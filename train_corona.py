"""
train_corona.py — Corona(日冕)训练入口
算法特性 (vs upstream):
  - Gated Attention Pooling + SiLU gate (替代FC+ReLU+sigmoid)
  - EMA-based residual decomposition (替代LayerNorm+ReLU)
  - Attention-weighted graph convolution (注意力加权各阶图矩阵)
  - Learned metric embedding + cosine similarity distance (替代内积)
  - Top-k sparse mask (替代element-wise mask)
  - Alternating row-col normalizer (替代行归一化)
  - LSTM + RoPE (替代GRU + sinusoidal PE)
  - Gated residual aggregation (替代简单sum)
  - Quantile loss (替代MAE)
  - RAdam + CosineAnnealingWarmRestarts (替代Adam + MultiStepLR)
"""
...
from walpurgis_corona.utils.train import set_config, EarlyStopping, data_reshaper, load_model
from walpurgis_corona.utils.load_data import load_dataset, load_adj
from walpurgis_corona.utils.log import TrainLogger
from walpurgis_corona.models.losses import masked_mae, masked_rmse, masked_mape, metric
from walpurgis_corona.models.trainer import trainer
from walpurgis_corona.models.model import D2STGNN

from walpurgis_corona import (_dbg, _is_debug, snapshot_model, register_activation_hooks,
...
def run(dataset, device_str='cpu', epochs_override=None, debug=False):
    if debug: os.environ['CORONA_DEBUG'] = '1'
    set_config(0)
    config_dir = os.path.join(_REPO_ROOT, 'src', 'walpurgis_corona', 'configs')
    ...
    if not os.path.exists(data_dir):
        data_dir = _resolve_path(config['data_args']['data_dir'], os.path.join(_REPO_ROOT, 'src', 'walpurgis_corona'))
        adj_path = _resolve_path(config['data_args']['adj_data_path'], os.path.join(_REPO_ROOT, 'src', 'walpurgis_corona'))
    ...
    print(f"\n[CR] Pipeline complete. Saved: {save_path}")
    return early_stopping.val_loss_min

def main():
    parser = argparse.ArgumentParser(description='Corona D2STGNN Training')
