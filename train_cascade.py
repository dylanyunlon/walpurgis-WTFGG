"""
train_cascade.py — Cascade训练入口
从项目根目录运行: python train_cascade.py --dataset SYNTH --debug
...
from walpurgis_cascade.utils.load_data import (
    load_dataset, load_adj)
from walpurgis_cascade.utils.log import TrainLogger
from walpurgis_cascade.models.losses import (
    masked_mae, masked_rmse, masked_mape, metric)
from walpurgis_cascade.models import trainer
from walpurgis_cascade.models.model import D2STGNN
...
def run(dataset, device_str='cpu', epochs_override=None,
        debug=False):
    if debug:
        os.environ['CASCADE_DEBUG'] = '1'

    set_config(0)
    config_dir = os.path.join(
        _REPO_ROOT, 'src', 'walpurgis_cascade', 'configs')
    ...
    if not os.path.exists(data_dir):
        cas_dir = os.path.join(
            _REPO_ROOT, 'src', 'walpurgis_cascade')
    ...
    print(f"\n{'='*60}")
    print(f"  Cascade Training Pipeline")
...
def main():
    parser = argparse.ArgumentParser(
        description='Cascade (D2STGNN) Training')
