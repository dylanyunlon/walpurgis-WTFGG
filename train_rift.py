"""
train_rift.py — Rift训练入口
从项目根目录运行: python train_rift.py --dataset SYNTH --debug

算法特性 (vs upstream):
  - Split-Recombine注意力 (hidden分K组独立处理后重组)
  - FFT域特征增强 (频域特征与时域特征concat后投影)
  - Polynomial decay LR: lr = base * (1 - t/T)^power
...
from walpurgis_rift.utils.train import (
    set_config, EarlyStopping, data_reshaper, load_model)
from walpurgis_rift.utils.load_data import (
    load_dataset, load_adj)
from walpurgis_rift.utils.log import TrainLogger
from walpurgis_rift.models.losses import (
    masked_mae, masked_rmse, masked_mape, metric)
from walpurgis_rift.models import trainer
from walpurgis_rift.models.model import D2STGNN
...
def run(dataset, device_str='cpu', epochs_override=None,
        debug=False):
    if debug:
        os.environ['RIFT_DEBUG'] = '1'

    set_config(0)
    config_dir = os.path.join(
        _REPO_ROOT, 'src', 'walpurgis_rift', 'configs')
    ...
    if not os.path.exists(data_dir):
        rf_dir = os.path.join(
            _REPO_ROOT, 'src', 'walpurgis_rift')
        data_dir = _resolve_path(
            config['data_args']['data_dir'], rf_dir)
        adj_path = _resolve_path(
            config['data_args']['adj_data_path'], rf_dir)
    ...
    print(f"\n{'='*60}")
    print(f"  Rift Training Pipeline")
    ...
    print(f"\n[RF] Pipeline complete. Saved: {save_path}")
    return early_stopping.val_loss_min


def main():
    parser = argparse.ArgumentParser(
        description='Rift (D2STGNN) Training')
