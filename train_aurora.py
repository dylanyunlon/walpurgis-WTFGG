"""
train_aurora.py — Aurora训练入口
从项目根目录运行: python train_aurora.py --dataset SYNTH --debug

算法特性 (vs upstream):
  - Multi-Scale Temporal Attention (多尺度时间注意力替代单一GRU)
  - Gated Fusion (sigmoid门控融合diffusion/inherent分支)
  - Spectral Graph Regularizer (自适应图拉普拉斯正则化)
  - CosineAnnealingWarmRestarts (替代MultiStepLR)
  - Huber Loss (替代纯MAE, outlier鲁棒)
...
from walpurgis_aurora.utils.train import (
    set_config, EarlyStopping, data_reshaper, load_model)
from walpurgis_aurora.utils.load_data import (
    load_dataset, load_adj)
from walpurgis_aurora.utils.log import TrainLogger
from walpurgis_aurora.models.losses import (
    masked_mae, masked_rmse, masked_mape, metric)
from walpurgis_aurora.models import trainer
from walpurgis_aurora.models.model import D2STGNN
from walpurgis_aurora import (
...
def run(dataset, device_str='cpu', epochs_override=None,
    ...
    config_dir = os.path.join(
        _REPO_ROOT, 'src', 'walpurgis_aurora', 'configs')
    ...
    if not os.path.exists(data_dir):
        nf_dir = os.path.join(
            _REPO_ROOT, 'src', 'walpurgis_aurora')
    ...
    print(f"\n{'='*60}")
    print(f"  Aurora Training Pipeline")
    ...
    if mode != 'test':
        for epoch in range(
            ...
            # Aurora: CosineAnnealingWarmRestarts per-epoch step
    ...
    print(f"\n[AR] Pipeline complete. Saved: {save_path}")
    return early_stopping.val_loss_min


def main():
    parser = argparse.ArgumentParser(
        description='Aurora (D2STGNN) Training')
