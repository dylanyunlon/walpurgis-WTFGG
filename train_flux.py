"""
train_flux.py — Flux训练入口
从项目根目录运行: python train_flux.py --dataset SYNTH --debug

算法特性 (vs upstream):
  - 滑动窗口流式推理 (streaming inference): 因果窗口截取+衰减权重
  - 渐进式解码 (progressive decode): 粗→细多步预测
  - Focal MAE损失 (focal loss): 对hard samples加权
  - 因果卷积嵌入 (causal conv embedding): 替代线性嵌入
  - ExponentialLR调度器 + warmup线性ramp
  - 流式特征缓存 (streaming feature cache)
  - 指数衰减位置编码 (exponential decay PE)
  - 分块因果注意力 (chunked causal attention)
  - 完整诊断: struct dump / activation probe / grad histogram
"""
import argparse
import os
import sys
...
from walpurgis_flux.utils.load_data import (
    load_dataset, load_adj)
from walpurgis_flux.utils.log import TrainLogger
from walpurgis_flux.models.losses import (
    masked_mae, masked_rmse, masked_mape, metric)
from walpurgis_flux.models import trainer
from walpurgis_flux.models.model import D2STGNN
...
def run(dataset, device_str='cpu', epochs_override=None,
    ...
    config_dir = os.path.join(
        _REPO_ROOT, 'src', 'walpurgis_flux', 'configs')
    ...
    data_dir = _resolve_path(
        config['data_args']['data_dir'])
    adj_path = _resolve_path(
        config['data_args']['adj_data_path'])

    if not os.path.exists(data_dir):
        fx_dir = os.path.join(
            _REPO_ROOT, 'src', 'walpurgis_flux')
        data_dir = _resolve_path(
            config['data_args']['data_dir'], fx_dir)
        adj_path = _resolve_path(
            config['data_args']['adj_data_path'],
            fx_dir)
    ...
    os.makedirs(os.path.join(
        _REPO_ROOT, 'output'), exist_ok=True)
    ...
    print(f"\n{'='*60}")
    print(f"  Flux Training Pipeline")
    ...
    if mode == 'test':
        ...
        resume_epoch = config[
            'start_up']['resume_epoch']
        if os.path.exists(save_path_resume):
            model = load_model(
                model, save_path_resume)
        batch_num_init = (
            resume_epoch *
            len(dataloader['train_loader']))
    ...
    if mode != 'test':
        for epoch in range(
            ...
            current_lr = engine.optimizer.param_groups[
                0]['lr']
...
def main():
    parser = argparse.ArgumentParser(
        description='Flux (D2STGNN) Training')
    parser.add_argument(
        '--dataset', type=str,
        default='SYNTH',
