#!/usr/bin/env python3
"""Eclipse training entry point — mirrors train_cardgame.py structure."""
import os
import sys
import time
import yaml
...
from walpurgis_eclipse.utils.load_data import load_dataset, load_adj
from walpurgis_eclipse.utils.train import set_config, EarlyStopping, data_reshaper
from walpurgis_eclipse.utils.log import TrainLogger
from walpurgis_eclipse.models.model import D2STGNN
from walpurgis_eclipse.models import trainer
from walpurgis_eclipse import _dbg, _is_debug, snapshot_model, register_activation_hooks, gradient_health_check


def main():
    parser = argparse.ArgumentParser(description='D2STGNN Eclipse Variant')
    ...
    if args.debug:
        os.environ['ECLIPSE_DEBUG'] = '1'

    # Load config
    cfg_path = os.path.join('src', 'walpurgis_eclipse', 'configs',
                            f'{args.dataset}.yaml')
    ...
    print(f"\n[ECL] D2STGNN Eclipse Variant")
    print(f"[ECL] Dataset: {dataset_name}")
    print(f"[ECL] Device: {device}")
    print(f"[ECL] Epochs: {cfg['optim_args']['epochs']}")
    ...
    if dataset_name == 'SYNTH':
        ...
        if not os.path.exists(os.path.join(synth_dir, 'train.npz')):
            print("[ECL] Generating synthetic data...")
    ...
    print(f"[ECL] Model params: {num_params:,}")
    ...
    print(f"\n[ECL] Starting training: {total_epochs} epochs")
    t_start = time.time()
    ...
    print(f"\n[ECL] Pipeline complete. Saved: {save_path_resume}")
