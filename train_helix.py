"""
train_helix.py — Helix训练入口
从项目根目录运行: python train_helix.py --dataset SYNTH --debug

算法特性 (vs upstream):
  - 螺旋卷积 (Helix Conv): 交替升维-降维螺旋结构
  - top-k自适应图稀疏化 (Adaptive Sparsification)
  - Label Smoothing MAE损失
  - 螺旋位置编码注入 (旋转相位调制embedding)
  - GELU + LayerNorm输出头 + helix channel rotation
  - ExponentialLR调度器 + 螺旋warm-up
...
from walpurgis_helix.utils.load_data import (
    load_dataset, load_adj)
from walpurgis_helix.utils.log import TrainLogger
from walpurgis_helix.models.losses import (
    masked_mae, masked_rmse, masked_mape, metric)
from walpurgis_helix.models import trainer
from walpurgis_helix.models.model import D2STGNN
...
def run(dataset, device_str='cpu', epochs_override=None,
        debug=False):
    if debug:
        os.environ['HELIX_DEBUG'] = '1'

    set_config(0)
    config_dir = os.path.join(
        _REPO_ROOT, 'src', 'walpurgis_helix', 'configs')
    ...
    if not os.path.exists(data_dir):
        hx_dir = os.path.join(
            _REPO_ROOT, 'src', 'walpurgis_helix')
        data_dir = _resolve_path(
            config['data_args']['data_dir'], hx_dir)
        adj_path = _resolve_path(
            config['data_args']['adj_data_path'], hx_dir)
    ...
    print(f"\n{'='*60}")
    print(f"  Helix Training Pipeline")
    ...
    if mode != 'test':
        ...
        if _is_debug():
            engine.perf.report()
            engine.lr_tracker.report()
            engine.sparsity_tracker.report()
    ...
    print(f"\n[HX] Pipeline complete. Saved: {save_path}")
    return early_stopping.val_loss_min


def main():
    parser = argparse.ArgumentParser(
        description='Helix (D2STGNN) Training')
