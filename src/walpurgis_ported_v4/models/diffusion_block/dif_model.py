"""
STLocalizedConv — walpurgis_ported_v4
Modifications:
  - gconv: added residual skip connection (output += X_0) after GCN aggregation
    to improve gradient flow through deep stacked layers
  - forward: prints shape evolution at each transformation step
  - get_graph: prints per-order mask coverage
"""
import torch
...
class STLocalizedConv(nn.Module):
    ...
    def gconv(self, support, X_k, X_0):
        """Graph convolution with residual skip (v4)."""
        out = [X_0]
        ...
        # v4: residual skip connection — keeps gradient path open
        out = out + X_0
        return out
    ...
    def get_graph(self, support):
        ...
        for graph in support:
            ...
            for k in range(2, self.k_s + 1):
                ...
                graph_ordered.append(k_1_order * mask)
                if _V4_DEBUG:
                    coverage = (k_1_order * mask > 1e-6).float().mean().item()
                    print(f"[v4-DBG][STLocalConv.get_graph] k={k} coverage={coverage:.4f}",
                          file=sys.stderr)
    ...
    def forward(self, X, dynamic_graph, static_graph):
        # X: [bs, seq, nodes, feat]
        if _V4_DEBUG:
            print(f"[v4-DBG][STLocalConv.forward] input X: {tuple(X.shape)}", file=sys.stderr)

        X = X.unfold(1, self.k_t, 1).permute(0, 1, 2, 4, 3)
        batch_size, seq_len, num_nodes, kernel_size, num_feat = X.shape

        if _V4_DEBUG:
            print(f"[v4-DBG][STLocalConv.forward] after unfold: "
                  f"bs={batch_size} seq={seq_len} N={num_nodes} "
                  f"kt={kernel_size} feat={num_feat}", file=sys.stderr)
        ...
        if _V4_DEBUG:
            print(f"[v4-DBG][STLocalConv.forward] n_support_graphs={len(support)}",
                  file=sys.stderr)
        ...
        if _V4_DEBUG:
            print(f"[v4-DBG][STLocalConv.forward] output: {tuple(hidden.shape)} "
                  f"norm={hidden.detach().norm().item():.4f}", file=sys.stderr)
