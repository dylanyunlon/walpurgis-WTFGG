"""generate_synth_data.py — Reverie合成数据生成"""
import os
import sys
import numpy as np
import pickle
...
def generate_synth_traffic(num_nodes=10, num_timesteps=500,
    ...
    print(f"[RV-SYNTH] Generating: {num_nodes} nodes, {num_timesteps} steps")
    ...
    for name, (_x, _y) in splits.items():
        np.savez_compressed(
            os.path.join(output_dir, f"{name}.npz"), x=_x, y=_y,
    ...
    pickle.dump({'mean': mean, 'std': std},
                open(os.path.join(output_dir, 'scaler.pkl'), 'wb'))
    print(f"[RV-SYNTH] Done. Output: {output_dir}")
    return output_dir
