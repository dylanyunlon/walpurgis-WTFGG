"""Cascade load_data: eps-guarded scaler + NaN detection."""
import pickle
import os
import numpy as np
import sys
...
class StandardScaler():
    def __init__(self, mean, std):
        self.mean = mean
        self.std = max(std, 1e-8)  # Cascade: eps-guarded to avoid div-by-zero
...
def load_dataset(data_dir, batch_size, valid_batch_size, test_batch_size, dataset_name):
    ...
    for mode in ['train', 'val', 'test']:
        ...
        x = _['x']
        y = _['y']
        # Cascade: NaN detection
        nan_x = np.isnan(x).sum()
        nan_y = np.isnan(y).sum()
        if nan_x > 0 or nan_y > 0:
            print(f"[CAS:load_data] WARNING NaN in {mode}: x={nan_x} y={nan_y}",
                  file=sys.stderr)
        ...
        data_dict['y_' + mode]  = y
        if _CAS_DBG:
            print(f"[CAS:load_data] {mode}: x={x.shape} y={y.shape} "
...
def load_adj(file_path, adj_type):
    ...
    if _CAS_DBG:
        print(f"[CAS:load_adj] type={adj_type} shape={adj[0].shape} "
              f"density={(np.array(adj[0])!=0).mean():.2%}",
              file=sys.stderr)
    return adj, adj_mx
