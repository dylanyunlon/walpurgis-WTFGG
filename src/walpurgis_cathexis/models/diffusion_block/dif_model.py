"""
Cathexis STLocalizedConv — 算法改写 #3
upstream: uniform matmul with support graphs
cathexis: GATv2-style per-hop attention weighting — learned attention coefficients
          per spatial hop before the graph convolution aggregation
"""
import torch
...
class STLocalizedConv(nn.Module):
    def __init__(self, hidden_dim, pre_defined_graph=None, use_pre=None, dy_graph=None, sta_graph=None, **model_args):
        ...
        # Cathexis: use num_modalities (always 2) for dynamic graph count, not len(pre_defined_graph)
        num_modalities = model_args.get('num_modalities', 2)
        self.num_matric = (int(use_pre)*len(self.pre_defined_graph) + num_modalities*int(dy_graph) + int(sta_graph)) * self.k_s + 1
        ...
        # Cathexis改写: GATv2-style per-hop attention
        self.hop_attn_key = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.hop_attn_query = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.hop_attn_vec = nn.Parameter(torch.randn(hidden_dim))
    ...
    def gconv(self, support, X_k, X_0):
        ...
        for graph in support:
            if len(graph.shape) == 2: pass
            else: graph = graph.unsqueeze(1)
            H_k = torch.matmul(graph, X_k)
            # Cathexis改写: per-hop attention scoring
            attn_score = torch.tanh(self.hop_attn_key(H_k) + self.hop_attn_query(X_0))
            attn_weight = torch.sigmoid((attn_score * self.hop_attn_vec).sum(dim=-1, keepdim=True))
            H_k = H_k * attn_weight
            out.append(H_k)
    ...
    def forward(self, X, dynamic_graph, static_graph):
        ...
        support = []
        if self.use_predefined_graph: support = support + self.pre_defined_graph
        if self.use_dynamic_hidden_graph: support = support + dynamic_graph
        if self.use_static__hidden_graph: support = support + self.get_graph(static_graph)
