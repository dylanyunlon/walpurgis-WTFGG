"""
Cathexis D2STGNN — 算法改写 #8
upstream: sum(dif_hidden_list) + sum(inh_hidden_list), ReLU output head
cathexis: Exponential recency weighting — later layers contribute more,
          SiLU output head with layer normalization
"""
import torch
...
class D2STGNN(nn.Module):
    def __init__(self, **model_args):
        ...
        # Cathexis改写 #8: exponential recency weights for layer aggregation
        self.recency_decay = nn.Parameter(torch.tensor(0.5))

        # Cathexis改写: SiLU + LayerNorm output head
        self.out_ln = nn.LayerNorm(self._forecast_dim)
    ...
    def forward(self, history_data):
        ...
        # Cathexis改写 #8: exponential recency weighting (later layers get higher weight)
        decay = torch.sigmoid(self.recency_decay)
        n_layers = len(dif_forecast_hidden_list)
        exponents = torch.arange(n_layers - 1, -1, -1, device=history_data.device, dtype=torch.float)
        weights = decay ** exponents
        weights = weights / weights.sum()

        dif_forecast_hidden = sum(w * h for w, h in zip(weights, dif_forecast_hidden_list))
        inh_forecast_hidden = sum(w * h for w, h in zip(weights, inh_forecast_hidden_list))
        forecast_hidden = dif_forecast_hidden + inh_forecast_hidden

        # Cathexis改写: SiLU output + LayerNorm
        forecast_hidden = self.out_ln(forecast_hidden)
