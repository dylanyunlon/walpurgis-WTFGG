"""Flux load_data: 带运行统计的scaler + NaN插补.
与upstream(固定mean/std)和vortex(eps-guard)不同,
Flux的StandardScaler使用运行中统计: 支持增量更新mean/std,
适配流式推理场景中数据分布可能漂移的情况."""
import pickle
import os
import numpy as np
import sys
...
def max_min_normalization(x, _max, _min):
    return 1. * (x - _min) / (_max - _min) * 2. - 1.


def re_max_min_normalization(x, _max, _min):
    return 1. * (x + 1.) / 2. * (_max - _min) + _min


class StandardScaler:
    """Flux: 带运行统计的StandardScaler.
    初始化时记录train set的mean/std,
    同时支持增量更新(update方法)用于流式推理."""
    def __init__(self, mean, std):
        self.mean = mean
        self.std = max(std, 1e-8)
        self._n = 1
        self._running_mean = mean
        self._running_var = std ** 2
    ...
    def update(self, new_data_mean, new_data_std, n=1):
        """增量更新运行统计 (Welford's online algorithm)"""
        old_n = self._n
        new_n = old_n + n
        delta = new_data_mean - self._running_mean
        self._running_mean += delta * n / new_n
        self._running_var = (
            (old_n * self._running_var +
             n * new_data_std ** 2 +
             delta ** 2 * old_n * n / new_n) / new_n)
        self._n = new_n
        if _FX_DBG:
            print(f"[FX:scaler.update] n={self._n} "
                  f"running_mean={self._running_mean:.4f} "
                  f"running_std="
                  f"{np.sqrt(self._running_var):.4f}",
                  file=sys.stderr)
...
def load_dataset(data_dir, batch_size, valid_batch_size,
    ...
    for mode in ['train', 'val', 'test']:
        ...
        if nan_x > 0 or nan_y > 0:
            print(f"[FX:load_data] WARNING NaN in {mode}: "
                  f"x={nan_x} y={nan_y}",
                  file=sys.stderr)
            # 用前值填充替代零填充
        ...
        data_dict['y_' + mode] = y
        if _FX_DBG:
            print(f"[FX:load_data] {mode}: x={x.shape} "
                  f"y={y.shape} "
    ...
    if dataset_name in ('PEMS04', 'PEMS08'):
        _min = pickle.load(
            open(os.path.join(
                "datasets", dataset_name, "min.pkl"), 'rb'))
        _max = pickle.load(
            open(os.path.join(
                "datasets", dataset_name, "max.pkl"), 'rb'))
        for mode in ['train', 'val', 'test']:
            y = np.squeeze(np.transpose(
                data_dict['y_' + mode],
                axes=[0, 2, 1, 3]), axis=-1)
            yn = max_min_normalization(
                y, _max[:, :, 0, :], _min[:, :, 0, :])
            data_dict['y_' + mode] = np.transpose(
                yn, axes=[0, 2, 1])
...
def load_adj(file_path, adj_type):
    ...
    if adj_type == "scalap":
        ...
        raise ValueError(
            f"adj type not defined: {adj_type}")
    if _FX_DBG:
        print(f"[FX:load_adj] type={adj_type} "
              f"shape={adj[0].shape} "
              f"density="
              f"{(np.array(adj[0])!=0).mean():.2%}",
              file=sys.stderr)
    return adj, adj_mx
