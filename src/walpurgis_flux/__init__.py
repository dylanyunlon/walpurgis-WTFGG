"""
walpurgis_flux — D2STGNN Flux变体
算法改写 (~20%):
  - 滑动窗口流式推理 (streaming inference): 无需全序列, 用因果滑动窗口处理
  - 渐进式解码 (progressive decode): 粗→细多步预测, 先预测低分辨率再细化
  - Focal loss: 对hard sample加权, 聚焦难预测时刻
  - 因果卷积嵌入 (causal conv embedding): 替代线性嵌入
  - ExponentialLR调度器 + warmup线性ramp
  - 流式特征缓存 (streaming feature cache): 减少冗余计算

全局调试: FLUX_DEBUG=1 开启
诊断日志: FLUX_DIAG_LOG=<path> 写入JSONL
"""
import os
import sys
import time
...
def _diag_write(record: dict):
    """写入诊断记录到JSONL（若设置了FLUX_DIAG_LOG）"""
...
def snapshot_model(model, epoch=0, step=0, top_k=10):
    ...
    if not _is_debug():
        return
    print(f"\n{'='*70}", file=sys.stderr)
    print(f"[FX-SNAPSHOT] epoch={epoch} step={step} "
...
class ActivationTracker:
    ...
    def report(self, top_k=12):
        print(f"\n[FX-ACTIVATION] {len(self.stats)} layers tracked:",
              file=sys.stderr)
        items = []
...
def gradient_health_check(model, explode_thresh=70.0,
    ...
    if issues and _is_debug():
        print("[FX-GRAD] Issues detected:", file=sys.stderr)
        for iss in issues:
...
def gradient_histogram(model, n_bins=10):
    ...
    if not all_grads:
        print("[FX-GRAD-HIST] No gradients computed.",
              file=sys.stderr)
        return
    ...
    print(f"[FX-GRAD-HIST] log10 gradient distribution:",
          file=sys.stderr)
    for i in range(n_bins):
...
def weight_diff(state_a, state_b, top_k=6):
    ...
    print(f"\n[FX-WEIGHT-DIFF] top-{top_k} changed:",
          file=sys.stderr)
...
def dump_struct_state(label, **kwargs):
    ...
    for k, v in kwargs.items():
        if isinstance(v, torch.Tensor):
            ...
            print(f"  {k:30s}: {type(v).__name__}",
                  file=sys.stderr)
    print(f"[FX-STRUCT:{label}] === End Dump ===\n",
          file=sys.stderr)
    _diag_write(record)
...
# ─── 流式窗口统计追踪（Flux特有）────────────────────────
class StreamWindowTracker:
    """追踪流式推理中滑动窗口的统计: 窗口命中率, 缓存效率, 延迟分布"""
    def __init__(self):
        self.window_hits = 0
    ...
    def report(self):
        ...
        hit_rate = self.window_hits / max(total, 1)
        print(f"\n[FX-STREAM] Window stats:",
              file=sys.stderr)
        print(f"  hits={self.window_hits} misses={self.window_misses} "
              f"hit_rate={hit_rate:.2%}",
              file=sys.stderr)
        if self.latencies:
            lat = np.array(self.latencies)
            print(f"  latency: mean={lat.mean():.2f}ms "
                  f"p50={np.median(lat):.2f}ms "
                  f"p99={np.percentile(lat, 99):.2f}ms",
                  file=sys.stderr)


# ─── 渐进式解码精度追踪（Flux特有）─────────────────────
class ProgressiveDecodeTracker:
    """追踪粗→细各阶段预测精度变化"""
    def __init__(self):
        self.stage_errors = defaultdict(list)

    def record(self, stage_name, error_val):
        self.stage_errors[stage_name].append(error_val)

    def report(self):
        if not _is_debug():
            return
        print(f"\n[FX-PROGRESSIVE] Decode stage errors:",
              file=sys.stderr)
        for stage, errors in self.stage_errors.items():
            e = np.array(errors)
            print(f"  {stage:25s}: mean={e.mean():.6f} "
                  f"std={e.std():.6f} last={e[-1]:.6f}",
                  file=sys.stderr)
        _diag_write({
            "ts": time.time(), "tag": "progressive_decode",
            "stages": {k: float(np.mean(v))
                       for k, v in self.stage_errors.items()}
        })
