"""
generate_synth_data.py — Flux合成数据生成
与Vortex共享相同的数据格式, 确保可比性
"""
import os
import sys
import numpy as np
...
def generate_synth_traffic(num_nodes=10,
                           num_timesteps=500,
                           num_feat=1,
                           seq_x=12, seq_y=12,
                           output_dir=None):
    ...
    print(f"[FX-SYNTH] Generating: {num_nodes} nodes, "
    ...
    xs, ys = [], []
    for t_idx in range(seq_x - 1,
                       num_timesteps - seq_y):
        xs.append(data[t_idx + x_offsets, ...])
        ys.append(
            data[t_idx + y_offsets, ..., :1])
    ...
    for name, (_x, _y) in splits.items():
        checksums[name] = {
            'x_mean': float(_x.mean()),
            'x_std': float(_x.std()),
            'y_mean': float(_y.mean()),
            'y_std': float(_y.std()),
    ...
    with open(os.path.join(output_dir,
                           'checksums.json'), 'w') as f:
        json.dump(checksums, f, indent=2)
    print(f"[FX-SYNTH] Done. Output: {output_dir}")
    return output_dir
