...
def _dbg(tag, tensor_or_val, module="eclipse"):
    ...
    if isinstance(tensor_or_val, torch.Tensor):
        ...
        if t.numel() == 0:
            print(f"[ECL:{tag}@{module}] EMPTY shape={list(t.shape)}", file=sys.stderr)
            return
...
def gradient_health_check(model):
    ...
    print(f"[ECL] === End Check ({issues} issues) ===\n", file=sys.stderr)
...
def weight_diff(state_a, state_b, top_k=5):
    ...
    print(f"\n[ECL] === Weight Diff (top {top_k}) ===", file=sys.stderr)
