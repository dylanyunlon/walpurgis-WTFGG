"""generate_synth_data.py — Umbra合成数据生成"""
import os
import sys
import numpy as np
import pickle
...
def generate_synth_traffic(num_nodes=10, num_timesteps=500,
    ...
    print(f"[UMB-SYNTH] Generating: {num_nodes} nodes, {num_timesteps} steps")
    ...
    for name, (_x, _y) in splits.items():
        checksums[name] = {
            'x_mean': float(_x.mean()), 'x_std': float(_x.std()),
    ...
    print(f"[UMB-SYNTH] Done. Output: {output_dir}")
    return output_dir
