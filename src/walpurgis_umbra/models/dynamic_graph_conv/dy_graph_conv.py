"""
DynamicGraphConstructor — Umbra变体
整合Hyperbolic Poincaré距离 + Differentiable Top-K + PageRank归一化
"""
...
class DynamicGraphConstructor(nn.Module):
    def __init__(self, **model_args):
        ...
        self.normalizer = Normalizer(damping=0.85, ppr_iters=5)
    ...
    def forward(self, **inputs):
        ...
        dist_mx = self.distance_function(
            X, E_d, E_u, T_D, D_W)
        dataflow_checkpoint("dygraph.raw_dist", dist_mx[0])

        dist_mx = self.mask(dist_mx)
        dist_mx = self.normalizer(dist_mx)

        dataflow_checkpoint("dygraph.normed", dist_mx[0])

        mul_mx = self.multi_order(dist_mx)
        dynamic_graphs = self.st_localization(mul_mx)

        _dbg("dygraph.num_graphs",
             f"{len(dynamic_graphs)}", "graph")
