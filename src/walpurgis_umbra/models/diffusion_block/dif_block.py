"""
DifBlock — Umbra变体
保持整体结构, 集成Random Walk Diffusion + Restart的STLocalizedConv
"""
...
class DifBlock(nn.Module):
    ...
    def forward(self, history_data, gated_history_data,
        ...
        dataflow_checkpoint("dif_block.input", history_data)

        hidden_states_dif = self.localized_st_conv(
            gated_history_data, dynamic_graph, static_graph)

        _dbg("dif_block.hidden_states",
             hidden_states_dif, "diffusion")
        ...
        _dbg("dif_block.forecast_energy",
             forecast_hidden.detach().norm(), "diffusion")
        _dbg("dif_block.backcast_energy",
             backcast_seq_res.detach().norm(), "diffusion")
