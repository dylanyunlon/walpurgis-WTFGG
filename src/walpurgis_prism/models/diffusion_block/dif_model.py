"""Prism STLocalizedConv: adds node-wise contrastive scaling to the graph convolution.
After standard graph conv, each node's output is scaled by a learned importance
factor derived from its connectivity pattern, promoting discriminative node representations."""
import torch
import torch.nn as nn


class NodeImportanceScaler(nn.Module):
    """Prism特有: 根据节点连接模式学习重要性缩放因子"""
    def __init__(self, hidden_dim):
        super().__init__()
        self.importance_fc = nn.Linear(hidden_dim, 1)

    def forward(self, x, support_count):
        # x: [B, L, N, D]
        # 用节点特征均值估算重要性
        node_feat = x.mean(dim=1)  # [B, N, D]
        importance = torch.sigmoid(
            self.importance_fc(node_feat))  # [B, N, 1]
        importance = importance.unsqueeze(1)  # [B, 1, N, 1]
        return x * importance
...
class STLocalizedConv(nn.Module):
    def __init__(self, hidden_dim,
        ...
        self.num_matric = (int(use_pre) * len(
            self.pre_defined_graph) + len(
            self.pre_defined_graph) * int(
            dy_graph) + int(sta_graph)) * self.k_s + 1
        ...
        # Prism特有: 节点重要性缩放
        self.node_scaler = NodeImportanceScaler(hidden_dim)
    ...
    def forward(self, X, dynamic_graph, static_graph):
        ...
        if self.use_static__hidden_graph:
            support = support + self.get_graph(
                static_graph)
        # parallelize
