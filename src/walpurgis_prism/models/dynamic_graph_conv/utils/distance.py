"""Prism distance function: adds frequency-domain distance as a third modality.
Besides the standard time-series and embedding distances, Prism computes
FFT-based spectral similarity between nodes, providing a frequency-aware
graph construction signal."""
import math
import torch
...
class DistanceFunction(nn.Module):
    def __init__(self, **model_args):
        ...
        # Prism特有: 频谱距离特征提取
        n_freq = self.input_seq_len // 2 + 1
        self.freq_fc = nn.Linear(n_freq, self.hidden_dim)
        # Distance Score
    ...
    def forward(self, X, E_d, E_u, T_D, D_W):
        ...
        dy_feat = self.fc_ts_emb2(self.dropout(
            self.bn(F.relu(self.fc_ts_emb1(X)))))
        dy_feat = dy_feat.view(
