"""Prism cal_adj: adjacency matrix calculations with numerical diagnostics."""
...
def check_nan_inf(tensor, raise_ex=True):
    ...
    if _PR_DBG and (nan or inf):
        nan_ct = torch.isnan(tensor).sum().item()
        inf_ct = torch.isinf(tensor).sum().item()
        total = tensor.numel()
        print(f"[PR:cal_adj] NaN={nan_ct}/{total} ({nan_ct/total:.2%}) "
        ...
        if valid.numel() > 0:
            ...
            print(f"[PR:cal_adj] percentiles: p1={q[0]:.4f} p25={q[1]:.4f} "
