...
def masked_mae(preds, labels, null_val=np.nan):
    ...
    # ── Adaptive-δ Huber ──
    if len(_residual_buf) >= 20:
        delta = float(np.percentile(list(_residual_buf), 90))
        delta = max(0.1, min(delta, 10.0))  # safety bounds
    else:
        delta = 1.0  # default until enough samples
    _t_adaptive_delta.record(delta)

    quadratic_region = 0.5 * diff * diff / delta  # scaled quadratic
    linear_region = abs_diff - 0.5 * delta          # linear tail
    loss = torch.where(abs_diff < delta, quadratic_region, linear_region)

    loss = loss * mask
    loss = torch.where(torch.isnan(loss), torch.zeros_like(loss), loss)

    # ── Exponential horizon decay ──
    if loss.dim() >= 3 and loss.shape[1] > 1:
        T = loss.shape[1]
        lam = 0.08
        hw = torch.tensor(
            [np.exp(-lam * t) for t in range(T)],
