"""Cascade InhBlock: depth-aware positional encoding + standard backcast.
Unlike upstream (static sinusoidal PE) and vortex (conditional PE + dual-path backcast),
Cascade uses depth-aware PE where the encoding is scaled by a learnable factor
that adapts based on the current layer's depth in the cascade. This allows
deeper layers to use different positional granularity than shallow ones."""
import math
...
class PositionalEncoding(nn.Module):
    """Cascade PE: standard sinusoidal + learnable depth scaling.
    The depth_scale parameter allows the cascade to adjust PE magnitude
    per layer, enabling deeper layers to attend at different granularity."""
    def __init__(self, d_model, dropout=None, max_len: int = 5000):
        ...
        # Cascade特有: learnable depth scaling factor
        self.depth_scale = nn.Parameter(torch.tensor(1.0))

    def forward(self, X):
        # Scale PE by learnable factor (cascade depth adaptation)
        X = X + self.pe[:X.size(0)] * self.depth_scale
        X = self.dropout(X)
        return X
...
class InhBlock(nn.Module):
    ...
    def forward(self, hidden_inherent_signal):
        ...
        if _CAS_DBG:
            print(f"[CAS:inhblk@inh_block] pe_scale={self.pos_encoder.depth_scale.item():.4f} "
                  f"fk_shape={list(forecast_hidden.shape)}",
                  file=sys.stderr)
