...
class DecoupleLayer(nn.Module):
    ...
    def _hard_concrete(self, log_alpha: torch.Tensor):
        """Hard concrete sampling (training) / deterministic (eval).

        Training: sample u ~ Uniform(0,1), compute stretched sigmoid,
        then clamp to [0,1].  This yields exact 0 or 1 with nonzero
        probability — natural L₀ sparsity.
        Eval: deterministic sigmoid stretched and clamped.
        """
        if self.training:
            u = torch.rand_like(log_alpha).clamp(1e-6, 1 - 1e-6)
            s = torch.sigmoid(
                (torch.log(u) - torch.log(1 - u) + log_alpha) / self._HC_BETA
            )
        else:
            s = torch.sigmoid(log_alpha)
        # Stretch to (gamma, zeta) then clamp to [0, 1]
        s_bar = s * (self._HC_ZETA - self._HC_GAMMA) + self._HC_GAMMA
    ...
    def forward(self, hist, dyn_g, sta_g, emb_u, emb_d, t_feat, d_feat):
        ...
        # ── Hard-concrete skip ──
        alpha = self._hard_concrete(self._skip_log_alpha)
...
class D2STGNN(nn.Module):
    ...
    def __init__(self, **kw):
        ...
        # ── Attention-pooled aggregation ──
        # A tiny 2-layer MLP that maps each layer's forecast to a scalar score
        self._agg_scorer = nn.Sequential(
            nn.Linear(self._forecast_dim, 64),
            nn.GELU(),
            nn.Linear(64, 1),
        )
    ...
    def _warmup_phase(self) -> float:
        """Return current warmup fraction (0→1 over warmup window)."""
        progress = min(self._fwd_n / max(self._warmup_steps, 1), 1.0)
        return 0.5 * (1.0 - math.cos(math.pi * progress))

    def _fast_fingerprint(self, tensor_sample: bytes) -> str:
        """xxhash-style fingerprint via Python hash on quantised bytes."""
        # Quantise to 8-bit and hash — ~5× faster than MD5
        h = hash(tensor_sample) & 0xFFFFFFFFFFFF
        return f"{h:012x}"
    ...
    def forward(self, history_data):
        ...
        # Structure fingerprint (fast xxhash-style)
        if dyn_g:
            with torch.no_grad():
                sample = dyn_g[0].detach().cpu().to(torch.float16).numpy().tobytes()[:512]
                self._structure_fingerprint = self._fast_fingerprint(sample)
        ...
        # Step 5 — attention-pooled aggregation
        # Score each layer's forecast, then softmax-normalise
        tau = self._agg_temp.clamp(min=0.1)
        combined_fcs = [d + i for d, i in zip(dif_fcs, inh_fcs)]  # [L × [B,N,F]]

        # Stack → [L, B*N, F] → score per layer
        stacked = torch.stack(combined_fcs, dim=0)       # [L, B, N, F]
        L, B, N, Fd = stacked.shape
        flat = stacked.reshape(L, B * N, Fd)              # [L, B*N, F]
        # Global avg across B*N to get per-layer representation → [L, F]
        layer_repr = flat.mean(dim=1)
        scores = self._agg_scorer(layer_repr).squeeze(-1) / tau   # [L]
        weights = F.softmax(scores, dim=0)                 # [L]

        self._last_agg_weights = [round(w.item(), 5) for w in weights]

        # Weighted sum
        agg = sum(w * fc for w, fc in zip(weights, combined_fcs))
