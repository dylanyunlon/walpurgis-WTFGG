...
def MinMaxnormalization(train, val, test):
    ...
    for name, arr in [('train', train_norm), ('val', val_norm),
                      ('test', test_norm)]:
...
def generate_graph_seq2seq_io_data(
    ...
    if add_day_in_week:
        ...
        day_in_week = np.tile(day_in_week, [1, num_nodes, 1]).transpose(
            (2, 1, 0))
...
def generate_train_val_test(args):
    ...
    num_train = (num_train // samples_per_week) * samples_per_week or round(
        num_samples * 0.6)
    num_test = (num_test // samples_per_week) * samples_per_week or round(
        num_samples * 0.2)
    ...
    # MinMax norm on traffic features
