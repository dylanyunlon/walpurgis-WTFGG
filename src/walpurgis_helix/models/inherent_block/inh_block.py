...
class HelixPositionalEncoding(nn.Module):
    """Helix特有: 螺旋位置编码
    在标准sin/cos编码基础上, 加入可学习的螺旋半径参数,
    使位置编码的幅度随位置变化, 形成螺旋状的编码轨迹"""
    def __init__(self, d_model, dropout=None, max_len: int = 5000):
        ...
        # Helix: 可学习的螺旋半径 — 控制PE幅度随位置衰减/增长
        self.helix_radius = nn.Parameter(torch.ones(1))

    def forward(self, X):
        # Helix: 半径调制 — PE幅度乘以可学习的radius
        radius = torch.sigmoid(self.helix_radius) * 2.0
        X = X + self.pe[:X.size(0)] * radius
        X = self.dropout(X)
        return X
...
class InhBlock(nn.Module):
    def __init__(self, hidden_dim, num_heads=4, bias=True, forecast_hidden_dim=256, **model_args):
        """Inherent block
        Helix改写: 使用HelixPositionalEncoding替代标准PE
        ...
        # inherent model — Helix: 螺旋位置编码
        self.pos_encoder    = HelixPositionalEncoding(hidden_dim, model_args['dropout'])
