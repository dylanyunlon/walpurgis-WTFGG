...
class DistanceFunction(nn.Module):
    ...
    def __init__(self, **model_args):
        ...
        # Helix特有: 螺旋距离温度参数
        self.helix_temperature = nn.Parameter(torch.tensor(1.0))
    ...
    def forward(self, X, E_d, E_u, T_D, D_W):
        ...
        X  = [X1, X2]
        adjacent_list = []
        # Helix特有: 用可学习的温度缩放注意力
        temp = torch.clamp(self.helix_temperature, min=0.1)
        for _ in X:
            Q = self.WQ(_)
            K = self.WK(_)
            QKT = torch.bmm(Q, K.transpose(-1, -2)) / (math.sqrt(self.hidden_dim) * temp)
