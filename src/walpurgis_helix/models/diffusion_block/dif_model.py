...
class STLocalizedConv(nn.Module):
    def __init__(self, hidden_dim, pre_defined_graph=None, use_pre=None, dy_graph=None, sta_graph=None, **model_args):
        ...
        # Helix特有: 螺旋时间核权重 — 可学习的时间核缩放
        # 对k_t个时间步施加不同的权重, 形成螺旋式的重要度分配
        self.helix_kernel_weight = nn.Parameter(
            torch.ones(self.k_t) / self.k_t)
    ...
    def forward(self, X, dynamic_graph, static_graph):
        ...
        # Helix特有: 对时间核维度施加螺旋权重
        kernel_weights = F.softmax(self.helix_kernel_weight, dim=0)
        kernel_weights = kernel_weights.view(1, 1, 1, kernel_size, 1)
        X = X * kernel_weights
