"""
walpurgis_helix — D2STGNN Helix变体
算法改写 (~20%):
  - 螺旋卷积 (Helix Conv): 交替升维-降维螺旋结构替代标准FC
  - 自适应图稀疏化 (Adaptive Sparsification): top-k图过滤
  - Label Smoothing Loss: 平滑标签替代hard target
  - Helix位置编码: 螺旋相位注入时间嵌入
  - 渐进式通道旋转: 层间hidden维度交替膨胀/收缩

全局调试: HELIX_DEBUG=1 开启
诊断日志: HELIX_DIAG_LOG=<path> 写入JSONL
"""
import os
import sys
import time
...
def _diag_write(record: dict):
    """写入诊断记录到JSONL（若设置了HELIX_DIAG_LOG）"""
...
def snapshot_model(model, epoch=0, step=0, top_k=10):
    ...
    if not _is_debug():
        return
    print(f"\n{'='*70}", file=sys.stderr)
    print(f"[HX-SNAPSHOT] epoch={epoch} step={step} "
...
class ActivationTracker:
    ...
    def report(self, top_k=12):
        print(f"\n[HX-ACTIVATION] {len(self.stats)} layers tracked:",
              file=sys.stderr)
        items = []
...
def gradient_health_check(model, explode_thresh=70.0,
    ...
    if issues and _is_debug():
        print("[HX-GRAD] Issues detected:", file=sys.stderr)
        for iss in issues:
...
def gradient_histogram(model, n_bins=10):
    ...
    if not all_grads:
        print("[HX-GRAD-HIST] No gradients computed.",
              file=sys.stderr)
        return
    ...
    print(f"[HX-GRAD-HIST] log10 gradient distribution:",
          file=sys.stderr)
    for i in range(n_bins):
...
def weight_diff(state_a, state_b, top_k=6):
    ...
    print(f"\n[HX-WEIGHT-DIFF] top-{top_k} changed:",
          file=sys.stderr)
...
def dump_struct_state(label, **kwargs):
    ...
    for k, v in kwargs.items():
        if isinstance(v, torch.Tensor):
            ...
            print(f"  {k:30s}: {type(v).__name__}",
                  file=sys.stderr)
    print(f"[HX-STRUCT:{label}] === End Dump ===\n",
          file=sys.stderr)
    _diag_write(record)
...
class SparsityTracker:
    ...
    def record(self, step, sparsity, k_ratio):
        self.history.append((step, sparsity, k_ratio))
        if _is_debug() and len(self.history) % 30 == 0:
            print(f"[HX-SPARSITY] step={step} "
                  f"sparsity={sparsity:.4f} "
                  f"k_ratio={k_ratio:.4f}",
                  file=sys.stderr)

    def report(self):
        if not self.history:
            return
        sparsities = [s for _, s, _ in self.history]
        print(f"\n[HX-SPARSITY] Summary: "
              f"initial={sparsities[0]:.4f} "
              f"final={sparsities[-1]:.4f} "
              f"min={min(sparsities):.4f} "
              f"max={max(sparsities):.4f} "
              f"steps={len(sparsities)}", file=sys.stderr)
...
class LRTracker:
    ...
    def record(self, step, lr):
        ...
        if _is_debug() and len(self.history) % 50 == 0:
            print(f"[HX-LR] step={step} lr={lr:.8f}",
                  file=sys.stderr)
