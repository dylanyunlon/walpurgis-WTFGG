"""
walpurgis_zenith — D2STGNN Zenith变体
算法改写:
  - 频域衰减门控 (spectral decay gating on graph convolution)
  - 自适应残差缩放 (learnable residual scaling per layer)
  - 梯度分桶直方图诊断 (gradient histogram binning)
  - 训练过程中实时输出每一层的数据流状态

全局调试: ZENITH_DEBUG=1 开启
"""
import os
import sys
import time
...
def _diag_write(record: dict):
    """写入诊断日志到JSONL文件（若配置了ZENITH_DIAG_LOG）"""
...
def snapshot_model(model, epoch=0, step=0, top_k=8):
    ...
    if not _is_debug():
        return
    print(f"\n{'='*65}", file=sys.stderr)
    print(f"[ZN-SNAPSHOT] epoch={epoch} step={step} "
...
class ActivationTracker:
    ...
    def report(self, top_k=10):
        print(f"\n[ZN-ACTIVATION] {len(self.stats)} layers tracked:",
              file=sys.stderr)
        items = []
...
def gradient_health_check(model, explode_thresh=80.0, vanish_thresh=1e-7):
    ...
    if issues and _is_debug():
        print("[ZN-GRAD] Issues detected:", file=sys.stderr)
        for iss in issues:
...
def gradient_histogram(model, n_bins=8):
    ...
    if not all_grads:
        print("[ZN-GRAD-HIST] No gradients computed yet.", file=sys.stderr)
        return
    ...
    print(f"[ZN-GRAD-HIST] log10 gradient distribution:", file=sys.stderr)
    for i in range(n_bins):
...
def weight_diff(state_a, state_b, top_k=5):
    ...
    print(f"\n[ZN-WEIGHT-DIFF] top-{top_k} changed parameters:", file=sys.stderr)
...
class PerfTimer:
    ...
    def report(self):
        if not _is_debug():
            return
        print(f"\n[ZN-PERF] Timing breakdown:", file=sys.stderr)
