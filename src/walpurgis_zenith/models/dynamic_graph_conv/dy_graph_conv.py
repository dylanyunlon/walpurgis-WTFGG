"""
DynamicGraphConstructor — Zenith变体
改写: st_localization中对图做temperature scaling
"""
...
class DynamicGraphConstructor(nn.Module):
    def __init__(self, **model_args):
        ...
        # Zenith: 可学习温度
        import torch
        self.graph_temp = nn.Parameter(torch.tensor(1.0))

    def st_localization(self, graph_ordered):
        import torch
        temp = torch.sigmoid(self.graph_temp) + 0.5
        st_local_graph = []
        for modality_i in graph_ordered:
            for k_order_graph in modality_i:
                k_order_graph = (k_order_graph * temp).unsqueeze(
