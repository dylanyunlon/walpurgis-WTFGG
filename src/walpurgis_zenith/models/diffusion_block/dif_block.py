"""
DifBlock — Zenith变体
改写: backcast用ELU激活, forecast分支前加dropout防过拟合
"""
...
class DifBlock(nn.Module):
    def __init__(self, hidden_dim, forecast_hidden_dim=256,
        ...
        self.bc_activation = nn.ELU(alpha=0.7)
        self.residual_decompose = ResidualDecomp(
            [-1, -1, -1, hidden_dim])
        self.forecast_dropout = nn.Dropout(
            model_args.get('dropout', 0.1) * 0.5)
    ...
    def forward(self, history_data, gated_history_data,
        ...
        forecast_hidden = self.forecast_dropout(forecast_hidden)
        backcast_seq = self.bc_activation(
            self.backcast_branch(hidden_states_dif))
