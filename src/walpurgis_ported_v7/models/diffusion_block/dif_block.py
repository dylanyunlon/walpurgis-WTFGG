...
_DBG_DIFBLK = ("--dbg-difblk" in sys.argv)
...
class DifBlock(nn.Module):
    def __init__(self, hidden_dim, forecast_hidden_dim=256,
        ...
        # 算法改动: 可学习的 skip-connection 缩放因子
        # 控制 backcast 分支对原始信号的保留程度
    ...
    def forward(self, history_data, gated_history_data,
        ...
        if _DBG_DIFBLK:
            with torch.no_grad():
                print(f"[DBG-DIFBLK] st_conv_out  "
                      f"shape={list(hidden_states_dif.shape)}  "
                      f"norm={hidden_states_dif.norm().item():.4f}")
        ...
        # 算法改动: 缩放 backcast
        alpha = torch.sigmoid(self.skip_scale)
        backcast_seq = backcast_seq * alpha
        ...
        if _DBG_DIFBLK:
            with torch.no_grad():
                print(f"[DBG-DIFBLK] skip_scale={alpha.item():.4f}  "
                      f"backcast_norm={backcast_seq.norm().item():.4f}  "
