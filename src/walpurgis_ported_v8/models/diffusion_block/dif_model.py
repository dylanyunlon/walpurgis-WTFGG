...
class STLocalizedConv(nn.Module):
    def __init__(self, hidden_dim, pre_defined_graph=None,
        ...
        # 算法改动: gcn_updt 加 spectral normalization
        # 原版: 普通 Linear
        # 改为: spectral_norm(Linear), 约束 Lipschitz 常数 <= 1,
        #       防止图卷积在多层堆叠时梯度爆炸
        self.gcn_updt = P.spectral_norm(
            nn.Linear(self.hidden_dim * self.num_matric, self.hidden_dim))

        # 算法改动: unfold 后加 LayerNorm
        self.unfold_ln = nn.LayerNorm(self.k_t * hidden_dim)
    ...
    def forward(self, X, dynamic_graph, static_graph):
        ...
        if _DBG:
            with torch.no_grad():
                print(f"[DBG][STLocalizedConv] out shape={list(hidden.shape)}  "
                      f"mean={hidden.mean().item():.5f}  "
                      f"std={hidden.std().item():.5f}  "
                      f"support_count={len(support)}", flush=True)
        return hidden
