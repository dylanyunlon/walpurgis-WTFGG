"""Rift DynamicGraphConstructor — from upstream with debug hooks"""
...
class DynamicGraphConstructor(nn.Module):
    ...
    def forward(self, **inputs):
        ...
        if _RF_DBG:
            print(f"[RF-DBG:dy_graph] num_graphs={len(dynamic_graphs)} "
                  f"graph0_shape={dynamic_graphs[0].shape if dynamic_graphs else 'N/A'}",
