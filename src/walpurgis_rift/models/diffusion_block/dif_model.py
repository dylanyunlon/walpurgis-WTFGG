"""Rift dif_model: STLocalizedConv with grouped temporal convolution.
Rift splits the temporal convolution into groups, processes each independently,
then recombines — applying the split-recombine philosophy at the conv level."""
import torch
...
class STLocalizedConv(nn.Module):
    def __init__(self, hidden_dim, pre_defined_graph=None, use_pre=None, dy_graph=None, sta_graph=None, **model_args):
        ...
        n_groups = min(4, self.k_t * hidden_dim)
        while (self.k_t * hidden_dim) % n_groups != 0:
            n_groups -= 1
        self.fc_list_updt = nn.Linear(self.k_t * hidden_dim, self.k_t * hidden_dim, bias=False)
        self.group_proj = nn.Conv1d(self.k_t * hidden_dim, self.k_t * hidden_dim,
                                     kernel_size=1, groups=n_groups, bias=False)
    ...
    def forward(self, X, dynamic_graph, static_graph):
        ...
        BLN = batch_size * seq_len * num_nodes
        kt_d = kernel_size * num_feat
        out_flat = out.reshape(BLN, kt_d, 1)
        out_flat = self.group_proj(out_flat)
        out = out_flat.reshape(batch_size, seq_len, num_nodes, kt_d)
        ...
        if _RF_DBG:
            print(f"[RF-DBG:st_conv] out_shape={hidden.shape} norm={hidden.norm().item():.4f}",
