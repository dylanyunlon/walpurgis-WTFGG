"""Rift EstimationGate — from upstream with debug hooks"""
import torch
...
class EstimationGate(nn.Module):
    ...
    def forward(self, node_embedding_u, node_embedding_d, time_in_day_feat, day_in_week_feat, history_data):
        ...
        if _RF_DBG:
            print(f"[RF-DBG:est_gate] gate_mean={estimation_gate.mean().item():.4f} "
                  f"gate_std={estimation_gate.std().item():.4f}",
