...
def _adbg(tag, val):
    ...
    if isinstance(val, torch.Tensor):
        print(f"[AUR:dygraph:{tag}] shape={list(val.shape)} mean={val.mean().item():.6f} nnz_frac={((val.abs()>1e-6).float().mean().item()):.2%}", file=sys.stderr)

class DynamicGraphConstructor(nn.Module):
    """aurora: 增加可学习时间权重对k_t步加权"""
