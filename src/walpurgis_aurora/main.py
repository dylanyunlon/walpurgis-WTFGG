...
from walpurgis_aurora.utils.train import set_config, data_reshaper, save_model, EarlyStopping, get_num_params
from walpurgis_aurora.utils.load_data import load_dataset, load_adj
from walpurgis_aurora.utils.log import TrainLogger
from walpurgis_aurora.models.model import D2STGNN
from walpurgis_aurora.models.trainer import trainer
from walpurgis_aurora import snapshot_model, register_activation_hooks, gradient_health_check, _is_debug

def _adbg(tag, val):
    if os.environ.get('AURORA_DEBUG','0')!='1': return
    print(f"[AUR:main:{tag}] {val}", file=sys.stderr)
...
def main(config_path, device_str='cpu', epochs_override=None):
    ...
    print(f"Aurora D2STGNN: {total_params:,} trainable parameters")
    ...
    logger = TrainLogger('aurora', dataset_name)
    early_stop = EarlyStopping(patience=train_cfg.get('PATIENCE', 15))

    save_path = train_cfg.get('SAVE_PATH', f'checkpoints/aurora_{dataset_name.lower()}')
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_loss, val_mape, val_rmse = engine.eval(device, dataloader, 'aurora', **kwargs_eval)
