# walpurgis_aurora — D2STGNN Aurora variant
# 算法方向: Mixture-of-Experts gate, GroupNorm, Linear attention, LAMB优化器
import os, sys, torch

_AURORA_DEBUG = os.environ.get('AURORA_DEBUG', '0') == '1'

def _is_debug():
    return _AURORA_DEBUG

def _dbg(tag, tensor_or_val, module="aurora"):
    """统一调试桩: 打印tensor统计或标量值, NaN/Inf自动告警"""
    if not _AURORA_DEBUG:
        return
    if isinstance(tensor_or_val, torch.Tensor):
        ...
        msg = (f"[AUR:{tag}@{module}] shape={list(t.shape)} dtype={t.dtype} "
        ...
        msg = f"[AUR:{tag}@{module}] value={tensor_or_val}"
...
def snapshot_model(model, epoch=0, step=0):
    ...
    if not _AURORA_DEBUG:
        return
    print(f"\n[AUR] === Model Snapshot (epoch={epoch}, step={step}) ===", file=sys.stderr)
    params = []
    frozen_count = 0
    ...
    print(f"[AUR] total_params={total_params:,} frozen={frozen_count}", file=sys.stderr)
    print(f"[AUR] === End Snapshot ===\n", file=sys.stderr)
...
class _ActivationTracker:
    ...
    def report(self):
        print(f"\n[AUR] === Activation Report ({len(self.records)} layers) ===", file=sys.stderr)
...
def gradient_health_check(model):
    ...
    print(f"\n[AUR] === Gradient Health Check ===", file=sys.stderr)
...
def weight_diff(state_a, state_b, top_k=10):
    ...
    print(f"\n[AUR] === Weight Diff (top-{top_k}) ===", file=sys.stderr)
