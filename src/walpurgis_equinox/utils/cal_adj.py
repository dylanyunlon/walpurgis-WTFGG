...
def _edbg(tag, val):
    ...
    if isinstance(val, torch.Tensor):
        print(f"[EQX:adj:{tag}] shape={list(val.shape)} nnz={int((val.abs()>1e-6).sum().item())}", file=sys.stderr)
    else:
        print(f"[EQX:adj:{tag}] {val}", file=sys.stderr)
