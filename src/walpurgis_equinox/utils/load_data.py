...
from walpurgis_equinox.utils.cal_adj import get_adjacency_matrix, calc_adj_dtw, calc_adj_correlation

def _edbg(tag, val):
    if os.environ.get('EQUINOX_DEBUG','0')!='1': return
    if isinstance(val, torch.Tensor):
        print(f"[EQX:load:{tag}] shape={list(val.shape)} mean={val.mean().item():.4f}", file=sys.stderr)
    elif isinstance(val, np.ndarray):
        print(f"[EQX:load:{tag}] shape={list(val.shape)} mean={val.mean():.4f}", file=sys.stderr)
    else:
        print(f"[EQX:load:{tag}] {val}", file=sys.stderr)
...
class StandardScaler:
    def __init__(self, mean, std):
        self.mean = mean
        self.std = std
        _edbg("scaler_init", f"mean={mean:.4f} std={std:.4f}")
...
def load_dataset(dataset_dir, batch_size, valid_batch_size=None, test_batch_size=None,
    ...
    from walpurgis_equinox.dataloader import DataLoaderM

    cat_dim = kwargs.get('normalizer', 'std')
    _edbg("loading", f"dir={dataset_dir} batch={batch_size} dataset={dataset_name}")
    ...
    _edbg("x_train", x_train)
    _edbg("y_train", y_train)
    ...
    _edbg("loaded", f"train={x_train.shape} val={x_val.shape} test={x_test.shape}")
    return data
