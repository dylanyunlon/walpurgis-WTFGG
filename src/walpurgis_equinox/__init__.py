...
def _is_debug():
    return _EQUINOX_DEBUG

def _dbg(tag, tensor_or_val, module="equinox"):
    """统一调试桩: 打印tensor统计或标量值, NaN/Inf自动告警"""
    if not _EQUINOX_DEBUG:
        return
    if isinstance(tensor_or_val, torch.Tensor):
        ...
        msg = (f"[EQX:{tag}@{module}] shape={list(t.shape)} dtype={t.dtype} "
        ...
        msg = f"[EQX:{tag}@{module}] value={tensor_or_val}"
...
def snapshot_model(model, epoch=0, step=0):
    ...
    if not _EQUINOX_DEBUG:
        return
    print(f"\n[EQX] === Model Snapshot (epoch={epoch}, step={step}) ===", file=sys.stderr)
    params = []
    frozen_count = 0
    ...
    print(f"[EQX] total_params={total_params:,} frozen={frozen_count}", file=sys.stderr)
    print(f"[EQX] === End Snapshot ===\n", file=sys.stderr)
...
class _ActivationTracker:
    ...
    def report(self):
        print(f"\n[EQX] === Activation Report ({len(self.records)} layers) ===", file=sys.stderr)
...
def gradient_health_check(model):
    ...
    print(f"\n[EQX] === Gradient Health Check ===", file=sys.stderr)
...
def weight_diff(state_a, state_b, top_k=10):
    ...
    print(f"\n[EQX] === Weight Diff (top-{top_k}) ===", file=sys.stderr)
