"""Tempest residual decomposition: PowerNorm + Swish + exponential scaling.
Unlike upstream (LayerNorm+ReLU) and eclipse (RMSNorm+Mish+alpha),
Tempest uses PowerNorm (power-mean normalization with learnable exponent p),
...
class ResidualDecomp(nn.Module):
    """Tempest residual decomposition.
    ...
    def forward(self, x, y):
        ...
        if _TEM_DBG:
            print(f"[TEM:resdecomp@residual_decomp] res_norm={residual.norm().item():.4f} "
