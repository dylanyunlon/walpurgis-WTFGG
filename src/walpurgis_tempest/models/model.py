"""Tempest D2STGNN: NAS-style weighted sum layer aggregation, Swish+LayerNorm+residual
output, conditional PE on time-of-day.
Unlike upstream (simple sum) and eclipse (EMA decay weights, GELU+SpectralNorm),
Tempest uses DARTS-style architecture: learnable logits -> softmax -> weighted sum
for layer aggregation, letting the network learn optimal layer contributions.
Output uses Swish + LayerNorm + additive residual from input embedding."""
...
class DecoupleLayer(nn.Module):
    ...
    def forward(self, history_data, dynamic_graph, static_graph, node_embedding_u,
                node_embedding_d, time_in_day_feat, day_in_week_feat):
        ...
        if _TEM_DBG:
            ...
            print(f"[TEM:decouple@model] dif_energy={de:.4f} inh_energy={ie:.4f} ratio={de/max(ie,1e-8):.4f}", file=sys.stderr)
...
class D2STGNN(nn.Module):
    def __init__(self, **model_args):
        ...
        # NAS-style learnable logits for layer aggregation (vs upstream sum, eclipse EMA)
        ...
        # Output: Swish + LayerNorm + residual (vs upstream ReLU, eclipse GELU+SpectralNorm)
        self.out_fc_1 = nn.Linear(self._forecast_dim, self._output_hidden)
        self.out_ln_1 = nn.LayerNorm(self._output_hidden)
        self.out_fc_2 = nn.Linear(self._output_hidden, model_args['gap'])
        self.out_ln_2 = nn.LayerNorm(model_args['gap'])
        # Residual projection from input embedding to output
        self.residual_proj = nn.Linear(self._hidden_dim, self._forecast_dim)
    ...
    def forward(self, history_data):
        ...
        static_graph, dynamic_graph = self._graph_constructor(
            node_embedding_u=neu, node_embedding_d=ned, history_data=history_data,
            time_in_day_feat=tid, day_in_week_feat=diw)
        history_data = self.embedding(history_data)
        # Save input embedding for output residual
        input_emb = history_data
        # NAS-style weighted layer aggregation
        dif_weights = torch.softmax(self.layer_logits_dif, dim=0)
        inh_weights = torch.softmax(self.layer_logits_inh, dim=0)
        dif_list, inh_list = [], []
        inh_bc = history_data
        for idx, layer in enumerate(self.layers):
            inh_bc, dif_fk, inh_fk = layer(inh_bc, dynamic_graph, static_graph, neu, ned, tid, diw)
            dif_list.append(dif_fk * dif_weights[idx])
            inh_list.append(inh_fk * inh_weights[idx])
            if _TEM_DBG:
                print(f"[TEM:layer{idx}@model] backcast={list(inh_bc.shape)} "
                      f"dif_w={dif_weights[idx].item():.4f} inh_w={inh_weights[idx].item():.4f}", file=sys.stderr)
        forecast_hidden = sum(dif_list) + sum(inh_list)
        # Output: Swish + LayerNorm + residual from input
        residual = self.residual_proj(input_emb[:, -forecast_hidden.shape[1]:, :, :])
        forecast_hidden = forecast_hidden + 0.1 * residual  # scaled residual skip
        forecast = self.out_ln_2(self.out_fc_2(swish(self.out_ln_1(self.out_fc_1(swish(forecast_hidden))))))
        forecast = forecast.transpose(1, 2).contiguous().view(forecast.shape[0], forecast.shape[2], -1)
        if _TEM_DBG:
            print(f"[TEM:output@model] forecast={list(forecast.shape)} range=[{forecast.min().item():.4f},{forecast.max().item():.4f}] "
                  f"dif_logits={self.layer_logits_dif.detach().tolist()} inh_logits={self.layer_logits_inh.detach().tolist()}", file=sys.stderr)
        return forecast
