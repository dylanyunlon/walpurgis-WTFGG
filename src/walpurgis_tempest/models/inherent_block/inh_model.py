"""Tempest inherent model: MinGRU + Transformer with relative positional bias.
...
class RNNLayer(nn.Module):
    """Tempest RNN: MinGRU + GroupNorm for stable recurrence."""
    ...
    def forward(self, X):
        ...
        if _TEM_DBG:
            print(f"[TEM:mingru@inh_model] hidden_mean={hx.mean().item():.4f} std={hx.std().item():.4f}", file=sys.stderr)
        return output

class TransformerLayer(nn.Module):
    """Tempest Transformer with learnable relative positional bias.
    ...
    def forward(self, X, K, V):
        ...
        out = X + out
        if _TEM_DBG:
            print(f"[TEM:transformer@inh_model] attn_mean={attn.mean().item():.4f} rel_bias_range=[{self.rel_pos_bias.min().item():.4f},{self.rel_pos_bias.max().item():.4f}]", file=sys.stderr)
        return out
