"""Tempest inherent forecast: cosine annealing weighting for future steps.
Unlike upstream (uniform weight) and eclipse (exponential step decay),
Tempest weights future predictions with a cosine schedule, providing
...
class Forecast(nn.Module):
    ...
    def forward(self, X, RNN_H, Z, transformer_layer, rnn_layer, pe):
        ...
        if _TEM_DBG:
            print(f"[TEM:forecast@inh_forecast] steps={predict.shape[1]} range=[{predict.min().item():.4f},{predict.max().item():.4f}]", file=sys.stderr)
        return predict
