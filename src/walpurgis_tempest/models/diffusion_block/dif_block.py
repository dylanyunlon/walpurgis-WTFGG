"""Tempest DifBlock: cross-attention backcast mechanism.
Unlike upstream (single FC backcast) and eclipse (2-layer MLP + sigmoid gate),
Tempest uses cross-attention where query=backcast, key/value=forecast hidden states.
...
class DifBlock(nn.Module):
    ...
    def forward(self, history_data, gated_history_data, dynamic_graph, static_graph):
        ...
        if _TEM_DBG:
            print(f"[TEM:difblk@dif_block] bc_e={bc.norm().item():.4f} fk_e={fk.norm().item():.4f} cross_attn_shape={list(bc.shape)}", file=sys.stderr)
        return res, fk
