"""Tempest STLocalizedConv: SpectralNorm + LeakyReLU + Chebyshev polynomial aggregation.
Unlike upstream (BatchNorm+ReLU, simple k-power graph) and eclipse (InstanceNorm+GELU+skip),
Tempest uses spectral normalization for training stability, LeakyReLU for avoiding dead
...
class STLocalizedConv(nn.Module):
    ...
    def gconv(self, support, X_k, X_0):
        ...
        if _TEM_DBG:
            print(f"[TEM:gconv@dif_model] out={list(out.shape)} cheb_coeffs={torch.softmax(self.cheb_coeffs,dim=0).detach().tolist()}", file=sys.stderr)
        return out
    ...
    def forward(self, X, dynamic_graph, static_graph):
        ...
        if _TEM_DBG:
            print(f"[TEM:forward@dif_model] hidden={list(hidden.shape)} range=[{hidden.min().item():.4f},{hidden.max().item():.4f}]", file=sys.stderr)
        return hidden
