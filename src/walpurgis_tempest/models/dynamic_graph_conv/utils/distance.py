"""Tempest distance: Mahalanobis distance with learnable covariance.
Unlike upstream (scaled dot-product attention) and eclipse (cosine similarity + bias),
Tempest uses a learnable Mahalanobis metric: d(q,k) = (q-k)^T M (q-k) where M = L^T L
...
class DistanceFunction(nn.Module):
    ...
    def forward(self, X, E_d, E_u, T_D, D_W):
        ...
        if _TEM_DBG:
            L = torch.tril(self.L_raw)
            print(f"[TEM:distance@distance] L_norm={L.norm().item():.4f} adj_mean={adj_list[0].mean().item():.6f} "
