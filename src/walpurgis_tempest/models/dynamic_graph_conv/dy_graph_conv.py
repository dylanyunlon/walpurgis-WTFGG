"""Tempest dynamic graph constructor: Mahalanobis + Sinkhorn + Bernoulli mask."""
...
class DynamicGraphConstructor(nn.Module):
    def __init__(self, **model_args):
        ...
        self.normalizer = Normalizer(num_iters=5)  # Sinkhorn with 5 iterations
    ...
    def forward(self, **inputs):
        dist_mx = self.distance_function(inputs['history_data'], inputs['node_embedding_d'],
                                          inputs['node_embedding_u'], inputs['time_in_day_feat'],
                                          inputs['day_in_week_feat'])
        dist_mx = self.mask(dist_mx)
        dist_mx = self.normalizer(dist_mx)
        mul_mx = self.multi_order(dist_mx)
        dg = self.st_localization(mul_mx)
        if _TEM_DBG:
            print(f"[TEM:dygraph@dy_graph_conv] n_graphs={len(dg)} sparsity={(dg[0]==0).float().mean().item():.2%}", file=sys.stderr)
        return dg
