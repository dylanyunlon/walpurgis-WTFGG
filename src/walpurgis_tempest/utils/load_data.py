"""Tempest load_data: robust scaler with eps-guard + NaN detection."""
...
def load_dataset(data_dir, batch_size, valid_batch_size, test_batch_size, dataset_name):
    ...
    for mode in ['train', 'val', 'test']:
        ...
        x = d['x']; y = d['y']
        # NaN detection with percentile info
        nan_x = np.isnan(x).sum(); nan_y = np.isnan(y).sum()
        if nan_x > 0 or nan_y > 0:
            print(f"[TEM:load_data] WARNING NaN in {mode}: x={nan_x} y={nan_y}", file=sys.stderr)
            x = np.nan_to_num(x, nan=0.0); y = np.nan_to_num(y, nan=0.0)
        data_dict['x_' + mode] = x; data_dict['y_' + mode] = y
        if _TEM_DBG:
            print(f"[TEM:load_data] {mode}: x={x.shape} y={y.shape} "
                  f"x_range=[{x.min():.2f},{x.max():.2f}] "
                  f"x_mean={x.mean():.4f} x_std={x.std():.4f}", file=sys.stderr)
...
def load_adj(file_path, adj_type):
    ...
    if _TEM_DBG:
        print(f"[TEM:load_adj] type={adj_type} shape={adj[0].shape} "
              f"density={(np.array(adj[0])!=0).mean():.2%}", file=sys.stderr)
    return adj, adj_mx
