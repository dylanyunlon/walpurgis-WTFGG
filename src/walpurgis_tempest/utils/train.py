"""Tempest train utils: SplitMix64 seed derivation, trend+curvature EarlyStopping, dtype-safe reshaper.
Unlike upstream (direct seed, patience-only stopping) and eclipse (Knuth hash, trend-slope stopping),
Tempest uses SplitMix64 for high-quality sub-seed derivation and adds curvature (2nd derivative)
...
def set_config(seed_val=0):
    ...
    if _TEM_DBG:
        print(f"[TEM:set_config@train] seed={seed_val} torch={s_torch} "
...
class EarlyStopping:
    """Tempest EarlyStopping: trend + curvature (2nd derivative) analysis.
    Unlike upstream (patience-only) and eclipse (trend slope triggering at half patience),
    Tempest monitors both loss trend slope AND curvature (2nd derivative of the loss curve).
    ...
    def _trend_and_curvature(self):
        ...
        if len(y) >= 4:
            ...
            curvature = 0.0
        if _TEM_DBG:
            print(f"[TEM:earlystop@train] slope={slope:.6f} curvature={curvature:.6f} "
