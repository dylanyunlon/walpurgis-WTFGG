"""Tempest log: JSONL + CSV dual dump with rolling statistics window."""
import time, os, json, csv, sys, numpy as np
...
class TrainLogger:
    def __init__(self, model_name, dataset):
        ...
        self._rolling = []  # rolling window for stats
        ...
        print(f"[TEM:log] dir={self.log_dir}", file=sys.stderr)

    def log_metrics(self, epoch, **metrics):
        record = {"epoch": epoch, "timestamp": time.time(), **metrics}
        # Rolling statistics
        self._rolling.append(metrics)
        if len(self._rolling) > 10: self._rolling = self._rolling[-10:]
        if len(self._rolling) >= 3:
            for key in metrics:
                if isinstance(metrics[key], (int, float)):
                    vals = [r[key] for r in self._rolling if key in r and isinstance(r[key], (int, float))]
                    if vals:
