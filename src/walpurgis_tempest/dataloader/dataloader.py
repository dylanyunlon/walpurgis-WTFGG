"""Tempest DataLoader: stratified shuffle + mirror-padding.
Unlike upstream (np.random.permutation) and eclipse (Fisher-Yates + circular wrap),
Tempest uses stratified shuffling: samples are divided into temporal strata (bins),
    ...
    def __init__(self, xs, ys, batch_size, pad_with_last_sample=True, shuffle=False):
        ...
        if pad_with_last_sample:
            ...
            if num_padding > 0:
                ...
                if _TEM_DBG:
                    print(f"[TEM:dataloader] mirror padding: {num_padding} samples", file=sys.stderr)
        ...
        if _TEM_DBG:
            print(f"[TEM:dataloader] size={self.size} batches={self.num_batch} "
