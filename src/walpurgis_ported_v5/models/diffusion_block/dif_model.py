...
# Delta vs upstream:
#   1. gconv: added residual connection (out += X_0)
#   2. Activation: ReLU → SiLU in the temporal projection
#   3. get_graph: mask uses soft thresholding (eps=1e-6) instead of binary
...
class STLocalizedConv(nn.Module):
    def __init__(self, hidden_dim, pre_defined_graph=None,
        ...
        self.activation = nn.SiLU()     # delta 2: SiLU replaces ReLU
    ...
    def gconv(self, support, X_k, X_0):
        ...
        # ── delta 1: residual from mean input ──
        out = out + X_0
        return out

    def get_graph(self, support):
        graph_ordered = []
        # ── delta 3: soft threshold instead of hard binary mask ──
    ...
    def forward(self, X, dynamic_graph, static_graph):
        ...
        out = self.activation(out)      # delta 2: SiLU
