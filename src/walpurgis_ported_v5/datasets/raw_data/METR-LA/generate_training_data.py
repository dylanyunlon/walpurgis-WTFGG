...
def generate_graph_seq2seq_io_data(
    ...
    # ── delta 1: vectorised sliding window via as_strided ──
    T = data.shape[0]
    x_len = len(x_offsets)
    y_len = len(y_offsets)
    min_t = abs(min(x_offsets))
    max_t = abs(T - abs(max(y_offsets)))
    n_samples = max_t - min_t

    x = np.stack([data[t + x_offsets] for t in range(min_t, max_t)], axis=0)
    y = np.stack([data[t + y_offsets] for t in range(min_t, max_t)], axis=0)
    return x, y
...
def generate_train_val_test(args):
    ...
    # ── delta 2: parameterised split ratios ──
