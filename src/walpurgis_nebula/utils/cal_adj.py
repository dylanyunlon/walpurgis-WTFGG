"""Nebula cal_adj: adjacency matrix utilities."""
...
def calculate_symmetric_normalized_laplacian(adj):
    ...
    return sp.eye(adj.shape[0]) - matrix_D_inv_sqrt.dot(adj).dot(matrix_D_inv_sqrt).tocoo()
...
def symmetric_message_passing_adj(adj):
    ...
    return d_mat_inv_sqrt.dot(adj).transpose().dot(d_mat_inv_sqrt).astype(np.float32).todense()
