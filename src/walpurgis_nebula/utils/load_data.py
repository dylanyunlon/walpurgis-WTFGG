"""Nebula load_data: eps-guarded scaler + NaN detection."""
...
class StandardScaler:
    def __init__(self, mean, std):
        self.mean = mean; self.std = max(std, 1e-8)
...
def load_dataset(data_dir, batch_size, valid_batch_size, test_batch_size, dataset_name):
    ...
    for mode in ['train', 'val', 'test']:
        ...
        if nan_x > 0 or nan_y > 0:
            print(f"[NEB:load_data] WARNING NaN in {mode}: x={nan_x} y={nan_y}", file=sys.stderr)
            x = np.nan_to_num(x, nan=0.0); y = np.nan_to_num(y, nan=0.0)
        data_dict['x_' + mode] = x; data_dict['y_' + mode] = y
        if _NEB_DBG: print(f"[NEB:load_data] {mode}: x={x.shape} y={y.shape} x_range=[{x.min():.2f},{x.max():.2f}]", file=sys.stderr)
...
def load_adj(file_path, adj_type):
    ...
    if _NEB_DBG: print(f"[NEB:load_adj] type={adj_type} shape={adj[0].shape}", file=sys.stderr)
    return adj, adj_mx
