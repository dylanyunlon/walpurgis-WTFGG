"""walpurgis_nebula — D2STGNN Nebula variant.
Algorithms: Log-Cosh + quantile loss, capsule routing gate, BatchNorm+ELU decomp,
GroupNorm+Hardswish depthwise-sep conv, highway backcast, hyperbolic distance,
top-k sparse mask, degree-aware norm, IndRNN+flash-attn, Fourier PE, DenseNet agg,
Maxout output, LAMB optimizer + poly decay, Levy flight synth, importance sampling."""
import os, sys, torch

_NEBULA_DEBUG = os.environ.get('NEBULA_DEBUG', '0') == '1'

def _is_debug():
    return _NEBULA_DEBUG

def _dbg(tag, tensor_or_val, module="nebula"):
    if not _NEBULA_DEBUG:
        return
    if isinstance(tensor_or_val, torch.Tensor):
        t = tensor_or_val.detach().float()
        if t.numel() == 0:
            print(f"[NEB:{tag}@{module}] EMPTY shape={list(t.shape)}", file=sys.stderr)
            return
        nan_ct = torch.isnan(t).sum().item()
        inf_ct = torch.isinf(t).sum().item()
        sp = (t == 0).float().mean().item()
        msg = (f"[NEB:{tag}@{module}] shape={list(t.shape)} dtype={t.dtype} "
        ...
        msg = f"[NEB:{tag}@{module}] value={tensor_or_val}"
    print(msg, file=sys.stderr)

def snapshot_model(model, epoch=0, step=0):
    if not _NEBULA_DEBUG: return
    print(f"\n[NEB] === Model Snapshot (epoch={epoch}, step={step}) ===", file=sys.stderr)
    recs = []
...
class _ActivationTracker:
    ...
    def _hook(self, name):
        ...
        return fn
    def report(self):
        print(f"\n[NEB] === Activation Report ({len(self.records)} layers) ===", file=sys.stderr)
...
def gradient_health_check(model):
    if not _NEBULA_DEBUG: return
    print(f"\n[NEB] === Gradient Health Check ===", file=sys.stderr)
    issues = 0
    for name, p in model.named_parameters():
    ...
    print(f"[NEB] === End Check ({issues} issues) ===\n", file=sys.stderr)
...
def weight_diff(state_a, state_b, top_k=5):
    ...
    print(f"\n[NEB] === Weight Diff (top {top_k}) ===", file=sys.stderr)
