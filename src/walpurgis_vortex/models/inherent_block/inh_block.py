"""Vortex InhBlock: conditional PE (conditioned on time-of-day), MinGRU, relative-bias Transformer.
Unlike upstream (static sinusoidal PE) and eclipse (learnable phase offset PE),
Vortex conditions the positional encoding on time-of-day features, making the PE
...
class InhBlock(nn.Module):
    ...
    def forward(self, hidden_inherent_signal):
        ...
        if _VX_DBG:
            print(f"[VX:inhblk@inh_block] merge_gate_mean={merge_gate.mean().item():.4f} fk_shape={list(fk.shape)}", file=sys.stderr)
        return bc_res, fk
