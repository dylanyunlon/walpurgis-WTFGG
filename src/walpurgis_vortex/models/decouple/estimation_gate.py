"""Vortex gate: Squeeze-Excitation with spatial attention.
Unlike upstream (2-layer FC+ReLU+sigmoid) and eclipse (3-layer FC+Swish+ChannelSE+tau),
Vortex uses a full SE block with explicit spatial (node-dim) attention pooling and a
...
class EstimationGate(nn.Module):
    """Vortex estimation gate: SE with spatial attention.
    Compared to upstream (simple FC+ReLU+sigmoid) and eclipse (Swish+ChannelSE+tau),
    Vortex adds explicit spatial attention across nodes and uses Hardswish activation."""
    ...
    def forward(self, node_embedding_u, node_embedding_d, time_in_day_feat, day_in_week_feat, history_data):
        ...
        if _VX_DBG:
            print(f"[VX:gate@estimation_gate] mean={gate.mean().item():.4f} std={gate.std().item():.4f} tau={tau.item():.4f}", file=sys.stderr)
