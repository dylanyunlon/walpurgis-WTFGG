"""Vortex dynamic graph constructor: Mahalanobis + Sinkhorn + Bernoulli mask."""
...
class DynamicGraphConstructor(nn.Module):
    ...
    def forward(self, **inputs):
        ...
        if _VX_DBG:
            print(f"[VX:dygraph@dy_graph_conv] n_graphs={len(dg)} sparsity={(dg[0]==0).float().mean().item():.2%}", file=sys.stderr)
        return dg
