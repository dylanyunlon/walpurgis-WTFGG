"""Vortex normalizer: doubly stochastic via Sinkhorn normalization + MultiOrder.
Unlike upstream (row-normalization D^{-1}A) and eclipse (symmetric D^{-1/2}AD^{-1/2}),
Vortex uses Sinkhorn iterations to produce doubly stochastic matrices where both
...
class Normalizer(nn.Module):
    ...
    def _sinkhorn_norm(self, graph):
        ...
        if _VX_DBG:
            ...
            print(f"[VX:sinkhorn@normalizer] row_dev={row_dev:.6f} col_dev={col_dev:.6f} "
