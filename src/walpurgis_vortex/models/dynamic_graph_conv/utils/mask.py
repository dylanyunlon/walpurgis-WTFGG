"""Vortex mask: straight-through Bernoulli (hard in forward, soft in backward).
Unlike upstream (fixed binary mask from adj) and eclipse (sigmoid soft-gating),
Vortex uses a straight-through estimator with Bernoulli sampling: the forward pass
...
class Mask(nn.Module):
    """Vortex mask: straight-through Bernoulli.
    ...
    def forward(self, adj):
        ...
        if _VX_DBG:
            ...
            print(f"[VX:mask@mask] sparsity={(result[0]==0).float().mean().item():.2%} "
