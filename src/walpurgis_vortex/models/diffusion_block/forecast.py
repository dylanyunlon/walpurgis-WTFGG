"""Vortex diffusion forecast: linear interpolation padding + GroupNorm.
Unlike upstream (last-sample repeat, no norm) and eclipse (reflect padding, LayerNorm),
Vortex uses linear interpolation for smoother padding transitions and GroupNorm
...
class Forecast(nn.Module):
    ...
    def forward(self, gated_history_data, hidden_states_dif, localized_st_conv, dynamic_graph, static_graph):
        ...
        if _VX_DBG:
            print(f"[VX:forecast@dif_forecast] steps={proj.shape[1]} range=[{proj.min().item():.4f},{proj.max().item():.4f}]", file=sys.stderr)
        return proj
