"""
walpurgis_vortex — D2STGNN Vortex变体
算法改写 (~20%):
  - EMA融合门控: dif/inh分支通过动量指数移动平均混合
  - 随机深度 (stochastic depth): 训练时以概率p跳过某些decouple层
  - 温度缩放聚合: 各层forecast hidden用可学习温度做softmax加权
  - Mish激活 + GroupNorm输出头 + 双路输出(主路+辅助路)
  - Cosine Annealing Warm Restarts调度器
  - 梯度噪声注入 (gradient noise injection)
  - Huber-MAE自适应混合损失

全局调试: VORTEX_DEBUG=1 开启
诊断日志: VORTEX_DIAG_LOG=<path> 写入JSONL
"""
import os
import sys
import time
...
def _diag_write(record: dict):
    """写入诊断记录到JSONL（若设置了VORTEX_DIAG_LOG）"""
...
def snapshot_model(model, epoch=0, step=0, top_k=10):
    ...
    if not _is_debug():
        return
    print(f"\n{'='*70}", file=sys.stderr)
    print(f"[VX-SNAPSHOT] epoch={epoch} step={step} "
...
class ActivationTracker:
    ...
    def report(self, top_k=12):
        print(f"\n[VX-ACTIVATION] {len(self.stats)} layers tracked:",
              file=sys.stderr)
        items = []
...
def gradient_health_check(model, explode_thresh=70.0,
    ...
    if issues and _is_debug():
        print("[VX-GRAD] Issues detected:", file=sys.stderr)
        for iss in issues:
...
def gradient_histogram(model, n_bins=10):
    ...
    if not all_grads:
        print("[VX-GRAD-HIST] No gradients computed.",
              file=sys.stderr)
        return
    ...
    print(f"[VX-GRAD-HIST] log10 gradient distribution:",
          file=sys.stderr)
    for i in range(n_bins):
...
def weight_diff(state_a, state_b, top_k=6):
    ...
    print(f"\n[VX-WEIGHT-DIFF] top-{top_k} changed:",
          file=sys.stderr)
...
def dump_struct_state(label, **kwargs):
    ...
    for k, v in kwargs.items():
        if isinstance(v, torch.Tensor):
            ...
            print(f"  {k:30s}: {type(v).__name__}",
                  file=sys.stderr)
    print(f"[VX-STRUCT:{label}] === End Dump ===\n",
          file=sys.stderr)
    _diag_write(record)
...
# ─── 学习率追踪 (Vortex特有) ────────────────────────────
class LRTracker:
    """记录每个step的学习率变化, 用于验证cosine schedule"""
    def __init__(self):
        self.history = []
    ...
    def record(self, step, lr):
        ...
        if _is_debug() and len(self.history) % 50 == 0:
            print(f"[VX-LR] step={step} lr={lr:.8f}",
                  file=sys.stderr)
