"""Vortex cal_adj: enhanced numerical diagnostics with percentile reporting."""
...
def check_nan_inf(tensor, raise_ex=True):
    ...
    if _VX_DBG and (nan or inf):
        nan_ct = torch.isnan(tensor).sum().item()
        inf_ct = torch.isinf(tensor).sum().item()
        total = tensor.numel()
        print(f"[VX:cal_adj] NaN={nan_ct}/{total} ({nan_ct/total:.2%}) "
        ...
        if valid.numel() > 0:
            ...
            print(f"[VX:cal_adj] percentiles: p1={q[0]:.4f} p25={q[1]:.4f} "
