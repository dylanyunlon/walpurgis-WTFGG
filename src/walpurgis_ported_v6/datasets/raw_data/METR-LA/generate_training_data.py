"""METR-LA training data generator — cyclic temporal encoding + variance-aware split.

Algorithm changes
-----------------
1. **Cyclic time encoding** — upstream encodes time-in-day as a linear
   fraction [0,1).  This creates a discontinuity at midnight (0.999→0.0).
   We encode it as a (sin, cos) pair: ``sin(2π·t/288), cos(2π·t/288)``.
   Same for day-of-week: (sin, cos) of 2π·d/7.
   The feature dimension goes from 3 (value, tid, dow) to 5
   (value, tid_sin, tid_cos, dow_sin, dow_cos).

2. **Variance-aware splitting** — instead of a strict 70/10/20 temporal
   cut, we compute per-sample variance and ensure each split has a
   similar variance distribution.  This prevents the common failure
   mode where the test set happens to contain only low-variance
   (nighttime) periods, inflating reported accuracy.
"""
...
def _cyclic_encode(values, period):
    """Encode a periodic signal as (sin, cos) pair."""
...
def generate_graph_seq2seq_io_data(
    ...
    if add_time_in_day:
        ...
        # cyclic: sin/cos instead of linear fraction
...
def _variance_aware_split(x, y, train_r=0.7, val_r=0.1):
    """Split with approximate variance stratification.

    Compute per-sample variance, sort into quantile buckets, and assign
    each bucket's samples to train/val/test in the target ratio.
    Temporal order is preserved *within* each bucket.
    """
    n = x.shape[0]
    # per-sample variance (over nodes and time)
    ...
    for b in range(n_buckets):
        idx = np.where(bucket_idx == b)[0]
        # preserve temporal order within bucket
        idx = np.sort(idx)
        nb = len(idx)
