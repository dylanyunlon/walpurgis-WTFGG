"""PEMS04 training data generator — robust normalization + cyclic encoding.

Algorithm changes
-----------------
1. **Robust MinMax normalization** — instead of using raw min/max (which
   can be skewed by a single sensor's spike), we use the 1st and 99th
   percentiles as the normalisation bounds.  Values outside the bounds
   are clipped, not extrapolated.

2. **Cyclic temporal encoding** — same as METR-LA: sin/cos pairs for
   time-in-day and day-of-week, replacing linear fractions.

3. **Variance-aware split** — 60/20/20 with stratified variance buckets.
"""
...
def RobustMinMaxNormalization(train, val, test, lo_pct=1, hi_pct=99):
    """Use percentile bounds instead of raw min/max."""
    assert train.shape[1:] == val.shape[1:] == test.shape[1:]

    # percentile-based bounds (robust to outlier spikes)
    _min = np.percentile(train, lo_pct, axis=(0, 1, 3), keepdims=True)
    _max = np.percentile(train, hi_pct, axis=(0, 1, 3), keepdims=True)

    spread = _max - _min
    spread[spread < 1e-6] = 1.0     # guard against flat signals

    print(f'  [robust] p{lo_pct} range: {_min.min():.2f} – {_max.max():.2f}')

    def normalize(x):
        x = np.clip(x, _min, _max)   # clip outliers
        x = (x - _min) / spread
        return 2.0 * x - 1.0
