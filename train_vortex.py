"""
train_vortex.py — Vortex训练入口
从项目根目录运行: python train_vortex.py --dataset SYNTH --debug

算法特性 (vs upstream):
  - EMA动量融合门控 (decouple层dif/inh混合)
  - 随机深度 (stochastic depth, 线性增长跳过概率)
  - 温度缩放聚合 (可学习temperature做softmax加权)
  - Mish激活 + GroupNorm输出头 + 双路输出
...
from walpurgis_vortex.utils.load_data import (
    load_dataset, load_adj)
from walpurgis_vortex.utils.log import TrainLogger
from walpurgis_vortex.models.losses import (
    masked_mae, masked_rmse, masked_mape, metric)
from walpurgis_vortex.models import trainer
from walpurgis_vortex.models.model import D2STGNN
...
def run(dataset, device_str='cpu', epochs_override=None,
    ...
    config_dir = os.path.join(
        _REPO_ROOT, 'src', 'walpurgis_vortex', 'configs')
    ...
    if not os.path.exists(data_dir):
        vx_dir = os.path.join(
            _REPO_ROOT, 'src', 'walpurgis_vortex')
        data_dir = _resolve_path(
            config['data_args']['data_dir'], vx_dir)
        adj_path = _resolve_path(
            config['data_args']['adj_data_path'], vx_dir)
    ...
    print(f"\n{'='*60}")
    print(f"  Vortex Training Pipeline")
    ...
    print(f"\n[VX] Pipeline complete. Saved: {save_path}")
    return early_stopping.val_loss_min


def main():
    parser = argparse.ArgumentParser(
        description='Vortex (D2STGNN) Training')
