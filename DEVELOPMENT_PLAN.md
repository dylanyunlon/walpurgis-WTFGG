...
| **#6** | M011–M012 | ✅ DONE (committed) — IntervalIndex + indexed temporal queries + benchmark. |
| **#7** | M013–M014 | ✅ DONE (+ self-review hardening) — **Partition-selection interval index**. M006 fixed the *intra*-partition scan but query_partitions still tested every partition: O(P) per query, which dominates under streaming as P grows. Added `src/core/partition_skiplist.hpp` — a skip list ordered by ts_lo, augmented with per-forward-pointer span-max(ts_hi), the skip-list analogue of an interval tree's subtree-max (CLRS §14.3). `overlaps()` prunes entire runs whose span-max < lo → O(log P + k). To avoid an O(N²log N) rebuild cliff under streaming, the bridge holds a `SegmentedPartitionIndex` (LSM/Lucene pattern): each flush appends ONE immutable segment over only the new partitions (O(M log M)), with threshold-triggered compaction. query_partitions runs the segmented walk; `query_partitions_linear(…, with_touch)` is retained as the runtime oracle. **Correctness**: 10M exhaustive + 32k random/adversarial skip-list cross-checks, 20k three-way (segmented/whole/brute) cross-checks, 900/900 runtime samples (compared by collision-free alloc_id) — all match brute force. TSan: 2.1M concurrent queries with live segment rebuild+compaction, no races/crashes. **Corrected performance** (fair comparison, both paths touch() every hit): narrow 4.1×, medium 1.75×, wide 0.85×. The index is at worst ~15% slower (wide) and up to 4× faster; it does NOT collapse 5× as a first, flawed measurement suggested (that earlier number charged N touch() calls solely to the index — a benchmark artifact, since touch() cost is identical on both paths). Pattern: Pugh skip list + CLRS interval-tree augmentation + LSM segmented index. Files: partition_skiplist.hpp (SkipList + SegmentedPartitionIndex), skiplist_selftest.cpp, partition_index_bench.cpp, philemon_partition_index_2000.json. |

> Self-review record (Claude #7, "Knuth-standard" pass — see REVIEW_M013_M014.md):
> Six issues were raised against the first M013–M014 commit and resolved before this state:
> S1 (streaming O(N²log N) rebuild) → SegmentedPartitionIndex incremental segments + compaction.
> S2 (benchmark validated via a hashable key that could collide → possible false "pass") → compare by unique alloc_id; samples raised 90→900, all pass.
> S3 (seqlock redundant under part_mu_; M007 "wait-free read" overstated) → documented honestly; seqlock retained only for migration metadata, no behavioral claim of lock-freedom for the index.
> S4 (public rebuild self-took a non-recursive lock → self-deadlock risk) → split into self-locking public methods and `_locked` internals with an explicit contract.
> S5 (index_ready_ a plain bool, safe only by accident) → `std::atomic<uint64_t> index_epoch_`.
> S6 (believed wide queries degraded 5×, added a width-ratio fallback) → DISPROVEN by profiling (pure selection: 3.7µs indexed vs 8.0µs linear even when wide). The slowdown was the S2 touch() artifact. The fallback was removed rather than kept — no heuristic, just the index. A genuine cost model for the intra-partition scan stays planned as M047.
