...
| **#7** | M013–M014 | ✅ DONE — **Partition-selection interval index**. M006 fixed the *intra*-partition scan but query_partitions still tested every partition: O(P) per query, which dominates under streaming as P grows. Added `src/core/partition_skiplist.hpp` — a skip list ordered by ts_lo, augmented with per-forward-pointer span-max(ts_hi), the skip-list analogue of an interval tree's subtree-max (CLRS §14.3). `overlaps()` prunes entire runs whose span-max < lo → O(log P + k). Wired into query_partitions; the old linear path kept as `query_partitions_linear` (runtime oracle). Validated: skiplist_selftest (32k random + adversarial nested-interval queries) and M014 bench (90/90 runtime samples at P=8000) both match brute force. Honest measured trade-off: narrow 7.0×, medium 1.3×, wide ~0.2× (index loses when a query touches nearly every partition). Pattern: Pugh skip list + CLRS interval-tree augmentation. Files: partition_skiplist.hpp, skiplist_selftest.cpp, partition_index_bench.cpp, philemon_partition_index_2000.json. |

> Note (Claude #7): the original plan slotted M013–M014 as the "RapidStore bridge". That work is deferred to a later milestone — the inter-partition O(P) selection was a more pressing, already-present bottleneck (the natural follow-on to M006), so M013–M014 now denote the partition skip-list index. RapidStore snapshot integration remains open for a future Claude.
