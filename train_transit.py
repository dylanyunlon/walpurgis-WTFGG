"""
train_transit.py — Transit训练入口 (M055)
从项目根目录运行: python train_transit.py --dataset SYNTH --debug
...
from walpurgis_transit.utils.train import (
    set_config, EarlyStopping, data_reshaper, load_model)
from walpurgis_transit.utils.load_data import (
    load_dataset, load_adj)
from walpurgis_transit.utils.log import TrainLogger
from walpurgis_transit.models.losses import (
    masked_mae, masked_rmse, masked_mape, metric)
from walpurgis_transit.models.trainer import trainer
from walpurgis_transit.models.model import D2STGNN
from walpurgis_transit import (
...
def run(dataset, device_str='cpu', epochs_override=None,
    ...
    config_dir = os.path.join(
        _REPO_ROOT, 'src', 'walpurgis_transit', 'configs')
    ...
    if not os.path.exists(data_dir):
        trn_dir = os.path.join(_REPO_ROOT, 'src', 'walpurgis_transit')
        data_dir = _resolve_path(config['data_args']['data_dir'], trn_dir)
        adj_path = _resolve_path(config['data_args']['adj_data_path'], trn_dir)
    ...
    print(f"\n{'='*60}")
    print(f"  Transit (D2STGNN) Training Pipeline — M055")
    ...
    print(f"\n[TRN] Pipeline complete. Saved: {save_path}")
    return early_stopping.val_loss_min


def main():
    parser = argparse.ArgumentParser(description='Transit (D2STGNN) Training — M055')
