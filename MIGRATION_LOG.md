...
## migrate 3f8dddf: [SKIP] resolve merge conflict — merge commit，无迁移价值

- **Upstream commit**: 3f8dddf (cugraph-gnn, Alexandria Barghi, 2025-06-09)
- **Commit message**: `resolve merge conflict`
- **Commit type**: MERGE (d491fae ← 78d3f72)
- **变更内容**:
  - `README.md` +5行: 保留合并冲突标记 `<<<<<<< HEAD / ======= / >>>>>>>` 原文（cuGraph-DGL deprecation 说明段），系 git conflict marker 未清理的 merge artifact
  - `datasets/karate.csv` 删除（156行纯数值 edge-list 数据文件）
- **跳过原因**: 纯 merge commit，diff 内容为遗留冲突标记与数据文件清理，零 Python/C++/CUDA 运行时代码变更，无任何可迁移算法内容
...
## migrate b89f57d: [SKIP] Enable device code compression — 纯 CMake C++ 构建配置，Walpurgis 无 C++ 编译体系
