...
## migrate 92c67a9: [SKIP] use 'rapids-init-pip' in wheel CI, other CI changes (#212) — 纯 CI/workflows 变更，无迁移价值

- **Upstream commit**: 92c67a9 (cugraph-gnn, James Lamb, 2025-05-22, PR #212)
- **跳过原因**: 10 个文件全为 GitHub Actions workflows 和 CI shell 脚本：
  - `.github/workflows/build.yaml`、`pr.yaml`、`test.yaml` — 工作流配置调整（script 参数化、triggers 修改）
  - `ci/build_wheel_cugraph-dgl.sh`、`ci/build_wheel_cugraph-pyg.sh`、`ci/build_wheel_libwholegraph.sh`、`ci/build_wheel_pylibwholegraph.sh` — 新增 `rapids-init-pip` 调用、移除 `PIP_CONSTRAINT` 重定义
  - `ci/test_wheel_cugraph-dgl.sh`、`ci/test_wheel_cugraph-pyg.sh`、`ci/test_wheel_pylibwholegraph.sh` — 同上
  - 变更描述：用 `rapids-init-pip` 统一 pip 初始化；明确传入 `script` 参数替代 workflow 默认值；阻止版本脚本修改触发 CI
  - Walpurgis 无 GitHub Actions CI 体系、无 RAPIDS wheel 构建流程、无 `rapids-init-pip` 工具依赖，受影响的所有文件在 Walpurgis 仓库中均不存在。
