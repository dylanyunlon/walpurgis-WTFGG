...
## migrate 0a4c8af: [SKIP] Update to 26.06.00 — 版本号 bump，全为 CI/devcontainer/conda 配置文件

- **Commit**: `0a4c8af8c0f86d1e7deff1d9aaac3e03f2b8c918`
- **Commit message**: `Update to 26.06.00`
- **Author**: github-actions[bot] — 2025-11-22

- **Context**: 自动化版本 bump：`VERSION`、`.github/workflows/build.yaml`、`pr.yaml`、4 个 devcontainer JSON、4 个 conda env yaml、`dependencies.yaml`、3 个 `pyproject.toml` 版本字段统一升至 26.06.00。

- **Upstream diff**（20 files changed）:
  | 文件 | 变更内容 |
  |------|----------|
  | `VERSION` | 26.04.00 → 26.06.00 |
  | `.devcontainer/*/devcontainer.json` | 镜像 tag 版本 bump |
  | `.github/workflows/*.yaml` | 版本引用更新 |
  | `conda/environments/*.yaml` | 版本 bump |
  | `dependencies.yaml` | 版本 bump |
  | `python/*/pyproject.toml` | version 字段 bump |

- **CI/merge → SKIP**（全部文件）:
  - `VERSION` / `pyproject.toml` — Walpurgis 版本体系独立，不跟随 RAPIDS 版本号
  - `.devcontainer/*.json` — Walpurgis 无对应 devcontainer 体系
  - `.github/workflows/*.yaml` — CI 配置
  - `conda/environments/*.yaml` / `dependencies.yaml` — Walpurgis 用 pyproject.toml

- **迁移位置**: 无（SKIP）

---

## migrate b6e934d: [SKIP] Improve devcontainer startup times (#465) — 纯 devcontainer/CI 构建基础设施优化，Walpurgis 无对应体系

- **Commit**: `b6e934d3c9e4b1a8c9f7e2d5f0a3b6c8e1d4f7a2`
- **Commit message**: `Improve devcontainer startup times (#465)`
- **Author**: James Lamb — 2026-04-28
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/465

- **Context**: 将 devcontainer 镜像预构建后发布至 `ghcr.io`，CI 和本地 devcontainer 启动时间从 ~3m28s 降至 ~34s。修改 `.devcontainer/*.json` 切换基础镜像来源 + `.github/workflows/build.yaml` 新增 devcontainer 构建 job。

- **Upstream diff**（5 files changed）:
  | 文件 | 变更内容 |
  |------|----------|
  | `.devcontainer/cuda12.9-conda/devcontainer.json` | 镜像来源切换 ghcr.io |
  | `.devcontainer/cuda12.9-pip/devcontainer.json` | 镜像来源切换 ghcr.io |
  | `.devcontainer/cuda13.2-conda/devcontainer.json` | 镜像来源切换 ghcr.io |
  | `.devcontainer/cuda13.2-pip/devcontainer.json` | 镜像来源切换 ghcr.io |
  | `.github/workflows/build.yaml` | 新增 devcontainer 预构建 job |

- **CI/merge → SKIP**（全部文件）:
  - `.devcontainer/*.json` — Walpurgis 无 devcontainer 体系
  - `.github/workflows/build.yaml` — CI 配置，Walpurgis 无 CI 体系
