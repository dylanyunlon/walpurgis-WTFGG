...
- **Upstream commit**: fcc9a2fe0365e3973cf40ee2f01e1086083924f2 (cugraph-gnn, Nate Rock, 2025-11-10, PR review suggestion by greptile-apps[bot])
- **Commit message**: `Apply suggestion from @greptile-apps[bot]`
- **Upstream diff** (1 file changed, 11 insertions):
  - `ci/release/update-version.sh`:
    - 在 `NEXT_FULL_TAG=$1` 赋值之后插入空值守卫:
      ```bash
      ...
      ```
    - 修复场景：版本参数缺失时，旧代码后续 awk/sed 操作因空字符串产生难以溯源的静默错误；新代码提前退出并提供可读提示

- **CI/merge → SKIP**:
  - `ci/release/update-version.sh` — SKIP：RAPIDS CI 发版基础设施脚本，Walpurgis 无 conda/RAPIDS release 体系

- **迁移位置**: `src/walpurgis/core/upstream_version_guard.py` — 新增

- **鲁迅拿法改写 (>20%)**:
  1. **`VersionArg` dataclass** — 将 bash 裸字符串 `$1` 封装为带类型验证的值对象，`from_str()` 同时处理存在性和格式（YY.MM.PP 正则），上游无任何格式验证
  2. **`RunContext` 枚举** — `MAIN`/`RELEASE` 两路，`from_env_or_flag()` 同时处理 CLI flag 与环境变量两种来源（三段 bash if/elif/else 的 Python 等价），`from_str()` 无效值抛 `InvalidRunContextError`
  3. **三层异常体系** — `MissingVersionArgError` / `InvalidVersionFormatError` / `InvalidRunContextError` 均继承 `VersionValidationError`，上游统一 `exit 1` 无法被 Python 层 try/except 分层捕获
  4. **`VersionGuard.validate()` 三层有序守卫** — 存在性 → 格式 → 上下文一致性（release 模式 patch!=0 警告），调用方可按错误类型区分原因，上游无此分层
  5. **`_dbg()` 全链路断点（5 处）** — 覆盖存在性检查、格式校验、RunContext 解析、环境变量来源决策、守卫通过摘要，WALPURGIS_DEBUG=1 时全可见，上游零调试输出

- **自测结果**: `WALPURGIS_DEBUG=1 python upstream_version_guard.py` → 11 项断言全通过，`[PASS]`
...
- **Commit message**: `Apply suggestion from @greptile-apps[bot]`
- **Upstream diff** (1 file changed, 11 insertions):
  - `ci/release/update-version.sh`: 在 `NEXT_FULL_TAG=$1` 赋值之后插入 11 行空值守卫（`if [[ -z "${NEXT_FULL_TAG}" ]]; then ... exit 1; fi`）。修复场景：版本参数缺失时，旧代码后续 awk/sed 操作因空字符串产生静默错误；新代码提前退出并提供 Usage + Example 说明。
- **CI/merge → SKIP**: `ci/release/update-version.sh` — SKIP：RAPIDS CI 发版基础设施脚本，Walpurgis 无 conda/RAPIDS release 体系
- **迁移位置**: `src/walpurgis/core/upstream_version_guard.py` — 新增
- **鲁迅拿法改写 (>20%)**:
  1. `VersionArg` dataclass — bash 裸字符串 `$1` → 类型化值对象，`from_str()` 同时处理存在性和 YY.MM.PP 格式校验，上游无任何格式验证
  2. `RunContext` 枚举 — bash if/elif/else 三段比较 → `MAIN`/`RELEASE` 枚举 + `from_env_or_flag()` 优先级链（CLI flag > 环境变量 > default）
  3. 三层异常体系 — `MissingVersionArgError` / `InvalidVersionFormatError` / `InvalidRunContextError`，上游统一 exit 1 无法被 Python 层分层捕获
  4. `VersionGuard.validate()` 三层有序守卫 — 存在性 → 格式 → 上下文一致性（release 模式 patch!=0 警告）
  5. `_dbg()` 全链路 5 处断点（WALPURGIS_DEBUG=1 门控）
- **自测结果**: `WALPURGIS_DEBUG=1 python upstream_version_guard.py` → 11 项断言全通过 `[PASS]`


## migrate a01924a: Pin Cython pre-3.2.0 and PyTest pre-9

- **Upstream commit**: a01924ae23feb84a371e9e2e170084377a1b39f0 (cugraph-gnn, jakirkham, 2025-11-25, PR #345)
- **Commit message**: `Pin Cython pre-3.2.0 and PyTest pre-9 (#345)`
- **Co-authored-by**: Alex Barghi, Gil Forsyth
- **Upstream diff** (12 files changed, 19 insertions, 19 deletions):
  ...
  - `dependencies.yaml` — cython/pytest 双处收紧
  - `python/cugraph-pyg/conda/cugraph_pyg_dev_cuda-129/130_arch-*.yaml` × 4 — pytest 上界收紧
  - `python/cugraph-pyg/pyproject.toml` — test extras `pytest` → `pytest<9.0.0a0`
  - `python/pylibwholegraph/pyproject.toml` — cython 上界 + pytest 上界；版权年 2023 → 2023-2025

- **约束动机**:
  - Cython 3.2.0 在 RAPIDS 构建中引入若干细微问题（rapidsai/build-planning#229、#230），固定到 `<3.2.0a0` 等待修复
  - PyTest 9 引入若干破坏性变更（rapidsai/build-planning#230），固定到 `<9.0.0a0` 等待适配

- **CI/conda/merge 文件 → SKIP**:
  - `conda/environments/*.yaml` — SKIP：conda 环境矩阵，Walpurgis 无 conda 体系
  - `conda/recipes/pylibwholegraph/` — SKIP：RAPIDS conda recipe，Walpurgis 不编译 pylibwholegraph
  - `dependencies.yaml` — SKIP：RAPIDS 构建依赖管理，Walpurgis 用 pyproject.toml
  - `cugraph-pyg/conda/*.yaml` — SKIP：conda 开发环境
  - `cugraph-pyg/pyproject.toml` — SKIP：上游包构建配置，非 Walpurgis 源码
  - `pylibwholegraph/pyproject.toml` — SKIP：上游包构建配置

- **迁移位置**: `src/walpurgis/core/dep_pin.py` — 新增

- **鲁迅拿法改写（≥20%）**:
  上游是散落在 12 个 yaml/toml 文件里的裸字符串，没有任何结构化约束理由、运行时守卫或可审计记录。Walpurgis 将其提炼为：
  1. **`PinReason` 枚举** — 约束动机类型（UPSTREAM_BUG / COMPAT_BREAK / BUILD_ISSUE / CI_STABILITY），上游无此抽象
  2. **`DepPin` dataclass（frozen）** — 包名 + 版本范围 + 动机 + 上游 commit + tracking issue + 预计修复版本；`pip_spec()` / `conda_spec()` 生成规范约束字符串；`dump()` 打印完整元信息
  3. **`PinPolicy`** — 运行时 import 前守卫，通过 `importlib.metadata` 检测已安装版本；`strict=True` 时 raise ImportError，宽松模式发出 UserWarning；上游完全没有 Python 层运行时防御
  4. **`PinAudit`** — 正则扫描 requirements 文本，确认约束仍然存在（供 CI 审计），防止版本钉被意外删除；上游靠人工维护 yaml
  5. **`WalpurgisPinEnv`** — 汇总当前 Cython/pytest 版本及符合性状态，`dump()` 一行打印快照；上游无此可视化
  6. **全链路 8 处 `WALPURGIS_DEBUG=1` 断点 print** — 模块加载、版本钉注册、上界越界检测、下界不足检测、审计扫描、批量检查入口、验证结果汇总、自测启动

- **自测结果**: `python src/walpurgis/core/dep_pin.py` → 6 项断言全部通过，环境快照正常打印 `[PASS]`
