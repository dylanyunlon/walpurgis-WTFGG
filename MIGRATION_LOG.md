...
- **Upstream commit**: 6ea54abfa41aaa6644db8b09daf218a0433d9a93 (cugraph-gnn, Bradley Dice, 2025-12-12, PR #372)
- **Commit message**: `Fix compiler warnings in scatter_op_impl_mapped.cu (#372)`
- **Upstream diff** (1 file changed, 11 insertions(+), 10 deletions(-)):
  - `cpp/src/wholememory_ops/scatter_op_impl_mapped.cu`:
    - 版权年份 `2019-2024` → `2019-2025`
    - `return scatter_func(...)` + 死代码 `WM_CUDA_CHECK(cudaStreamSynchronize(stream))`
      → `WHOLEMEMORY_RETURN_ON_FAIL(scatter_func(...))` + `WM_CUDA_DEBUG_SYNC_STREAM(stream)` + `return WHOLEMEMORY_SUCCESS`
    - 修复与 `gather_op_impl_mapped.cu` 的模式对齐（该文件已是修复后状态）

- **修复的编译器警告**:
  - `#128-D: loop is not reachable`（第 34 行旧代码：`return` 之后的 `WM_CUDA_CHECK` 是死代码）
  - `#940-D: missing return statement at end of non-void function`（函数无显式 `return`）
...
- **实际迁移位置**: `src/walpurgis/core/wholememory/scatter_op_compat.py` — 新增

- **鲁迅拿法改写（≥20%）**:
  鲁迅在《坟·论睁了眼看》中写道：「不敢正视各方面，用了种种方法，来回避，来隐蔽，才能苟安于目前。」
  本模块以同等精神，将上游「不看返回值直接 return」的鸵鸟写法，改写为「用 RETURN_ON_FAIL 正视每一个错误码」的可审计策略体系：

  | 上游变更（C++） | Walpurgis 迁移（Python） |
  |---|---|
  | `WHOLEMEMORY_RETURN_ON_FAIL(scatter_func(...))` | `ScatterReturnPolicy.MACRO_GUARD` 枚举值 |
  | `WM_CUDA_DEBUG_SYNC_STREAM(stream)` | `ScatterSyncMode.DEBUG_SYNC` 枚举值 |
  | `return WHOLEMEMORY_SUCCESS` | `ScatterReturnPolicy.EXPLICIT_SUCCESS` 枚举值 |
  | 死代码 `WM_CUDA_CHECK(cudaStreamSynchronize)` | `ScatterSyncMode.UNREACHABLE_DEAD`（PRE_FIX 版本记录） |
  | 无版本快照 | `ScatterOpCompatSpec` 冻结数据类（PRE + POST 两个快照） |
  | 无 debug 输出 | 4 处断点 print（WALPURGIS_DEBUG=1 开关） |

- **断点分布**（4 处，`WALPURGIS_DEBUG=1` 启用）:
  - BP-1: `ScatterOpCompatSpec.__post_init__` — spec 构建时打印 return_policy/sync_mode/commit 摘要
  - BP-2: `ScatterOpCompatSpec.is_fully_fixed` — 打印两维度修复状态及综合结果
  - BP-3: `ScatterOpCompatSpec.validate` — validate 通过时打印 PASS 确认
  - BP-4: `get_scatter_op_spec` — 首次查询时打印 cache_miss + spec 内容

- **自测结果**: 6 项断言全通过（`python src/walpurgis/core/wholememory/scatter_op_compat.py`）
