...
## migrate 8099bd2: [SKIP] Remove Old Content and Fix Broken Links in README — 纯文档清理，Walpurgis 独立文档体系

- **Upstream commit**: 8099bd2b0bcf2b530f2611490c55f8fd2571e3e7 (cugraph-gnn, Alex Barghi, 2025-10-01, PR #326)
- **Commit message**: `Remove Old Content and Fix Broken Links in README (#326)`
- **Approvers**: Tingyu Wang
- **Upstream diff** (`README.md | 11 +++--------`, 1 file changed, 3 insertions, 8 deletions):

  | 上游变更 | 具体内容 |
  |---|---|
  | 删除导航区两行 | `[Getting cuGraph](./docs/.../getting_cugraph.md)` 和 `[Graph Algorithms](./docs/.../algorithms.md)` — 本地 docs 路径在仓库中已不存在（失效链接） |
  | 修复三处安装链接 | `./docs/cugraph/source/installation/getting_cugraph.md` → `https://docs.rapids.ai/api/cugraph/stable/installation/getting_cugraph/`；source_build 和 CONTRIBUTING.md 同样替换为 docs.rapids.ai 外链 |
  | 删除三行 TOC 条目 | `Current list of algorithms`（本地 md，已失效）、`Blogs and Presentation`（.rst 文件）、`Performance`（本地 md） |

- **变更语义**: 上游 cugraph-gnn 从 rapidsai/cugraph 主仓库剥离后，本地 `docs/cugraph/source/` 目录已不再维护，对应文档迁移到 docs.rapids.ai。此 commit 将 README 死链替换为存活外部 URL，删除无对应页面的 TOC 条目。属于纯文档卫生性维护，零算法/API/运行时代码改动。

- **Knuth 审查**:
  1. **diff 完整性**: 1 个文件，11 行差异，全部为 Markdown 链接文本和 URL 变更，无 Python/C++/CUDA 可执行代码
  2. **用户角度**: 旧链接点击后 404（本地相对路径对 GitHub 渲染失效），新链接指向 docs.rapids.ai 稳定版，用户体验提升；删除的三条 TOC 条目（algorithms、blogs、performance）仍可通过 docs.rapids.ai 站内搜索找到，无信息损失
  3. **系统安全**: 纯 Markdown 文档，零安全影响；docs.rapids.ai 是 NVIDIA 官方文档站，外链可信

- **CI/merge → SKIP 原因**:
  - 单文件 `README.md` 变更，全部为文档层操作
  - Walpurgis 有完全独立的 README（项目描述/Quick Start/Structure/Target/Upstream/Development Plan），与 cugraph-gnn 包导航页无任何结构重叠
  - cugraph-gnn README 的安装路径（getting_cugraph、source_build、contributing）属于 RAPIDS 生态基础设施文档，Walpurgis 安装/贡献流程通过自身 README 和 `experiments/setup_env.sh` 独立描述
  - Walpurgis 无 `docs/cugraph/source/` 子目录，无指向 Graph Algorithms 或 Performance 的 TOC
