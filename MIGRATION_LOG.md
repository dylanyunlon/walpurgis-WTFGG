...
- **Commit**: `13e00d6`
- **Commit message**: `Update CuPy Version to 13.2 (#132)`
- **Author**: Alex Barghi — 2025-02-05

- **变更摘要**: 将 6 个 conda/recipe/dependencies 文件中的 `cupy>=12.0.0` 统一升级为 `cupy>=13.2.0`。  
  背景：CuPy 13.2 修复了与 NCCL 多 GPU 通信层的长期 bug（rapidsai/cugraph#4465）。  
  上游说明："At this point, we are relying on other constraints to enforce the CuPy version, but to be safe, we should explicitly upgrade it."

- **CI/conda → SKIP（6 个文件）**:
  - `conda/environments/all_cuda-118_arch-x86_64.yaml` — SKIP: RAPIDS conda 环境矩阵，Walpurgis 无此构建体系
  - `conda/environments/all_cuda-121_arch-x86_64.yaml` — SKIP: 同上
  - `conda/environments/all_cuda-124_arch-x86_64.yaml` — SKIP: 同上
  - `conda/recipes/cugraph-dgl/meta.yaml` — SKIP: RAPIDS conda recipe，Walpurgis 不编译 cugraph-dgl
  - `conda/recipes/cugraph-pyg/meta.yaml` — SKIP: RAPIDS conda recipe，Walpurgis 不编译 cugraph-pyg
  - `dependencies.yaml` — SKIP: RAPIDS 构建依赖管理（cupy-cuda12x / cupy-cuda11x / cupy 三处），Walpurgis 用 pyproject.toml

- **迁移位置**: `src/walpurgis/core/cupy_version_policy.py`（新增）

- **鲁迅拿法改写（≥20%）**:
  1. `NcclFixTrigger` 枚举 — 建模版本升级触发器类型（NCCL_BUG_FIX / SECURITY / PERF / COMPAT），上游无此抽象，6 个文件仅有裸字符串替换
  2. `CupyVersionSpec` dataclass — 将 `min_version / prev_min_version / trigger / nccl_issue_ref / cuda_variants` 收口为单一值对象，`__post_init__` 校验 semver 格式（上游完全无校验），附 `was_upgrade()` 方向判断
  3. `CudaVariantResolver` — 封装上游 `dependencies.yaml` 三段 matrix 逻辑（cuda 12.* / 11.* / null fallback），提供 `pip_spec_for_cuda()` 统一接口，上游无此封装
  4. `CupyNcclCompatGuard` — 运行时 import 守卫，检查已安装 cupy 版本 >=13.2.0；strict 模式 raise ImportError，宽松模式 UserWarning；上游只在 yaml 声明约束，安装后无 Python 层检查
  5. `CupyVersionAudit` — 扫描 requirements 文本验证 `cupy>=13.2.x` 约束存在，替代上游"靠人工维护 6 个 yaml 不遗漏"的隐性假设
  6. 全链路 `WALPURGIS_DEBUG=1` 断点，8 处覆盖：模块加载 → VersionSpec 解析 → NcclFixTrigger 注册 → CudaVariantResolver 映射 → NcclCompatGuard 检查 → VersionAudit 扫描 → 自测启动 → 自测通过
