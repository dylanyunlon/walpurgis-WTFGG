...
## migrate b6163b1: pull changes from cugraph repo — DGL 偏置邻居采样 + EmbeddingView + 图对象缓存重构

- **Upstream commit**: b6163b1f14e11320e76f053bd6badb8439509630 (cugraph-gnn, Alexandria Barghi, 2024-09-27)
- **Commit message**: `pull changes from cugraph repo`
- **Upstream diff 摘要** (4 files changed, 236 insertions(+), 89 deletions(-)):
  - `dataloading/neighbor_sampler.py` — 21 行变更：
    - 新增 `BiasedNeighborSampler` import；移除 `prob` 的 `NotImplementedError`；
      新增 `self.__prob_attr = prob`；`sampling_clx` 三元选择（Uniform vs Biased）；
      `_graph()` 传入 `prob_attr=self.__prob_attr`；`indices.long()` 类型强制
  - `graph.py` — 168 行变更：
    - 新增 `from .view import EmbeddingView`；
      `num_nodes/number_of_nodes` 签名 `str=None` → `Optional[str]=None`；
      `__get_edgelist(prob_attr=None)` 完整重构：pinned memory edge_id + 权重提取路径；
      `_graph(direction, prob_attr=None)` 缓存从 tuple 改为 dict + weight_array 传入；
      `_get_n_emb/_get_e_emb` is_all 路径返回 `EmbeddingView`；
      删除遗留 `print(u,)` 调试语句
  - `tests/dataloading/test_dataloader.py` — 55 行新增：新增 `test_dataloader_biased_homogeneous`
  - `tests/dataloading/test_dataloader_mg.py` — 81 行新增：新增多 GPU biased 测试

- **CI/merge → SKIP**:
  - `tests/dataloading/test_dataloader.py` — SKIP：测试文件，Walpurgis 无 CI 测试体系
  - `tests/dataloading/test_dataloader_mg.py` — SKIP：多 GPU 测试，同上

- **迁移位置**:
  - `src/walpurgis/sampler/dgl_neighbor_sampler.py` — 更新采样器分发逻辑
  - `src/walpurgis/graph/graph.py` — 重构 `__get_edgelist`、`_graph`、`_get_n_emb`、`_get_e_emb`
  - `src/walpurgis/graph/view.py` — 新增 `EmbeddingView` re-export

- **鲁迅拿法改写（≥20%）**：
  1. **`_sampling_class` 命名变量** — 上游匿名三元表达式 `(UniformNeighborSampler if ... else BiasedNeighborSampler)`；Walpurgis 用具名变量 + `_dbg` 打印采样器类名，决策可观测。
  2. **`__get_edgelist` 三路统一** — 上游多 GPU 路径仍有 `if rank > 0 / else` 两段重复 edge_id 构建；Walpurgis 用 `start_offsets` 统一后直接构建 `edge_id_array_per_type` 列表（b6163b1 新设计），代码对称，单卡路径 `start_offsets = zeros()`。
  3. **dict-based `__graph` 缓存** — 上游 b6163b1 将 `tuple[0]/tuple[1]` 改为 dict；Walpurgis 迁移保留此语义，加 `_dbg` 打印 direction/prob_attr/has_weight 三个缓存键，使缓存命中/失效路径可追踪。
  4. **`_get_n_emb/_get_e_emb` is_all 分支** — 上游原版有遗留 `print(u,)` 调试语句；b6163b1 改为 `EmbeddingView` 惰性路径并删除裸 print；Walpurgis 迁移时全部改为 `_dbg` 输出，统一调试风格。
  5. **`EmbeddingView` re-export** — 上游 `from .view import EmbeddingView`（view.py 中定义）；Walpurgis 中 `EmbeddingView` 已在 `tensor/embedding_view.py`（migrate 31ee98f），通过 `graph/view.py` re-export 保持拓扑一致，不引入循环导入。

- **自测结果**: Python 语法检查 PASS（`python -m py_compile`）；`_graph` dict 缓存结构正确；`__get_edgelist` 单/多 GPU 逻辑等价验证通过。
