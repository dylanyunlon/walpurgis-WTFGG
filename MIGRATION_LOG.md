...
---
---
## c46205b — Use GCC 14 in conda builds (#228)

- **Commit**: `c46205ba54260952f8968d3e51a9653b181ade86`
- **Commit message**: `Use GCC 14 in conda builds. (#228)`
- **Author**: Vyas Ramasubramani (vyasr, NVIDIA) / reviewers: Alex Barghi, Bradley Dice, Gil Forsyth
- **Context**: conda-forge 正迁移到 GCC 14（rapidsai/build-planning#188），本 PR 是 RAPIDS 25.10 对 conda-forge GCC 14 migration 的同步对齐。核心变更：(1) gcc_linux-64/aarch64 版本 glob 13.* → 14.*，体现在 conda 环境 yaml × 2 和 conda_build_config.yaml × 2；(2) cmake>=3.30.4 加上 <4.0.0 上界，防止 cmake 4.x 破坏性变更影响 RAPIDS C++ 构建；(3) 三个 .cu 测试文件补充 `#include <algorithm>`（GCC 14 移除了间接引入路径）并更新版权年 2024→2025。

- **Upstream diff** (10 files changed, 19 insertions(+), 16 deletions(−)):
  | 文件 | 变更内容 |
  |------|----------|
  | `conda/environments/all_cuda-129_arch-aarch64.yaml` | gcc_linux-aarch64 13.* → 14.*；cmake 加 <4.0.0 上界 |
  | `conda/environments/all_cuda-129_arch-x86_64.yaml` | gcc_linux-64 13.* → 14.*；cmake 加 <4.0.0 上界 |
  | `conda/recipes/libwholegraph/conda_build_config.yaml` | c/cxx_compiler_version 13 → 14 |
  | `conda/recipes/pylibwholegraph/conda_build_config.yaml` | c/cxx_compiler_version 13 → 14 |
  | `cpp/tests/graph_ops/append_unique_test_utils.cu` | 版权年 2024→2025；补 `#include <algorithm>` |
  | `cpp/tests/graph_ops/append_unique_tests.cu` | 版权年 2024→2025；补 `#include <algorithm>` |
  | `cpp/tests/graph_ops/csr_add_self_loop_utils.cu` | 版权年 2024→2025；补 `#include <algorithm>` |
  | `dependencies.yaml` | gcc 13.* → 14.*（x86/aarch64）；cmake 加 <4.0.0 |
  | `python/libwholegraph/pyproject.toml` | cmake>=3.30.4 → cmake>=3.30.4,<4.0.0 |
  | `python/pylibwholegraph/pyproject.toml` | cmake>=3.30.4 → cmake>=3.30.4,<4.0.0 |

- **CI/merge → SKIP**（全部 10 个上游文件）:
  - `conda/environments/*.yaml` — Walpurgis 无 conda 构建矩阵
  - `conda/recipes/libwholegraph/` — RAPIDS C++ conda recipe，Walpurgis 不编译 libwholegraph
  - `conda/recipes/pylibwholegraph/` — 同上
  - `cpp/tests/graph_ops/*.cu` — WG C++ 测试源码，非 Walpurgis Python 包
  - `dependencies.yaml` — RAPIDS 构建依赖生成器，Walpurgis 用 pyproject.toml 独立管理
  - `python/libwholegraph/pyproject.toml` — 上游 C++ Python 绑定包，非 Walpurgis 包
  - `python/pylibwholegraph/pyproject.toml` — 同上

- **迁移位置**: `src/walpurgis/core/gcc_build_policy.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`CompilerArch` 枚举**: X86_64/AARCH64 双架构显式建模——上游仅通过两个 yaml 文件名隐含架构差异，无任何 Python 层枚举
  2. **`CompilerSpec` dataclass**: 结构化 `compiler_name/arch/version_glob/upstream_commit/migration_reason/previous_version_glob`，`GCC14_X86_64`/`GCC14_AARCH64` 携带完整溯源与 `conda_dep_line()`/`conda_build_config_entry()`/`major_version()` 方法——上游只有 yaml 的裸字符串替换
  3. **`CmakeUpperBound` dataclass**: 建模 cmake <4.0.0 约束，携带 `reason`（cmake 4.x CMP0169 破坏性变更）和 `expected_unpin`，`is_version_allowed()` 程序化版本边界检查——上游只有两个 pyproject.toml 的一行字符串变更，无任何解释
  4. **`GccMigrationRecord`**: 将 GCC 升级事件结构化，`expected_gcc_major()` 程序化查询期望版本，`conda_build_config_block()` 生成标准格式输出——上游散落在 4 个 yaml 文件中
  5. **`CppHeaderAudit`**: 审计 .cu/.cpp 文件是否补充了 `#include <algorithm>` 且版权年已更新——上游只是在三个 .cu 文件中补了几行，无任何 Python 层审计机制
  6. **`BuildPolicySnapshot`**: 汇总 GCC + cmake 构建策略，`dump()` 单次调用打印完整快照，`validate_cmake_version()` 策略验证代理——上游无任何 Python 层汇总
  7. **`CopyrightYearChecker`**: 批量扫描 C++/CUDA 文件版权年的独立工具，与 `CppHeaderAudit` 职责分离（单文件审计 vs 批量目录扫描）——上游无此机制
  8. 全链路 `WALPURGIS_DEBUG=1` 断点（8 处）
