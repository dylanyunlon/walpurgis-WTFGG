...
- **Upstream commit**: 2ef236753 (Megatron-LM #8)
- **Commit message**: `Support latest PyTorch RNG state API. (#8)`
- **Upstream diff 摘要** (1 file changed, 20 insertions(+), 3 deletions(-))：

  | 文件 | 变更内容 |
  |------|----------|
  | `mpu/random.py` | `_set_cuda_rng_state` 从单一 `_C._cuda_setRNGState` 路径，改为旧版/新版 PyTorch 双路径自适应 |

- **双路径逻辑**：
  - **older PyTorch**: `hasattr(_C, '_cuda_setRNGState')` 为真 → `with device_ctx_manager(device): _C._cuda_setRNGState(new_state)`
  - **newer PyTorch**: `_C._cuda_setRNGState` 不存在 → device 归一化（-1/str/int → torch.device）→ `torch.cuda.default_generators[idx].set_state(new_state)`

- **迁移位置**：`src/walpurgis/models/cuda_rng_state.py`（新增）

- **鲁迅拿法改写（≥20%）**：
  上游改动的本质是「新旧交替的双轨迁移」——_C._cuda_setRNGState 是一道厚重的铁门，
  default_generator.set_state 是悄然开启的侧门。鲁迅写《药》里人血馒头，是说旧物件
  里藏着时代的断层；_C._cuda_setRNGState 的消失同理：一个内部 C++ 符号悄然退场，
  无声无息，只留下 hasattr 的探针和 default_generators 的新路。
  Walpurgis 将此次双轨迁移结构化为四个可程序化结构：
  1. **`RngBackend` 枚举** — 显式建模 LEGACY/MODERN 两条路径，替代上游内联 hasattr 判断
  2. **`DeviceNormalizer`** — 封装 device=-1/str/int → torch.device 归一化逻辑，替代上游 if/elif 三段式（散落在 else 分支，无复用）
  3. **`CudaRngStateSetter`** — 封装 _set_cuda_rng_state 完整逻辑，构造时探测并缓存 backend（上游每次调用都重新 hasattr），持有 `_build_legacy_cb()` / `_build_modern_cb()` 两条 callback 构造路径
  4. **`LazyCudaCallable`** — 封装 `_lazy_call(cb)` 调度语义，支持注入真实 _lazy_call 或单机直接执行（改写新增降级路径）

  全链路 `WALPURGIS_DEBUG=1` 断点 print（backend 探测×2、device 归一化×4、cb 构造×2、cb 执行×3、_lazy_call 调度×2），覆盖旧版/新版两条完整执行路径。

- **三维度审查（Knuth）**：
  - **正确性**：上游 `_set_cuda_rng_state` 双路径逻辑完整保留——LEGACY 路径保持 device_ctx_manager 语义不变，MODERN 路径 device 归一化顺序（-1→str→int）与上游 if/elif 完全对应。`default_generators[idx]` 的 `idx is None` 降级（`torch.cuda.current_device()`）与上游逻辑等价。
  - **性能**：backend 探测结果缓存（`self._backend`），避免上游每次调用都重新 `hasattr`（O(1) → O(1) 常数优化）。`DeviceNormalizer.normalize()` 为 O(1) 类型判断。`LazyCudaCallable` 无额外分配。
  - **可读性**：上游 `_set_cuda_rng_state` 为 30 行裸函数，混合 backend 探测、device 归一化、callback 构造、_lazy_call 调度四种关注点。改写将四种关注点分离为四个独立结构，每条路径均有明确的调试输出，LEGACY/MODERN 分支可独立测试和 mock。
