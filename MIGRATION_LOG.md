...
## migrate 474b10e: [BUG] Fix Broken DGL Test — 删除 pylibcugraph 废弃 API 参数

- **Upstream commit**: 474b10e (cugraph-gnn, Alex Barghi, 2025-06-04)
- **Commit message**: `[BUG] Fix Broken DGL Test (#223)`
- **变更内容**: 2 files changed, 4 deletions
  - `python/cugraph-dgl/cugraph_dgl/tests/test_graph.py`
  - `python/cugraph-dgl/cugraph_dgl/tests/test_graph_mg.py`
  - 修复内容：删除 `pylibcugraph.uniform_neighbor_sample()` 调用中的
    `with_edge_properties=True` 和 `return_dict=True` 两个废弃参数
- **修复原因**: pylibcugraph 新版 API 移除了这两个参数，继续传入会触发 TypeError，
  导致异构图采样测试全面崩溃
- **Walpurgis 迁移**:
  - 新增 `src/walpurgis/tests/graph/test_graph.py`：迁移 SG 路径的同构/异构图测试，
    使用修复后的 `uniform_neighbor_sample()` 调用（去掉两个废弃参数）
  - 新增 `src/walpurgis/tests/graph/conftest.py`：`karate_bipartite` fixture
    及 `_KarateBipartiteInfo` dataclass（20% 改写：tuple → dataclass）
  - MG 路径（`test_graph_mg.py`）标记 skip，Walpurgis 不支持 MG 图对象
  - 20% 改写：`_SamplingOutputInspector` dataclass 封装边校验循环 + WALPURGIS_DEBUG 断点

## migrate 474b10e: [BUG] Fix Broken DGL Test — 删除 pylibcugraph 废弃 API 参数 with_edge_properties/return_dict
