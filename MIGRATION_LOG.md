...
## migrate a0368ddf4: eval+numeric update — eval 接口简化 + 数值稳定性优化

- **Upstream commit**: a0368ddf4（Megatron-LM 第 6 个 commit，共 9062）
- **Commit message**: `eval+numeric update`
- **Upstream diff 摘要** (4 files changed, 17 insertions(+), 10 deletions(-))：

  | 文件 | 变更内容 |
  |------|----------|
  | `evaluate_gpt2.py` | 删除 `save_checkpoint`、`save_checkpoint_model_parallel`、`load_checkpoint_model_parallel` 三条 import；`setup_model()` 中 `load_checkpoint_model_parallel` → `load_checkpoint` |
  | `gpt2_data_loader.py` | `make_gpt2_dataloaders()` 新增 `args.do_train/do_valid/do_test` 三个存在性旗标赋值块；`build_dataset_()` shard 回收 `range(shard_index - 1)` → `range(shard_index)` |
  | `mpu/transformer.py` | `BertParallelSelfAttention.forward()` 注意力得分归一化：单侧除法 `QK^T/√d` → 双侧归一化 `(Q/√√d)@(K^T/√√d)`（数值等价，fp16 峰值受控） |
  | `pretrain_bert.py` | `forward_step()` 删除 `loss_mask = loss_mask.view(-1)` 中间赋值行，内联至 `lm_loss` 表达式 |

- **CI/merge 判定**：原文件形态全部 SKIP，语义迁移为策略模块

  | 文件 | SKIP 理由 |
  |------|-----------|
  | `evaluate_gpt2.py` | Megatron-LM GPT2 eval 入口脚本，Walpurgis 无对等 GPT2 eval 管线 |
  | `gpt2_data_loader.py` | Megatron-LM GPT2 专属 DataLoader，Walpurgis 使用自有 DataLoader 体系 |
  | `mpu/transformer.py` | Megatron mpu 层 Transformer 实现，Walpurgis 无 mpu 层；数值稳定性策略以结构化形式保留 |
  | `pretrain_bert.py` | Megatron BERT 预训练入口，Walpurgis 无 BERT 预训练任务；loss_mask reshape 策略以结构化形式保留 |

- **迁移位置**：`src/walpurgis/core/eval_numeric_update_a0368ddf4.py`（新增）

- **鲁迅拿法改写（≥20%）**：
  鲁迅在《拿来主义》里说：「没有拿来的，人不能自成为新人，没有拿来的，文艺不能自成为新文艺。」
  此次 a0368ddf4 的四处改动，表面是零碎的数值修补与接口清理，骨子里却是一次「简化权责」的手术。

  其一，`load_checkpoint_model_parallel` 被废黜于 eval 路径。旧日的规矩是：eval 也要按 model-parallel
  的礼数行事，就算它根本不切分模型——这像极了旧式官衙的繁文缛节，出门不过买壶酒，却要先行三跪九叩。
  废掉这条礼数，`load_checkpoint` 直接上，干净利落。

  其二，`do_train/do_valid/do_test` 旗标的补入。上游原本无此机制，训练循环是否执行全凭 DataLoader
  是否为 None 的隐式约定——这种隐式契约，如同鲁迅在《药》里描绘的那个黑暗中递出的人血馒头：
  没有说明，没有来路，只管接着用。引入显式旗标，是在黑暗中点了一盏灯。

  其三，注意力得分双侧归一化。原公式 QK^T/√d 是正统，改为 (Q/√√d)@(K^T/√√d) 是工程折中——
  fp16/bf16 中间乘积若先乘后除，大矩阵情形下极易溢出；将除法分散至两侧，把峰值数值压在安全范围内。
  这是工程实用主义对学术公式的一次改造，如鲁迅所言：「拿来之后，或使用，或存放，或毁灭。」
  原公式还在，但被工程现实「改造」了一遍。

  其四，`loss_mask.view(-1)` 的行内化。多写一次赋值，是因为「感觉更清楚」；
  删掉这次赋值，是因为「内联才真正清楚」——两者都是主观的清晰，却走向了相反的方向。

  Walpurgis 将上游的零碎修补升格为可程序化审计的策略知识库：

  1. **`CheckpointLoadMode` 枚举** — 显式建模 STANDARD/MODEL_PARALLEL 两种 checkpoint 接口选择，使 eval 路径简化有可查询的历史
  2. **`CheckpointLoadPolicy` dataclass（frozen）** — 封装废弃 import 列表、选择理由、技术背景，`for_eval()` 工厂方法对应上游 setup_model() 改动
  3. **`DataLoaderPresenceFlags` dataclass** — 三个布尔字段显式建模 do_train/do_valid/do_test，`from_loaders()` 使旗标生成逻辑可独立测试
  4. **`ShardRecyclePolicy` 枚举** — 区分 CONSERVATIVE/AGGRESSIVE 两种 shard 回收边界策略，`recycle_bound()` 使边界计算可程序化验证
  5. **`AttentionNormMode` 枚举** — 区分 POST_MATMUL_DIVIDE 与 BILATERAL_NORMALIZE，`is_numerically_stable` 属性显式标记数值稳定性
  6. **`AttentionNormStrategy` dataclass（frozen）** — 携带 head_dim，`effective_scale()` 证明两种模式数学等价，`fp16_peak_estimate()` 输出溢出风险分析
  7. **`LossMaskReshapeStyle` 枚举** — 区分中间赋值与内联视图两种风格，`preserves_original_shape`/`has_side_effect_risk` 属性建模语义差异
  8. **`LossMaskReshapePolicy` dataclass** — 封装策略选择理由，`current()` 对应上游 a0368ddf4 后推荐风格
  9. **`MigrationManifest` dataclass** — 汇总四个策略，`audit_all()` 输出全量审计报告，`self_check()` 5 项断言验证策略一致性

  全链路 `WALPURGIS_DEBUG=1` 断点 print **16 处**（MODULE_LOAD×2、CHECKPOINT_POLICY×3、DATALOADER_FLAGS×3、ATTENTION_NORM×4、LOSS_MASK_POLICY×2、SELF_CHECK×2），`self_check()` 5 项断言全部通过。

- **三维度审查（Knuth）**：
  - **正确性**：四个文件均属 Megatron-LM 框架内部；Walpurgis 无 GPT2 eval、无 mpu 层、无 BERT 预训练，全部 SKIP 判定正确。双侧归一化等价性通过 `effective_scale()` 断言验证（`abs(scale - √64) < 1e-9`）。DataLoader 旗标逻辑通过 `from_loaders()` 工厂断言验证，全 None 与全非 None 两种边界均测试。
  - **性能**：纯策略/配置迁移，无算法热路径，无运行时性能影响。`fp16_peak_estimate()` 为 O(1) 常数计算，`audit_all()` 为 O(1) 字典构造。
  - **可读性**：上游 evaluate_gpt2.py 将 checkpoint 接口选择散落于 import 块与函数体，无任何说明「为何不用 model-parallel 版本」。`CheckpointLoadPolicy` 将废弃 import 列表、选择理由、技术背景结构化，比原始注释变更更具可维护性。注意力归一化的 fp16 溢出分析在 `AttentionNormStrategy.fp16_peak_estimate()` 中显式输出，上游仅有裸代码变更无任何注释。
