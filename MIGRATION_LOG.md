...
## migrate bd703b3: add wholegraph to repo

- **Upstream commit**: bd703b3 (cugraph-gnn, NVIDIA / Alexandria Barghi, 2024-07-31)
- **Commit message**: `add wholegraph to repo`
- **Upstream diff** (208 文件变动, 46965 行新增):
  - `cpp/` — 全套 WholeGraph C++/CUDA 实现（wholememory / wholegraph_ops / graph_ops / communicator / embedding / file_io / bench / tests）
  - `python/pylibwholegraph/pylibwholegraph/torch/` — Python 高层封装层（comm / tensor / embedding / graph_structure / wholegraph_ops / graph_ops / initialize / wholegraph_env）
  - `python/pylibwholegraph/pylibwholegraph/binding/wholememory_binding.pyx` — Cython binding（2034行）
  - `python/pylibwholegraph/pylibwholegraph/torch/gnn_model.py` / `data_loader.py` / `distributed_launch.py` / `common_options.py` — 训练入口脚手架

- **Knuth 审查**:
  1. **diff 对比源**:
     | 上游 bd703b3 | Walpurgis 迁移 |
     |---|---|
     | `default_wholegraph_env_context` / `torch_cpp_ext_loaded` / `torch_cpp_ext_lib` 三个模块级变量散落 wholegraph_env.py | `_WalpurgisEnvCtx` dataclass 集中管理，compile_cpp_extension 改为惰性单例防重入 |
     | `global_communicators` / `local_node_communicator` 等五个模块级变量散落 comm.py | `_WalpurgisCommRegistry` dataclass 统一注册表，`_CommKey` NamedTuple 防 backend 字符串拼写错误 |
     | `TorchMemoryContext`: `get_c_context()` 在 cpp_ext / fallback 两路语义不同（真实 handle vs id(self)），调用方需知道内部状态 | `_OutputBuffer`: 统一 `c_handle()` / `tensor()` 接口，隐藏路径差异 |
     | `WholeMemoryEmbedding` 用 `need_apply` / `sparse_indices` / `sparse_grads` 三个散落属性管理梯度累积 | `_GradAccumulator` dataclass 封装累积生命周期，`accumulate()` / `flush_tensors()` / `reset()` 职责清晰 |
     | `unweighted/weighted_sample_without_replacement` 末尾 4 路 if/elif/else 返回不同长度 tuple | `_SampleOutput` dataclass 统一返回，`.as_tuple()` 保留上游兼容接口 |
     | `GraphStructure.multilayer_sample_without_replacement` for 循环内重复 weight/unweight if/else 分支 | 提取 `_one_hop_sample()` 内部方法；`_MultilayerSampleResult` / `_HopResult` dataclass 替代裸 list |
     | `get_all_chunked_tensor` 存在拼写错误: `wmb_tensor.get_global_tensorget_all_chunked_tensor(...)` | 迁移修正为 `wmb_tensor.get_all_chunked_tensor(...)` |
     | `_LocalTensorView`: get_local_tensor 返回裸 tuple (tensor, element_offset)，调用方需知道解包顺序 | 具名 dataclass，`tensor` / `element_offset` 字段消除歧义 |
     | `WholeMemoryCommunicator.destroy()` 无重入保护 | 加 `if self.wmb_comm is not None` 防 double-free，加 _dbg 输出 |
     | `_WalpurgisInitState`: 上游无初始化阶段追踪，多次 init() 会导致 wmb.init 重入 | dataclass 追踪 wmb_initialized / torch_env_initialized，init / finalize 均有幂等保护 |

  2. **用户角度 bug**:
     - `get_all_chunked_tensor` 拼写错误（已修正，上游 bd703b3 原版 bug）
     - `create_builtin_cache_policy("all_devices", ...)` 当 `embedding_memory_location=="cuda"` 时上游用 `print("[WARNING]...")` 输出到 stdout，Walpurgis 改为 `_dbg`，避免在 multi-GPU 训练时所有 rank 同时污染 stdout
     - `init_torch_env` 中 MASTER_ADDR/PORT fallback 同样改为 `_dbg`

  3. **系统角度安全**:
     - `_WalpurgisEnvCtx.compile_cpp_extension()` 惰性单例: 上游每次调用均尝试 importlib.import_module，无幂等保护；Walpurgis 首次成功后缓存，避免多进程并发重入
     - `_CommKey` NamedTuple: 上游用裸字符串 `distributed_backend` 作 dict key，拼写错误会静默创建新 comm；Walpurgis 用 NamedTuple 使 IDE 可类型检查

### Walpurgis 迁移位置

**新增文件**（均在 `src/walpurgis/core/wholememory/`）:
- `__init__.py` — 子包导出，覆盖全部迁移符号
- `env.py` — 运行时环境: `_WalpurgisEnvCtx` / `_OutputBuffer` / `wrap_torch_tensor` / `get_stream` / `get_wholegraph_env_fns` / `compile_cpp_extension`
- `env_fn_utils.py` — dtype/location/type 转换工具（独立以避免循环导入）
- `comm.py` — `_WalpurgisCommRegistry` / `WholeMemoryCommunicator` / 通信器创建/获取 API
- `tensor.py` — `WholeMemoryTensor` / `_LocalTensorView` / gather / scatter / 文件 IO / 工厂函数
- `embedding.py` — `_GradAccumulator` / `WholeMemoryEmbedding` / `WholeMemoryOptimizer` / `WholeMemoryCachePolicy` / 工厂函数
- `wholegraph_ops.py` — `_SampleOutput` / unweighted/weighted 采样 / CPU 随机数工具
- `graph_ops.py` — `_AppendUniqueResult` / `_CsrSelfLoopResult` / `append_unique` / `add_csr_self_loop`
- `graph_structure.py` — `_WalpurgisGraphMeta` / `GraphStructure` / `_HopResult` / `_MultilayerSampleResult`
- `initialize.py` — `_WalpurgisInitState` / `init` / `init_torch_env` / `init_torch_env_and_create_wm_comm` / `finalize`

**未迁移**:
- `cpp/` — C++/CUDA 实现层（需 CUDA 工具链 + cmake，属于 build 范畴）
- `wholememory_binding.pyx` — Cython binding（依赖 cpp 层编译产物）
- `gnn_model.py` / `data_loader.py` / `distributed_launch.py` / `common_options.py` — 训练入口脚手架，与 walpurgis 自有 `main.py` / `trainer.py` 功能重叠，不迁移

**改写20%（鲁迅拿法）**:
- `_WalpurgisEnvCtx` / `_WalpurgisCommRegistry` / `_WalpurgisInitState` — 三处模块级散变量集中化
- `_OutputBuffer` — 替代 `TorchMemoryContext`，统一 cpp_ext/fallback 两路接口
- `_GradAccumulator` — 替代 embedding 中三个散落梯度属性
- `_SampleOutput` / `_AppendUniqueResult` / `_CsrSelfLoopResult` / `_LocalTensorView` — 四处裸 tuple 返回具名化
- `_WalpurgisGraphMeta` / `_HopResult` / `_MultilayerSampleResult` — 图结构元数据和多跳结果具名化
- `_one_hop_sample()` — 消除 multilayer for 循环内 weight/unweight 重复分支
- 修正 `get_all_chunked_tensor` 拼写 bug
- 全链路 `WALPURGIS_DEBUG=1` 断点 print，覆盖:
  env 初始化 → comm 创建/split → tensor gather/scatter → embedding gather/accumulate/apply →
  cache policy 构建 → 采样参数/seed/输出 shape → append_unique → CSR self-loop → multilayer 每跳
...
- **Upstream diff** (1 file changed, `.github/workflows/pr.yaml`):
  ...
  - `wheel-test-pylibwholegraph` job: 新增 `and .CUDA_VER != "11.4.3"` 过滤条件
  - `wheel-build-cugraph-dgl` / `wheel-build-cugraph-pyg` job: 同样新增 CUDA 11.4 过滤
- **迁移价值**: 无
- **原因**: 纯 CI workflow 配置（`.github/workflows/pr.yaml`），Walpurgis 无 GitHub Actions CI 体系，无 CUDA 版本矩阵管理，无可迁移内容
