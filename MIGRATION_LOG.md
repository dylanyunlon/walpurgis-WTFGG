...
- **Upstream commit**: ff67374 (cugraph-gnn, James Lamb, 2025-07-01, PR #235)
- **Commit message**: `add docs on CI workflow inputs (#235)`
- **Authors**: James Lamb (jameslamb); Approvers: Jake Awe (AyodeAwe), Vyas Ramasubramani (vyasr)
- **Upstream diff** (2 files changed, 12 insertions):
  - `.github/workflows/build.yaml` — `workflow_dispatch.inputs` 下 `branch`/`date`/`sha`/`build_type` 四个输入字段各新增 `description:` 字段，解释每个参数的格式与用途（如 `branch` 需 YYYY-MM-DD 格式说明、`sha` 需完整 40 位 SHA 说明）
  - `.github/workflows/test.yaml` — 完全对称，同样四个字段加同样的 `description:` 说明

- **跳过原因**: 纯 GitHub Actions `workflow_dispatch` 手动触发界面文档改进——`description:` 字段仅在 GitHub UI 的"Run workflow"下拉面板中渲染为提示文本，零运行时代码变更。Walpurgis 无 RAPIDS/rapidsai GitHub Actions CI 体系，无 `.github/workflows/` 目录，零迁移价值。

- **Knuth 审查**:
  1. **diff 对比源**: 上游 build.yaml 新增的 `branch` description 包含两行 block scalar（`|`），第二行 "Required even when 'sha' is provided because it is also used for organizing artifacts."——这是有价值的 CI 行为说明，但仅对 RAPIDS CI 使用者有意义，与 Walpurgis 运行时逻辑零交叉
  2. **用户角度 bug**: 无运行时影响；原来缺少 description 时 CI 操作者不知道 `date` 参数的 `YYYY-MM-DD` 格式要求，可能传入错误格式导致 nightly artifacts 路径混乱——修复后 UI 提示清晰，防止人为错误
  3. **系统角度安全**: 无安全影响；`description:` 是 GitHub Actions YAML 的纯元数据字段，不参与 workflow 执行逻辑，不影响权限边界


## migrate a58ba34: [SKIP] cugraph_gnn RAPIDS dependency now use the new rapids-cmake branch info — 纯 CMake 构建基础设施变更，无迁移价值

- **Upstream commit**: a58ba34854ddaf8a18c5b98a47587922ce40e4f9 (cugraph-gnn, Robert Maynard, 2025-07-30, PR #263)
- **Commit message**: `cugraph_gnn RAPIDS dependency now use the new rapids-cmake branch info (#263)`
- **Approvers**: Bradley Dice, Alex Barghi
- **Upstream diff** (1 file changed, 2 insertions, 3 deletions):
  - `cpp/cmake/thirdparty/get_raft.cmake`:
    - 删除 `set(WHOLEGRAPH_BRANCH_VERSION_raft ...)` 变量声明（该变量拼接 `${RAPIDS_VERSION_MAJOR}.${RAPIDS_VERSION_MINOR}` 用于构造 `branch-YY.MM` 格式的 raft pinned tag）
    - `if(PKG_CLONE_ON_PIN AND NOT PKG_PINNED_TAG STREQUAL "branch-${WHOLEGRAPH_BRANCH_VERSION_raft}")` → `STREQUAL "${rapids-cmake-checkout-tag}"`
    - `PINNED_TAG branch-${WHOLEGRAPH_BRANCH_VERSION_raft}` → `PINNED_TAG ${rapids-cmake-checkout-tag}`

- **变更语义**:
  RAPIDS 正在切换分支命名策略（从 `branch-YY.MM` 静态格式过渡到由 `rapids-cmake` 动态提供 `${rapids-cmake-checkout-tag}` 变量）。旧代码在 CMake 配置层手工拼接版本字符串 `branch-25.10`，新策略由上游 `rapids-cmake` 模块统一管理 checkout tag，解耦了 cugraph-gnn 自身对 RAPIDS 版本格式的感知。两处改动逻辑等价：`CLONE_ON_PIN` 守卫条件和 `PINNED_TAG` 调用点均替换为由 rapids-cmake 注入的统一变量，无算法或 API 语义变化。

- **跳过原因**: 全部改动位于 `cpp/cmake/thirdparty/get_raft.cmake`，属于 CMake 构建脚本层。Walpurgis 无 `cpp/` C++ 构建体系，无 CMake/CPM 依赖管理，无 rapids-cmake 集成；raft 作为采样算法底层库以预编译形式通过 `pylibcugraph` wheel 间接使用，不在 Walpurgis 内部重新编译。此 commit 等同于已 SKIP 的同类 CMake 基础设施变更（参见 `3816ca8`、`71675d8`、`f6e3654`、`af22a12`、`954a7ba`）。零迁移价值。

- **Knuth 审查**:
  1. **diff 完整性**: 仅 1 个文件，3 处字符串替换，已全量审查；无任何 `.py`/`.cu`/`.cpp`/`.h` 源码改动
  2. **用户角度 bug**: 旧代码在 RAPIDS 改变分支命名约定后（如从 `branch-25.10` 改为其他格式）会硬编码失效，导致 raft 被错误地按 pin 克隆到本地目录而非使用系统安装版本，构建时间显著增加但无报错；新代码动态跟随 rapids-cmake 决策，健壮性提升。对 Walpurgis 用户无影响
  3. **系统角度安全**: `${rapids-cmake-checkout-tag}` 变量由 `include(rapids-cmake/rapids-cmake.cmake)` 注入，为 CMake 配置时常量，非运行时变量，无注入风险；CMake `STREQUAL` 比较是纯字符串操作，无安全面扩展
