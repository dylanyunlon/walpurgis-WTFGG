...
## migrate 8a6ba4a: [SKIP] Use shared-workflows branch-25.04 (#138) — CI runner 分支切换

- **Commit**: `8a6ba4aaec83b59695b777a54053b04e8b33dbc7`
- **Commit message**: `Use shared-workflows branch-25.04 (#138)`
- **Author**: Bradley Dice (bdice) — 2025-02-20
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/138

- **Context**: 完成向 NVKS runners 的迁移后，将 shared-workflows 引用从 `@nvks-runners` 全部切换至 `@branch-25.04`。涉及 `.github/workflows/build.yaml`（10处）、`.github/workflows/pr.yaml`（16处）、`.github/workflows/test.yaml`（6处）、`.github/workflows/trigger-breaking-change-alert.yaml`（1处），共33处字符串替换，零源码变动。该变更对应 rapidsai/build-infra#184 / shared-workflows#273，标志 NVKS runners 全面就绪并稳定化为具名分支。

- **CI/merge → SKIP**:
  - `.github/workflows/` — 纯 CI runner 版本引用，33处 `@nvks-runners` → `@branch-25.04`，Walpurgis 不维护 RAPIDS CI 体系

- **迁移位置**: 无（SKIP）

---

## migrate b405f73: fix(docker): use versioned `-latest` tag for all `rapidsai` images (#246)

- **Upstream commit**: b405f737def44280316cbb01a614c03682d5580e (cugraph-gnn, Gil Forsyth, 2025-07-17)
- **Commit message**: `fix(docker): use versioned -latest tag for all rapidsai images (#246)`
- **Upstream diff 摘要** (4 files changed, 5 insertions(+), 4 deletions(-))：
  - `.github/workflows/build.yaml` — `rapidsai/ci-conda:latest` → `rapidsai/ci-conda:25.08-latest`
  - `.github/workflows/pr.yaml` — 两处 `rapidsai/ci-conda:latest` → `rapidsai/ci-conda:25.08-latest`
  - `.github/workflows/test.yaml` — `rapidsai/ci-conda:latest` → `rapidsai/ci-conda:25.08-latest`
  - `ci/release/update-version.sh` — 新增 sed 规则：branch 创建时自动将 `:<OLD_CALVER>-` 替换为 `:<NEXT_SHORT_TAG>-`，使镜像 tag 随版本号自动滚动

- **CI/merge → SKIP**：
  - `.github/workflows/build.yaml` — GitHub Actions 构建 workflow，Walpurgis 无 GitHub Actions CI 体系
  - `.github/workflows/pr.yaml` — GitHub Actions PR workflow，同上
  - `.github/workflows/test.yaml` — GitHub Actions 测试 workflow，同上
  - `ci/release/update-version.sh` — RAPIDS 专属版本号管理脚本，Walpurgis 使用独立版本策略，无 CalVer 自动滚动需求

- **迁移位置**：无新增文件（全量 SKIP）

- **鲁迅拿法改写（≥20%）**：N/A — 本 commit 四个文件均为 GitHub Actions YAML 与 RAPIDS CI shell 脚本，与 Walpurgis Python 源码体系无交集，改写无意义
