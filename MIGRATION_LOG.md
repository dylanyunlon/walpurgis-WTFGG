...
- **Upstream commit**: e8ed23d20598e560f00daaf011275604472f8e72 (cugraph-gnn, Alex Barghi, 2025-12-15)
- **Commit message**: `Update PyG pin to <2.8 (#360)`
- **Upstream diff 摘要** (10 files changed, 11 insertions(+), 11 deletions(-))：

  | 文件 | 变更 | 说明 |
  |---|---|---|
  | `conda/environments/all_cuda-{129,130}_arch-{aarch64,x86_64}.yaml` × 4 | ±1 × 4 | `pytorch_geometric>=2.5,<2.7` → `>=2.5,<2.8` |
  | `dependencies.yaml` | ±2 | conda 行 + pyproject 行各 1 处（`pytorch_geometric` + `torch-geometric`）|
  | `python/cugraph-pyg/conda/cugraph_pyg_dev_cuda-{129,130}_arch-{aarch64,x86_64}.yaml` × 4 | ±1 × 4 | 同上 |
  | `python/cugraph-pyg/pyproject.toml` | ±1 | `torch-geometric>=2.5,<2.7` → `>=2.5,<2.8` |

- **CI/merge → SKIP**（全部 10 个文件）：
  - `conda/environments/*.yaml` — SKIP：Walpurgis 无 conda 环境矩阵，conda env yaml 无迁移目标
  - `dependencies.yaml` — SKIP：RAPIDS 构建依赖清单，Walpurgis 用 pyproject.toml 独立管理
  - `python/cugraph-pyg/conda/*.yaml` — SKIP：上游 conda 开发环境，同上
  - `python/cugraph-pyg/pyproject.toml` — SKIP：上游包构建配置，非 Walpurgis 源码

- **迁移位置**: `src/walpurgis/core/pyg_pin_upgrade.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`PygVersionRange` dataclass**: 将上游裸字符串 `>=2.5,<2.8` 强类型化为可操作对象，`contains(v)`/`overlaps(r)`/`as_pip_spec()`/`as_conda_spec()` 四种视图——上游只有字符串替换，无任何行为
  2. **`PygPinBump` dataclass**: 封装"FROM→TO"跃迁事实，携带 `commit_sha`/`pr_url`/`author`/`rationale`/`compat_notes`，`upper_delta()`/`is_forward()`/`unlocks_version()` 方法均为上游所无；`E8ED23D_BUMP` 实例化本 commit 的全部跃迁元数据
  3. **`PygCompatMatrix`**: 建模 PyG 版本与 CUDA 12.x/13.x 的兼容性矩阵，`lookup(pyg_version)` / `supports_cuda()` 将上游隐式散落在 yaml 文件名（cuda-129/cuda-130）中的兼容信息集中可查——上游无此 Python 层查询接口
  4. **`PygPinAudit`**: 同时扫描旧约束（`<2.7`）和新约束（`<2.8`），`assert_upgraded(path)` 在 CI 中防止回退和漏更新；正则采用 `(?m)^[^\S\n]*` 行首断言精确排除注释行——上游无任何 Python 层守卫
  5. **`PygUpgradeImpact` dataclass**: 将上游 PR 空白描述补全为 5 项 PyG 2.7.x 新特性说明，并与 Walpurgis 模型层（sampler/dataloader/graph）逐一交叉评估影响，`action_required=False` 给出明确结论——上游 PR 只写"support latest version of PyG"
  6. 全链路 `WALPURGIS_DEBUG=1` 断点（8 处）：覆盖模块加载→PygVersionRange 构造→contains 检查→PygPinBump 跃迁注册→compat_matrix lookup→old/new pin 扫描→assert_upgraded→self_test 各阶段
