...
## migrate d491fae: [SKIP] Remove CUDA 11 from dependencies.yaml — 纯依赖矩阵清理，无迁移价值

- **Upstream commit**: d491fae (cugraph-gnn, Kyle Edwards, 2025-06-04, PR #224)
...
- **Upstream diff** (5 files changed, 3 insertions, 229 deletions):
  ...
  - `dependencies.yaml` — 移除所有 `cuda: "11.*"` 矩阵条目及对应的 cudatoolkit/cuda-nvtx/gcc_linux-*/nvcc_linux-*/cu118 PyTorch index/pylibwholegraph-cu11/libraft-cu11/librmm-cu11 等依赖块
...
- **迁移价值**: 无
- **原因**: 纯 conda 环境文件 + RAPIDS `dependencies.yaml` 矩阵配置清理，移除 CUDA 11.x 支持。无任何 `.py`/`.cu`/`.cpp`/`.h` 源码改动。Walpurgis 无 conda 构建体系、无 `dependencies.yaml`、无 CUDA 版本矩阵管理，受影响的所有文件在 Walpurgis 仓库中均不存在。
- **Knuth 审查**:
  1. diff 完整性: 5 个文件全量审查，均为构建基础设施产物，无业务逻辑
  2. 用户角度 bug: 无运行时代码改动，零 bug 风险
  3. 内存/并发/性能安全: 不适用（无可执行代码）
