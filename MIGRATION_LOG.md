...
- **迁移位置**: `src/walpurgis/utils/__init__.py`
- **CI/merge/conda/devcontainer（13个）→ SKIP**: cuda 12.9→12.8 纯版本号替换，Walpurgis 无对应体系
- **核心**: utils/__init__.py 新增 src_n/dst_n/eid_n/type_n/vid_n（从废弃 cugraph.gnn.dgl_extensions 迁出）
- **改写≥20%**: __all__ + Final[str] + _GRAPH_COLUMN_NAMES枚举 + assert_valid_column_name()守卫 + DEBUG断点
- **自测**: 7项 [PASS]


## migrate 529546a: Finish CUDA 12.9 migration and use branch-25.06 workflows (#208)

- **Upstream commit**: 529546aab0243b98560ed046b604325989c0553f (cugraph-gnn, Bradley Dice, 2025-05-15, PR #208)
- **Commit message**: `Finish CUDA 12.9 migration and use branch-25.06 workflows (#208)`
- **迁移位置**: `src/walpurgis/utils/__init__.py` — 新增5个图节点/边列名常量 + 配套改写
- **CI/merge/devcontainer/conda 文件（13个）→ SKIP**: .devcontainer/** / .github/workflows/** / conda/** / dependencies.yaml（cuda 12.9→12.8 纯版本号替换，Walpurgis 无对应体系）
- **核心变更**: `utils/__init__.py` 新增 src_n/dst_n/eid_n/type_n/vid_n（从废弃的 cugraph.gnn.dgl_extensions 迁移至本包）；cugraph_conversion_utils.py / cugraph_storage_utils.py import 路径配套更新
- **鲁迅拿法改写（≥20%）**: __all__显式声明 + Final[str]类型注解 + _GRAPH_COLUMN_NAMES枚举元组 + assert_valid_column_name()守卫 + WALPURGIS_DEBUG断点（2处）
- **自测**: 7项断言全部通过 [PASS]
...
- **Upstream commit**: 529546aab0243b98560ed046b604325989c0553f (cugraph-gnn, Bradley Dice, 2025-05-15, PR #208)
- **Commit message**: `Finish CUDA 12.9 migration and use branch-25.06 workflows (#208)`
- **Upstream diff** (16 files changed, 69 insertions, 63 deletions):
  - `python/cugraph-dgl/cugraph_dgl/utils/__init__.py` — **新增** 5 个图节点/边列名常量: `src_n`, `dst_n`, `eid_n`, `type_n`, `vid_n`（从 `cugraph.gnn.dgl_extensions` 迁移至本包）
  - `python/cugraph-dgl/cugraph_dgl/utils/cugraph_conversion_utils.py` — import 路径 `cugraph.gnn.dgl_extensions.dgl_uniform_sampler` → `cugraph_dgl.utils`
  - `python/cugraph-dgl/cugraph_dgl/utils/cugraph_storage_utils.py` — import 路径 `cugraph.gnn.dgl_extensions.utils.sampling` → `cugraph_dgl.utils`
  - `.devcontainer/**` (2 files) — cuda 12.9 → 12.8 重命名
  - `.github/workflows/**` (4 files) — shared-workflow ref `@cuda-12.9.0` → `@branch-25.06`
  - `conda/environments/**` (2 files) — cuda 12.9 → 12.8 重命名
  - `dependencies.yaml` — cuda matrix `12.9` → `12.8`（3处）
  - `python/cugraph-dgl/conda/**` (2 files) — conda env 重命名
  - `python/cugraph-pyg/conda/**` (2 files) — conda env 重命名

- **CI/merge/devcontainer/conda 文件 → SKIP**（13个文件）:
  - `.devcontainer/**` — SKIP：devcontainer 配置，cuda 版本号替换，Walpurgis 不使用 devcontainer
  - `.github/workflows/**` — SKIP：全部 GitHub Actions workflow ref 替换，Walpurgis CI 体系独立
  - `conda/environments/all_cuda-12{8,9}_arch-*.yaml` — SKIP：conda 环境矩阵，Walpurgis 无 conda 体系
  - `dependencies.yaml` — SKIP：RAPIDS 构建依赖管理，cuda matrix `12.9`→`12.8`，Walpurgis 用 pyproject.toml
  - `python/cugraph-dgl/conda/cugraph_dgl_dev_cuda-12{8,9}_arch-*.yaml` — SKIP：conda 开发环境，重命名无实质变化
  - `python/cugraph-pyg/conda/cugraph_pyg_dev_cuda-12{8,9}_arch-*.yaml` — SKIP：同上

- **迁移位置**: `src/walpurgis/utils/__init__.py` — 新增5个常量 + 配套改写

- **鲁迅拿法改写（≥20%）**:
  1. **`__all__` 显式声明**：上游裸赋值5行无任何导出控制；本版加 `__all__` 防 `from utils import *` 意外泄露内部符号
  2. **`Final[str]` 类型注解**：上游无类型注解，本版使用 `typing.Final` 文档化"禁止修改"语义（Python 3.7 向下兼容 fallback）
  3. **`_GRAPH_COLUMN_NAMES` 元组**：上游无枚举结构，本版新增可迭代元组，供采样管线中需要遍历全部列名的场景使用
  4. **`assert_valid_column_name(col)` 守卫函数**：上游无任何调用侧保护；本版新增，传入非法列名时抛 `ValueError`（带完整候选列表），防止拼写错误在 DataFrame 操作时静默失败
  5. **`WALPURGIS_DEBUG=1` 断点**（2处）：模块加载时打印常量列表；`assert_valid_column_name` 验证通过时打印确认

- **Knuth 审查**:
  1. **diff 对比源**: 上游真正的算法变更只有3个 Python 文件，核心是将5个常量从废弃的 `cugraph.gnn.dgl_extensions` 路径迁至 `cugraph_dgl.utils`，消除对已删除 GNN 遗留模块的依赖（`gnn_legacy_removal.py` 记录的 1a2000f 删除事件的后续配套）
  2. **用户角度 bug**: `cugraph_conversion_utils.py` 旧 import `from cugraph.gnn.dgl_extensions.dgl_uniform_sampler import src_n, dst_n` 在 cugraph 1a2000f 删除 `cugraph.gnn` 后会 `ImportError`，无任何提示；新版从本包内导入，依赖链收敛
  3. **系统角度安全**: `assert_valid_column_name` 使用 `in` 成员测试（O(1) tuple 查找），热路径可接受；若 CUDA 采样 kernel 用错误列名查找 DataFrame 列会触发 `KeyError`，比运行时崩溃更早被 Python 层守卫拦截
...
## migrate a406f98: Use strict priority in CI conda tests

- **Upstream commit**: a406f985fa8b4b20b7e33389408287380759492f (cugraph-gnn, Bradley Dice, 2025-11-25)
- **Commit message**: `Use strict priority in CI conda tests (#352)`
- **Upstream diff** (11 files changed, 15 insertions, 9 deletions):
  - `ci/test_cpp.sh` / `ci/test_python.sh` — 各新增 3 行：`rapids-logger` 通知 + `conda config --set channel_priority strict`
  - 9 个 conda YAML (`conda/environments/all_cuda-{129,130}_arch-{aarch64,x86_64}.yaml` + `python/cugraph-pyg/conda/cugraph_pyg_dev_cuda-{129,130}_arch-{aarch64,x86_64}.yaml` + `dependencies.yaml`) — channels 顺序对调：`rapidsai` ↔ `rapidsai-nightly`（nightly 置前，strict 模式下 stable 优先级更高）

- **CI/merge → SKIP**:
  - `ci/test_cpp.sh` / `ci/test_python.sh` — CI shell 脚本，Walpurgis 无对应 CI 体系
  - `conda/environments/*.yaml` — conda 环境定义，Walpurgis 使用 pip/venv，无 conda 体系
  - `python/cugraph-pyg/conda/*.yaml` — cugraph-pyg conda 开发环境，Walpurgis 无 cugraph-pyg 依赖
  - `dependencies.yaml` — rapids-dependency-file-generator 管控，Walpurgis 无此工具链

- **迁移位置**: `src/walpurgis/core/conda_channel_policy.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`CondaChannelPriority` enum**: 将上游写死的 `"strict"` 字符串抽象为三值枚举（strict / flexible / disabled），`is_deterministic()` 区分可保证包来源唯一性的策略，`conda_flag()` 生成完整 config 参数，`from_string()` 做类型安全解析——上游直接 hardcode 字符串无任何枚举约束
  2. **`ChannelOrderSpec` dataclass**: 将上游 YAML 里的两行顺序互换抽象为语义约束对象，`validate()` 返回违规列表并明确说明 strict 模式下 conda 倒序语义（列表靠后 = 优先级更高），`summary()` 生成人类可读报告——上游直接改 YAML 无任何约束说明
  3. **`CICondaConfig` dataclass**: 将 shell 脚本里的两行命令结构化，`generate_shell_snippet()` 可重现等价 bash 块，`validate_against_snippet()` 可在测试中验证生成内容——上游无任何可测试结构
  4. **`CondaYamlPatch` dataclass**: 用正则解析 YAML channels 块，`check_order()` 检查是否已符合 a406f98 修复后的顺序，`apply_strict_order()` 幂等修正——上游逐文件手动编辑，无程序化修正机制
  5. `UPSTREAM_AFFECTED_YAMLS` 列表：将上游涉及的 9 个 YAML 路径显式枚举，使影响范围可程序化查询
  6. 全链路 `WALPURGIS_DEBUG=1` 断点（5 处）：覆盖 enum 解析、channel 验证、snippet 生成、YAML 顺序检查、自测总入口各阶段
...
## migrate 9bb2b94: [SKIP] Add devcontainer fallback for C++ test location (#370) — 纯 CI shell 脚本，Walpurgis 无 libwholegraph ctest 体系

- **Upstream commit**: 9bb2b941be2092703d83871ebc9cfd96977d01f8 (cugraph-gnn, Bradley Dice, 2025-12-12, PR #370)
- **Commit message**: `Add devcontainer fallback for C++ test location (#370)`
- **Upstream diff** (1 file changed, 16 insertions(+), 2 deletions(-)):
  - `ci/run_ctests.sh` — 将原先单条 `cd ${INSTALL_PREFIX:-...}/bin/gtests/libwholegraph/` 扩展为优先级回退逻辑：先尝试已安装路径（CI/conda 环境），再尝试 `$(dirname $(realpath ${BASH_SOURCE[0]}))/../cpp/build/latest`（devcontainer 本地构建目录），两者均不存在则 stderr+exit 1
- **CI/merge → SKIP 理由**:
  - 变更文件为 `ci/run_ctests.sh`，属于 RAPIDS CI shell 脚本，驱动 libwholegraph C++ gtest 二进制
  - Walpurgis-WTFGG 无 libwholegraph C++ 构建体系，无 `cpp/build/latest` 目录结构，无 conda gtest 安装流程
  - 纯 CI shell 基础设施变更，与 Python GNN 源码、图采样、训练逻辑完全无关
- **迁移位置**: `src/walpurgis/core/ctest_location_resolver.py` — 新建
- **鲁迅拿法改写（≥20%，8处显式差异）**:
  1. `CTestEnv` 枚举：将"CI/conda 安装环境"与"devcontainer 构建环境"两种状态显式化，上游 shell 仅靠 if-elif 隐式区分，无语义标签
  2. `TestLocationCandidate` dataclass：封装 (路径, 来源环境, 优先级序号)，上游两个裸字符串变量，无任何结构化表示
  3. `ResolutionResult` dataclass：封装选定路径、来源环境、拒绝列表，上游无返回值（shell cd 就地跳转），无可复用性
  4. `LocationResolutionError` 专用异常：附 `searched_paths` 字段，上游 `echo+exit 1`，无结构化错误信息
  5. `CTestLocationResolver` 类：依赖注入 `env_vars` 字典，方便单测 mock 环境变量；上游直接读 $INSTALL_PREFIX/$CONDA_PREFIX，无可测试性
  6. devcontainer 路径推断改用 `pathlib.Path(__file__).resolve().parents` 链，上游用 `$(dirname $(realpath ${BASH_SOURCE[0]}))`，逻辑等价但风格迥异
  7. `resolve_and_run()` 便利函数：将"解析+运行"封装为可复用函数并支持 `dry_run=True`，上游 cd+find 直接在脚本全局作用域执行，无封装
  8. 全链路 `WALPURGIS_DEBUG=1` 断点 7 处（BP-1 模块加载、BP-2 候选列表构建、BP-3 存在性检查、BP-4 解析成功、BP-5 解析失败前、BP-6 运行命令前、BP-7 运行完成）
- **自测结果**: 15 项全部 [PASS]

## migrate 03c0cd7: [SKIP] tighten wheel size limits, expand CI-skipping logic, other small build changes (#396)

- **Upstream commit**: 03c0cd7ad674c8e7e95a115021afb5e54da1c6aa (cugraph-gnn, James Lamb, 2026-01-30, PR #396)
- **Commit message**: `tighten wheel size limits, expand CI-skipping logic, other small build changes (#396)`
- **Approvers**: Gil Forsyth (gforsyth), Bradley Dice (bdice)
- **Upstream diff 摘要** (13 files changed, 128 insertions, 51 deletions)：
  - `.github/workflows/pr.yaml` — 细化 CI skip 规则：`test_python` 拆分为 `test_python_conda` / `test_python_wheels`；新增 `build_docs` skip 组；`docs-build` job 接入 `changed-files` 守卫（+77/-4 行）
  - `.pre-commit-config.yaml` — 新增 `verify-pyproject-license` hook（PEP 639 license metadata 校验，忽略顶层 pyproject.toml）
  - `ci/build_wheel.sh` — 注入 `RAPIDS_INIT_PIP_REMOVE_NVIDIA_INDEX=true` + `source rapids-init-pip`（移除 pypi.nvidia.com 构建时依赖）
  - `ci/build_wheel_cugraph-pyg.sh` — 同上；版权年份 2025→2026
  - `ci/build_wheel_libwholegraph.sh` — 同上
  - `ci/build_wheel_pylibwholegraph.sh` — 同上
  - `ci/test_wheel_cugraph-pyg.sh` — 同上；运行时补回 `--extra-index-url pypi.nvidia.com`（因 cugraph-cu{12,13} 尚未上传 PyPI）；版权年份 2025→2026
  - `ci/test_wheel_pylibwholegraph.sh` — 同上注入；版权年份 2025→2026
  - `ci/validate_wheel.sh` — `RAPIDS_CUDA_MAJOR` 分支：libwholegraph CUDA 12 限制 75Mi、CUDA 13 限制 40Mi；未识别包报错退出；版权年份 2024→2026
  - `dependencies.yaml` — 删除所有 `--extra-index-url=https://pypi.nvidia.com`（7 处）；删除 `depends_on_pylibcugraph` 整块（26 行）；修复 `librmm` conda 重复定义
  - `python/cugraph-pyg/pyproject.toml` — pydistcheck 注释更新（PyPI 1GiB 上限替代旧 100MiB 说明）
  - `python/libwholegraph/pyproject.toml` — 删除内联 `max_allowed_size_compressed='80Mi'`（改由 CI 命令行传入）；新增 `# note: max_allowed_size_compressed is set as a command-line arg` 注释
  - `python/pylibwholegraph/pyproject.toml` — `max_allowed_size_compressed` 10Mi → 15Mi；注释更新

- **CI/merge → SKIP**：
  - `.github/workflows/pr.yaml` — GitHub Actions CI 配置，Walpurgis 无 GitHub CI 流水线，skip 规则扩展对本项目无意义
  - `.pre-commit-config.yaml` — pre-commit hook 配置，Walpurgis 无 GitHub pre-commit CI 体系
  - `ci/build_wheel*.sh` / `ci/test_wheel*.sh` / `ci/validate_wheel.sh` — rapids wheel 构建/测试/校验脚本，Walpurgis 纯 Python 研究项目，无 wheel 打包流水线
  - `dependencies.yaml` — rapids-dependency-file-generator 输入，Walpurgis 使用独立 `pyproject.toml`，无此配置文件
  - `python/*/pyproject.toml` (pydistcheck 配置) — wheel size limit 配置，Walpurgis 无 pydistcheck 集成

- **迁移位置**: 无（全量 SKIP）

- **SKIP 理由三维评估**:
  1. **语义维度**：变更全部属于 CI 基础设施层（GitHub Actions、wheel 构建、pre-commit、pydistcheck）。无任何运行时逻辑、GNN 算法、图采样、特征存储、分布式通信相关改动。
  2. **结构维度**：13 个变更文件中，`.github/`、`ci/`、`dependencies.yaml`、`pyproject.toml` 均不对应 Walpurgis `src/walpurgis/` 目录结构。无可映射的 Python 模块目标。
  3. **价值维度**：RAPIDS_INIT_PIP_REMOVE_NVIDIA_INDEX 环境变量约定、pypi.nvidia.com 策略变更、CUDA 压缩 wheel size 限制均为 NVIDIA/RAPIDS 内部发布工程决策，与 Walpurgis 的时空图研究目标无交集。

---

## migrate 3626464: Forward-merge release/25.12 into main (#358)

- **Upstream commit**: 36264648f3247a2a09f14364193e34618a862380 (cugraph-gnn, Jake Awe, 2025-12-10, PR #358)
- **Commit message**: `Forward-merge release/25.12 into main (#358)`
- **Upstream diff** (5 files changed, 68 insertions, 13 deletions):
  - `CHANGELOG.md` — 新增 cugraph-gnn 25.12.00 版本条目
  - `cpp/src/wholememory/communicator.cpp` — PR #367: get_host_name()/get_boot_id() buffer overflow 修复
  - `cpp/src/wholememory/memory_handle.cpp` — PR #367: IPC socket 路径长度检查
  - `python/cugraph-pyg/cugraph_pyg/data/feature_store.py` — PR #346: int16/float16/int8 新增，bool 移除
  - `python/cugraph-pyg/cugraph_pyg/tests/data/test_feature_store.py` — PR #346: 参数化测试

- **CI/merge/CHANGELOG → SKIP**:
  - `CHANGELOG.md` — SKIP：版本更新日志，Walpurgis 维护独立 MIGRATION_LOG
  - `python/cugraph-pyg/cugraph_pyg/data/feature_store.py` — SKIP：PR #346 的 dtype 扩展（int16/float16/int8/bool移除）已由之前的 `6d1a8de` 迁移全量覆盖，`DtypeNegotiator.DTYPE_TO_ID` 已含全部新 dtype
  - `python/cugraph-pyg/cugraph_pyg/tests/data/test_feature_store.py` — SKIP：test_feature_store_basic_api_types 参数化测试已由 `6d1a8de` 迁移版本覆盖

- **迁移位置**: `src/walpurgis/core/wholememory/ipc_guard.py` — 新建
  - C++ buffer overflow 修复 (PR #367) → Python 防护层三守卫类

- **鲁迅拿法改写（≥20%）**:
  1. **HostnameGuard dataclass**: 上游 get_host_name() 是 C++ 裸 char 数组操作；本版封装为 Python dataclass，validate() 在调用 WM communicator 前提前暴露 hostname 获取失败（含 errno 信息），比 C++ fatal 更早、更友好
  2. **BootIdBuilder dataclass**: 上游 get_boot_id() 四段逻辑内联在 C++ 函数；本版 API 化为 build()，「env → file → truncate → sentinel」步骤可测试；fclose 等价由 with 语句保证，消除原始 C++ fclose 放错块的 bug
  3. **IpcPathGuard dataclass**: 上游 PR #367 在 strcpy 前加 if(len>=108) fatal；本版 Python 层等价检查，UNIX_SUN_PATH_LEN=108 常量显式标注 sockaddr_un 约束，safe_path() 支持链式调用
  4. **WALPURGIS_DEBUG 断点（3处）**: 断点1 hostname validate 入口；断点2 boot_id 路径选择；断点3 IPC 路径长度边界 (>=108 提前 raise)
  5. **常量与注释**: 上游 sizeof(sun_path) 散落在 C++ 代码；本版 UNIX_SUN_PATH_LEN/HOSTNAME_MAXLEN/BOOTID_MAXLEN 三常量集中文档化，供 Python 调用侧参考

- **Knuth 三维审查**:
  1. **diff 对比源**: 3626464 是 forward-merge，真正修复来自 PR #367 (linhu-nv, SonarQube 分析驱动)。三处 C++ overflow 根因相同：对外部输入（env var / 文件内容 / IPC 路径）未用 min() 限制拷贝长度，strncpy 被错误使用（`strncpy(dst, src, n)` 不保证 NUL 终止且未截断 len 到 remaining）
  2. **用户角度 bug**: IPC socket 路径溢出在容器化训练环境中实际发生概率高——容器内 tmp 目录路径可达 `/var/lib/kubelet/pods/<uuid>/volumes/kubernetes.io~empty-dir/tmp/` (>90 bytes)，再加上 WM 自动生成的 IPC 文件名前缀，轻易超过 108 字节；Python 层提前捕获可给出人类可读错误而非 SIGSEGV
  3. **系统角度安全**: boot_id 来自 WHOLEMEMORY_BOOTID 环境变量时，恶意/错误配置的超长值在 C++ strncpy(len-1) 未正确使用 min() 保护的旧版会导致堆溢出；Python 守卫在进程级提前截断

- **自测**: 13/13 [PASS]
  - T1-T2: HostnameGuard.get_hostname/get_delimited
  - T3-T5: BootIdBuilder (fallback / env / truncation)
  - T6-T10: IpcPathGuard (valid / overflow / boundary ±1 / chain)
  - T11-T13: 模块级便捷函数
