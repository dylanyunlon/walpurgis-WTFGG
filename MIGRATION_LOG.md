...
## migrate c50d56d: [SKIP] cugraph-pyg: remove unnecessary dependency on 'numba' — 纯 conda/CI 依赖清理，Walpurgis 无对应结构

- **Upstream commit**: c50d56d0d4987c587e6f377dd4eb9537f3d0440b (cugraph-gnn, James Lamb, 2025-08-27, PR #287)
- **Commit message**: `cugraph-pyg: remove unnecessary dependency on 'numba' (#287)`
- **Approvers**: Mike Sarahan

**Upstream diff** (5 files changed, 25 deletions, 0 insertions):

| 文件 | 变更内容 |
|---|---|
| `ci/release/update-version.sh` | 从 DEPENDENCIES 数组中删除 `dask-cudf` 和 `rapids-dask-dependency` 两个条目 |
| `conda/environments/all_cuda-129_arch-aarch64.yaml` | 删除 `dask-cudf==25.10.*,>=0.0.0a0` 依赖条目 |
| `conda/environments/all_cuda-129_arch-x86_64.yaml` | 删除 `dask-cudf==25.10.*,>=0.0.0a0` 依赖条目 |
| `conda/recipes/cugraph-pyg/recipe.yaml` | 删除 `numba >=0.57` run 依赖 |
| `dependencies.yaml` | 删除整个 `depends_on_dask_cudf` 依赖块（20行），含 conda/requirements/pyproject 三套声明 |

**变更原因**:
- `numba` 对 `cugraph-pyg` wheels 从未声明依赖，且 `git grep numba` 无直接使用
- `dask-cudf` / `rapids-dask-dependency` 的大部分 dask 相关代码已在 PR #199、#166、#168 中移除，仅剩 `cugraph-gnn/mg_utils` 目录中有少量残留

**CI/merge → SKIP 原因**:
- 5 个文件全部位于 conda/CI 配置层：`ci/release/` 发布脚本、`conda/environments/` 测试环境、`conda/recipes/` 构建配方、`dependencies.yaml` RAPIDS 依赖矩阵
- Walpurgis 项目无 `conda/` 目录、无 `ci/release/` 脚本、无 `dependencies.yaml` RAPIDS 依赖体系
- Walpurgis 依赖由自身 `pyproject.toml` 管理，不从 RAPIDS conda 体系继承
- `numba` / `dask-cudf` 在 Walpurgis 中均无直接引用，清理无意义

**Knuth 审查**:
1. **diff 完整性**: 5 个文件逐一确认，全部属于 conda/CI 基础设施层，无 Python/C++/CUDA 运行时代码
2. **用户角度**: 移除 numba 声明对 wheel 用户行为无影响（wheel 本已不声明），conda 用户安装时减少一个无用依赖
3. **系统安全**: 无安全影响；删除依赖声明不影响任何功能路径
