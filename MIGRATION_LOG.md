...
## migrate c254763: Use static linkage for CUDA runtime

- **Upstream commit**: c254763d18dbaf4fb88a0aed3ecd07b18ee535c2 (cugraph-gnn, Bradley Dice, 2025-12-11, PR #369)
- **Commit message**: `Use static linkage for CUDA runtime (#369)`
- **Upstream diff** (9 files changed, 4 insertions, 21 deletions):
  - `ci/build_wheel_libwholegraph.sh` — 移除 `-DCUDA_STATIC_RUNTIME=ON`（已默认，无需显式传入）
  - `ci/build_wheel_pylibwholegraph.sh` — 同上
  - `conda/environments/all_cuda-129_arch-aarch64.yaml` — 移除 `cuda-cudart-dev` 依赖
  ...
  - `conda/recipes/libwholegraph/recipe.yaml` — 移除 `cuda-cudart-dev`（build host）、`cuda-cudart`（run）及 `ignore_run_exports.by_name` 中对应条目（共 7 行）
  - `cpp/CMakeLists.txt` — 删除 `option(CUDA_STATIC_RUNTIME ...)` + `_ctk_static_suffix` 变量逻辑（9 行 → 1 行）；`rapids_cuda_init_runtime(USE_STATIC ${CUDA_STATIC_RUNTIME})` → `rapids_cuda_init_runtime(USE_STATIC ON)`；`CUDA::cudart${_ctk_static_suffix}` → `CUDA::cudart_static`
  - `dependencies.yaml` — 移除 `cuda-cudart-dev`（conda build 依赖块）

- **变更语义**:
  上游将 libwholegraph 从「可选静态 CUDA runtime（默认关闭）」改为「始终静态链接 CUDA runtime」。
  旧 CMake option `CUDA_STATIC_RUNTIME OFF`（默认）被彻底删除，`USE_STATIC ON` 硬编码。
  主要效果：
  1. **运行时依赖消除**: `cuda-cudart` 共享库不再是 wheel 运行时依赖，libcudart 直接嵌入 `libwholegraph.so`，约 +4 MiB 二进制体积换取依赖链简化。
  2. **conda recipe 精简**: `cuda-cudart` / `cuda-cudart-dev` 从 build/run/ignore_run_exports 三处全部移除，recipe 结构更清晰。
  3. **CI 脚本简化**: `build_wheel_*.sh` 中显式传入的 `-DCUDA_STATIC_RUNTIME=ON` 已为冗余，随之删除。
  4. **符合 build-planning#235**: RAPIDS 生态统一向静态 CUDA runtime 迁移，此 commit 是 cugraph-gnn 侧的实现。

- **CI/merge → SKIP 文件清单**:
  | 文件 | 跳过原因 |
  |------|---------| 
  | `ci/build_wheel_libwholegraph.sh` | RAPIDS wheel CI 脚本，Walpurgis 无 wheel 构建体系 |
  | `ci/build_wheel_pylibwholegraph.sh` | 同上 |
  | `conda/environments/all_cuda-129_arch-*.yaml` | conda 环境 YAML，Walpurgis 无 conda 体系 |
  | `conda/environments/all_cuda-130_arch-*.yaml` | 同上 |
  | `conda/recipes/libwholegraph/recipe.yaml` | conda recipe，Walpurgis 不打包 libwholegraph |
  | `dependencies.yaml` | RAPIDS 构建依赖矩阵，Walpurgis 用 pyproject.toml |
  | `cpp/CMakeLists.txt` | C++ CMake 配置，Walpurgis 不重新编译 libwholegraph |

- **实际迁移位置**: `src/walpurgis/cuda_build_config.py` — 追加 c254763 迁移块

- **鲁迅拿法改写（≥20%）**:
  鲁迅在《呐喊·自序》中写道："我在年青时候也曾经做过许多梦，后来大半忘却了……"
  上游删除了一个"曾经存在的选项"——`CUDA_STATIC_RUNTIME OFF`。选项存在的岁月如梦，
  删除意味着从此只有一条路：静态链接。本迁移以同等精神，将这个"消失的选项"重新
  结构化为可审计的 Python 层记录，让每一个调用者都能知道「曾经有 OFF，如今只有 ON」。

  上游 9 个文件的变更语义被重构为 3 个 Python 组件：

  | 上游变更 | Walpurgis 迁移类 | 增量 |
  |---------|----------------|------|
  | `CUDA_STATIC_RUNTIME` option 删除 | `CudaRuntimeLinkage` 枚举 | 显式化 STATIC/DYNAMIC 两种语义，枚举值保留历史轨迹 |
  | `USE_STATIC ON` 硬编码 | `CudaStaticRuntimePolicy` dataclass | `is_static()` / `link_flag()` / `cmake_target` 全链路可查 |
  | `cuda-cudart{-dev}` conda 依赖移除 | `CudaCudartDepRemoval` dataclass | `was_removed(pkg)` / `audit_report()` 可程序化审计 |
  | 跨 9 个文件的删除操作 | `CUDA_STATIC_RUNTIME_POLICY` / `CUDA_CUDART_DEP_REMOVAL` 模块级单例 | 单点查询，无副作用 |

- **断点分布**（5 处，均为 `WALPURGIS_DEBUG=1` 门控的 `_dbg_c254763()` 调用）:
  - BP-1: `CudaStaticRuntimePolicy` 模块级单例初始化（策略对象构建时打印）
  - BP-2: `get_cuda_runtime_policy()` 每次调用时打印策略摘要
  - BP-3: `nvflags_with_cuda_runtime()` 叠加 fatbin flag 后、追加 runtime flag 前打印中间状态
  - BP-4: `nvflags_with_cuda_runtime()` 链接模式决策结果打印
  - BP-5: `CudaCudartDepRemoval` 模块级单例初始化（依赖移除记录构建时打印）

- **自测结果**: 7 项断言全通过（`python -c "... 7 assertions PASS"`）
  - `policy.is_static()` → True
  - `policy.cmake_target` → `"CUDA::cudart_static"`
  - `policy.link_flag()` → `"-lcudart_static"`
  - `CUDA_CUDART_DEP_REMOVAL.was_removed("cuda-cudart")` → True
  - `CUDA_CUDART_DEP_REMOVAL.was_removed("cuda-cudart-dev")` → True
  - `"-lcudart_static" in nvflags_with_cuda_runtime(["-std=c++14"])` → True
  - `CudaRuntimeLinkage.STATIC.value == "static"` → True
