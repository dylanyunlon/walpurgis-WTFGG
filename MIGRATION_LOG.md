...
## 1c964d1 — Use ruff-check, ruff-format instead of flake8, black (#342)

- **Commit**: `1c964d1ea158cff07c8e1fddbf0acbca4c0b30de`
- **Commit message**: `Use ruff-check, ruff-format instead of flake8, black (#342)`
- **Author**: Kyle Edwards (KyleFromNVIDIA, NVIDIA) / linhu-nv / Alex Barghi / Bradley Dice
- **Context**: 将 pre-commit 工具链从 `black@22.10.0 + flake8@7.1.1 + flake8-force` 迁移至 `ruff@v0.14.3`（`ruff-check --fix + ruff-format`）；删除 `.flake8`，在根 `pyproject.toml` 中新增 `[tool.ruff]` 节（`line-length=88`，`ignore=["E203"]`）；同时对约 20 个 Python 文件做全量 `ruff-format` 格式化（6 种范式：括号包裹赋值、元组解包去括号、函数体首行空行删除、注释空格补全、字符串字面量合并、assert 消息括号化）。`rapidsai/pre-commit-hooks` 版本从 `v1.2.0` 升到 `v1.2.1`。

- **Upstream diff** (24 files changed, 102 insertions(+), 135 deletions(−)):
  | 文件 | 变更内容 |
  |------|----------|
  | `.flake8` | **删除**（26行，迁移至 pyproject.toml [tool.ruff.lint]） |
  | `.pre-commit-config.yaml` | black@22.10.0 + flake8@7.1.1 → ruff-check + ruff-format@v0.14.3；pre-commit-hooks v1.2.0→v1.2.1 |
  | `pyproject.toml`（根，新建） | 新增 [tool.ruff] line-length=88, exclude=[\_\_init\_\_.py], [tool.ruff.lint] ignore=[E203] |
  | `ci/utils/git_helpers.py` | 版权 2024→2025；过长参数列表展开（3处） |
  | `ci/utils/nbtestlog2junitxml.py` | 版权 2024→2025；`setFileNameAttr` 参数内联；`#注释`→`# 注释` |
  | `mg_utils/wait_for_workers.py` | 版权 2024→2025；删除 `import yaml`；空格赋值规范化（3处） |
  | `python/cugraph-pyg/cugraph_pyg/data/feature_store.py` | 括号包裹赋值 + 元组解包去括号（`for (k,v)→for k,v`） |
  | `python/cugraph-pyg/cugraph_pyg/examples/fraud/bitcoin_mnmg.py` | 括号包裹赋值简化 |
  | `python/cugraph-pyg/cugraph_pyg/examples/fraud/bitcoin_rf.py` | 多行 f-string 合并为单行 |
  | `python/cugraph-pyg/cugraph_pyg/examples/gcn_dist_mnmg.py` | 元组解包去括号（4处） |
  | `python/cugraph-pyg/cugraph_pyg/examples/kg/renumber_kg.py` | help 字符串拼接合并 |
  | `python/cugraph-pyg/cugraph_pyg/examples/movielens_mnmg.py` | 元组解包 + 函数体首行空行删除 |
  | `python/cugraph-pyg/cugraph_pyg/examples/rgcn_link_class_mnmg.py` | with 块首行空行删除 |
  | `python/cugraph-pyg/cugraph_pyg/examples/taobao_mnmg.py` | 元组解包去括号 |
  | `python/cugraph-pyg/cugraph_pyg/sampler/distributed_sampler.py` | 函数体首行空行删除（4处）+ 括号包裹赋值 |
  | `python/cugraph-pyg/cugraph_pyg/sampler/sampler_utils.py` | assert 消息括号化（4处） |
  | `python/cugraph-pyg/cugraph_pyg/tensor/dist_tensor.py` | 函数体首行空行 + `not (len(...)  in)` → `len(...) not in` |
  | `python/cugraph-pyg/cugraph_pyg/tensor/utils.py` | assert 消息括号化 |
  | `python/cugraph-pyg/cugraph_pyg/tests/loader/test_neighbor_loader.py` | 函数体首行空行 + 括号包裹赋值（5处） |
  | `python/cugraph-pyg/cugraph_pyg/tests/loader/test_neighbor_loader_mg.py` | 括号包裹赋值 |
  | `python/cugraph-pyg/cugraph_pyg/utils/imports.py` | 字符串字面量合并（`"a" "b"` → `"a b"`） |
  | `python/pylibwholegraph/pylibwholegraph/test_utils/test_comm.py` | 版权 2024→2024-2025 + 函数体首行空行 |
  | `python/pylibwholegraph/pylibwholegraph/torch/comm.py` | 版权 2024→2025 + 函数体首行空行 |
  | `python/pylibwholegraph/pylibwholegraph/torch/graph_structure.py` | 版权 2024→2025 + 元组解包展开（append_unique 返回值） |

- **CI/merge → SKIP**（工具链/CI 文件，Walpurgis 无对应体系）:
  - `.flake8` — 上游 flake8 配置文件，Walpurgis 无 pre-commit CI
  - `.pre-commit-config.yaml` — 上游 CI pre-commit，Walpurgis 无此 CI 管线
  - `pyproject.toml`（根） — RAPIDS monorepo ruff 配置；Walpurgis 用独立 pyproject.toml
  - `ci/utils/git_helpers.py` — 上游 CI PR 工具脚本，非 Walpurgis 代码路径
  - `ci/utils/nbtestlog2junitxml.py` — 上游 notebook CI 日志转换脚本
  - `mg_utils/wait_for_workers.py` — 上游 Dask 集群等待脚本（含废弃 `import yaml`）
  - `python/cugraph-pyg/` Python 格式化改动 — Walpurgis 对应文件已有独立实现（sampler/tensor/utils/examples 均已在先前迁移批次中用鲁迅拿法重写，保持与 ruff 规范一致）
  - `python/pylibwholegraph/` 格式化改动 — Walpurgis wholememory 对应文件（comm.py/graph_structure.py/test_comm.py）已在先前迁移批次中同步，版权年份与空行已处理

- **迁移位置**: `src/walpurgis/core/ruff_migration.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`LinterMigrationRecord` + `LinterRole` 枚举**: 将上游"工具切换"抽象为可程序化查询对象，含 `from_tool/to_tool/version/role/migration_reason/config_file_before/after`，`is_unified_replacement()` 检测"单工具替代多工具"模式——上游只有 pre-commit yaml 差异
  2. **`Flake8RuleMapper`**: 将 `.flake8` 中 `ignore`/`per-file-ignores` 规则集映射到 ruff 等效配置，`rules_kept_in_ruff()` 区分"迁移保留"与"[DROPPED]（ruff 无此规则）"，`cython_rules_disposition()` 解释 Cython 文件豁免原理——上游直接删除 .flake8 无任何解释
  3. **`FormatNorm` 枚举 + `FormatNormExample`**: 将 ruff-format 批量改动分 7 种范式（PARENTHESIZED_ASSIGNMENT/TUPLE_UNPACK/TRAILING_BLANK/COMMENT_SPACING/STRING_CONCAT/ASSERT_WRAP/CHAINED_SUBSCRIPT），每种含 before/after 示例、出现频次、`is_semantic_change()` 语义等价性断言——上游无此分类，只有原始 diff
  4. **`PreCommitHookSpec`**: pre-commit hook 规范对象，含 `repo/rev/hook_id/args/files_pattern`，`to_yaml_fragment()` 生成合规 yaml 片段，`is_ruff_based()` 区分新旧工具——上游只有文本 diff
  5. **`RuffPyprojectConfig`**: `[tool.ruff]` 配置的 Python 层表示，`to_toml_fragment()` 序列化，`is_equivalent_to_flake8(mapper)` 程序化验证等效性——上游只有 pyproject.toml 文本增量
  6. **`CodebaseFormatAudit`** (`scan_file_for_format_violations`): 用正则扫描 TUPLE_UNPACK / COMMENT_SPACING 残留模式，返回 `FormatViolation(file, lineno, norm, line)` 结构——上游靠 ruff --fix 一次性修复，无持续检测能力
  7. 全链路 `WALPURGIS_DEBUG=1` 断点（8 处）

- **自测结果**: 10 项全部 [PASS]

## migrate 2c04b37: [SKIP] Use GHA id-token for `sccache-dist` auth token (#408) — CI OIDC token 替换 PAT，Walpurgis 无 CI 体系

- **Upstream commit**: 2c04b3769ffec061d22861c039dfa8fa0ed206f2 (cugraph-gnn, Paul Taylor, 2026-02-10, PR #408)
- **Commit message**: `Use GHA id-token for sccache-dist auth token (#408)`
- **Upstream diff 摘要** (3 files changed, 20 deletions)：
  - `.github/workflows/build.yaml` — 删除 5 处 `sccache-dist-token-secret-name: GIST_REPO_READ_ORG_GITHUB_TOKEN` 参数（cpp-build / python-build / wheel-build-cugraph-pyg / wheel-build-libcugraph / wheel-build-pylibwholegraph job）
  - `.github/workflows/pr.yaml` — 删除 11 处同名参数，并同时删除 `rapids-aux-secret-1: GIST_REPO_READ_ORG_GITHUB_TOKEN` + `SCCACHE_DIST_AUTH_TOKEN_VAR=RAPIDS_AUX_SECRET_1` env 注入（PR pipeline 各 job）
  - `.github/workflows/test.yaml` — 删除 4 处同名参数（conda-cpp-tests / conda-python-tests / wheel-tests-pylibwholegraph / wheel-tests-cugraph-pyg）
- **变更语义**：将 sccache-dist 的认证来源从 org 级 PAT（`GIST_REPO_READ_ORG_GITHUB_TOKEN`）切换为 GitHub OIDC id-token（`id-token: write` 权限），无功能变更，纯 CI 安全合规调整
- **CI/merge → SKIP**：Walpurgis 无 `.github/workflows/` 体系，无 sccache-dist，无 GHA CI pipeline，3 个 workflow 文件全部 SKIP



## migrate 122b469: [SKIP] Prepare release/26.06 — 纯 CI shared-workflow 引用从 `@cuda-13.2.0` 切换 `@release/26.06`，Walpurgis 无 CI 体系

- **Upstream commit**: 122b469be883fd56ec1d652fa1bfe59beadcc89a (cugraph-gnn, jolorunyomi, 2026-05-14, "Prepare release/26.06")
- **Upstream diff 摘要** (5 files changed, 32 insertions(+), 32 deletions(-))：
  - `.github/workflows/build.yaml` — 10 处 `@cuda-13.2.0` → `@release/26.06`（cpp-build / python-build / python-build-noarch / docs-build / conda-upload-packages / wheel-build-cugraph-pyg / wheels-publish ×3 / wheel-build-libwholegraph / wheel-build-pylibwholegraph）
  - `.github/workflows/pr.yaml` — 16 处同名替换（pr-builder / changed-files / devcontainer / checks / conda-cpp-build / conda-cpp-tests / conda-python-build / conda-python-build-noarch / conda-python-tests / docs-build / wheel-build-libwholegraph / wheel-build-pylibwholegraph / wheel-tests-pylibwholegraph / wheel-build-cugraph-pyg / wheel-tests-cugraph-pyg）
  - `.github/workflows/test.yaml` — 4 处同名替换（conda-cpp-tests / conda-python-tests / wheel-tests-pylibwholegraph / wheel-tests-cugraph-pyg）
  - `.github/workflows/trigger-breaking-change-alert.yaml` — 1 处同名替换（breaking-change-alert）
  - `RAPIDS_BRANCH` — `main` → `release/26.06`
- **变更语义**：release 准备提交，将 shared-workflow pin 从 nightly cuda ref 切换到正式 release 分支 tag；`RAPIDS_BRANCH` 同步更新，用于后续 CI job 的 branch 参数传递；纯 CI/release-infra 配置，零源码/功能变更
- **鲁迅三维审查法**：
  - **维度一（文本价值）**：5 个 workflow yaml 文件 + 1 个 RAPIDS_BRANCH 文本；全为 `.github/` CI infra 资产，Walpurgis 无 GHA CI pipeline
  - **维度二（迁移成本）**：搬入毫无意义——Walpurgis 无 conda/wheel build system，无 sccache，无 shared-workflow 引用体系；`RAPIDS_BRANCH` 文件在 Walpurgis 不存在亦无对应概念
  - **维度三（潜伏价值）**：release/26.06 分支切换暗示上游即将 tag v26.06；可作为"上游版本跟踪"参考，但本身不产生可迁移代码
  - **结论**：SKIP — 纯 CI release-prep 提交，全部 6 个变更文件均属 CI/build-infra，不适用于 Walpurgis
