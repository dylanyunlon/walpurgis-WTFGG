...
## 120501e — [BUG] Fix DGL Test (#209)

- **上游 commit**: `120501ed3138fe253dee7be3c91a793e2f282687`
- **Commit message**: `[BUG] Fix DGL Test (#209)`
- **Author**: Alex Barghi (alexbarghi-nv, NVIDIA)
- **Context**: 修复 `test_graph_make_homogeneous_graph_mg` 在 3+ GPU 场景下崩溃/挂死的问题。根因：旧方案用等步长切割节点分片（`len(node_x)*rank` ~ `len(node_x)*(rank+1)`），当 `global_num_nodes` 不被 `world_size` 整除时末尾 rank 的 `ix` 越界；断言右侧也从 `torch.as_tensor(node_x)` 改为 `ix`（全局索引），与 `add_nodes` 时实际写入的值一致。

- **Upstream diff** (3 files changed, 4 insertions(+), 4 deletions(−)):
  | 文件 | 变更 |
  |------|------|
  | `ci/test_notebooks.sh` | 删除 `--prepend-channel "dglteam/label/th24_cu124"` |
  | `dependencies.yaml` | 删除 `includes` 中 `- depends_on_cugraph_dgl` |
  | `python/cugraph-dgl/cugraph_dgl/tests/test_graph_mg.py` | 修复 `ix` 计算方式 + 修复断言右侧 |

- **CI/merge → SKIP**（2 个文件）:
  - `ci/test_notebooks.sh` — Walpurgis 无 RAPIDS Notebook CI 脚本，DGL conda channel 移除与本项目无关
  - `dependencies.yaml` — RAPIDS 构建依赖清单，`depends_on_cugraph_dgl` 是上游 conda 构建矩阵条目，Walpurgis 不使用此文件

- **迁移位置**: `src/walpurgis/core/dgl_mg_graph_tensor_split.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`NodePartitionScheme` dataclass**: 将上游修复后的裸 `torch.tensor_split` 一行强类型化为含校验的数据类，`__post_init__` 检查 `world_size > 0` / `global_num_nodes > 0`，`rank_slice()` 方法返回 `RankSlice` 命名元组，`summary()` 方法打印各 rank 边界——上游无任何此类封装
  2. **`RankSlice` NamedTuple**: 携带 `rank/world_size/indices` 三元，取代上游裸 `ix = ...` 局部变量
  3. **`partition_nodes()` 工厂函数**: 统一生成各 rank CUDA 索引 tensor，上游每次在函数体内临时构造
  4. **`validate_node_feature_assignment()`**: 将上游两行断言（`is not None` + `== ix`）拆成两步并加语义说明，失败时打印 `actual[:10]` vs `ix[:10]` 辅助定位，上游仅有裸 `assert`
  5. **`HomogeneousGraphMgTestSpec` dataclass**: 收束 `run_test_graph_make_homogeneous_graph_mg` 的四个入参，`__post_init__` 校验 `direction` 合法性与 `rank` 范围，上游靠位置参数传递
  6. **`_split_summary()` 辅助函数**: 在 `WALPURGIS_DEBUG=1` 时打印分片边界，生产模式为空操作，上游无此辅助
  7. 全链路 `WALPURGIS_DEBUG=1` 断点（6 处）：覆盖 scheme 初始化→分片计算→特征验证→规格校验各阶段
  8. **`_selftest_partition_logic()`**: 用纯 Python 验证 `tensor_split` 分区语义，同时通过故意复现旧等步长方案的越界来确认 bug 存在，无需 CUDA 环境即可运行

- **自测结果**: 2 项全部 [PASS]（`partition_logic` + `旧方案越界确认`）
