...
<<<<<<< HEAD
=======
## migrate 47e64e8: [BUG] Fix optional dependencies (#278)

- **Commit**: `47e64e8658b2ddf7107e971c29acb4cb7a6a693d`
- **Commit message**: `[BUG] Fix optional dependencies (#278)`
- **Author**: Alex Barghi — 2025-08-15
- **上游 PR**: https://github.com/rapidsai/cugraph-gnn/pull/278

- **问题**: `cudf`/`torch` 作为可选依赖，但在若干文件中以顶层硬 import 形式引入，
  导致缺少 GPU 环境时模块级 `ImportError`。同时 `DistSampleReader` 类
  从 `cugraph` 迁移时被误带入 `reader.py`，直接依赖 `cudf.read_parquet`，
  与可选依赖原则矛盾。
...
| 维度 | 结论 |
|------|------|
| **正确性** | 所有改动均为"硬 import → `import_optional` 懒代理"或"删除冗余局部重导入"，不影响运行时行为；`TensorType` 改为字符串前向引用安全 |
| **关联性** | 4 个上游文件中，`graph_store.py` 和 `loader/utils.py` 在 Walpurgis 无对应文件，SKIP；`distributed_sampler.py` 和 `sampler/io/reader.py` 有对应，MIGRATE |
| **完整性** | Walpurgis 已有 `walpurgis.utils.imports.import_optional` + `MissingModule`，import 路径从 `cugraph.utilities.utils` 保持不变（Walpurgis 的 `utils/imports.py` 兼容接口），无需额外变更 |

**迁移文件**:

- `src/walpurgis/sampler/distributed_sampler.py`
  - 删除 `import cudf`，改为 `cudf = import_optional("cudf")`
  - `torch = MissingModule("torch")` → `torch = import_optional("torch")`
  - 删除 5 处函数内部冗余 `torch = import_optional("torch")`：
    `get_start_batch_offset` / `__sample_from_nodes_func` /
    `__get_call_groups` / `sample_from_nodes` / `__sample_from_edges_func`
  - `TensorType` 中 `cudf.Series` → `"cudf.Series"`（字符串前向引用）
  - 删除 `MissingModule` import（不再使用）

- `src/walpurgis/sampler/io/reader.py`
  - `DistSampleReader` 整个类已在上一版本迁移时确认不带入（Walpurgis 无此需求），本次同步确认清洁
  - `torch = MissingModule("torch")` → `torch = import_optional("torch")`
  - 新增 `cudf = import_optional("cudf")`
  - 删除 `BufferedSampleReader.__init__` 中 `global torch; torch = import_optional("torch")`（冗余）
  - 删除 `MissingModule` import

**SKIP 文件（CI/非业务逻辑）**:

- `python/cugraph-pyg/cugraph_pyg/data/graph_store.py` — SKIP: Walpurgis 无对应 `graph_store.py`
- `python/cugraph-pyg/cugraph_pyg/loader/utils.py` — SKIP: Walpurgis 无对应 `loader/utils.py`
...
>>>>>>> 46a8ed1 (migrate 47e64e8: [BUG] Fix optional dependencies — cudf/torch 改为 import_optional)
