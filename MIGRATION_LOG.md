...
- **Upstream commit**: 5c7a7da28c5260b077e81325d940a9bc0b80b342 (cugraph-gnn, James Lamb, 2025-01-30, PR #397)
- **Commit message**: `remove unused CI jobs, code, and configuration for notebooks (#397)`
- **Upstream diff** (13 files changed, 612 deletions, 0 insertions):
  - `.github/CODEOWNERS` — 删除 `*.ipynb @rapidsai/cugraph-notebook-codeowners` 一行
  - `.github/workflows/pr.yaml` — 删除 `conda-notebook-tests` job（约 23 行）及 `test_notebooks` changed-file 触发组
  - `.github/workflows/test.yaml` — 删除 `conda-notebook-tests` job（约 12 行）
  - `ci/notebook_list.py` — **删除**（95 行）：notebook 扫描/过滤脚本，依赖 `numba.cuda`
  - `ci/test_notebooks.sh` — **删除**（64 行）：bash CI 测试脚本
  - `ci/utils/git_helpers.py` — **删除**（155 行）：git branch diff/status 工具集
  - `ci/utils/nbtest.sh` — **删除**（66 行）：bash notebook 执行脚本
  - `ci/utils/nbtestlog2junitxml.py` — **删除**（162 行）：nbtest 日志 → JUnit XML 转换
  - `conda/environments/all_cuda-129_arch-aarch64.yaml` — 删除 ipython/nbconvert/notebook/ogb 四个依赖
  - `conda/environments/all_cuda-129_arch-x86_64.yaml` — 同上
  - `conda/environments/all_cuda-131_arch-aarch64.yaml` — 同上
  - `conda/environments/all_cuda-131_arch-x86_64.yaml` — 同上
  - `dependencies.yaml` — 删除 `test_notebook` 依赖块 + `test_notebooks` 文件配置节

- **CI/merge/docs 文件 → SKIP**:
  - `.github/workflows/pr.yaml` — SKIP：Walpurgis 无 GitHub Actions CI 体系
  - `.github/workflows/test.yaml` — SKIP：同上
  - `.github/CODEOWNERS` (notebook 行) — SKIP：无对应 notebook codeowners 规则
  - `ci/test_notebooks.sh` — SKIP：纯 bash CI 脚本
  - `ci/utils/nbtest.sh` — SKIP：纯 bash CI 脚本
  - `conda/environments/all_cuda-*.yaml` (四个) — SKIP：Walpurgis 无 conda 体系
  - `dependencies.yaml` (test_notebook 节) — SKIP：RAPIDS 构建依赖配置，Walpurgis 用 pyproject.toml

- **迁移位置**: `src/walpurgis/core/notebook_ci_helpers.py` — 新增

- **鲁迅拿法改写（≥20%）**:
  1. **WalpurgisGitHelpers**（改写自 ci/utils/git_helpers.py）:
     - 删除 `shell=True`（安全隐患），所有 subprocess 调用改为 args 列表 + `check=True`
     - `_run_git()` 统一错误处理，失败时抛 `RuntimeError` 含 returncode/stderr 信息（上游裸抛 CalledProcessError）
     - 新增 `GitEnvSnapshot` dataclass，汇总 TARGET_BRANCH/COMMIT_HASH/currentBranch 三字段，替代上游三个散落局部变量
     - `uncommitted_files()` 通过 `include_staged/include_modified` 参数控制过滤白名单，上游硬编码 M/A
     - `_checkout()` 提取为私有辅助，消除 `changed_files_between()` 中三次重复 git checkout 调用
  2. **WalpurgisNotebookScanner**（改写自 ci/notebook_list.py）:
     - 删除 `numba.cuda` 硬依赖，改为 `is_ampere: bool` 构造参数（降级到无 GPU 环境可运行）
     - `NotebookSkipReason` 枚举替代上游裸字符串 skip 理由（上游 5 种理由均以 f-string 散落）
     - `NotebookScanResult` dataclass 替代布尔 skip flag，每个决策携带 skip_reason/skip_detail，可审计
     - `scan()` 改为纯扫描+分类，不直接 print 到 stdout（副作用分离）；`report()` 方法控制输出
     - `runtype` 参数在构造时提前校验，上游在 `if runtype not in runtype_dict.keys()` 后 `exit()` 中途退出
  3. **WalpurgisJunitBuilder**（改写自 ci/utils/nbtestlog2junitxml.py）:
     - 不写文件（去副作用），`parse_log()` 返回 `Element`，`build_tree()` 返回 `ElementTree`，由调用方决定是否写出
     - `TestRecord` dataclass 替代上游裸 `attrDict = {}` dict（无类型约束，字段名靠命名约定维护）
     - `_ParseState` 枚举替代上游 `Enum("parserStateEnum", "...")` 动态枚举（上游用法在 Python 3.11+ 触发 DeprecationWarning）
     - `_count_incr()` 模块函数替代上游 `incrNumAttr()`，命名更直白
  4. **全链路 8 处 WALPURGIS_DEBUG=1 断点**：git_run 命令 → GitEnvSnapshot 捕获 → uncommitted_files 结果 → checkout 操作 → scan_notebook 入口/skip/pass → JunitBuilder 解析开始/skip/result/完成

- **自测结果**: `python src/walpurgis/core/notebook_ci_helpers.py` → 5 项断言全通过，`[PASS]`

## migrate b6e934d: [SKIP] Improve devcontainer startup times — 纯 devcontainer/CI 配置，Walpurgis 无对应结构

- **Upstream commit**: b6e934d2a046cc63af6e9d987c40c8416bd85f68 (cugraph-gnn, Paul Taylor @trxcllnt, 2026-05-22, PR #465)
- **Commit message**: `Improve devcontainer startup times (#465)`
- **Upstream diff** (5 files changed, 25 insertions, 4 deletions):
  - `.devcontainer/cuda12.9-conda/devcontainer.json` — `buildOptions` 内新增 `cacheFrom: ["ghcr.io/rapidsai/cugraph-gnn/devcontainer:26.08-cuda12.9-conda"]`
  - `.devcontainer/cuda12.9-pip/devcontainer.json` — 同上，镜像名 `26.08-cuda12.9-pip`
  - `.devcontainer/cuda13.2-conda/devcontainer.json` — 同上，镜像名 `26.08-cuda13.2-conda`
  - `.devcontainer/cuda13.2-pip/devcontainer.json` — 同上，镜像名 `26.08-cuda13.2-pip`
  - `.github/workflows/build.yaml` — 新增 `devcontainers` job：调用 `rapidsai/shared-workflows/.github/workflows/build-devcontainers.yaml@main`，`push: true`，cuda 矩阵 `["12.9", "13.2"]`，权限 `packages: write`

- **变更语义**: 将 cugraph-gnn 的 devcontainer 镜像预构建并推送到 `ghcr.io/rapidsai/cugraph-gnn/devcontainer`，使后续 `docker build` 可从远端缓存层启动，将 devcontainer 冷启动时间从 3m28s 压缩到 34s。这是纯粹的 CI/开发环境优化，不含任何 Python/C++/CUDA 运行时算法代码。

- **跳过原因**: 全部 5 个变更文件均属于 devcontainer 配置（`.devcontainer/`）或 GitHub Actions CI workflow（`.github/workflows/`）。Walpurgis 项目无 `.devcontainer/` 目录、无 RAPIDS devcontainer 体系、无 `rapidsai/shared-workflows` 依赖、无 ghcr.io 镜像发布流程。此前 `68bad40`（Allow latest OS in devcontainers）已 SKIP，本 commit 遵循一致决策。零迁移价值。

- **Knuth 审查**:
  1. **diff 完整性**: 5 个文件逐一审查；4 个 devcontainer.json 各新增 3 行（`cacheFrom` 数组），build.yaml 新增 9 行 job 定义，全量覆盖，无隐藏算法改动
  2. **用户角度**: `cacheFrom` 是 Docker BuildKit 的层缓存拉取指令，仅在 `docker build` 执行时生效；若 ghcr.io 镜像不存在或网络不通，构建降级为全量构建（不报错，只是慢）。对 Walpurgis Python 运行时用户零影响
  3. **系统角度安全**: `build-devcontainers.yaml` workflow 使用 `secrets: inherit`（上游有 `# zizmor: ignore[secrets-inherit]` 豁免注释），继承父 workflow 的所有 secret 用于 ghcr.io 推送；`packages: write` 权限限于 GitHub Packages 写入。Walpurgis 无此 workflow，零安全面扩展
