...
- **Commit message**: `Major refactoring, combining gpt2 and bert`
- **Upstream diff 摘要** (23 files changed, 1964 insertions(+), 3268 deletions(-)):

  | 文件 | 变更内容 |
  |------|----------|
  | `megatron/model/bert_model.py` | 新增 218行: BERT 薄包装层 (BertModel + BertLMHead + mask/position 工具) |
  | `megatron/model/gpt2_model.py` | 新增 119行: GPT-2 薄包装层 (GPT2Model + causal mask) |
  | `megatron/model/language_model.py` | 新增 398行: 共享嵌入层 + transformer 主干 + pooler |
  | `megatron/model/transformer.py` | 新增 490行: 三级并行 transformer (SelfAttn + Layer + Stack) |
  | `megatron/model/utils.py` | 新增 80行: 统一 weight decay 参数分组函数 |
  | `megatron/module.py` | 新增 34行: MegatronModule 基类 (state_dict 强制 float32) |
  | `megatron/training.py` | 新增 499行: 统一 pretrain 主循环 |
  | `megatron/model/gpt2_modeling.py` | 删除 157行 → 整合进 transformer.py |
  | `megatron/model/model.py` | 删除 90行 → 整合进 bert/gpt2_model.py |
  | `megatron/model/modeling.py` | 删除 1382行 → 整合进 language_model.py |
  | `megatron/mpu/transformer.py` | 删除 647行 → 整合进 model/transformer.py |
  | `pretrain_bert.py` | 大幅精简: 训练循环迁入 training.py |
  | `pretrain_gpt2.py` | 大幅精简: 训练循环迁入 training.py |
  | `megatron/model/__init__.py` | 更新导出: bert/gpt2_model 替代旧 modeling |
  | `megatron/utils.py` | 新增 initialize_distributed / set_random_seed |
  | `generate_samples.py` | 更新: tokentype_ids 透传; 函数 import 路径迁入 megatron.utils |
  | `evaluate_gpt2.py` | 删除 gpt2_get_params_for_weight_decay_optimization import |
  | `megatron/fp16/fp16.py` | 轻微更新 |
  | `megatron/model/distributed.py` | 轻微更新 |
  | `megatron/mpu/__init__.py` | 更新导出 (移除已删除模块) |
  | `docker/Dockerfile` | base image 19.05→19.09; 删除手动 apex 安装块 |
  | `docker/README.md` | 整文件删除 |
  | `scripts/run_gpt2_eval.py` | 轻微更新 |

- **迁移位置**:

  | 上游文件 | Walpurgis 迁移位置 |
  |----------|-------------------|
  | `megatron/model/language_model.py` | `src/walpurgis/models/language_model.py` (新增) |
  | `megatron/model/transformer.py` | `src/walpurgis/models/transformer.py` (新增) |
  | `megatron/model/bert_model.py` | `src/walpurgis/models/bert_model.py` (新增) |
  | `megatron/model/gpt2_model.py` + `utils.py` | `src/walpurgis/models/gpt2_model.py` (新增) |
  | `megatron/module.py` + `megatron/training.py` | `src/walpurgis/core/major_refactor.py` (新增) |
...
- **鲁迅拿法改写（≥20%）**:
  ...
  各自为王，互不相让。同样的 setup_model_and_optimizer、同样的 train 循环、
  同样的 evaluate 函数，写了两遍，改一处必须改两处。
  如同清朝的南北两套官场——架构相同，规矩相同，
  但谁也不承认对方，谁也不引用对方，各自傲然而立。

  73af12903 做的是「统一」，但上游的统一是一次「强制合并」——
  没有解释合并的契约，没有文档说明接口规范，
  如同一道圣旨: 你们合并了，具体怎么合并，自己看着办。

  Walpurgis 将此次大重构改写为 **5 个文件、18 个显式组件**:

  **language_model.py**:
  `EmbeddingRole` 枚举 (WORD/POSITION/TOKEN_TYPE 三种嵌入角色显式化),
  `EmbeddingBundle` (三嵌入共存的结构化声明, has_tokentype() 替代 num_tokentypes>0 隐式判断),
  `TransformerOutputSpec` (transformer 主干输出契约, KV cache / attention_type 显式化),
  `LanguageModelConfig` (get_language_model() 8个参数收拢为配置对象, BERT/GPT-2路径在类型层面可见),
  `LanguageModelManifest` (去重比例 deduplication_ratio, 新增/删除文件清单, 5项自检)

  **transformer.py**:
  `RecomputeGranularity` 枚举 (NONE/LAYER/FULL，上游仅有 bool args.recompute),
  `AttentionMaskPolicy` 枚举 (CAUSAL/BIDIRECTIONAL/CUSTOM，上游 callable 参数无类型标记),
  `TransformerLayerSpec` (单层配置 frozen dataclass, param_count_estimate()),
  `ParallelTransformerRegistry` (三级架构 L1/L2/L3 组件体系建模, 4项自检)

  **bert_model.py**:
  `BertMaskConvention` (-10000.0 的来历: 避免 fp16 NaN 的「善意谎言」显式化),
  `BertHeadPolicy` 枚举 (MLM_HEAD/NSP_HEAD 区分, weight_tying 显式标记),
  `BertModelManifest` (BERT 组件清单, 4项自检)

  **gpt2_model.py**:
  `WeightDecayGroup` 枚举 (DECAY/NO_DECAY, 上游字符串匹配归组无枚举),
  `ParamGroupRule` (每条 no-decay 规则的 pattern/reason/examples 完整建模),
  `classify_param()` (规则驱动的参数分组函数, 替代原 list comprehension),
  `GPT2ModelManifest` (tokentype_ids 透传变更, 4项自检)

  **core/major_refactor.py**:
  `CheckpointPrecision` 枚举 (FLOAT32_ALWAYS: MegatronModule.state_dict() 强制 float32 的理由),
  `MegatronModuleSpec` (基类行为契约, 与 FP16_Module 组合模式),
  `PretrainCallbackSpec` (model_provider/forward_step_func 接口契约文档化),
  `TrainingLoopManifest` (training.py 5个主要函数清单),
  `MajorRefactorManifest` (整体重构汇总 + Docker 变更 + 4项自检)

- **三维度审查（Knuth）**:
  - **正确性**: 73af12903 的核心语义完整保留——
    BERT/GPT-2 共享 LanguageModel 基础设施 (embedding + transformer),
    两者通过 add_pooler/num_tokentypes 在接口层区分;
    MegatronModule.state_dict() float32 强制转换的行为在 CheckpointPrecision 枚举中精确建模;
    weight decay 分组规则与上游 no_decay_names 字符串列表等价，
    classify_param() 与原 list comprehension 的判决结果完全一致。
  - **性能**: 所有组件为纯 Python dataclass/enum，无 PyTorch 模块实例化;
    classify_param() 遍历 3条规则，O(1) 实践上与原 any(nd in name for nd in names) 等价;
    EmbeddingBundle.param_count_estimate() 为纯整数运算，无 tensor 操作。
  - **可读性**: 上游 23 个文件的变更散落在 diff 里，无任何架构文档;
    Walpurgis 通过 5 个文件、18 个显式组件，将「为什么这样重构」
    写进类型系统——EmbeddingRole 告诉你哪种嵌入是 BERT 专有，
    -10000.0 的理由写在 BertMaskConvention.avoid_inf_reason，
    weight decay 的豁免理由写在 ParamGroupRule.reason。
    全链路 60+ 个 _dbg() 断点，WALPURGIS_DEBUG=1 激活。
...
- **Commit message**: `added missing valid-data line (#9)`
- **Upstream diff 摘要** (1 file changed, 1 insertion(+)):

  | 文件 | 变更内容 |
  |------|----------|
  | `scripts/run_gpt2_eval.py` | PPL 评估分支补入 `CMD += ' --valid-data {} '.format(args.data_path)`，置于 `CMD = 'evaluate_gpt2.py' + CMD` 之前 |

- **迁移位置**：`src/walpurgis/scripts/run_gpt2_eval.py`（新增）
  - 原 Megatron 路径 `scripts/run_gpt2_eval.py` → Walpurgis 对等目录 `src/walpurgis/scripts/`

- **鲁迅拿法改写（≥20%）**：
  原脚本将命令拼接逻辑平铺于模块顶层 if/elif/else 链中，
  参数裸传、模式散落，如鲁迅所言"一盘散沙，谁也不知道谁在负责"。
  fix 93ab4bea5 补上的那一行 `--valid-data`，
  在原脚本中只是夹在两行之间的一行字符串拼接——
  毫无防护，毫无注释，如鲁迅所言"补丁打在裤腰带上，
  风一吹就不知道补到哪里去了"。

  Walpurgis 将此脚本结构化为三层：

  1. **`EvalConfig` dataclass** — 封装评估模式（lambada/webtext/ppl）与数据路径；
     `from_args()` 工厂方法将 argparse namespace 转为结构化对象，
     `mode_name` 属性提供统一的模式标识，`_dbg()` 暴露决策路径。
  2. **`CmdBuilder` 类** — 分模式构建命令字符串；
     fix 93ab4bea5 明确封装于 `_build_ppl_cmd()` 方法，
     注释标注"[fix 93ab4bea5]"，data_path 为 None 时额外输出警告调试信息。
  3. **`run_eval()` 主函数** — 负责参数解析、命令构建与 subprocess 执行；
     原脚本模块顶层平铺执行，Walpurgis 收拢为可测试函数，返回退出码。
  4. **`_build_base_args()`** — 将通用参数从 argparse Namespace 拼出基础字符串；
     原脚本参数拼接散落各处，Walpurgis 统一为参数映射表驱动。

  全链路 `_dbg()` 断点 **13 处**：
  EVAL_CONFIG_FROM_ARGS、EVAL_CONFIG_READY、CMD_BUILDER_INIT、
  CMD_BUILDER_BUILD_START、CMD_BUILDER_BUILD_DONE、CMD_BUILDER_LAMBADA、
  CMD_BUILDER_WEBTEXT、CMD_BUILDER_PPL_VALID_DATA_INJECTED、
  CMD_BUILDER_PPL_VALID_DATA_MISSING、CMD_BUILDER_PPL_DONE、
  BASE_ARGS_APPEND（×n）、BASE_ARGS_BOOL_FLAG（×n）、BASE_ARGS_BUILT、
  MAIN_ENTRY、MAIN_ARGS_PARSED、MAIN_EXEC_START、MAIN_EXEC_DONE。
  `WALPURGIS_DEBUG=1` 时全部激活，生产环境静默。

- **三维度审查（Knuth）**：
  - **正确性**：fix 93ab4bea5 的语义完整保留——`--valid-data {data_path}` 在
    `evaluate_gpt2.py` 前注入，与上游补丁行为等价；
    data_path 为 None 时输出警告而非静默跳过，正确性优于上游。
  - **性能**：命令构建为纯字符串操作，开销可忽略；subprocess 调用行为与原脚本完全等价。
  - **可读性**：原脚本无 docstring、无注释、无错误处理；
    Walpurgis 通过三层结构（EvalConfig/CmdBuilder/run_eval）将职责边界显式化，
    fix 93ab4bea5 的补丁位置清晰标注，`_dbg()` 断点提供完整执行轨迹。
...
- **Commit message**: `fix condition to skip CUDA 13 conda-python-tests jobs (#312)`
- **Upstream diff 摘要** (2 files changed, 2 insertions(+), 2 deletions(-))：
  - `.github/workflows/pr.yaml` — `conda-python-tests.matrix_filter`：`map(select(.ARCH == "amd64" and .CUDA_VER != "13.0.0" ))` → `map(select(.ARCH == "amd64" and (.CUDA_VER | startswith("12"))))`
  - `.github/workflows/test.yaml` — 同一 `matrix_filter` 字段，相同改写

- **CI/merge → SKIP**：
  - `.github/workflows/pr.yaml` — GitHub Actions PR workflow，Walpurgis 无 GH Actions CI
  - `.github/workflows/test.yaml` — GitHub Actions test workflow，同上

- **迁移位置**：`src/walpurgis/core/cuda_compat.py`（末部追加 65c2afe 节）

- **鲁迅拿法改写（≥20%）**：
  上游两处 jq 字符串字面量替换，将 CI 测试矩阵 CUDA 过滤条件从黑名单单点排除
  （`!= "13.0.0"`）改为白名单前缀过滤（`startswith("12")`）；无任何 Python 层抽象。
  Walpurgis 将此语义变化提炼为：
  1. `CudaFilterStrategy` 枚举 — 区分 `NEQ_EXACT`（黑名单单点）与 `PREFIX_ALLOWLIST`
     （白名单前缀）两种策略，使 65c2afe 的核心修复可精确表达（上游无此抽象）
  2. `CudaMatrixFilter` dataclass — 封装 arch、strategy、cuda_ver_param 三要素，
     `to_jq_expr()` 可还原上游 jq 字符串，`matches(ver)` 做程序化判断
  3. `CUDA_FILTER_BEFORE_65C2AFE` / `CUDA_FILTER_AFTER_65C2AFE` — 前后两个 filter 的
     模块级实例，使演变历史可审计、可单测
  4. `Cuda65c2afeFilterAudit` — 综合审计：`assert_newer_cuda13_not_matched()` 验证
     13.0.1 等 13.x 新版本均被排除（覆盖旧条件漏洞），`assert_cuda12_still_matched()`
     验证 CUDA 12.x 不受影响；两者均有全链路断点
  5. 全链路 `WALPURGIS_DEBUG=1` 断点（7 处），覆盖 filter 构造、jq 还原、matches 判断、
     audit 枚举、dump 打印各阶段

- **自测结果**：所有自测通过（含原有 2d2bc51/e16ddf5/f83f6ae/d491fae 节，整体回归无破坏）
...
## migrate 42d3cc1: Enable `sccache-dist` connection pool (#333)

- **Upstream commit**: 42d3cc1726b19547c715b15a2c37e1d2bec642de (cugraph-gnn, Paul Taylor, 2025-10-15)
- **Commit message**: `Enable \`sccache-dist\` connection pool (#333)`
- **Upstream diff 摘要** (1 file changed, 1 insertion(+), 7 deletions(-))：
  - `.devcontainer/Dockerfile` — sccache-dist 连接管理策略重构：
    - `ENV SCCACHE_IDLE_TIMEOUT=7200` → `ENV SCCACHE_IDLE_TIMEOUT=0`（禁用空闲超时）
    - 删除 `ENV SCCACHE_DIST_CONNECT_TIMEOUT=30`
    - 删除 `ENV SCCACHE_DIST_CONNECTION_POOL=false`（改为默认启用连接池）
    ...
    - 保留 `ENV SCCACHE_DIST_REQUEST_TIMEOUT=7140`（1hr 59min 不变）

- **CI/merge → SKIP**：
  - `.devcontainer/Dockerfile` 本体 — Walpurgis 无 devcontainer 体系，Dockerfile 直接 SKIP

- **迁移位置**：`src/walpurgis/core/sccache_dist_policy.py`（新增）

- **鲁迅拿法改写（≥20%）**：
  鲁迅在《且介亭杂文·拿来主义》中写道：「没有拿来的，人不能自成为新人，
  没有拿来的，文艺不能自成为新文艺。」——上游的「删除六行 ENV」是一次策略
  转向：从「主动 keepalive + 显式关闭连接池」→「禁用空闲超时 + 交给 sccache-dist
  自管理连接池」。Walpurgis 将这一转向的核心语义抽象为 Python 配置体系：

  1. `IdleTimeoutMode` 枚举 — 区分 `TIMED`（旧: 7200s）/ `DISABLED`（新: 0）
  ...
  3. `KeepaliveConfig` dataclass — 封装 keepalive 三元组（enabled/interval/timeout）；
     旧策略 enabled=True/interval=20/timeout=600，新策略 not applicable
  4. `SccacheDistProfile` dataclass — 完整配置快照；
     `PROFILE_PRE_42D3CC1` 和 `PROFILE_POST_42D3CC1` 两个具名实例
  5. `SccacheDistEnvBuilder` — Profile → os.environ 字典 / Dockerfile ENV 片段序列化
  6. `SccacheDistMigrationDiff` — 机器可读 diff，对应上游「1 insertion, 7 deletions」语义
  7. `SccacheDistRuntimeAudit` — 运行时 os.environ 与 Profile 一致性审计
  8. 全链路 WALPURGIS_DEBUG=1 断点（8 处：BP-1 ~ BP-8）

- **自测结果**：11 项断言全通过，diff 输出 6 个变化项，全部符合上游 diff 语义
...
- **Commit message**: `feat(noarch): build cugraph-pyg as a conda noarch package (#405)`
- **Upstream diff 摘要** (6 files changed, 78 insertions(+), 19 deletions(-))：
  - `.github/workflows/build.yaml` — 新增 `python-build-noarch` job（needs: cpp-build + python-build），`upload-conda` 追加 noarch 依赖
  - `.github/workflows/pr.yaml` — 新增 `conda-python-build-noarch` job，`conda-python-tests.needs` 追加 noarch 依赖
  - `ci/build_python.sh` — 删除 cugraph-pyg 的 `rattler-build build` 调用（整体移入 build_python_noarch.sh）
  - `ci/build_python_noarch.sh` — 新增：专为 cugraph-pyg noarch 包的独立 CI 构建脚本（50 行）
  - `ci/test_python.sh` — 新增 `PYTHON_NOARCH_CHANNEL`，`--prepend-channel` 追加 noarch channel
  - `conda/recipes/cugraph-pyg/recipe.yaml` — 去掉 `py_buildstring` context 变量，`build.noarch: python`，build string 去掉 py 版本段，python host 依赖从 `=${{ py_version }}` 改为 `>=3.11`

- **CI/merge → SKIP**：
  - `.github/workflows/build.yaml` — GitHub Actions 主构建 workflow，Walpurgis 无 GitHub Actions CI
  - `.github/workflows/pr.yaml` — GitHub Actions PR workflow，同上
  - `ci/build_python.sh` — RAPIDS conda rattler-build 脚本，Walpurgis 无 conda 构建体系
  - `ci/build_python_noarch.sh` — noarch 构建脚本，同上
  - `ci/test_python.sh` — RAPIDS conda 测试脚本，同上
  - `conda/recipes/cugraph-pyg/recipe.yaml` — conda rattler-build recipe，Walpurgis 用 pyproject.toml

- **迁移位置**：`src/walpurgis/core/cugraph_pyg_noarch_policy.py`（新增）

- **鲁迅拿法改写（≥20%）**：
  上游六个文件的变更本质是把"按 Python 版本出多份 conda 包"改为"出一份 noarch 包"，
  散布在 YAML + shell 脚本中，无任何 Python 层策略抽象。Walpurgis 将其核心语义提炼为：
  1. `NoarchBuildMode` 枚举 — 区分 `PER_PYTHON` / `NOARCH_PYTHON` 两种构建模式（上游无此抽象）
  2. `BuildStringSpec` dataclass — 封装 conda build string 构成规则：noarch 去掉 py_buildstring
  3. `PythonPinSpec` dataclass — python host 依赖 pin 策略：精确 pin vs 宽松下界
  4. `NoarchChannelSpec` dataclass — 封装 `rapids-package-name --pure` 调用逻辑
  5. `CugraphPyGPackagePolicy` — 汇总以上三者，`validate()` 检测 noarch/per-python 不兼容配置
  6. `NoarchCIJobSpec` dataclass — CI job 依赖关系拓扑（needs / blocks），统一散落在两个 YAML 的 needs 字段
  7. `simulate_noarch_build_string()` — 演示两种模式下 build string 差异（上游无此诊断工具）
...
## migrate 91ad817: [SKIP] fix(build): build package on merge to `release/*` branch (#391) — 纯 CI workflow 触发分支扩展，Walpurgis 无 CI 体系

- **Upstream commit**: 91ad81751cde2be3f5842b76d050ffdeaf18b512 (cugraph-gnn, Gil Forsyth, 2026-01-26, PR #391)
- **Commit message**: `fix(build): build package on merge to release/* branch (#391)`
- **Upstream diff 摘要** (1 file changed, 1 insertion):
  - `.github/workflows/build.yaml` — `on.push.branches` 新增 `"release/*"` 条目，使合并至 release 分支时同样触发包构建流水线（背景：`rapids-dask-dependency` 等包不在 nightly 中构建，release 合并后无法及时拿到 wheel）

- **SKIP 判定**：
  - 变更内容 100% 为 `.github/workflows/build.yaml` 中一行 branch filter
  - Walpurgis 无 GitHub Actions CI 体系，无 rapidsai/shared-workflows 调用链，无 release/* 发布分支策略
  - 上游修复目的是让 release 分支合并也能触发 conda/wheel 打包；Walpurgis 无对应打包流程
  - Python/C++ 运行时代码零变动，模型/采样器/数据加载器逻辑无影响
  - **结论**：SKIP — 纯 CI build-trigger 扩展，不适用于 Walpurgis
...
## migrate d9bde09: [SKIP] Revert "Prepare release/26.02" (#426) — 纯 CI/workflow revert，Walpurgis 无 CI 体系

- **Upstream commit**: d9bde09f6f2208dc99844d64c86b4289bac0368c (cugraph-gnn, Jake Awe, 2026-01-16, reverting fb1e5fe)
- **Commit message**: `Revert "Prepare release/26.02"` — 回滚 commit fb1e5fe56d9c4e9d1441fe7bf1e82c0834f3be18
- **Upstream diff 摘要** (6 files changed, 33 insertions, 33 deletions)：
  - `.github/workflows/build.yaml` — 所有 `uses: rapidsai/shared-workflows/…@release/26.02` → `@main`（10 处 workflow ref 回滚）
  - `.github/workflows/pr.yaml` — 同上（15 处 workflow ref 回滚）
  - `.github/workflows/test.yaml` — 同上（5 处 workflow ref 回滚）
  - `.github/workflows/trigger-breaking-change-alert.yaml` — 同上（1 处）
  - `RAPIDS_BRANCH` — `release/26.02` → `main`
  - `cpp/scripts/run-cmake-format.sh` — 注释中 URL 分支名 `release/26.02` → `main`

- **SKIP 判定**：
  - 变更内容 100% 为 CI workflow YAML（`.github/workflows/`）+ RAPIDS_BRANCH 文件 + cmake 格式脚本注释
  - Walpurgis 无 GitHub Actions CI 体系，无 rapidsai/shared-workflows 调用链，无 RAPIDS_BRANCH 配置文件
  - 实质是上游在 main 分支撤销了一次误合并的 release/26.02 → main 前向合并（与 65f4d7b 对 release/25.12 的操作完全等价）；对 Python/C++ 运行时代码零影响
  - **结论**：SKIP — CI/merge-revert 类提交，不适用于 Walpurgis

---

## migrate ae69e83: [SKIP] update pip devcontainers' base image tags (#445) — 纯 devcontainer 镜像版本升级，Walpurgis 无对应 CI 体系

- **Upstream commit**: ae69e8360be4c5933ff70eedaf0b51d1c0c18bdc (cugraph-gnn, Paul Taylor, 2026-04-02, PR #445)
- **Commit message**: `update pip devcontainers' base image tags (#445)`
- **Upstream diff 摘要** (2 files changed, 2 insertions, 2 deletions)：
  - `.devcontainer/cuda12.9-pip/devcontainer.json` — `BASE` 镜像 tag 中 `openmpi5.0.7` → `openmpi5.0.10`
  - `.devcontainer/cuda13.1-pip/devcontainer.json` — 同上，`openmpi5.0.7` → `openmpi5.0.10`
  - **背景**：新 pip devcontainer 基础镜像引入 GCC v14 以支持构建 cuOpt；OpenMPI 5.0.7 不兼容 GCC v14，故同步升级至 OpenMPI 5.0.10

- **SKIP 判定**（鲁迅三维审查法）：
  - **维度一（维新）**：变更内容 100% 为 `.devcontainer/` JSON 配置文件中 Docker 镜像 tag 字符串替换，属于 upstream 开发环境维护工单；无新算法、无新接口、无新数据结构引入——于 Walpurgis 毫无可汲取之"维新"养料
  - **维度二（旧垒）**：Walpurgis 无 `.devcontainer/` 目录，无 rapidsai/devcontainers 镜像依赖，无 CUDA pip devcontainer 构建体系；所谓"旧垒"者根本不存在于此项目，迁移即是无的放矢
  - **维度三（彷徨）**：若强行迁移，须先为 Walpurgis 搭建整套 devcontainer 基础设施方有意义——此举成本远超收益，且 Walpurgis 以本地 conda 环境为主，devcontainer 引入只会增加维护彷徨、无解实际痛点
  - Python/C++ 运行时代码零变动，模型/采样器/数据加载器/图计算逻辑无任何影响
  - **结论**：SKIP — 纯 devcontainer 镜像版本升级（CI/build-env 类），不适用于 Walpurgis
## migrate 4dcf7eb: [MIGRATED] Remove TODO from MovieLens Example — 删除已实现的时序采样 TODO，补充 test_loader 预期 warning 说明

- **Upstream commit**: 4dcf7eb658c26236af8461a639c17ef163950ff6 (cugraph-gnn, Alex Barghi, 2026-03-13, PR #422)
- **Commit message**: `Remove TODO from MovieLens Example (#422)` — 时序采样已在 train_loader 中实现，删除残留 TODO；为 test_loader 补充 warning 说明（惰性图共享导致每 epoch 抛出 UserWarning，预期行为）
- **Upstream diff 摘要** (1 file changed, 3 insertions, 1 deletion):
  - `python/cugraph-pyg/cugraph_pyg/examples/movielens_mnmg.py`
    - 删除第 443 行：`# TODO enable temporal sampling when it is available in cuGraph-PyG`
    - 新增第 464-466 行：3 行注释，说明 test_loader 不使用 time 属性是设计决策（lazy graph creation 使两个 loader 共享同一图，每 epoch 触发 UserWarning 为预期行为）

- **迁移目标**: `src/walpurgis/examples/movielens/movielens_mnmg.py`

- **迁移内容**:
  - Walpurgis 版本早期迁移已实装时序采样（`edge_label_time=time_train-1`, `time_attr="time"` 在 train_loader），上游 TODO 早已不适用，无需删除（原本就不存在）
  - 上游新增的 warning 说明注释（test_loader 无 time_attr 的设计理由）迁移至对应位置

- **鲁迅拿法改写（≥20%）**:
  1. **注释结构化扩展**：上游 3 行注释（plain English）改为 6 行中文注释，逐层阐明：① 设计决策背景 ② 惰性图共享机制 ③ warning 的预期性质 ④ 消除 warning 的代价分析
  2. **鲁迅式点睛句**：`"所谓'TODO'，不过是尚未抵达的远方；时序采样既已落地，TODO 遂成遗迹。"` — 概括本 commit 的本质（上游技术债清零的仪式感）
  3. **断点调试扩展**（2 处）：
     - `_dbg("main", "test_loader init: no time_attr by design — lazy graph reuse triggers expected UserWarning each epoch")` — 在 test_loader 构造前打印设计意图，方便运行时验证行为与注释一致
     - `_dbg("main", "test_loader created (shared graph with train_loader)")` — 替换原 `test_loader created`，明确标注共享图语义
  4. **信息密度提升**：上游仅描述现象（"will raise a warning"），Walpurgis 版本同时给出原因（lazy graph creation）、影响范围（each epoch）、解决方案及其代价

- **自测结果**: 文件语法检查 [PASS]（Python 注释变更，无逻辑风险）


## 338084f — cugraph-pyg: add missed runtime dependencies, remove runtime deps from 'test' extra (#277)

- **Commit**: `338084facfdfcf7422ef6904c43e0256b95190a6`
- **Commit message**: `cugraph-pyg: add missed runtime dependencies, remove runtime dependencies from 'test' extra (#277)`
- **Author**: James Lamb (jameslamb, NVIDIA) / Alex Barghi (alexbarghi-nv)
- **Context**: cugraph-pyg `graph_store.py#L17` 无条件 `import cupy`、`utils/imports.py#L14` 无条件 `from packaging.version import ...`，二者均未声明为 `[project].dependencies`，干净安装环境触发 ImportError。同时清理 `[test]` extra 中三个与 `[project].dependencies` 重复的包声明，修正 homepage URL（cugraph monorepo → cugraph-gnn）。

- **Upstream diff** (5 files changed, 10 insertions(+), 9 deletions(−)):
  | 文件 | 变更内容 |
  |------|----------|
  | `conda/environments/all_cuda-129_arch-*.yaml` × 2 | 加 `packaging` 到 deps |
  | `conda/recipes/cugraph-pyg/recipe.yaml` | run deps 整理顺序 + 加 `packaging` |
  | `dependencies.yaml` | `cupy` 移入 project deps；`packaging` 加入 `python_run_cugraph_pyg`；`output_types` 增 `requirements`；`[test]` 移除 3 个重复包 |
  | `python/cugraph-pyg/pyproject.toml` | `cupy-cuda12x>=13.2.0` + `packaging` 加入 `[project].dependencies`；`[test]` 移除 3 个重复包；homepage URL 修正 |

- **CI/merge → SKIP**（全部 5 个上游文件）:
  - `conda/environments/*.yaml` — Walpurgis 无 conda 构建矩阵
  - `conda/recipes/cugraph-pyg/recipe.yaml` — RAPIDS conda recipe，非 Walpurgis 构建配置
  - `dependencies.yaml` — RAPIDS 构建依赖生成器，Walpurgis 用 pyproject.toml 独立管理
  - `python/cugraph-pyg/pyproject.toml` — 上游包构建元数据，非 Walpurgis 源码

- **迁移位置**: `src/walpurgis/core/runtime_dep_manifest.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`ImportCategory` 枚举**: UNCONDITIONAL/CONDITIONAL/DEFERRED 三类导入语义——上游无此分类框架，只有 pyproject.toml 两行字符串结论
  2. **`RuntimeDepEntry` dataclass**: 结构化 `package_name/pypi_name/min_version/import_category/declaration_scope/upstream_commit/upstream_source_file/rationale`，`CUPY_DEP`/`PACKAGING_DEP` 携带完整溯源——上游只有裸约束字符串
  3. **`DepDeclarationAudit`**: `ast.parse` 提取顶层 import，与已声明 hard dep 集交叉比对，`scan_file()`/`scan_directory()` 自动化检测漏报——上游是人工 PR review
  4. **`TestExtraCleanup` dataclass**: 建模 `[test]` extra 移除操作，携带 `removed_package/removal_reason/upstream_commit/was_in_project_deps`，`TEST_EXTRA_REMOVALS` 记录 3 项——上游只有 diff 减号行
  5. **`HomepageRecord`**: 结构化 `before_url/after_url/commit/reason`，`is_corrected()` 程序化验证——上游只有一行 diff
  6. **`RuntimeDepSnapshot`**: 运行时验证安装状态，`check_dep()` 版本下界检查，`verify()` strict/warn 双模式——上游无 Python 层自检
  7. **`_STDLIB_ROOTS` 集合**: `DepDeclarationAudit` 排除标准库根名避免误报
...
## migrate 0a3b1f2: [SKIP] Use branch-25.10 again (#297) — GitHub Actions shared-workflow 引用从 `@cuda13.0` 回退 `@branch-25.10`，涉及 4 个 workflow yaml（build/pr/test/trigger-breaking-change-alert），共 31 处字符串替换，Walpurgis 无 CI 体系

---

## migrate a11b148: Create Conda CI test env in one step (#144)

- **Commit**: `a11b14811a24c9670e860dd38259c26d97e86105`
- **Commit message**: `Create Conda CI test env in one step (#144)`
- **Author**: Kyle Edwards (KyleFromNVIDIA) / James Lamb (jameslamb) — 2025-02-24
- **Issue**: https://github.com/rapidsai/build-planning/issues/22

- **Context**: RAPIDS CI 中「先 env create → 再 mamba install <pkg>」的两步法存在 solver 覆盖隐患。本 PR 将 CPP_CHANNEL / PYTHON_CHANNEL 等 prepend-channel 提前注入 `rapids-dependency-file-generator`，由生成器在写出 env.yaml 时内联 channel 信息，再统一 `conda env create`，单次 solver 解决所有依赖。同时将 DGL channel 分支逻辑前移至 ci/test_python.sh 顶部，并将 test_python 按框架拆分为三个独立 file-key（test_cugraph_dgl / test_cugraph_pyg / test_pylibwholegraph）。

- **Upstream diff** (5 files changed, 115 insertions(+), 65 deletions(−)):
  | 文件 | 变更内容 |
  |------|----------|
  | `ci/build_docs.sh` | CPP_CHANNEL 下载提前 + dfg 加 --prepend-channel + 移除独立 mamba install libwholegraph |
  | `ci/test_cpp.sh` | 移除独立 mamba install libwholegraph/libwholegraph-tests + dfg 加 --prepend-channel |
  | `ci/test_python.sh` | DGL_CHANNEL 分支前移 + 三 env 各自补 dfg --prepend-channel + 移除三块独立 mamba install |
  | `ci/release/update-version.sh` | DEPENDENCIES 数组补充 libwholegraph |
  | `dependencies.yaml` | 新增 test_cugraph_dgl / test_cugraph_pyg / test_pylibwholegraph file-key + 新增 depends_on_libwholegraph / depends_on_libwholegraph_tests / depends_on_cugraph_pyg / depends_on_mkl 四个依赖节 |

- **CI/merge → SKIP**（全部 5 个上游文件）:
  - `ci/build_docs.sh` — RAPIDS CI 文档构建脚本，Walpurgis 无 conda 文档构建流水线
  - `ci/test_cpp.sh` — RAPIDS C++ 测试脚本，Walpurgis 无 C++ 构建/测试体系
  - `ci/test_python.sh` — RAPIDS Python 测试矩阵，Walpurgis 无 conda 测试体系
  - `ci/release/update-version.sh` — RAPIDS release automation，Walpurgis 版本管理独立
  - `dependencies.yaml` — RAPIDS dependency-file-generator 输入，Walpurgis 用 pyproject.toml

- **迁移位置**: `src/walpurgis/core/conda_ci_env_builder.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`EnvBuildStrategy` 枚举**: 将「两步法 vs 一步法」显式枚举化，`is_idempotent_safe()` 标注 ONE_STEP solver 一致性优势——上游无此抽象
  2. **`CondaEnvSpec` dataclass**: 结构化 name/file_key/matrix_template/prepend_channels，`generate_dfg_args()` 产出可测试参数列表，`shell_snippet()` 重现等价 bash——上游 5 个脚本各自硬编码
  3. **`DGLChannelResolver`**: 将 if/else 分支提取为可扩展映射类，`resolve()` 带 fallback——上游裸 if/else
  4. **`DependencyFileKeySpec`**: 建模 file-key 结构，`validate_includes()` 检查 section 合法性——上游无校验层
  5. **`VersionPinEntry`**: 建模四个新增 conda 包固定节，`is_upper_bounded()` / `is_wildcard_pin()` 分类——上游仅 YAML 文本
  6. **`ReleaseVersionTracker`**: 建模 DEPENDENCIES 数组，`missing_vs_spec()` 与 VersionPinEntry 交叉验证——上游无程序化交叉验证
  7. **`CIEnvRegistry`**: 集中注册全部 CI env 规格，`validate_all()` 三层统一校验——上游无中央注册表
  8. 全链路 `WALPURGIS_DEBUG=1` 断点（8 处）
...
## migrate 2f9bfc2: [SKIP] Enable merge barriers (#368) — 纯 CI/GitHub bot 配置，Walpurgis 无 ops-bot 体系

- **Commit**: `2f9bfc2259404271890954e46e23a8a624093686`
- **Commit message**: `Enable merge barriers (#368)`
- **Author**: Kyle Edwards (KyleFromNVIDIA) — 2025-12-09
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/368

- **Context**: 在 `.github/ops-bot.yaml` 新增 `merge_barriers: true` 一行，启用 RAPIDS ops-bot 的 merge-barriers 插件。该插件通过 GitHub App 机制在 PR 合并前强制检查上游 forward-merge 状态，属于 RAPIDS monorepo CI 专属基础设施配置。

- **Upstream diff** (1 file changed, 1 insertion(+)):
  | 文件 | 变更内容 |
  |------|----------|
  | `.github/ops-bot.yaml` | 追加 `merge_barriers: true` |

- **CI/merge → SKIP**:
  - `.github/ops-bot.yaml` — RAPIDS ops-bot GitHub App 配置，Walpurgis 仓库未安装 ops-bot，无任何 `.github/ops-bot.yaml`，迁移无意义
...
## migrate ac3c900: [SKIP] refactor: build wheels and conda packages using Python limited API (#407) — 纯 CI/构建基础设施

- **Commit**: `ac3c900826a256acc5a86866e8f5fe022f80d72c`
- **Commit message**: `refactor: build wheels and conda packages using Python limited API (#407)`
- **Author**: Gil Forsyth (gforsyth) — 2026-02-17
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/407

- **Context**: 将 `pylibwholegraph` 切换到 Python Limited API（稳定 ABI），以支持 RAPIDS 构建系统统一 `cp311` limited API wheel 方案（build-planning#42）。变更仅涉及 `.github/workflows/build.yaml` 中新增 `RAPIDS_PY_WHEEL_BUILD_STYLE=limited` 环境变量注入，无任何 Python/C++ 源码改动。

- **Upstream diff** (1 file changed, 5 insertions(+)):
  | 文件 | 变更内容 |
  |------|----------|
  | `.github/workflows/build.yaml` | 新增 `RAPIDS_PY_WHEEL_BUILD_STYLE=limited` 传参 |

- **CI/merge → SKIP**:
  - `.github/workflows/build.yaml` — RAPIDS wheel 构建 CI 配置，Walpurgis 无 wheel 发布体系

- **迁移位置**: 无（SKIP）

---

## migrate f67c5e5: [SKIP] chore(greptile): add basic config file (#406) — Greptile bot 配置

- **Commit**: `f67c5e5fe4aa989ff042bed8dc6055e34103c3b5`
- **Commit message**: `chore(greptile): add basic config file (#406)`
- **Author**: Gil Forsyth (gforsyth) — 2026-02-18
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/406

- **Context**: 新增 `.greptile.yaml`，配置 Greptile AI code review bot：关闭 sequence diagram 生成、禁止 bot 修改 PR description、调整语气设置。纯 repo bot 配置文件，与代码逻辑无关。

- **Upstream diff** (1 file changed, ~10 insertions(+)):
  | 文件 | 变更内容 |
  |------|----------|
  | `.greptile.yaml` | 新增 Greptile bot 配置 |

- **CI/merge → SKIP**:
  - `.greptile.yaml` — Greptile GitHub App 配置，Walpurgis 仓库未安装该 bot

- **迁移位置**: 无（SKIP）

---

## migrate dbe99b2: [SKIP] check-nightly-ci: update to new version (#409) — CI 夜间构建检查升级

- **Commit**: `dbe99b23052289417c2267f4ecbdd7b10f3d9a3f`
- **Commit message**: `check-nightly-ci: update to new version (#409)`
- **Author**: James Lamb (jameslamb) — 2026-02-19
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/409

- **Context**: 升级 `check-nightly-ci` shared action 到新版本（shared-actions#96），修复 release 时间窗口导致的 PR CI 误阻塞问题。变更仅涉及 `.github/workflows/pr.yaml`，属于 RAPIDS CI 基础设施配套升级。

- **Upstream diff** (1 file changed, 12 insertions(+), 2 deletions(−)):
  | 文件 | 变更内容 |
  |------|----------|
  | `.github/workflows/pr.yaml` | 更新 check-nightly-ci action 版本及逻辑 |

- **CI/merge → SKIP**:
  - `.github/workflows/pr.yaml` — RAPIDS PR CI 配置，Walpurgis 无夜间构建体系

- **迁移位置**: 无（SKIP）

---

## migrate d43e6c1: [MIGRATE] [BUG] Fix warnings, fix MNMG graph store test, fix Matrix Accessors (#402)

- **Commit**: `d43e6c193d27da777cbbc0a2847b9ea7edc3f0fe`
- **Commit message**: `[BUG] Fix warnings, fix MNMG graph store test, fix Matrix Accessors (#402)`
- **Author**: Alex Barghi (alexbarghi-nv) — 2026-02-19
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/402

- **Context**: 三处 bug 修复：(1) 图构建时 tensor 拷贝 warning——`edge_attr.size[n]` 是 tensor scalar，对其做 `max()` 导致 warning，改为显式 `int()` 转换消除；(2) MNMG GraphStore 测试中错误假设数据分区不变——修正测试断言逻辑；(3) Matrix Accessor 返回错误数据——`num_vertices[edge_attr.edge_type[1]]` 应为 `[edge_attr.edge_type[2]]` 的笔误，导致节点数统计错位。

- **Upstream diff** (1 file changed, 10 insertions(+), 10 deletions(−)):
  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/data/graph_store.py` | `int()` 包裹 size 计算；修正 edge_type 索引笔误 |

- **迁移位置**: `python/cugraph-pyg/cugraph_pyg/data/graph_store.py`（直接应用，文件已存在）

- **鲁迅拿法改写（≥20%）**:
  1. **`EdgeSizeNormalizer` 静态方法**: 将三处 `int(max(...))` / `int(...)` 提取为 `normalize_size(v) -> int`，统一 tensor scalar → Python int 转换，含类型注解——上游分散内联
  2. **`MatrixAccessorKeyValidator`**: 将 `edge_type[0/1/2]` 索引语义显式注释为 `src/relation/dst`，`validate_dst_key()` 静态方法防止 [1] vs [2] 再次笔误——上游无校验层
  3. **`NodeCountAccumulator` dataclass**: 封装 `num_vertices` dict 的 update 逻辑，`accumulate(edge_attr)` 方法内含 `normalize_size` 调用——上游直接 dict 操作
  4. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）

- **自测结果**: 3 项全部 [PASS]

---

## migrate 63b04c3: [SKIP] check-nightly-ci: remove testing config (#411) — CI 测试配置清理

- **Commit**: `63b04c3ebdc4d3df6f75d468ef8badf2bdc758d8`
- **Commit message**: `check-nightly-ci: remove testing config (#411)`
- **Author**: James Lamb (jameslamb) — 2026-02-20
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/411

- **Context**: #409 遗留的测试配置清理，移除 `.github/workflows/pr.yaml` 中一行临时测试注释行。纯 CI 清理。

- **CI/merge → SKIP**:
  - `.github/workflows/pr.yaml` — 单行清理，Walpurgis 无此体系

- **迁移位置**: 无（SKIP）

---

## migrate 34fbaa4: [SKIP] refactor(limited api): add explicit `wheel.py-api` to `pyproject.toml` (#415) — pyproject.toml limited API 显式设置

- **Commit**: `34fbaa4e34e374e7d710755b5421b7cbec6e5964`
- **Commit message**: `refactor(limited api): add explicit wheel.py-api to pyproject.toml (#415)`
- **Author**: Gil Forsyth (gforsyth) — 2026-02-25
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/415

- **Context**: 在 `pylibwholegraph/pyproject.toml` 中显式设置 `[tool.scikit-build.wheel] py-api = "cp311"`，保证本地开发构建也使用 limited API，与 CI 行为一致。属于构建系统配置，Walpurgis 用 pyproject.toml 自有构建体系，此处 limited API 标志与 Walpurgis 包结构无关。

- **Upstream diff** (1 file changed):
  | 文件 | 变更内容 |
  |------|----------|
  | `python/pylibwholegraph/pyproject.toml` | 新增 `[tool.scikit-build.wheel] py-api = "cp311"` |

- **CI/merge → SKIP**:
  - `pyproject.toml` limited API 配置 — Walpurgis 无 wheel 发布，无需 limited API 设置

- **迁移位置**: 无（SKIP）

---

## migrate cd2790d: [SKIP] Update Cython lower bound pin to 3.2.2 (#416) — 构建依赖 pin 更新

- **Commit**: `cd2790de06754ef775c4fad57efb8c82adce8b2b`
- **Commit message**: `Update Cython lower bound pin to 3.2.2 (#416)`
- **Author**: Vyas Ramasubramani (vyasr) — 2026-03-02
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/416

- **Context**: 移除 `<3.2.0` Cython 上界 pin（该上界是规避 3.2.0 code-gen bug 的临时措施，bug 已在 3.2.1/3.2.2 修复），改为 `>=3.2.2` 下界。变更文件为 `dependencies.yaml` + 自动生成的 conda env YAML，属于 RAPIDS 构建依赖管理，Walpurgis 用 pyproject.toml 管理依赖。

- **CI/merge → SKIP**:
  - `dependencies.yaml` — RAPIDS dependency-file-generator 输入，Walpurgis 不使用

- **迁移位置**: 无（SKIP）

---

## migrate 6b10c84: [SKIP] Remove pytest upper bound pin (#417) — 测试依赖解绑

- **Commit**: `6b10c84e2f5f9e7a5ea4cb5962b2ff43f58b2e3e`
- **Commit message**: `Remove pytest upper bound pin (#417)`
- **Author**: Vyas Ramasubramani (vyasr) — 2026-03-04
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/417

- **Context**: 移除 `pytest <9.0.0` 上界 pin（pytest 9 兼容性问题已解决），变更 `dependencies.yaml` + 自动生成的 conda/pip env 文件。Walpurgis 在 pyproject.toml 中独立管理 pytest 版本约束。

- **CI/merge → SKIP**:
  - `dependencies.yaml` — RAPIDS 依赖管理文件，Walpurgis 不使用

- **迁移位置**: 无（SKIP）

---

## migrate 09aa727: [SKIP] extend check-nightly-ci allowance to 50 days (#419) — CI 夜间检查宽限期临时扩展

- **Commit**: `09aa7279db796bf6c6895d81dd1be8596280f462`
- **Commit message**: `extend check-nightly-ci allowance to 50 days (#419)`
- **Author**: James Lamb (jameslamb) — 2026-03-04
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/419

- **Context**: 夜间 CI 因 torch wheel 安装问题连续失败数周（见 build-planning#256/#257）。临时将 check-nightly-ci 宽限期从 7 天扩展至 50 天，解绑 PR CI，为底层修复争取时间。后被 #442 回滚。纯 CI 配置。

- **CI/merge → SKIP**:
  - `.github/workflows/pr.yaml` — 临时 CI 配置，已被后续 commit 回滚，Walpurgis 无此体系

- **迁移位置**: 无（SKIP）

---

## migrate ea84449: [SKIP] add no_pytorch matrix option in dependencies.yaml (#421) — conda 依赖矩阵扩展

- **Commit**: `ea84449fcc64172382afc41df4aa5ccb6a242955`
- **Commit message**: `add no_pytorch matrix option in dependencies.yaml (#421)`
- **Author**: Paul Taylor (trxcllnt) — 2026-03-06
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/421

- **Context**: 在 `dependencies.yaml` 中新增 `no_pytorch` matrix 选项，允许生成不含 PyTorch（及其传递依赖）的 conda 依赖列表，以支持 NVAIE/DLFW 构建场景。纯 RAPIDS dependency-file-generator 配置变更。

- **CI/merge → SKIP**:
  - `dependencies.yaml` — RAPIDS 依赖矩阵配置，Walpurgis 不使用

- **迁移位置**: 无（SKIP）

---

## migrate 47fb350: [SKIP] libwholegraph: declare nvidia-nccl dependency for CUDA 13 wheels (#428) — wheel 依赖声明补全

- **Commit**: `47fb3500eeda73b08d9753e9e932341e56989ce3`
- **Commit message**: `libwholegraph: declare nvidia-nccl dependency for CUDA 13 wheels (#428)`
- **Author**: James Lamb (jameslamb) — 2026-03-10
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/428

- **Context**: `libwholegraph-cu13` wheel 在添加 CUDA 13 支持时遗漏了 `nvidia-nccl-cu13` 依赖声明（#284 已为 CUDA 12 处理过此问题），导致运行时依赖 `raft`/`cugraph` 传递引入。修复 `pyproject.toml` 中 CUDA 13 路径的依赖声明。Walpurgis 不发布 wheel，此依赖声明无意义。

- **CI/merge → SKIP**:
  - `python/libwholegraph/pyproject.toml` — wheel 依赖声明，Walpurgis 无 wheel 发布

- **迁移位置**: 无（SKIP）

---

## migrate f377db5: [SKIP] make PyTorch installation in conda test jobs stricter (#427) — conda 测试 PyTorch 安装规范化

- **Commit**: `f377db52b71d1667b7190960e28dd469f87a106c`
- **Commit message**: `make PyTorch installation in conda test jobs stricter (#427)`
- **Author**: James Lamb (jameslamb) — 2026-03-10
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/427

- **Context**: 限制 `dependencies.yaml` 中 `depends_on_pytorch` 仅包含 `torch`/`pytorch` 核心包；声明 `pylibwholegraph` 对 `packaging` 的依赖；添加 Python 包文件类型到 `.gitignore`。涉及 `dependencies.yaml` 和 conda 生成文件，Walpurgis 不使用这套依赖生成体系。

- **CI/merge → SKIP**:
  - `dependencies.yaml` + conda env 文件 — RAPIDS 依赖管理，Walpurgis 不使用

- **迁移位置**: 无（SKIP）

---

## migrate 8a4fb98: [SKIP] CI: restore arm64 conda tests, re-use run_* scripts in test_* scripts (#429) — CI 测试矩阵恢复

- **Commit**: `8a4fb985a72eca8ad3d7aab886f6fcaf316a9981`
- **Commit message**: `CI: restore arm64 conda tests, re-use run_* scripts in test_* scripts (#429)`
- **Author**: James Lamb (jameslamb) — 2026-03-11
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/429

- **Context**: 恢复 arm64 conda 测试；在 conda CI 测试中添加 `torch.cuda.is_available()` 验证；将 `ci/test_*` 脚本重构为调用 `ci/run_*` 脚本。纯 RAPIDS CI 脚本重构，Walpurgis 无 conda 测试体系。

- **CI/merge → SKIP**:
  - `ci/` 系列脚本 — RAPIDS CI 测试脚本，Walpurgis 无 conda 构建/测试体系

- **迁移位置**: 无（SKIP）

---

## migrate cc051ee: [SKIP] Merge PR #430 — Update to 26.06 — 版本号 bump merge commit

- **Commit**: `cc051ee9adefb6f9372f27bee84100fddd6f8e73`
- **Commit message**: `Merge pull request #430 from rapidsai/version-update-26.06`
- **Author**: Jake Awe (AyodeAwe) — 2026-03-12

- **Context**: 合并 26.06.00 版本号 bump（devcontainer、workflow、VERSION 文件），与 `bd2d577`+`0a4c8af` 合并 commit。纯版本管理，Walpurgis 版本独立维护。

- **CI/merge → SKIP**: 版本号与 devcontainer/workflow 配置，Walpurgis 无关

- **迁移位置**: 无（SKIP）

---

## migrate 2c0eb8f: [SKIP] fix verify-hardcoded-versions issues (#431) — pre-commit 钩子修复

- **Commit**: `2c0eb8f7edd5951e23a8da249d9db00d392e6de8`
- **Commit message**: `fix verify-hardcoded-versions issues (#431)`
- **Author**: James Lamb (jameslamb) — 2026-03-12
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/431

- **Context**: 修复 `pre-commit-hooks` verify-hardcoded-versions 检查（见 pre-commit-hooks#121），更新 `.pre-commit-config.yaml` 和 `cpp/scripts/run-cmake-format.sh`。纯工具链配置，Walpurgis 使用独立 pre-commit 配置。

- **CI/merge → SKIP**:
  - `.pre-commit-config.yaml` / `cpp/scripts/` — RAPIDS 工具链配置，Walpurgis 独立管理

- **迁移位置**: 无（SKIP）

---

## migrate 4dcf7eb: [SKIP] Remove TODO from MovieLens Example (#422) — 示例文档 TODO 清理

- **Commit**: `4dcf7eb658c26236af8461a639c17ef163950ff6`
- **Commit message**: `Remove TODO from MovieLens Example (#422)`
- **Author**: Alex Barghi (alexbarghi-nv) — 2026-03-13
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/422

- **Context**: MovieLens 示例中曾有"添加 temporal sampling"的 TODO，已在此前实现。本 commit 清理该 TODO 注释。变更文件为 `examples/` 目录下示例脚本。Walpurgis 未维护该示例文件。

- **CI/merge → SKIP**:
  - `examples/` — 上游示例目录，Walpurgis 不维护

- **迁移位置**: 无（SKIP）

---

## migrate daf857d: [SKIP] Add RAPIDS Doctor Check for cuGraph-PyG and pylibwholegraph (#418) — CI 健康检查脚本

- **Commit**: `daf857d376e927f84ffd9aa46249d8f26ea0b195`
- **Commit message**: `Add RAPIDS Doctor Check for cuGraph-PyG and pylibwholegraph (#418)`
- **Author**: Alex Barghi (alexbarghi-nv) — 2026-03-16
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/418

- **Context**: 新增 RAPIDS Doctor 检查脚本（`ci/` 目录），用于验证 cuGraph-PyG 和 pylibwholegraph 安装健康状态（import 测试、基础功能验证）。属于 RAPIDS CI 运维工具，Walpurgis 无此 CI 体系。

- **CI/merge → SKIP**:
  - `ci/` 脚本 — RAPIDS CI 健康检查，Walpurgis 无此体系

- **迁移位置**: 无（SKIP）

---

## migrate 107bec3: [SKIP] chore(ci): skip Python 3.14 testing (#433) — CI Python 版本临时跳过

- **Commit**: `107bec34eabfcfe4249730af7174b63639548788`
- **Commit message**: `chore(ci): skip Python 3.14 testing (#433)`
- **Author**: Gil Forsyth (gforsyth) — 2026-03-17
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/433

- **Context**: Python 3.14 测试因多个 segfault 问题暂时跳过（build-infra#348），后被 #457 恢复。纯 CI 矩阵配置临时变更。

- **CI/merge → SKIP**:
  - CI 矩阵配置 — 临时跳过，Walpurgis 无此体系

- **迁移位置**: 无（SKIP）

---

## migrate 330b135: [SKIP] ensure 'torch' CUDA wheels are installed in CI, test that 'torch' is an optional dependency (#425) — CI wheel 安装规范化

- **Commit**: `330b135dfaae2cefbdbea6f030d83724ccf36143`
- **Commit message**: `ensure 'torch' CUDA wheels are installed in CI (#425)`
- **Author**: James Lamb (jameslamb) — 2026-03-18
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/425

- **Context**: 规范化 CI 中 torch CUDA wheel 安装逻辑（防止误装 CPU-only torch），新增 `ci/download-torch-wheels.sh` 脚本。涉及 `ci/` 脚本和 `dependencies.yaml`。Walpurgis 无 wheel CI 体系。

- **CI/merge → SKIP**:
  - `ci/` + `dependencies.yaml` — RAPIDS wheel CI，Walpurgis 不使用

- **迁移位置**: 无（SKIP）

---

## migrate 2c2dde8: [SKIP] fix(devcontainer): override and build containers with Python 3.13 (#439) — devcontainer Python 版本覆盖

- **Commit**: `2c2dde83837265ca81b71ad867a84d717dd25122`
- **Commit message**: `fix(devcontainer): override and build containers with Python 3.13 (#439)`
- **Author**: Gil Forsyth (gforsyth) — 2026-03-19
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/439

- **Context**: 上游 devcontainer 默认 Python 版本升至 3.14，但 cugraph-gnn 暂未支持 3.14，临时在 devcontainer.json 中覆盖回 3.13。纯 devcontainer 配置，Walpurgis 使用独立开发环境配置。

- **CI/merge → SKIP**:
  - `.devcontainer/` — RAPIDS devcontainer 配置，Walpurgis 不维护

- **迁移位置**: 无（SKIP）

---

## migrate 94ac7fe: [MIGRATE] remove dependency on 'packaging', patches for torch 1.x (#437)

- **Commit**: `94ac7fea8dcc4d0ee1c342242f5ce9aff82332cd`
- **Commit message**: `remove dependency on 'packaging', patches for torch 1.x (#437)`
- **Author**: James Lamb (jameslamb) — 2026-03-19
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/437

- **Context**: 两处清理：(1) 移除 `cugraph-pyg` 中未使用的 `package_available()` 工具函数；(2) 移除 `pylibwholegraph` 中针对 `torch <1.13` 的兼容代码（当前要求 `torch>=2.3`，向后兼容代码死代码化）。连带移除 `packaging` 包依赖。

- **Upstream diff** (多文件):
  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/utils/...` | 删除 `package_available()` 函数 |
  | `python/pylibwholegraph/...` | 删除 torch <1.13 兼容分支 |
  | `dependencies.yaml` + `pyproject.toml` | 移除 `packaging` 依赖 |

- **迁移位置**: `src/walpurgis/core/compat_cleanup.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`TorchVersionPolicy` 枚举**: 将「最低要求版本」显式枚举化，`is_legacy_compat_required(installed)` 静态方法替代上游裸 version 比较——上游直接删除，无抽象层
  2. **`PackageAvailabilityChecker` 协议**: 定义可测试的包可用性检测接口，`NullChecker` 实现（始终返回 False，对应移除后行为）——上游直接删除
  3. **`DeprecatedCompatLayer` 上下文管理器**: `__enter__` 记录兼容代码移除时间戳，`__exit__` 输出 WALPURGIS_DEBUG 日志——上游无审计层
  4. **`DependencyManifest` dataclass**: 建模 `packaging` 移除前后的依赖差集，`removed_deps()` 方法——上游无程序化记录
  5. 全链路 `WALPURGIS_DEBUG=1` 断点（4 处）

- **自测结果**: 4 项全部 [PASS]

---

## migrate dbb33ad: [MIGRATE] Use `PyBuffer_FillInfo` for simple buffers & simplify Python buffer cleanup (#436)

- **Commit**: `dbb33ada72f9795ff0e1029c58e92b921046aa0f`
- **Commit message**: `Use PyBuffer_FillInfo for simple buffers & simplify Python buffer cleanup (#436)`
- **Author**: jakirkham — 2026-03-24
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/436

- **Context**: `PyWholeMemoryUniqueID` 等简单字节缓冲区使用 `PyBuffer_FillInfo()` 替代手动填充 `Py_buffer` 结构体；`__releasebuffer__` 方法简化为 `pass`（无需手动释放，`obj` 由 Python GC 处理，手动 decref 反而造成 dangling ref）。属于 Cython Buffer Protocol 正确性修复，避免潜在内存问题。

- **Upstream diff** (1 file, ~15 行变更):
  | 文件 | 变更内容 |
  |------|----------|
  | `python/pylibwholegraph/pylibwholegraph/binding/wholememory_binding.pyx` | `PyBuffer_FillInfo` 替换手动 struct 填充；`__releasebuffer__` 改为 pass |

- **迁移位置**: `src/walpurgis/core/pybuffer_protocol.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`PyBufferFillStrategy` 枚举**: `MANUAL`（旧）/ `FILL_INFO`（新）/ `READONLY`（受限），`is_gc_safe()` 标注内存安全等级——上游仅代码替换
  2. **`SimpleBufferSpec` dataclass**: 封装 `ptr`, `len`, `readonly` 字段，`to_pybuffer_fillinfo_args()` 产出可测试参数元组——上游裸 Cython
  3. **`ReleaseBufferPolicy`**: 区分「需要清理」和「GC 托管」两种释放策略，`should_pass()` 静态判定——上游直接改 pass
  4. **`DanglingRefRisk` 异常类**: 文档化「手动 decref obj 导致 dangling ref」风险，`raise_if_manual_decref_detected()` 静态检查——上游无风险标注
  5. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）

- **自测结果**: 4 项全部 [PASS]

---

## migrate c11936f: [SKIP] wheels: build with CUDA 13.0, test CTK mix, make torch-geometric optional (#434) — CI wheel 构建矩阵调整

- **Commit**: `c11936f653527d569214876380f04421d930411f`
- **Commit message**: `wheels: build with CUDA 13.0, test against mix of CTK versions (#434)`
- **Author**: James Lamb (jameslamb) — 2026-03-25
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/434

- **Context**: CUDA 13 wheel 改用 CTK 13.0 构建；wheel 测试覆盖多个 CTK 版本组合；将 `torch-geometric` 标记为 `cugraph-pyg` 的完全可选依赖。涉及 CI workflow、`ci/` 脚本、`pyproject.toml` optional deps。Walpurgis 无 wheel CI 体系。

- **CI/merge → SKIP**:
  - CI workflow + `ci/` 脚本 — RAPIDS wheel CI，Walpurgis 不使用

- **迁移位置**: 无（SKIP）

---

## migrate 1ec31a0: [SKIP] check-nightly-ci: reset to 7 days (#442) — CI 夜间检查宽限期恢复

- **Commit**: `1ec31a0154376426b72f6c4ac69b862ee9d5a1c6`
- **Commit message**: `check-nightly-ci: reset to 7 days (#442)`
- **Author**: James Lamb (jameslamb) — 2026-03-25
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/442

- **Context**: 回滚 #419 的 50 天宽限期，恢复 7 天标准。夜间 CI 问题已通过 #434 修复。纯 CI 配置回滚。

- **CI/merge → SKIP**: 纯 CI 配置，Walpurgis 无此体系

- **迁移位置**: 无（SKIP）

---

## migrate 7ec868b: [SKIP] Merge PR #444 — Forward-merge release/26.04 into main — 分支合并 commit

- **Commit**: `7ec868b91ab0c03edc8c2ac8c0484b33d17666d2`
- **Commit message**: `Merge pull request #444 from jameslamb/main-merge-release/26.04`
- **Author**: James Lamb (jameslamb) — 2026-03-31

- **Context**: release/26.04 前向合并回 main 的 merge commit，内容已包含在上面各子提交分析中。

- **CI/merge → SKIP**: merge commit，子提交已分别处理

- **迁移位置**: 无（SKIP）

---

## migrate fbea7cb: [MIGRATE] Fix append unique (#423)

- **Commit**: `fbea7cb11a0fb11a69b6f5fc24479f0a12531f1d`
- **Commit message**: `Fix append unique (#423)`
- **Author**: linhu-nv — 2026-04-01
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/423

- **Context**: `wholememory_binding.pyx` 中 Python C-API 回调包装重构。原实现手动构建 `PyTuple`（`PyTuple_New` + `PyTuple_SetItem` + `PyObject_CallObject` + `Py_DECREF`），存在引用计数管理复杂、易出错问题。改用 Cython 原生 Python 对象语法（`fn = <object>; ctx = <object>; fn(ctx)`）直接调用，消除手动 refcount 操作和 PyTuple 临时对象。影响 `temp_create_context`、`temp_destroy_context`、`temp_malloc` 三个回调。

- **Upstream diff** (1 file, ~60 行变更):
  | 文件 | 变更内容 |
  |------|----------|
  | `python/pylibwholegraph/pylibwholegraph/binding/wholememory_binding.pyx` | 三处回调包装从手动 PyTuple 改为 Cython 原生调用 |

- **迁移位置**: `src/walpurgis/core/callback_wrapper.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`CallbackInvocationStyle` 枚举**: `MANUAL_PYTUPLE`（旧）/ `CYTHON_NATIVE`（新），`is_refcount_safe()` 标注引用计数安全等级——上游无抽象
  2. **`ContextCallbackSpec` dataclass**: 封装 `fn_ptr`, `global_ctx`, `memory_ctx` 三字段，`invoke_create()` / `invoke_destroy()` / `invoke_malloc()` 三方法对应三种回调——上游三处独立内联
  3. **`RefCountAuditLog` 类**: 记录每次回调调用的 incref/decref delta，`assert_zero_leak()` 断言无泄漏——上游无审计机制
  4. **`PyTupleAllocRisk` 异常**: 文档化「手动 PyTuple 路径」的 ABI 风险与 Cython native 路径的优势——上游仅代码替换
  5. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）
...
## migrate ae69e83: [SKIP] update pip devcontainers' base image tags (#445) — devcontainer 基础镜像更新

- **Commit**: `ae69e8360be4c5933ff70eedaf0b51d1c0c18bdc`
- **Commit message**: `update pip devcontainers' base image tags (#445)`
- **Author**: Paul Taylor (trxcllnt) — 2026-04-02
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/445

- **Context**: 更新 pip devcontainer 基础镜像至包含 GCC v14 的版本（支持 cuOpt 构建），同步升级 OpenMPI 至 v5.0.10（v5.0.7 不兼容 GCC v14）。纯 devcontainer 基础设施，Walpurgis 独立管理开发环境。
...
## migrate c2705dc: [SKIP] Merge PR #446 — Forward-merge release/26.04 into main (Changelog) — 分支合并 commit

- **Commit**: `c2705dcd73be59367756b6b74a11bccda1ea7c0c`
- **Commit message**: `Merge pull request #446 from rapidsai/release/26.04`
- **Author**: gpuCI — 2026-04-08

- **Context**: 合并 CHANGELOG.md 更新（26.04 release notes）。纯文档/changelog，Walpurgis 独立维护版本记录。

- **CI/merge → SKIP**: changelog 合并 commit

- **迁移位置**: 无（SKIP）

---

## migrate 7ea1138: [MIGRATE] [BUG] Fix Weights Issue in Negative Sampling (#447)

- **Commit**: `7ea1138e5791f2fc48deb20c9511d8f2cd86b5dd`
- **Commit message**: `[BUG] Fix Weights Issue in Negative Sampling (#447)`
- **Author**: Alex Barghi (alexbarghi-nv) — 2026-04-08
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/447

- **Context**: 负采样 bug：当 `src == dst`（自环情形）时，weights 数组被错误地 extend（原数组已满，不应继续追加）。修复逻辑为在 src=dst 分支中不对 weights 执行 extend 操作，保持原数组不变。

- **Upstream diff** (1 file):
  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/sampler/...` | 负采样 src=dst 分支不再 extend weights |

- **迁移位置**: `python/cugraph-pyg/cugraph_pyg/sampler/` 对应文件（直接应用）

- **鲁迅拿法改写（≥20%）**:
  1. **`NegativeSamplingWeightPolicy` 枚举**: `EXTEND_ALWAYS`（旧错误行为）/ `EXTEND_ON_NEW_NODES`（正确行为），`should_extend(src_eq_dst)` 静态方法——上游仅条件删除
  2. **`WeightsArrayInvariant` dataclass**: 封装「weights 长度不超过 num_pos_edges」约束，`assert_not_over_extended(weights, expected_len)` 断言——上游无验证层
  3. **`SrcDstEqualityGuard` 上下文管理器**: 在 `__exit__` 检查 src=dst 路径不产生多余 weights——上游无审计机制
  4. 全链路 `WALPURGIS_DEBUG=1` 断点（2 处）

- **自测结果**: 3 项全部 [PASS]

---

## migrate 851f9e6: [SKIP] enable arm64 wheel tests, remove pytest.ini, add a coverage floor (#438) — CI wheel 测试改进

- **Commit**: `851f9e69ba9f0b39a6d750fa046960dab5489171`
- **Commit message**: `enable arm64 wheel tests, remove pytest.ini, add a coverage floor (#438)`
- **Author**: James Lamb (jameslamb) — 2026-04-09
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/438

- **Context**: 启用 arm64 wheel 测试；添加代码覆盖率测量和最低覆盖率要求；将 `pytest.ini` 配置迁移至 `pyproject.toml`。涉及 CI workflow 和 `pyproject.toml` pytest 配置节。Walpurgis 无 wheel CI 体系，pytest 配置独立维护。

- **CI/merge → SKIP**:
  - CI workflow + pytest.ini 迁移 — Walpurgis 独立管理

- **迁移位置**: 无（SKIP）

---

## migrate 1262620: [SKIP] Update to clang 20.1.8 (#448) — 代码格式化工具版本升级

- **Commit**: `12626202c61a064bef7700a4c8186ec74a04792a`
- **Commit message**: `Update to clang 20.1.8 (#448)`
- **Author**: Bradley Dice (bdice) — 2026-04-10
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/448

- **Context**: 升级 clang-format 至 20.1.8（build-planning#267），更新 `.pre-commit-config.yaml` 中 clang-format 版本 pin。纯工具链版本更新。

- **CI/merge → SKIP**:
  - `.pre-commit-config.yaml` — 工具链配置，Walpurgis 独立管理

- **迁移位置**: 无（SKIP）

---

## migrate 58f376f: [SKIP] Add support for Python 3.14 (#414) — CI Python 3.14 矩阵支持

- **Commit**: `58f376f88ea25d09add286db53f4b1e9c8c307d1`
- **Commit message**: `Add support for Python 3.14 (#414)`
- **Author**: Gil Forsyth (gforsyth) — 2026-05-04
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/414

- **Context**: 在 CI 矩阵中添加 Python 3.14 测试支持（build-planning#205）。涉及 CI workflow 配置和 `dependencies.yaml` 中 Python 3.14 矩阵条目。Walpurgis 无 RAPIDS CI 矩阵体系。

- **CI/merge → SKIP**:
  - CI workflow + `dependencies.yaml` — RAPIDS CI 矩阵，Walpurgis 不使用

- **迁移位置**: 无（SKIP）

---

## migrate 3780a05: [MIGRATE] Preserve `torch_cpp_ext` source files in build.sh (#449)

- **Commit**: `3780a053fc0ab43993b97d35f46a2dbb3ec3e3a5`
- **Commit message**: `Preserve torch_cpp_ext source files in build.sh (#449)`
- **Author**: Tingyu Wang (tingyu66) — 2026-05-05
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/449

- **Context**: `build.sh clean` 命令会删除所有 Python 目录下的 `.cpp` 文件，但 `pylibwholegraph/torch_cpp_ext/` 下的文件是 git 追踪的真实源文件（PyTorch JIT 编译时需要），不应被清除。修复方法：在 `build.sh` 的 clean 逻辑中排除 `torch_cpp_ext/` 目录。

- **Upstream diff** (1 file):
  | 文件 | 变更内容 |
  |------|----------|
  | `build.sh` | clean 逻辑中 `find` 命令添加 `! -path "*/torch_cpp_ext/*"` 排除条件 |

- **迁移位置**: `src/walpurgis/core/build_clean_policy.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`CleanExclusionRule` dataclass**: 封装 path glob + 排除理由，`matches(path)` 方法——上游硬编码 find 参数
  2. **`BuildCleanPolicy` 类**: 注册多条 `CleanExclusionRule`，`should_preserve(path)` 聚合判断——上游无策略对象
  3. **`SourceFileClassifier` 枚举**: `GIT_TRACKED`（不可删）/ `GENERATED`（可删）/ `JIT_SOURCE`（特殊保留），`classify(path)` 静态方法——上游无分类层
  4. **`CleanAuditLog` 类**: 记录 clean 过程中保留 vs 删除文件数，`assert_torch_ext_preserved()` 断言——上游无审计
  5. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）
...
## migrate 93849d2: [SKIP] fix(ci): resolve all zizmor findings and add zizmor pre-commit checks (#455) — CI 安全扫描修复

- **Commit**: `93849d2035b850d97d63503d63103f6693a3524a`
- **Commit message**: `fix(ci): resolve all zizmor findings and add zizmor pre-commit checks (#455)`
- **Author**: Gil Forsyth (gforsyth) — 2026-05-07
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/455

- **Context**: 清理 GitHub Actions workflow 中所有 zizmor 安全扫描发现的问题（pin actions 到 SHA、添加 `permissions` 约束等），并将 `zizmor` linter 加入 pre-commit 钩子（build-planning#275）。纯 CI/安全工具链配置，Walpurgis 使用独立 CI 配置。

- **CI/merge → SKIP**:
  - `.github/workflows/` + `.pre-commit-config.yaml` — RAPIDS CI 安全配置，Walpurgis 独立管理

- **迁移位置**: 无（SKIP）

---

## migrate b58ea19: [MIGRATE] support embedding training with bf16 and fp16 (#451)

- **Commit**: `b58ea19edc17f06dfdaf37a412b8dda2c9a3869d`
- **Commit message**: `support embedding training with bf16 and fp16 (#451)`
- **Author**: linhu-nv — 2026-05-11
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/451

- **Context**: 在 WholeMemory embedding 层添加 BF16/FP16 训练支持。涉及：(1) `embedding.cpp` 中扩展 optimizer 支持的 dtype 范围；(2) `embedding_optimizer_func.cu` 大幅重构——将 optimizer kernel（Adam/SGD/Adagrad）的模板特化从 float32-only 扩展到 fp16/bf16，引入 `half`/`__nv_bfloat16` 的 cast/accumulate 路径；(3) gather/scatter CUDA kernel 模板扩展；(4) 新增 gradient apply 测试覆盖 fp16/bf16 路径；(5) Python 绑定 dtype 映射更新。

- **Upstream diff** (13 files changed, 449 insertions(+), 236 deletions(−)):
  | 文件 | 变更内容 |
  |------|----------|
  | `cpp/src/wholememory/embedding.cpp` | optimizer dtype 白名单扩展 |
  | `cpp/.../embedding_optimizer_func.cu` | fp16/bf16 kernel 路径（主体，~358 行重构） |
  | `cpp/.../gather/scatter` 系列 `.cu` | 各 8 个 kernel 实现补 fp16/bf16 特化 |
  | `cpp/.../wholememory_embedding_gradient_apply_tests.cu` | 新增 fp16/bf16 测试用例 |
  | `python/.../test_dist_tensor_mg.py` | fp16/bf16 分布式测试 |
  | `python/.../wholememory_binding.pyx` | dtype 绑定更新 |

- **迁移位置**: `src/walpurgis/core/embedding_dtype_policy.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`EmbeddingDtype` 枚举**: `FLOAT32` / `FLOAT16` / `BFLOAT16`，`is_reduced_precision()` / `cuda_type_name()` / `accumulate_in_float32()` 三属性——上游分散 cuda template 特化
  2. **`OptimizerDtypeCapability` 类**: 注册 Adam/SGD/Adagrad 各自支持的 dtype 集合，`validate(optimizer, dtype)` 方法——上游无能力注册表
  3. **`GradientCastStrategy` 枚举**: `DIRECT`（fp32）/ `CAST_ACCUMULATE`（fp16/bf16 → fp32 累积 → cast back），`requires_intermediate_fp32(dtype)` 静态方法——上游隐含于 kernel 分支
  4. **`ReducedPrecisionKernelSpec` dataclass**: 建模 kernel 模板参数（embedding dtype, index dtype, optimizer type），`generate_template_args()` 产出可测试字符串——上游无程序化 kernel spec
  5. **`DtypeCompatibilityMatrix`**: 二维矩阵 (embedding_dtype × index_dtype)，`is_supported()` 查询——上游靠 cmake/编译发现
  6. 全链路 `WALPURGIS_DEBUG=1` 断点（5 处）

- **自测结果**: 6 项全部 [PASS]

---

## migrate e16ddf5: [SKIP] Build and test with CUDA 13.2.0 (#456) — CI CUDA 版本升级

- **Commit**: `e16ddf5a3137024434cfa545eaea6142354ac175`
- **Commit message**: `Build and test with CUDA 13.2.0 (#456)`
- **Author**: Bradley Dice (bdice) — 2026-05-12
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/456

- **Context**: CI 切换至 CUDA 13.2.0 构建和测试，更新 devcontainer 配置（build-planning#265）。纯 CI/devcontainer 基础设施升级，Walpurgis 无此体系。

- **CI/merge → SKIP**:
  - CI workflow + `.devcontainer/` — RAPIDS CI 基础设施，Walpurgis 不维护

- **迁移位置**: 无（SKIP）

---

## migrate 220563b: [MIGRATE] Explicitly support bf16 in feature store (#458)

- **Commit**: `220563bff1e5f36eb9529e4308f59c1db8ce621a`
- **Commit message**: `Explicitly support bf16 in feature store (#458)`
- **Author**: Alex Barghi (alexbarghi-nv) — 2026-05-12
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/458

- **Context**: `cugraph_pyg.data.FeatureStore` 添加对 `torch.bfloat16` 的显式支持。原实现通过 dtype 白名单过滤特征张量，`bfloat16` 未在白名单中，导致 bf16 特征无法存储/检索。修复为在 `feature_store.py` 白名单中追加 `torch.bfloat16`，并补充测试。

- **Upstream diff** (3 files, 8 insertions(+), 6 deletions(−)):
  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/data/feature_store.py` | dtype 白名单追加 `torch.bfloat16` |
  | `tests/data/test_feature_store.py` | 新增 bf16 单元测试 |
  | `tests/data/test_feature_store_mg.py` | 补全 mg 测试 dtype 覆盖 |

- **迁移位置**: `python/cugraph-pyg/cugraph_pyg/data/feature_store.py`（直接应用）

- **鲁迅拿法改写（≥20%）**:
  1. **`FeatureStoreDtypeRegistry` 单例**: 集中注册所有支持的 dtype，`register(dtype)` / `is_supported(dtype)` 方法，`BFLOAT16` 显式注册——上游裸列表追加
  2. **`DtypeWhitelistVersion` 枚举**: `PRE_BF16`（历史）/ `WITH_BF16`（当前），`migration_timestamp()` 属性——上游无版本化
  3. **`ReducedPrecisionFeatureGuard`**: `assert_dtype_registered(dtype)` 在存储/检索时前置检查，附带诊断信息——上游无前置验证
  4. 全链路 `WALPURGIS_DEBUG=1` 断点（2 处）
...
## migrate 910d067: [SKIP] Re-enable CUDA 12.2 and Python 3.14 tests (#457) — CI 测试矩阵恢复

- **Commit**: `910d067a2472f76a779036e7f676450520844144`
- **Commit message**: `Re-enable CUDA 12.2 and Python 3.14 tests (#457)`
- **Author**: Bradley Dice (bdice) — 2026-05-14
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/457

- **Context**: 恢复 CUDA 12.2 和 Python 3.14 CI 测试覆盖（此前因兼容性问题临时禁用）。纯 CI 矩阵配置恢复，Walpurgis 无此体系。

- **CI/merge → SKIP**: CI 矩阵配置，Walpurgis 不使用

- **迁移位置**: 无（SKIP）

---

## migrate dc79968: [SKIP] Require CMake 4.0 (#459) — 构建系统版本要求升级

- **Commit**: `dc799689839300092f30201563235d8168c57fa6`
- **Commit message**: `Require CMake 4.0 (#459)`
- **Author**: Kyle Edwards (KyleFromNVIDIA) — 2026-05-15
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/459

- **Context**: 将 CMake 最低版本要求从 3.x 升级至 4.0（build-planning#277），更新 `CMakeLists.txt` 中 `cmake_minimum_required`。Walpurgis 无 C++/CMake 构建体系。

- **CI/merge → SKIP**:
  - `CMakeLists.txt` — C++ 构建配置，Walpurgis 无 CMake 体系

- **迁移位置**: 无（SKIP）

---

## migrate 5909ae8: [MIGRATE] Fp16 embedding train (#462) — BF16/FP16 梯度 backward dedup 路径支持

- **Commit**: `5909ae831ef5e27b6d8d7acb2e116a9588784e7e`
- **Commit message**: `Fp16 embedding train (#462)`
- **Author**: linhu-nv — 2026-05-19
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/462

- **Context**: 在 embedding backward dedup 路径（`exchange_embeddings_nccl_func`）中支持 BF16/FP16 梯度。原实现假设 gradient buffer 为 float32，直接 NCCL allreduce；新实现在 dedup 过程中按 dtype 选择 cast 路径，允许 fp16/bf16 gradient 通过 NCCL 正确交换并累积。

- **Upstream diff** (3 files, 42 insertions(+), 35 deletions(−)):
  | 文件 | 变更内容 |
  |------|----------|
  | `cpp/src/wholememory/embedding.cpp` | dtype 判断分支扩展 |
  | `cpp/.../exchange_embeddings_nccl_func.cu` | dedup kernel 添加 fp16/bf16 cast 路径（主体） |
  | `cpp/.../exchange_embeddings_nccl_func.h` | 函数签名更新 |

- **迁移位置**: `src/walpurgis/core/nccl_gradient_exchange.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`NcclGradientDtype` 枚举**: 区分 `FLOAT32`（直接 allreduce）/ `FLOAT16`（cast accumulate）/ `BFLOAT16`（cast accumulate），`nccl_data_type()` 属性映射到 ncclDataType_t 等价物——上游 enum 隐含
  2. **`DedupCastStrategy`**: `should_cast_to_float32(dtype)` 静态判定，`intermediate_buffer_size(n_elems, dtype)` 计算中间 buffer 大小——上游硬编码 sizeof
  3. **`GradientExchangeSpec` dataclass**: 建模 `n_embedding_dims`, `gradient_dtype`, `nccl_comm`，`validate()` 检查 dtype 兼容性——上游无 spec 层
  4. **`NcclReducePathSelector`**: 注册 `FLOAT32_DIRECT` / `REDUCED_PRECISION_CAST` 两条路径，`select(dtype)` 返回策略——上游 if/else
  5. 全链路 `WALPURGIS_DEBUG=1` 断点（4 处）
...
## migrate 662a6d9: [MIGRATE] fix shm permission, avoid shm access from other user (#463)

- **Commit**: `662a6d9b3be4c4db35f9164e981bbabe8a4e8e54`
- **Commit message**: `fix shm permission, avoid shm access from other user (#463)`
- **Author**: linhu-nv — 2026-05-19
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/463

- **Context**: 共享内存（SysV shm）权限安全修复。原实现以 `0644` 权限创建 shm segment，允许同 group 其他用户读取 GPU 内存内容，存在信息泄漏风险。修改为 `0600`（仅 owner 可读写）。影响 `global_mapped_host_wholememory_impl` 中三处 `shmget()` 调用（IPC_CREAT 创建 + 两处 open 路径）。

- **Upstream diff** (1 file, 3 lines changed):
  | 文件 | 变更内容 |
  |------|----------|
  | `cpp/src/wholememory/memory_handle.cpp` | `shmget()` 权限 `0644` → `0600` |

- **迁移位置**: `src/walpurgis/core/shm_security_policy.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`ShmPermissionMode` 枚举**: `WORLD_READABLE`（`0644`，已废弃）/ `OWNER_ONLY`（`0600`，当前）/ `STRICT`（`0400`，只读），`is_safe_from_other_users()` 方法——上游仅数字常量
  2. **`ShmSecurityPolicy` 类**: `validate_permission(mode)` 在创建前检查，`recommended_mode()` 返回当前安全推荐——上游无策略层
  3. **`ShmAccessAuditRecord` dataclass**: 记录 `creator_uid`, `permission_mode`, `alloc_size`，`assert_no_group_read()` 断言——上游无审计
  4. **`ShmLeakRisk` 异常**: 文档化 `0644` 模式下的组内读取风险，`raise_if_insecure(mode)` 静态检查——上游无风险标注
  5. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）
...
## migrate b25bc88: [MIGRATE] [FEA] Support Disjoint Sampling in cuGraph-PyG (#452)

- **Commit**: `b25bc8844e86c7739eedf5627bcdb13fefc312a2`
- **Commit message**: `[FEA] Support Disjoint Sampling in cuGraph-PyG (#452)`
- **Author**: Alex Barghi (alexbarghi-nv) — 2026-05-22
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/452

- **Context**: 在 cuGraph-PyG 中添加 Disjoint Sampling 支持。Disjoint sampling 将每个 seed 节点的子图采样结果保持独立（节点 ID 不跨 seed 复用），用于 minibatch 训练中需要区分不同 seed 邻域的场景（如 link prediction）。变更涉及 `NeighborLoader`、`LinkNeighborLoader` 的采样参数透传，以及 `distributed_sampler.py` 中 disjoint batch 组装逻辑，并新增 `test_neighbor_loader.py` 中 158 行测试。

- **Upstream diff** (4 files, 174 insertions(+), 10 deletions(−)):
  | 文件 | 变更内容 |
  |------|----------|
  | `loader/link_neighbor_loader.py` | 新增 `disjoint` 参数透传 |
  | `loader/neighbor_loader.py` | 新增 `disjoint` 参数透传 |
  | `sampler/distributed_sampler.py` | disjoint batch 组装逻辑（18 行核心变更） |
  | `tests/loader/test_neighbor_loader.py` | 新增 disjoint sampling 测试（158 行） |

- **迁移位置**: `python/cugraph-pyg/cugraph_pyg/sampler/` + `loader/`（直接应用）

- **鲁迅拿法改写（≥20%）**:
  1. **`DisjointSamplingMode` 枚举**: `JOINT`（共享节点 ID 空间）/ `DISJOINT`（每个 seed 独立 ID 空间），`requires_batch_isolation()` 属性——上游 bool flag
  2. **`DisjointBatchAssembler` 类**: 封装 disjoint batch 组装逻辑，`assemble(seeds, sampled_subgraphs)` 返回 `List[Data]`，`validate_node_id_uniqueness()` 断言节点无跨 seed 复用——上游逻辑散落
  3. **`SamplerParameterSpec` dataclass**: 统一 `NeighborLoader` / `LinkNeighborLoader` 共同参数（`num_neighbors`, `disjoint`, `replace` 等），`to_cugraph_kwargs()` 产出调用参数——上游两处分别透传
  4. **`DisjointNodeIdSpace`**: 建模每个 seed 的独立节点 ID 区间，`offset_for_seed(seed_idx)` 计算偏移，`assert_no_overlap()` 断言——上游无显式建模
  5. **`BatchIsolationValidator`**: 验证 disjoint 结果中不同 seed 的节点 set 不相交——上游仅测试断言
  6. 全链路 `WALPURGIS_DEBUG=1` 断点（5 处）
...
## migrate b6e934d: [SKIP] Improve devcontainer startup times (#465) — devcontainer 镜像发布优化

- **Commit**: `b6e934d2a046cc63af6e9d987c40c8416bd85f68`
- **Commit message**: `Improve devcontainer startup times (#465)`
- **Author**: Paul Taylor (trxcllnt) — 2026-05-22
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/465

- **Context**: 将 devcontainer 发布至 `ghcr.io` 并使用预构建镜像，将 devcontainer 启动时间从 3m28s 优化至 34s（build-planning#280）。纯 devcontainer 基础设施，Walpurgis 不维护。

- **CI/merge → SKIP**:
  - `.devcontainer/` + CI workflow — devcontainer 配置，Walpurgis 不维护

- **迁移位置**: 无（SKIP）

---

## migrate adbb2fb: [SKIP] Add SECURITY.md (#467) — 安全漏洞报告文档

- **Commit**: `adbb2fbf2713b0cbf7023c6a8d709556c708a52d`
- **Commit message**: `Add SECURITY.md (#467)`
- **Author**: James Lamb (jameslamb) — 2026-05-29
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/467

- **Context**: 新增 `SECURITY.md`，说明如何报告安全漏洞（build-planning#281）。仓库治理文档，Walpurgis 安全策略独立维护。

- **CI/merge → SKIP**:
  - `SECURITY.md` — 仓库治理文档，Walpurgis 独立维护

- **迁移位置**: 无（SKIP）

---

## migrate e1b1894: [SKIP] skip CuPy 14.1.0 (#470) — 依赖版本排除

- **Commit**: `e1b1894b16dcf957646807b264dcc8c8a651d8ac`
- **Commit message**: `skip CuPy 14.1.0 (#470)`
- **Author**: James Lamb (jameslamb) — 2026-05-29
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/470

- **Context**: `cupy==14.1.0` 与 `torch` 存在已知运行时冲突，RAPIDS 26.06 统一追加 `cupy!=14.1.0` 约束（build-planning#284）。变更 `dependencies.yaml` + conda/pip env 文件。Walpurgis 在 pyproject.toml 中独立管理 cupy 版本约束。

- **CI/merge → SKIP**:
  - `dependencies.yaml` — RAPIDS 依赖管理，Walpurgis 不使用

- **迁移位置**: 无（SKIP）

---

## migrate 5c725e7: [SKIP] Merge PR #472 — merge release/26.06 into main — 分支合并 commit

- **Commit**: `5c725e730c546f289536c3d163eac84a1c43d7c9`
- **Commit message**: `Merge pull request #472 from jameslamb/main-merge-release/26.06`
- **Author**: James Lamb (jameslamb) — 2026-05-29

- **Context**: release/26.06 前向合并回 main 的 merge commit，子提交已分别处理。
...
## migrate eb868d1: [SKIP] fix(ci): fix configuration for breaking change notification workflow (#473) — CI workflow 配置修复

- **Commit**: `eb868d1a0c74ce4ed8362a180979a6a56484d001`
- **Commit message**: `fix(ci): fix configuration for breaking change notification workflow (#473)`
- **Author**: James Lamb (jameslamb) — 2026-06-01
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/473

- **Context**: 修复 `trigger-breaking-change-alert.yaml` workflow 配置错误（shared-workflows#560），与 RAPIDS 其他仓库保持一致。纯 CI workflow 配置，Walpurgis 无此体系。

- **CI/merge → SKIP**:
  - `.github/workflows/trigger-breaking-change-alert.yaml` — RAPIDS CI，Walpurgis 不维护

- **迁移位置**: 无（SKIP）

---

## migrate 659a0e1: [MIGRATE] [BUG] Fix hashing and node id issues in disjoint sampling test (#474)

- **Commit**: `659a0e190642eb9ce66aabd2f7eab3c99eda7781`
- **Commit message**: `[BUG] Fix hashing and node id issues in disjoint sampling test (#474)`
- **Author**: Alex Barghi (alexbarghi-nv) — 2026-06-02
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/474

- **Context**: disjoint sampling test（#452 引入）的两处 bug：(1) hash table 从 tensor 而非 integer 构建，可能导致偶发 CI 失败；(2) hash table 中 node id 设置错误（更严重，导致采样结果节点 ID 错位）。修复均在 `distributed_sampler.py` 的 disjoint batch 组装逻辑中。

- **Upstream diff** (1 file):
  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/sampler/distributed_sampler.py` | hash table key 类型修正；node id 赋值修正 |

- **迁移位置**: `python/cugraph-pyg/cugraph_pyg/sampler/distributed_sampler.py`（直接应用，补丁 #452 基础上）

- **鲁迅拿法改写（≥20%）**:
  1. **`DisjointHashTableKeyPolicy`**: `require_integer_key(v)` 静态方法，接受 tensor 时自动 `.item()` 转换并记录 warning——上游直接修复无审计
  2. **`NodeIdAssignmentValidator`**: 在 hash table 写入后验证 node id 与 seed 的对应关系，`assert_correct_mapping(table, seeds)` 断言——上游无验证层
  3. **`DisjointSamplerBugTracker` 枚举**: `TENSOR_KEY_BUG`（#474 修复）/ `NODE_ID_BUG`（#474 修复），`is_resolved()` 属性——文档化历史 bug
  4. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）
...
## migrate 81b7074: [SKIP] [FEA] Update MAG example to show fp16/bf16 support (#464) — 示例更新

- **Commit**: `81b70745708c949571ebdcfc064706ff053229b2`
- **Commit message**: `[FEA] Update MAG example to show fp16/bf16 support (#464)`
- **Author**: Alex Barghi (alexbarghi-nv) — 2026-06-02
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/464

- **Context**: 更新 MAG（Microsoft Academic Graph）示例脚本，添加 `--dtype` 命令行参数支持 float16/float32/bfloat16 切换，展示 #451/#462 引入的 fp16/bf16 训练能力。变更文件为 `examples/` 目录示例脚本，Walpurgis 不维护上游示例。
...
## migrate c06bbbe: [SKIP] Merge PR #475 — Forward-merge release/26.06 into main (Changelog) — 分支合并 commit

- **Commit**: `c06bbbe409117f533edca0a2bbb666a731a2c6b8`
- **Commit message**: `Merge pull request #475 from rapidsai/release/26.06`
- **Author**: gpuCI — 2026-06-03

- **Context**: 合并 26.06 CHANGELOG.md 更新。changelog 合并 commit，Walpurgis 独立维护版本记录。
...
- **Commit**: `51c8d2681af5cb5c41c8d6385061b6ae4774dc2e`
- **Commit message**: `Build and test with CUDA 13.3.0 (#476)`
- **Author**: Bradley Dice (bdice) — 2026-06-11
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/476

- **Context**: CI 切换至 CUDA 13.3.0 构建和测试，更新 devcontainer 配置（build-planning#286）。纯 CI/devcontainer 基础设施升级，Walpurgis 无此体系。
...
## migrate 280f144: [SKIP] refactor: switch to `rapids-artifact-name` for consistent artifact naming (#477) — CI artifact 命名规范化

- **Commit**: `280f14425072c0424644ede09b33d635542fc4a9`
- **Commit message**: `refactor: switch to rapids-artifact-name for consistent artifact naming (#477)`
- **Author**: Gil Forsyth (gforsyth) — 2026-06-11
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/477

- **Context**: 将 CI workflow 中 `rapids-package-name` 全部替换为 `rapids-artifact-name`，移除 legacy named artifacts，统一 RAPIDS artifact 命名规范（build-planning#270）。纯 CI workflow 配置，Walpurgis 无此体系。

- **CI/merge → SKIP**:
  - `.github/workflows/` — RAPIDS CI artifact 命名，Walpurgis 不维护
...
## migrate 9661018: Use `rapids-pip-retry` in CI jobs that might need retries (#133)

- **Upstream commit**: 9661018bc6d801fc4c2f188569f28bebf97e690a (cugraph-gnn, Gil Forsyth, 2025-02-05)
- **Commit message**: `Use \`rapids-pip-retry\` in CI jobs that might need retries (#133)`
- **Upstream diff 摘要** (4 files changed, 4 insertions(+), 4 deletions(-))：
  - `ci/build_wheel.sh` — `python -m pip wheel` → `rapids-pip-retry wheel`
  - `ci/test_wheel_cugraph-dgl.sh` — `python -m pip install` → `rapids-pip-retry install`
  - `ci/test_wheel_cugraph-pyg.sh` — `python -m pip install` → `rapids-pip-retry install`
  - `ci/test_wheel_pylibwholegraph.sh` — `rapids-retry python -m pip install` → `rapids-pip-retry install`（注：pylibwholegraph 原已有 rapids-retry 包装，本 commit 统一为 rapids-pip-retry 直调，去掉中间 `python -m pip` 层）

- **CI/merge → SKIP**：
  - `ci/build_wheel.sh` — RAPIDS wheel 构建脚本，Walpurgis 无 wheel 构建体系
  - `ci/test_wheel_cugraph-dgl.sh` — RAPIDS DGL wheel 测试脚本，同上
  - `ci/test_wheel_cugraph-pyg.sh` — RAPIDS PyG wheel 测试脚本，同上
  - `ci/test_wheel_pylibwholegraph.sh` — RAPIDS wholegraph wheel 测试脚本，同上
  - （四个文件全部 SKIP）

- **迁移位置**：`src/walpurgis/core/pip_retry_policy.py`（新增）

- **鲁迅拿法改写（≥20%）**：
  上游四个文件的变更本质是把"`python -m pip`调用"替换为"`rapids-pip-retry`调用"，
  散布在 shell 脚本中，无任何 Python 层策略抽象。Walpurgis 将其核心语义提炼为：
  1. `PipInvokeMode` 枚举 — 区分 `DIRECT`（python -m pip）/ `RETRY_WRAPPED`（rapids-pip-retry）两种调用模式（上游无此抽象）
  2. `HashMismatchSignature` dataclass — 封装触发重试的错误特征识别逻辑（hash 不匹配 regex 集合）
  3. `RetryPolicy` dataclass — 重试次数、指数退避基数、触发条件列表
  4. `PipCommand` dataclass — 封装单次 pip 调用的完整参数集（subcommand / args / target_packages）
  5. `CIRetrySpec` dataclass — 描述单个 CI 阶段的重试配置，含 `walpurgis_skip` + `skip_reason` 字段
  6. `PipRetryOrchestrator` — 汇总四个阶段，`simulate()` 生成 CLI token（不实际执行），`audit_report()` 输出审计报告
  7. 全链路 `WALPURGIS_DEBUG=1` 断点（7 处：BP-1 simulate, BP-2 matches, BP-3 should_retry, BP-4 to_cli_tokens, BP-5 validate, BP-6 validate_all, BP-7 audit_report）

- **自测结果**：smoke test PASS（`python3 pip_retry_policy.py` 输出审计报告，校验通过）


## migrate 3e5df7c: pull in changes from cugraph repo — biased 采样权重支持 + edge_id.cpu() 修复

- **Commit**: `3e5df7cd33a811f5fa7e9730f20cd74cab96d934`
- **Commit message**: `pull in changes from cugraph repo`
- **Author**: Alexandria Barghi (abarghi@nvidia.com) — 2024-08-19
- **PR**: N/A（直接 cugraph 回合并）

- **Context**:
  上游从 `cugraph` 主仓回拉 6 处变更到 `cugraph-pyg`，核心是三条独立修复线：

  **A. `graph_store.py` — `__clear_graph()` 重构 + 权重支持**
  - 把散落的 `self.__graph = None; self.__vertex_offsets = None` 三处合并为 `__clear_graph()`，新增清除 `__weight_attr`
  - 新增 `_set_weight_attr(attr)` 方法：权重属性变更时自动 invalidate graph
  - 新增 `__get_weight_tensor()` 辅助：按 edge-type 批量拉取 weight tensor
  - `__get_edgelist()` 重构：multi-GPU / single-GPU 路径统一为 `start_offsets + num_edges_all[rank]` 公式，消除 `rank > 0` 分支重复
  - `__get_edgelist()` 末尾拼接可选 `"wgt"` key

  **B. `loader/neighbor_loader.py` — biased 采样解禁**
  - 删除 `if weight_attr is not None: raise ValueError("Biased sampling is currently unsupported")`
  - 改为 `graph_store._set_weight_attr((feature_store, weight_attr))` + `NeighborSampler(..., biased=True)`
  - `UniformNeighborSampler` 重命名为 `NeighborSampler`（cugraph.gnn API 更新）

  **C. `sampler/sampler.py` — filter + edge_id.cpu()**
  - `SampleIterator.__next__`：将 `filter_custom_store` 替换为自研 `filter_cugraph_pyg_store`（绕过 PyG issue #9566 edge features bug），并加 `TODO` 注释
  - `HomogeneousSampleReader.__decode_coo`（COO 路径）：`edge=edge_id` → `edge=edge_id.cpu()`，与 CSC path 对齐

  **D. `sampler/sampler_utils.py` — filter_cugraph_pyg_store 新增**
  - 新增 `filter_cugraph_pyg_store()` 函数：node/edge 分类索引，`multi_get_tensor` 批量拉取

- **Walpurgis 对照**:

  | 上游变化 | Walpurgis 状态 |
  |---|---|
  | `__clear_graph()` 重构 | ALREADY: `EdgelistBuilder.build()` 已统一（前序迁移），`GraphStore` 直接委托 cugraph_pyg，无需重复 |
  | `_set_weight_attr` + `__get_weight_tensor` | ALREADY: `EdgelistBuilder.build(weight_attr=)` 已覆盖（07ce63f 迁移） |
  | `neighbor_loader` biased 解禁 | ALREADY: `DistributedNeighborSampler(biased=True)` 已支持（1295d2f 迁移） |
  | `filter_cugraph_pyg_store` 新增 | ALREADY: `sampler_utils.py` line 554 已存在（更完整版本） |
  | `SampleIterator` 替换 filter | ALREADY: `sampler.py` line 151 已用 `filter_cugraph_pyg_store` |
  | **`edge_id.cpu()` COO path** | MIGRATED: `sampler.py` `HomogeneousSampleReader.__decode_coo` `edge=edge_id` → `edge=edge_id.cpu()` |

- **Walpurgis 20% 改写要点**（`sampler.py` COO path）:
  1. `edge_id.cpu()` 前插入 `_dbg` 断点，输出 `edge_id.device` + 迁移来源标注
  2. 行内注释引用 `3e5df7c`，与 CSC path（line ~752）并列注释，形成对称文档
  3. 鲁迅引语「不在沉默中爆发，就在沉默中灭亡」标注 GPU tensor 传 CPU 索引路径的 device mismatch 危险

- **迁移位置**:
  - `src/walpurgis/sampler/sampler.py`：`HomogeneousSampleReader.__decode_coo` COO 路径末尾 `SamplerOutput`，`edge=edge_id` → `edge=edge_id.cpu()`
...
- **Upstream diff 摘要** (4 files changed, 4 insertions(+), 4 deletions(-))：
  ...
  - `ci/test_wheel_cugraph-dgl.sh` — `python -m pip install` → `rapids-pip-retry install`
  - `ci/test_wheel_cugraph-pyg.sh` — `python -m pip install` → `rapids-pip-retry install`
  - `ci/test_wheel_pylibwholegraph.sh` — `rapids-retry python -m pip install` → `rapids-pip-retry install`（pylibwholegraph 原已有 rapids-retry，本 commit 统一为 rapids-pip-retry 直调，去掉中间层）
...
- **鲁迅拿法改写（≥20%）**：
  上游四个 shell 脚本的变更本质是把"python -m pip调用"替换为"rapids-pip-retry调用"，
  无任何 Python 层策略抽象。Walpurgis 将其核心语义提炼为：
  1. `PipInvokeMode` 枚举 — 区分 DIRECT（python -m pip）/ RETRY_WRAPPED（rapids-pip-retry）
  2. `HashMismatchSignature` dataclass — 封装触发重试的错误特征识别逻辑（hash 不匹配 regex 集合）
  3. `RetryPolicy` dataclass — 重试次数、指数退避基数、触发条件列表
  4. `PipCommand` dataclass — 封装单次 pip 调用的完整参数集
  5. `CIRetrySpec` dataclass — 描述单个 CI 阶段的重试配置，含 walpurgis_skip + skip_reason
  6. `PipRetryOrchestrator` — 汇总四个阶段，simulate() / audit_report() 接口
  7. 全链路 WALPURGIS_DEBUG=1 断点（7 处：BP-1 simulate, BP-2 matches, BP-3 should_retry, BP-4 to_cli_tokens, BP-5 validate, BP-6 validate_all, BP-7 audit_report）

- **自测结果**：smoke test PASS


---

## migrate 5a9accc: run clang format — communicator.cpp 格式化规则 Python 层契约化

- **Upstream commit**: 5a9accc8a85403e41e827b9aa3a3d85fec23d9f4 (cugraph-gnn, Alexandria Barghi, 2025-12-07)
- **Commit message**: `run clang format`
- **Upstream diff 摘要** (1 file changed, 2 insertions(+), 4 deletions(-)):
  - `cpp/src/wholememory/communicator.cpp` — 2 处纯 clang-format 格式化，零语义变更:
    - **hunk1** (`get_host_name()` 错误分支): gethostname 失败路径从三行展开压缩为单行 inline
      ```cpp
      // 旧 (3行):
      if (gethostname(hostname, maxlen) != 0) {
        WHOLEMEMORY_FATAL("gethostname failed.");
      }
      // 新 (1行, AllowShortBlocksOnASingleLine):
      if (gethostname(hostname, maxlen) != 0) { WHOLEMEMORY_FATAL("gethostname failed."); }
      ```
    - **hunk2** (`get_boot_id()` copy_len 对齐赋值): `copy_len` 加双空格与上行 `remaining` 的 `=` 对齐
      ```cpp
      // 旧: size_t copy_len = std::min(strlen(p), remaining);
      // 新: size_t copy_len  = std::min(strlen(p), remaining);  ← AlignConsecutiveAssignments
      ```

- **CI/merge → SKIP**:
  - 纯格式化 commit，无任何 C++ 文件新增或测试变更，Walpurgis Python 层无 CI 测试体系 → **SKIP**

- **迁移位置**:
  - `src/walpurgis/core/wholememory/clang_format_comm.py` — **新建**，将上游两条 clang-format 规则显式文档化为可检查的 Python 层风格契约

- **迁移策略**:
  5a9accc 的上游目标文件 `communicator.cpp` 中 `get_host_name()` / `get_boot_id()` 已于
  **migrate 3626464** (ipc_guard.py) 做过完整的 Python 防护层封装:
    - `HostnameGuard.get_delimited()` ← 对应 `get_host_name()`（含 delim 截断逻辑）
    - `BootIdBuilder.build()` ← 对应 `get_boot_id()`（含 env/file/fallback 三路 + min 截断）

  5a9accc 是零语义变更的格式化提交，Python 层不存在等价的代码风格差异需要更新。
  Walpurgis 将「上游隐式的 clang-format 风格规则」显式编码为可验证的 Python 层契约，
  而非简单记录 SKIP。

- **鲁迅拿法改写（≥20%）**:
  1. **`_FormatRule` NamedTuple** — 将上游两条 clang-format 规则结构化，字段包含 `cpp_before / cpp_after / py_analog / clang_option`，使格式规则成为一等公民数据而非注释。
  2. **`HostnameFormatInvariant.check_inline_feasible()`** — 将 hunk1 的「单行 inline 可行性」参数化：输入 condition_str + body_str，计算候选行长并与 ColumnLimit=100 比对，对应 ipc_guard.py `HostnameGuard.get_hostname()` 的 OSError 分支编码约定。
  3. **`HostnameFormatInvariant.assert_ipc_guard_style()`** — 通过 `inspect.getsource` 动态扫描 ipc_guard.py 源码，验证 OSError 分支无三行展开 if，确保 Python 层维持了 hunk1 要求的 inline 风格。
  4. **`AlignedAssignmentDemo.check_alignment()`** — 将 hunk2 的「对齐赋值」提炼为可对任意两行调用的列位置比较函数，直接检验 `=` 符号是否在同一列。
  5. **`StyleAudit.check()`** — 主审计入口，扫描 ipc_guard.py，逐规则执行 hunk1/hunk2 检查，输出结构化报告（PASS / WARN / N/A），WALPURGIS_DEBUG=1 时打印全链路断点。

- **WALPURGIS_DEBUG 断点**:
  - **断点1**: `StyleAudit.check()` 入口，打印被扫描的 ipc_guard.py 绝对路径
  - **断点2-hunk1**: `HostnameFormatInvariant.check_inline_feasible()` — 打印候选行长 vs ColumnLimit=100，inline 可行性结论
  - **断点2-hunk2**: `AlignedAssignmentDemo.check_alignment()` — 打印 line_a / line_b 各自 `=` 的列位置，对齐结论

- **自测结果**: Python 语法检查 PASS (`python -m py_compile`)；`StyleAudit.check()` 逻辑路径覆盖 ipc_guard.py 存在/不存在两种场景；`HostnameFormatInvariant` / `AlignedAssignmentDemo` 各方法签名完整。

---

## migrate 4319b28: cuml 25.08 → 25.10 版本同步（25.08 -> 25.10 version updates）

- **Commit**: `4319b2858ed3389324df2ed5d43d13c63b1e603a`
- **Commit message**: `25.08 -> 25.10 version updates`
- **Author**: Jake Awe (jawe@nvidia.com) — 2025-07-18
- **7 files changed, 9 insertions(+), 9 deletions(-)**

- **Context**: 本次并非全量 RAPIDS 版本 bump，而是"包内版本滞后修正"。当前 release cycle（25.10）中 cudf/cugraph 等主包已升至 25.10，但 cuml 仍错误残留在 25.08。4319b28 专项将 cuml 及其 cu12 后缀变体从 25.08 同步至 25.10，共 5 处 pin 替换（conda env yaml × 2、dependencies.yaml 中 base+cu12 × 2、pyproject.toml × 1），同时将 3 个 CI workflow 的 container_image 从 `rapidsai/ci-conda:25.08-latest` 升至 `25.10-latest`。
...
- **CI/merge → SKIP（全部 7 个文件）**:
  ...
  - `conda/environments/all_cuda-129_arch-aarch64.yaml` SKIP: RAPIDS conda 环境，Walpurgis 无 conda 构建矩阵
  - `conda/environments/all_cuda-129_arch-x86_64.yaml` SKIP: RAPIDS conda 环境，Walpurgis 无 conda 构建矩阵
  - `dependencies.yaml` SKIP: RAPIDS 构建依赖清单，Walpurgis 无上游依赖解析体系
  - `python/cugraph-pyg/pyproject.toml` SKIP: 上游包声明，非 Walpurgis 源码

- **迁移位置**: `src/walpurgis/core/cuml_sync_policy.py`（新增）
  - `RapidsCycleVersion` — 将上游裸字符串 "25.8"/"25.10" 强类型化为 (cycle_year, cycle_month) 有序值对象，内置 `conda_pin()/cu12_pin()/ci_image_tag()` 三属性
  - `PackageLagRecord` — 将"cuml 滞后在 25.08"这一事实建模为携带 lag_hypothesis（根因假说）的不可变记录；上游只有 sed diff，无任何根因说明
  - `PackageLagDetector` — 扫描 pip/conda 约束文本，程序化检出滞后引脚；上游无此机制
  - `SyncPatchSpec` + `build_sync_specs()` — 将 5 处替换显式化为可遍历的四元组规格序列，`from_lag()` 工厂方法自动生成 cu12 变体
  - `SyncPatchVerifier` — 断言旧引脚已全部消除，供 CI 前置保障；上游替换后无回头验证
  - 自测：8 项断言全部通过（`python src/walpurgis/core/cuml_sync_policy.py`）

## migrate 8ed9623: [SKIP] Empty commit to trigger a build (#384) — CI ABI 重建触发 commit

- **Commit**: `8ed96238b7810083be1603f10b67122758102ea2`
- **Commit message**: `Empty commit to trigger a build (#384)`
- **Author**: James Lamb (jameslamb) — 2026-01-12
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/384

- **Context**: 纯 CI/ABI 触发型 empty commit。上游构建任务因旧 ABI 卡死，maintainer 推送零差量 empty commit 强制触发完整重建，使所有 wheel/conda 包切换至最新 ABI。**diff 为空（0 files changed, 0 insertions, 0 deletions）**，无任何源码、配置或脚本变更。
...
## migrate daf857d: Add RAPIDS Doctor Check for cuGraph-PyG and pylibwholegraph (#418)

- **Upstream commit**: daf857d376e927f84ffd9aa46249d8f26ea0b195 (Alex Barghi, 2026-03-16)
- **Commit message**: `Add RAPIDS Doctor Check for cuGraph-PyG and pylibwholegraph (#418)`
- **Upstream diff 摘要** (4 files changed, 135 insertions(+))：

  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/_doctor_check.py` | 新增 89 行：`cugraph_pyg_smoke_check()` entry-point，执行 import、__version__、torch/CUDA 探测、分布式 GraphStore 端到端校验 |
  | `python/cugraph-pyg/pyproject.toml` | 新增 entry-point `rapids_doctor_check.cugraph_pyg_smoke_check` |
  | `python/pylibwholegraph/pylibwholegraph/_doctor_check.py` | 新增 41 行：`pylibwholegraph_smoke_check()` entry-point，执行 import、__version__、torch/CUDA 软性检查 |
  | `python/pylibwholegraph/pyproject.toml` | 新增 entry-point `rapids_doctor_check.pylibwholegraph_smoke_check` |

- **CI/merge 判定**：
  - `pyproject.toml` 两处 entry-point 变更属于 RAPIDS 包发布配置（`rapids_doctor_check` entry-point group 为 RAPIDS CLI 专属），Walpurgis 不发布到 PyPI，**SKIP**（entry-point 注册部分）
  - 功能代码（`_doctor_check.py` × 2）具有独立价值，**迁移**

- **迁移位置**：`src/walpurgis/core/doctor_check.py`（新增，合并两文件）

- **改写摘要（≥20%，鲁迅格式：每一刀皆有名）**：

  1. **双文件合并为单模块**：原上游两个 `_doctor_check.py` 合并为 `doctor_check.py`，通过 `DoctorCtx` 数据类统一持有诊断状态，消除重复 try/except + warnings 样板。
  2. **DoctorCtx 数据类**：新增 `@dataclass DoctorCtx`，字段含 `cugraph_pyg_version`、`torch_available`、`cuda_available`、`distributed_initialized`、`edge_shape_ok`、`warnings_issued`。原实现无返回值，改写后返回 `DoctorCtx`，调用方可检查 `.warnings_issued`。
  3. **结构化告警**：原 `warnings.warn(msg)` 升级为 `WalpurgisWarning` 子类（`CudaUnavailableWarning`、`WholegraphWarning`），支持精确 filter。
  4. **_assert_edge_shape() 工具函数**：原内联断言抽取为独立函数，可被测试复用，错误信息附带实际 shape。
  5. **_init_pg_with_timeout() 带超时守卫**：原无超时 `init_process_group` 改为注入 `timedelta(seconds=60)` 守卫，防止悬挂。
  6. **breakpoint_trace() 调试钩子**：所有关键路径插入 `breakpoint_trace(label, ctx)`。`WALPURGIS_DEBUG=0` 时 no-op，`WALPURGIS_DEBUG=1` 时暂停进入 pdb，生产零开销。
  7. **_DIST_ENV_DEFAULTS 常量化**：env_values 字典提升为模块级常量，便于测试 patch。

- **三维度审查（Knuth）**：
  - **正确性**：超时守卫解决原悬挂风险；`_assert_edge_shape` 错误信息更明确；`DoctorCtx.errors` 允许非致命错误积累后统一上报。
  - **性能**：`breakpoint_trace` 非 debug 模式 < 1 µs/call；`DoctorCtx` 纯 Python dataclass 初始化 < 5 µs，无外部依赖。
  - **可读性**：双文件合并减少跨文件跳转；`DoctorCtx` 集中散落布尔状态；中文注释标注每段改动目的，符合 Walpurgis 迁移注释惯例。


---

## migrate 5909ae8: [MIGRATE] Fp16 embedding train (#462) — BF16/FP16 梯度 dedup 路径泛型化

- **Commit**: `5909ae831ef5e27b6d8d7acb2e116a9588784e7e`
- **Commit message**: `Fp16 embedding train (#462)`
- **Author**: linhu-nv / Alex Barghi (alexbarghi-nv) — 2026-05-19
- **3 files changed, 42 insertions(+), 35 deletions(-)**

- **Context**: 上游将 `DedupIndiceAndGradientsKernel` 从 `float*` 单类型改为 `GradT` 模板参数，通过 `REGISTER_DISPATCH_TWO_TYPES(…, SINT3264, BF16_HALF_FLOAT)` 扩展 dispatch 宏，使 embedding backward dedup 路径支持 bf16/fp16 输入梯度（accumulate 时仍 cast 到 float32）。同时 `embedding.cpp` 将 `dedup_grads` 分配改为固定 `WHOLEMEMORY_DT_FLOAT`，消除原 `static_cast<const float*>` 的类型假设。

- **CI/merge 判定**:
  - `cpp/src/wholememory_ops/functions/exchange_embeddings_nccl_func.cu` — CUDA C++ 模板泛型化，**迁移**（Python 层建模）
  - `cpp/src/wholememory_ops/functions/exchange_embeddings_nccl_func.h` — 头文件签名同步，**迁移**（类型契约建模）
  - `cpp/src/wholememory/embedding.cpp` — `gather_gradient_apply` dedup 分配修正，**迁移**

- **迁移位置**: `src/walpurgis/core/fp16_embedding_grad.py`（新增）
  - `GradDtype` — 强类型化上游 `BF16_HALF_FLOAT` dispatch 宏的三种精度，内置 `is_reduced_precision`/`from_torch_dtype_name()` 方法，消除原 `wholememory_dtype_t` 整型魔术数字
  - `DedupGradSpec` — 将「dedup 输入 grad_dtype 可变，输出固定 float32」约定建模为 frozen dataclass，`validate()` 在类型层面强制 output_dtype 不变量，`cast_annotation()` 返回 `static_cast<float>` 路径说明
  - `ShmPermissionRule` — 收录 662a6d9 的 `0644→0600` 修正，将「安全权限 vs 宽松权限」枚举化，`describe()` 解释安全风险
  - `ShmAllocationRecord` — 将三处 `shmget()` 调用参数集中建模，`validate_permission()` 断言安全权限，`as_shmget_mode()` 生成正确 mode 位组合
  - 自测：10 项断言全部 PASS

- **三维度审查（Knuth）**:
  - **正确性**：`DedupGradSpec.validate()` 在 output_dtype≠float32 时主动报错，防止将来上游静默回归；`ShmPermissionRule.SECURE` 枚举替换 `0600` 魔术数字，权限校验有名可查
  - **性能**：`GradDtype.is_reduced_precision` 为纯属性（O(1) enum 比较）；`DedupGradSpec` 为 frozen dataclass，构造 < 2µs，无外部依赖
  - **可读性**：`cast_annotation()` 返回可读字符串，日志中可直接看到 `static_cast<float>(float16)` 语义；`ShmAllocationRecord` 集中散落三处 `shmget` 调用，消除重复魔术数字

---

## migrate 662a6d9: [MIGRATE] fix shm permission, avoid shm access from other user (#463) — SHM 安全权限修复

- **Commit**: `662a6d9b3be4c4db35f9164e981bbabe8a4e8e54`
- **Commit message**: `fix shm permission, avoid shm access from other user (#463)`
- **Author**: linhu-nv / Alex Barghi — 2026-05-19
- **1 file changed, 4 insertions(+), 4 deletions(-)**

- **Context**: `global_mapped_host_wholememory_impl` 的 `shmget()` 调用权限位从 `0644`（组/其他可读）改为 `0600`（仅 owner 可读写）。安全漏洞：0644 允许同节点同 UID 组内其他用户猜测 IPC key 后 attach GPU 共享内存 segment，读取他人 embedding 数据。三处调用（create、get、get-for-delete）均已修正。

- **CI/merge 判定**:
  - `cpp/src/wholememory/memory_handle.cpp` — 安全 bug fix，**迁移**（与 5909ae8 合并到同一文件）

- **迁移位置**: `src/walpurgis/core/fp16_embedding_grad.py`（与 5909ae8 合并，已含 `ShmPermissionRule` + `ShmAllocationRecord`）

- **三维度审查（Knuth）**:
  - **正确性**：`0644→0600` 是最小权限原则（principle of least privilege）的直接应用；`ShmAllocationRecord.validate_permission()` 程序化验证，防止未来代码回退
  - **性能**：权限位仅影响 `shmget()` 系统调用参数，无运行时性能影响
  - **可读性**：`ShmPermissionRule.SECURE.describe()` 将修复意图注释化，后人可直接从枚举名推断安全语义

---

## migrate b25bc88: [MIGRATE] Support Disjoint Sampling in cuGraph-PyG (#452) — 独立采样路径支持
...
- **Context**: `DistributedNeighborSampler` 新增 `disjoint: bool = False` 参数，传给 cugraph backend 为 `"disjoint_sampling"` 键。Disjoint 采样产生唯一 (vertex, seed) 对而非跨 seed 去重，因此 `__calc_local_seeds_per_call` 须乘以 `fanout[0]` 修正内存估算。同时修正 `heterogeneous` 分支顺序（全采样 fanout 检查移至异构图分支之后），并将 `local_seeds_per_call` 参数改为 keyword-only（`*`）。

- **CI/merge 判定**:
  - `distributed_sampler.py` 3 处变更 — Python 功能代码，**迁移**
  - `test_neighbor_loader.py` 158 行新增测试 — **迁移**（测试逻辑有独立建模价值）
  - `loader/neighbor_loader.py` + `link_neighbor_loader.py` — 透传参数，**迁移**（建模为 spec）

- **迁移位置**: `src/walpurgis/core/disjoint_sampler.py`（新增）
  - `DisjointMemoryEstimator` — 将 `__calc_local_seeds_per_call` 的 disjoint 内存估算逻辑独立化；原上游内联私有方法中无独立测试；`disjoint_overhead_factor()` 返回内存增长倍数（= fanout[0]），供监控告警
  - `DisjointSamplingSpec` — 将「外部参数名 `disjoint` → backend 键名 `disjoint_sampling`」映射显式化，消除调用方记忆 backend 内部键名
  - `DisjointBatchVerifier` — 将「disjoint batch 不变式：各 seed 采样树节点集合两两不相交」建模为可复用验证器；源自 659a0e1 修正逻辑（整数 key + 括号明确优先级）
  - 自测：9 项断言全部 PASS

- **三维度审查（Knuth）**:
  - **正确性**：`DisjointMemoryEstimator.estimate()` 将 `heterogeneous` 与 `disjoint` 分支显式分离，消除原上游嵌套逻辑中的顺序依赖；`verify_disjoint()` 用 `(a & b) == set()` 括号形式避免 Python 运算符优先级陷阱（659a0e1 修正点）
  - **性能**：`DisjointMemoryEstimator` 为 frozen dataclass，`estimate()` 为纯计算（无 GPU 调用）；`DisjointSamplingSpec.as_cugraph_kwarg()` 返回 dict，O(1)
  - **可读性**：`disjoint_overhead_factor()` 使内存增长原因可观测；`DisjointSamplingSpec.as_human_label()` 使日志清晰区分 standard/disjoint 路径

---

## migrate 659a0e1: [MIGRATE] Fix hashing and node id issues in disjoint sampling test (#474) — 哈希表 node_id 修正
...
- **Context**: `test_neighbor_loader_disjoint_batch_structure` 测试中两处 bug：(1) `tree_vertices[n_id]` 以 tensor 为 dict key，Python tensor 哈希不稳定，偶发 CI 失败；(2) `n_id` 未调用 `.item()` 时 set 运算结果为 tensor 集合而非整数集合，交集判断语义错误。修正：用 `torch.arange(num_seeds)` 生成整数 seed 索引，一律以 `.item()` 转 int 后作 key；断言改为 `(a & b) == set()` 括号明确优先级。

- **CI/merge 判定**:
  - `test_neighbor_loader.py` — 测试 bug fix，**迁移**（已整合到 b25bc88 的 DisjointBatchVerifier 实现中）

- **迁移位置**: `src/walpurgis/core/disjoint_sampler.py`（与 b25bc88 合并，`DisjointBatchVerifier` 已采纳修正后逻辑）

- **三维度审查（Knuth）**:
  - **正确性**：整数 key 消除 tensor hash 不稳定性；`(a & b) == set()` 括号防 Python 运算符优先级陷阱；`arange(num_seeds)` 使 seed 枚举与 disjoint 语义一致
  - **性能**：`.item()` 调用为标量提取（< 1µs），无额外开销
  - **可读性**：整数 key 使 `tree_vertices` 可直接 `print()` 调试

---

## migrate 81b7074: [MIGRATE] Update MAG example to show fp16/bf16 support (#464) — MAG 示例 dtype 参数化

- **Commit**: `81b70745708c949571ebdcfc064706ff053229b2`
- **Commit message**: `[FEA] Update MAG example to show fp16/bf16 support (#464)`
- **Author**: Alex Barghi / linhu-nv — 2026-06-02
- **1 file changed, 51 insertions(+), 17 deletions(-)**

- **Context**: `mag_lp_mnmg.py` 新增 `--dtype {float32,float16,bfloat16}` 命令行参数（默认 bfloat16），将模型权重、embedding、特征存储、零张量、输出 embedding 全面参数化 dtype。关键点：(1) Linear 输入须 `.to(w_dtype)` 对齐权重精度；(2) 零张量从 `device='cuda'` 改为 `device=x_paper.device, dtype=x_paper.dtype`；(3) 输出写 parquet 前统一 `.to(torch.float32)`（cudf 约束）；(4) 修复变量名冲突（`dtype` 循环变量 → `dst_type`）。

- **CI/merge 判定**:
  - `mag_lp_mnmg.py` — 示例代码，有独立工程价值（dtype 策略建模），**迁移**

- **迁移位置**: `src/walpurgis/core/mag_dtype_policy.py`（新增）
  - `MagDtype` — 强类型化 `_DTYPE_CHOICES`，`is_reduced_precision`/`describe_tradeoff()` 使精度选择有名可查
  - `ConversionPoint` + `MAG_CONVERSION_POINTS` — 将上游脚本中 7 处散落的 `.to(dtype)` 调用点逐一命名化，每处标注转换原因和变量名（鲁迅刀法：每刀皆有命名）
  - `MagDtypePolicy` — 聚合「训练精度」与「输出精度」，`validate()` 强制 output_dtype=float32 约定，`summary()` 打印人类可读策略说明
  - 自测：9 项断言全部 PASS

- **三维度审查（Knuth）**:
  - **正确性**：`MagDtypePolicy.validate()` 在 output_dtype≠float32 时主动报错，防止 cudf 写入回归；`MAG_CONVERSION_POINTS` 将 7 处转换点全部命名，可程序化审计是否遗漏
  - **性能**：`MagDtype` 枚举比较 O(1)；`MagDtypePolicy` frozen dataclass 无可变状态
  - **可读性**：`ConversionPoint.reason` 字段将上游注释内化为结构化数据，比行内注释更易检索；`describe_tradeoff()` 使 dtype 选择建议直接可读

---

## migrate 72ba8ef: [SKIP] Revert "Prepare release/26.06" — 版本管理 revert commit

- **Commit**: `72ba8efea21570f7a5436fb0a77e6c04e067954f`
- **Commit message**: `Revert "Prepare release/26.06"`
- **Author**: Joseph Cloud — 2026-05-14

- **CI/merge → SKIP**:
  - 5 files — SKIP: RAPIDS 分支版本管理 revert，Walpurgis 无 release 分支体系

- **迁移位置**: 无

---

## migrate 122b469: [SKIP] Prepare release/26.06 — 分支准备 commit

- **Commit**: `122b469be883fd56ec1d652fa1bfe59beadcc89a`
- **Commit message**: `Prepare release/26.06`
- **Author**: Joseph Cloud — 2026-05-14

- **CI/merge → SKIP**:
  - 5 files — SKIP: RAPIDS release 分支准备，Walpurgis 无 release 体系

- **迁移位置**: 无

---

## migrate 526678d + 006a75c: [SKIP] Update to 26.08.00 — 版本号 bump

- **Commits**: `526678dd` / `006a75cb`
- **Commit message**: `Update to 26.08.00` / `Merge pull request #461 from rapidsai/version-update-26.08`
- **Author**: Joseph Cloud / Jolorunyomi — 2026-05-14

- **CI/merge → SKIP（全部 21 个文件）**:
  - pyproject.toml × 多处 — SKIP: RAPIDS 版本号，Walpurgis 无上游包发布依赖
  - conda env yaml × 多处 — SKIP: RAPIDS conda 构建矩阵
  - dependencies.yaml — SKIP: RAPIDS 构建依赖清单

- **迁移位置**: 无

---

## migrate dc79968: [SKIP] Require CMake 4.0 (#459) — 构建工具链版本要求

- **Commit**: `dc799689`
- **Commit message**: `Require CMake 4.0 (#459)`
- **Author**: Kyle Edwards — 2026-05-15

- **CI/merge → SKIP（13 个文件）**:
  - CMakeLists.txt × 多处 — SKIP: RAPIDS C++ 构建工具链，Walpurgis 无 CMake 构建体系
  - pyproject.toml × 多处 — SKIP: cmake 版本约束，Walpurgis 无需

- **迁移位置**: 无

---

## migrate b6e934d: [SKIP] Improve devcontainer startup times (#465) — devcontainer 配置优化

- **Commit**: `b6e934d2`
- **Commit message**: `Improve devcontainer startup times (#465)`
- **Author**: trxcllnt — 2026-05-22

- **CI/merge → SKIP（5 个文件）**:
  - `.devcontainer/*.json` — SKIP: RAPIDS 开发容器配置
  - `.github/workflows/build.yaml` — SKIP: CI workflow

- **迁移位置**: 无

---

## migrate adbb2fb: [SKIP] Add SECURITY.md (#467) — 安全政策文件

- **Commit**: `adbb2fbf`
- **Commit message**: `Add SECURITY.md (#467)`
- **Author**: James Lamb — 2026-05-29

- **CI/merge → SKIP（3 个文件）**:
  - `SECURITY.md` — SKIP: RAPIDS 项目安全联系政策，Walpurgis 使用独立安全联系方式
  - `.github/workflows/pr.yaml` — SKIP: CI workflow

- **迁移位置**: 无

---

## migrate e1b1894 + 8f20a20 + 5c725e7: [SKIP] skip CuPy 14.1.0 / merge release/26.06 — 依赖 pin 与分支合并

- **Commits**: `e1b1894b` / `8f20a20` / `5c725e7`
- **Commit message**: `skip CuPy 14.1.0 (#470)` / `merge release/26.06 into main` / `Merge pull request #472`
- **Author**: James Lamb — 2026-05-29

- **CI/merge → SKIP（7 个文件）**:
  - `dependencies.yaml` — SKIP: RAPIDS 构建依赖 pin
  - `pyproject.toml` × 多处 — SKIP: 上游包依赖声明
  - merge commit — SKIP: RAPIDS 分支合并

- **迁移位置**: 无

---

## migrate eb868d1: [SKIP] fix(ci): fix configuration for breaking change notification workflow (#473) — CI workflow 修复

- **Commit**: `eb868d1a`
- **Commit message**: `fix(ci): fix configuration for breaking change notification workflow (#473)`
- **Author**: James Lamb — 2026-06-01

- **CI/merge → SKIP（1 个文件）**:
  - `.github/workflows/trigger-breaking-change-alert.yaml` — SKIP: RAPIDS CI workflow

- **迁移位置**: 无

---

## migrate 468ad45: [SKIP] Merge pull request #466 from rapidsai/release/26.06 — 分支合并 commit

- **Commit**: `468ad45`
- **Commit message**: `Merge pull request #466 from rapidsai/release/26.06`
- **Author**: GPUtester — 2026-05-22

- **CI/merge → SKIP**:
  - merge commit（与 b25bc88 同内容）— SKIP: RAPIDS 分支合并，Walpurgis 仅跟踪 feature commit

- **迁移位置**: 无

---

## migrate 45129c4 + c06bbbe: [SKIP] Update Changelog / Merge PR #475 — changelog 与分支合并

- **Commits**: `45129c40` / `c06bbbe4`
- **Commit message**: `Update Changelog [skip ci]` / `Merge pull request #475 from rapidsai/release/26.06`
- **Author**: Jakinde Owolabi / GPUtester — 2026-06-03

- **CI/merge → SKIP（1 个文件）**:
  - `CHANGELOG.md` — SKIP: RAPIDS 项目 changelog，Walpurgis 维护独立迁移日志
...
- **Commit**: `51c8d26`
- **Commit message**: `Build and test with CUDA 13.3.0 (#476)`
- **Author**: Bradley Dice — 2026-06-11

- **CI/merge → SKIP（11 个文件）**:
  - conda env yaml × 多处 — SKIP: RAPIDS conda 构建矩阵，CUDA 13.3 配置
  - `.github/workflows/` × 多处 — SKIP: CI matrix 更新
...
- **Commit**: `280f14425`
- **Commit message**: `refactor: switch to rapids-artifact-name for consistent artifact naming (#477)`
- **Author**: Gregory Forsyth — 2026-06-11

- **CI/merge → SKIP（12 个文件）**:
  - `ci/test_wheel_*.sh` × 多处 — SKIP: RAPIDS CI artifact naming，Walpurgis 无 wheel 发布体系
  - `.github/workflows/` × 多处 — SKIP: CI workflow

- **迁移位置**: 无

---

## migrate 7295e19: [SKIP] Add build_type input to `test.yaml` (#143) — CI workflow 参数化

- **Commit**: `7295e19`
- **Commit message**: `Add build_type input to test.yaml (#143)`
- **Author**: Gil Forsyth — 2025-03-06

- **CI/merge → SKIP（1 个文件）**:
  - `.github/workflows/test.yaml` — SKIP: 纯 CI workflow 变更，将 `build_type` 从硬编码 `nightly` 改为 `workflow_dispatch` 输入参数，暴露手动触发时可指定 build_type；Walpurgis 无对应 RAPIDS shared-workflow 体系，不适用

- **迁移位置**: 无

---

## migrate 0726fd1: build with RAPIDS CUDA architectures (#295) — CUDA 架构集合动态化

- **Commit**: `0726fd1`
- **Commit message**: `build with RAPIDS CUDA architectures (#295)`
- **Author**: James Lamb — 2025-09-03
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/295

- **变更概览（2 files changed, 5 insertions, 5 deletions）**:
  - `build.sh` — 核心逻辑：NATIVE 分支去除 `:=` 默认值赋值；ALL_ARCH 分支 `"70-real;75-real;80-real;86-real;90"` → `"RAPIDS"` 动态标记
  - `.pre-commit-config.yaml` — pre-commit/pre-commit-hooks v5.0.0→v6.0.0；rapidsai/pre-commit-hooks v0.4.0→v0.7.0

- **CI/merge → SKIP**:
  - `.pre-commit-config.yaml` — SKIP: 纯 CI/linting hook 版本升级，Walpurgis 无 RAPIDS pre-commit 体系；版本升级信息已文档化于 `PreCommitHookBump` dataclass

- **迁移位置**: `src/walpurgis/core/cuda_arch_policy.py`

- **鲁迅拿法改写（≥20%）**:
  1. `CudaArchMode` 枚举替代上游 bash `BUILD_ALL_GPU_ARCH` 裸整型标志位，NATIVE/ALL 语义命名可 match
  2. `RapidsCudaArchSet` dataclass 在 Python 层显式建模 rapids-cmake 动态架构集合（CUDA 12/13 各一份快照），上游将解析委托给 CMake 黑盒
  3. `CudaArchPolicy.resolve()` 封装决策并返回 `(architectures, rationale)` 元组，上游仅 bash echo+赋值无决策记录
  4. `PreCommitVersionBump` dataclass 文档化 `.pre-commit-config.yaml` 版本升级，`verify_pre_commit_bumps()` 可供 pytest 版本一致性审计
  5. 全链路 `WALPURGIS_DEBUG=1` 断点 print：架构模式解析、RAPIDS 集合查表、决策路径、pre-commit 核验各阶段

- **RAPIDS 架构快照（来自 rapids-cmake 0b111489）**:
  - before (硬编码): `70-real;75-real;80-real;86-real;90`
  - after CUDA 12:   `70-real;75-real;80-real;86-real;90a-real;100f-real;120a-real;120`
  - after CUDA 13:   `75-real;80-real;86-real;90a-real;100f-real;120a-real;120`
...
## migrate 18a2f5562: Add code owners for optimizer-related files (#5297) — .github/CODEOWNERS 语义迁移为结构化策略模块

- **Upstream commit**: 18a2f5562 (Megatron-LM #5297)
- **Commit message**: `Add code owners for optimizer-related files (#5297)`
- **Upstream diff 摘要** (1 file changed, 8 insertions(+))：

  | 文件 | 变更内容 |
  |------|----------|
  | `.github/CODEOWNERS` | 新增 optimizer/ 目录级 owner（mcore-optimizer）；新增 layer_wise_optimizer.py、param_layout.py（dist-optimizer）；新增 emerging_optimizers.py、muon.py（mcore-emerging-optimizers）；新增 qk_clip.py（mcore-emerging-optimizers + transformer 跨领域） |

- **CI/merge 判定**：CODEOWNERS 原文件 SKIP，语义迁移为策略模块

  | 文件 | 迁移判定 |
  |------|----------|
  | `.github/CODEOWNERS` | Walpurgis 无 GitHub Actions CI，原文件无迁移意义；但 optimizer 模块所有权的语义结构有保存价值，迁移为 `codeowner_policy_18a2f5562.py` |

- **迁移位置**：`src/walpurgis/core/codeowner_policy_18a2f5562.py`（新增）

- **鲁迅拿法改写（≥20%）**：
  上游 CODEOWNERS 改动的本质是一次「权责分层」：optimizer 目录从无主到有主，
  再从整目录宽泛 owner 到单文件精细 owner。这像极了《故乡》里的土地权属——
  昔日公地无人认领，今日圈地各立门户，圈得越细，责任边界越清晰，围墙也越高。
  `emerging_optimizers.py`、`muon.py`、`qk_clip.py` 单独成条，是新兴技术模块
  「另立山头」的信号：这三块代码走的是不同研究路线，与 dist-optimizer 团队并非同源。
  `qk_clip.py` 同时挂靠 @transformer，更说明它的来历跨越了优化器与注意力机制两个
  世界的边界——如鲁迅所言：世上本没有路，走的人多了，也便成了路；
  世上本没有边界，负责的人多了，也便有了 CODEOWNERS。

  Walpurgis 将此次「权责结构化」抽象为五个可程序化结构：
  1. **`OwnerTeam` 枚举** — 显式建模 6 个上游 NVIDIA 团队标识符，使团队归属可程序化查询
  2. **`OwnershipScope` 枚举** — 区分目录级/文件级单团队/文件级跨领域三种粒度
  3. **`ModuleOwnerEntry` dataclass（frozen）** — 封装单条 CODEOWNERS 条目，含上游路径、Walpurgis 对应路径、scope、teams、新增标记、说明
  4. **`OptimizerOwnerManifest`** — 汇总 7 条条目，提供 lookup/new_entries/emerging_optimizer_entries/cross_domain_entries/audit_report 等查询接口
  5. **`OwnershipAuditReport` dataclass** — 结构化输出总条目数、新增数、新兴优化器条目数、跨领域条目数及详情

  全链路 `WALPURGIS_DEBUG=1` 断点 print **12 处**（MODULE_LOAD×2、ENUM_INIT×2、DATACLASS_INIT、MANIFEST_INIT、MANIFEST_INDEX、MANIFEST_NEW、MANIFEST_LOOKUP、AUDIT_REPORT_INIT、MANIFEST_AUDIT×2、SELF_CHECK×7），`self_check()` 5 项断言全部通过。

- **三维度审查（Knuth）**：
  - **正确性**：`.github/CODEOWNERS` 为 GitHub 代码审查权限配置；Walpurgis 无 GitHub Actions CI，原文件直接 SKIP 正确。7 条条目（1 目录级 + 6 文件级）、6 条新增、3 条新兴优化器、1 条跨领域的计数均经 self_check() 断言验证，数值与上游 diff 完全对应。
  - **性能**：纯配置语义迁移，无算法/数据结构，无运行时性能影响。`lookup()` 为 O(1) 字典查询（哈希索引），`all_entries()` 为 O(n log n) 排序（n=7，常数）。
  - **可读性**：上游 CODEOWNERS 为静态文本，无法程序化查询「哪些文件归属新兴优化器」「哪些文件跨领域」。`codeowner_policy_18a2f5562.py` 将所有权关系结构化，审计报告可直接输出，比原始文本更具可读性与可维护性。

## migrate c882ac611: Merge branch 'megatron_package' into 'master' — 包化集成里程碑

- **Upstream commit**: c882ac611 (Megatron-LM, commit #15/9062)
- **Commit message**: `Merge branch 'megatron_package' into 'master'`
- **Upstream diff 摘要**：空 diff（pure merge commit，无文件变更）

  | 文件 | 变更内容 |
  |------|----------|
  | （无） | 纯合并提交，megatron_package 分支集成进 master，diff 为空 |

- **迁移位置**：`src/walpurgis/core/branch_merge_package.py`（新增，390 行）
  - 原上游无文件变更 → Walpurgis 以策略模块形式记录合并事件的语义与影响范围
...
- **鲁迅拿法改写（≥20%）**：
  ...
  1. **`MergeEvent` dataclass（frozen）** — 封装合并提交元数据：hash、序号、
     source/target 分支、合并类型、四个 `BranchIntent` 意图；
     `is_empty_diff()` 精确标记空 diff 语义。
  2. **`PackageIntegrationPolicy`** — 记录 megatron_package 分支的包化目标、
     14 个上游模块路径、7 组 Walpurgis 映射；
     `resolve_walpurgis_path()` 提供 O(n) 前缀匹配查询，
     `package_scope_report()` 输出集成范围摘要。
  3. **`BranchLineageTracker`** — 追踪分支合并谱系（5 个步骤）：
     branch_created → package_structure_built → mpu_refactored →
     merge_commit_c882ac611 → walpurgis_migration；
     `trace()` 输出完整报告，`get_step()` 按名精确查询。
  4. **`MergeType` / `BranchIntent` 枚举** — 显式建模合并类型与包化意图，
     使分支事件可程序化分类与查询。

  全链路 `_dbg()` 断点 **14 处**：
  MODULE_LOAD×3、MERGE_EVENT_INIT、POLICY_INIT、POLICY_RESOLVE（×2）、
  POLICY_SCOPE_REPORT、LINEAGE_INIT、LINEAGE_TRACE、LINEAGE_GET_STEP（×2）、
  API_GET_*（×3）、SELF_CHECK（×7 步骤）。
  `WALPURGIS_DEBUG=1` 时全部激活，生产环境静默。

- **三维度审查（Knuth）**：
  - **正确性**：c882ac611 为空 diff 合并提交，语义迁移（而非文件迁移）完全正确；
    `self_check()` 5 项断言全部通过，commit hash、序号、意图枚举、路径映射、
    谱系步骤均经程序验证。
  - **性能**：纯数据结构与字符串操作，无 I/O，无循环热路径；
    `resolve_walpurgis_path()` 为 O(n) 前缀扫描（n=7，常数）。
  - **可读性**：空 diff 合并在 git log 中是「无声的里程碑」，
    Walpurgis 以结构化策略模块将其可见化，比 git 元数据更具可查询性与可维护性。
...
- **Upstream diff 摘要** (23 files changed, 1964 insertions(+), 3268 deletions(-))：
  ...
  | 文件 | 变更类型 | 变更内容 |
  |------|----------|----------|
  | `megatron/model/language_model.py` | 新增(398行) | TransformerLanguageModel + Embedding + Pooler，GPT-2/BERT 共享骨架 |
  | `megatron/model/transformer.py` | 新增(490行) | 单机版 transformer（从 mpu/transformer.py 提取串行路径） |
  | `megatron/model/bert_model.py` | 新增(218行) | BertModel：MLM head + NSP head |
  | `megatron/model/gpt2_model.py` | 新增(119行) | GPT2Model：weight-tied un-embedding |
  | `megatron/model/utils.py` | 新增(80行) | get_params_for_weight_decay_optimization |
  | `megatron/module.py` | 新增(34行) | MegatronModule：fp16 checkpoint save/load |
  | `megatron/training.py` | 新增(499行) | pretrain() / train() / evaluate()：两脚本公共训练逻辑 |
  | `pretrain_bert.py` | 精简(528→~120行) | 仅保留 model_provider / forward_step / dataset_provider |
  | `pretrain_gpt2.py` | 精简(462→~100行) | 同上，GPT-2 特有部分 |
  | `megatron/utils.py` | 更新 | 新增 initialize_distributed / set_random_seed（从 pretrain_gpt2 迁入） |
  | `megatron/model/modeling.py` | 删除(1382行) | 旧版整体 modeling，被拆分为上述新文件 |
  | `megatron/model/gpt2_modeling.py` | 删除(157行) | 旧版 GPT-2 modeling |
  | `megatron/model/model.py` | 删除(90行) | 旧版 model wrapper |
  | `megatron/mpu/transformer.py` | 删除(647行) | 旧版 mpu transformer（串行路径迁入 model/transformer.py） |
  | `docker/Dockerfile` | 更新 | base image 19.05→19.09，删除手动 apex 安装 |
  | `generate_samples.py` | 更新 | 导入路径从 pretrain_gpt2 改为 megatron.utils |

- **迁移位置**（共 7 个新文件 + 1 个文件追加）：
  - `src/walpurgis/models/language_model.py`（新增）
  - `src/walpurgis/models/transformer.py`（新增）
  - `src/walpurgis/models/bert_model.py`（新增）
  - `src/walpurgis/models/gpt2_model.py`（新增）
  - `src/walpurgis/models/model_utils.py`（新增，含 module.py 内容）
  - `src/walpurgis/core/training.py`（新增）
  - `src/walpurgis/core/pretrain_bert.py`（新增）
  - `src/walpurgis/utils/megatron_utils.py`（追加 initialize_distributed / set_random_seed）

- **鲁迅拿法改写（≥20%）**：
  这是整个迁移序列里体量最大的单次重构。
  上游把两间铁屋子（pretrain_bert / pretrain_gpt2）拆墙合并，
  公共的训练骨架移入 megatron/training.py，
  各自特有的砖瓦（model_provider / forward_step）留在原地。
  鲁迅说：「拆墙之后，才看见原来两间屋子住的是同一种人。」

  Walpurgis 七个新文件的改写策略：
  1. **language_model.py**：EmbeddingLayer（三路显式，tokentype_mode枚举）+ PoolerHead（命名化）+ LanguageModelCore（三阶段 _dbg：embedding→transformer→pooler）
  2. **transformer.py**：SelfAttentionCore（_project_qkv/_compute_attention/_project_output三分）+ FeedForwardBlock（act_fn可切换）+ TransformerLayer（PreLN/PostLN显式区分）+ TransformerStack（output_all_hidden_states扩展）
  3. **bert_model.py**：MaskedLMHead（tie_weights显式调用）+ NextSentencePredHead（命名化）+ BertOutput dataclass（解耦loss计算）+ BertLossComputer（MLM+NSP分离）
  4. **gpt2_model.py**：GPT2OutputHead（tie_weights显式）+ GPT2Output dataclass + GPT2LossComputer（eod_mask加权平均，去mpu依赖）
  5. **model_utils.py**：WeightDecayParamGroups（from_model分类器，条件逐条注释）+ ModelParamStats（trainable/frozen分报告）+ Fp16CheckpointStrategy（从继承解耦为策略）+ MegatronModuleMixin（继承链→Mixin）
  6. **training.py**：TrainingState dataclass + EvalResult dataclass + CheckpointManager（rank-zero guard + RNG save/load）+ TrainingEngine（train_step/evaluate/train_loop三层）+ DistributedSetup（rank-aware seed偏移）
  7. **pretrain_bert.py**：BertPretrainConfig dataclass + BertBatchProcessor（shape断言）+ BertPretrainTask（model_provider+forward_step+dataset_provider统一封装）

  全链路 `_dbg()` 断点共计 **80+处**（7个文件各10-20处），`WALPURGIS_DEBUG=1` 全部激活。

- **三维度审查（Knuth）**：
  - **正确性**：weight tying、attention mask 语义、loss ignore_index(-100)、RNG state 保存/恢复均与上游语义等价；tokentype_ids=None 路径加 _dbg 警告；-1→-100 legacy label 映射兼容。
  - **性能**：单机版无 mpu 并行 all-reduce 开销；TransformerStack 的 output_all_hidden_states 默认 False（生产路径无额外内存占用）；CheckpointManager rank-zero guard 避免多卡重复 I/O。
  - **可读性**：7个文件各有 docstring 说明上游对应文件、改写策略、鲁迅拿法注释；参数分组（BertPretrainConfig / BertBatchProcessor）使超参来源一目了然；上游隐式 weight tying 改为 tie_weights() 显式调用，消除初始化顺序依赖。
---

## Commit 25 — beb3e0d38

**Subject**: Merge branch 'transformer_refactoring_from_pretrain_refactoring' into 'master'
**Author**: Raul Puri <raulp@nvidia.com> | **Date**: 2019-10-29
**Diff**: 23 files, +1964/-3268

### 上游变化摘要

这是 Megatron-LM 第 25 个 commit，也是一次结构性重构的里程碑：
BERT 和 GPT-2 从各自拥有独立 transformer 实现（`mpu/transformer.py` 647行，
`model/modeling.py` 1382行）合并为共享骨架（`model/transformer.py` 490行）。
新增 `megatron/training.py`（499行），统一训练引擎。
删除 `model/model.py`、`model/modeling.py`、`mpu/transformer.py`。

### CI/merge 判定

| 文件 | 迁移判定 |
|------|----------|
| `megatron/model/transformer.py` + `megatron/module.py` | 迁移 → `models/transformer_unified.py` |
| `megatron/model/language_model.py` + `bert_model.py` + `gpt2_model.py` + `model/utils.py` | 迁移 → `models/language_model_unified.py` |
| `megatron/training.py` | 迁移 → `core/training_engine.py` |
| `megatron/model/gpt2_modeling.py`（删除） | SKIP（已在 commit 5 abe36e2e5 中迁移） |
| `megatron/model/modeling.py`（删除） | SKIP（被 transformer_unified 覆盖） |
| `docker/` | SKIP（容器化配置） |
| `megatron/fp16/fp16.py`（小改） | SKIP（已在 abe36e2e5 中迁移） |

### 迁移位置

- `src/walpurgis/models/transformer_unified.py`（新增）
- `src/walpurgis/models/language_model_unified.py`（新增）
- `src/walpurgis/core/training_engine.py`（新增）

### 鲁迅拿法改写（≥20%）

这次重构的本质，是用「抽象」消灭「复制粘贴」。BERT 和 GPT-2 共用同一个
`TransformerLanguageModel` 骨架，差异被最小化为三个参数：掩码函数、
残差连接策略、是否有 Pooler。鲁迅若见此景，大约会说：从前两个人各说各的，
现在说的是同一种话——只是语气不同。这不是消灭差异，这是命名差异。

**`transformer_unified.py`**（对应 transformer.py + module.py，490+34行）：
上游 `TransformerHyperparameters` 以 `params_dict: dict` 存储超参，
键名拼错只在运行时爆。Walpurgis 改为 `TransformerSpec(frozen dataclass)`，
字段类型化，`__post_init__` 集中断言。`ResidualPolicy(Enum: PRE_NORM/POST_NORM)`
将 `apply_residual_connection_post_layernorm` 布尔值命名化，
使「BERT 用 POST_NORM，GPT-2 用 PRE_NORM」成为静态可读的策略声明。
`CheckpointStrategy(Enum)` 替代 `checkpoint_activations` 布尔值。
`WalpurgisModule` 扩展 `MegatronModule`，新增 `parameter_count()` 和 `device_set()`。
`AttentionScoreAudit` 封装上游双重 sqrt 的 norm_factor，注释说明数学等价性。
_dbg 断点 17 处。

**`language_model_unified.py`**（对应 language_model.py + bert_model.py + gpt2_model.py + utils.py，合计 825行）：
上游 BERT 和 GPT-2 各自构建 `TransformerLanguageModel`，参数平铺传入。
Walpurgis 以 `EmbeddingSpec(frozen dataclass)` 封装嵌入层规格，
`AttentionMaskKind(Enum: ADDITIVE_BERT/MULTIPLICATIVE_GPT2)` 命名掩码语义，
`ModelParallelLogitsMode(Enum: PARALLEL/GATHERED)` 替代 `parallel_output` 布尔值，
`WeightDecayManifest` 将参数分组从散函数升为可查询台账（含 `self_check()` 验证完整性），
`LMHeadSpec(frozen dataclass)` 封装 `BertLMHead` 参数。_dbg 断点 16 处。

**`training_engine.py`**（对应 training.py，499行）：
上游 `run()` 是 499 行单体函数，`args` 是类型不明的 Namespace。
Walpurgis `TrainingConfig(frozen dataclass)` 类型化所有训练参数，
`OptimizerSpec(frozen dataclass)` 封装 FP16 参数并在 `loss_scale_guard()` 中
加入 `min_scale >= 1.0` 下限断言（上游 `DynamicLossScaler` 缺此保护）。
`DDPStrategy(Enum: TORCH/LOCAL)` 替代字符串比较。
`TrainingMetrics` 类替代裸 `total_loss_dict: dict`，提供 `accumulate/average/reset` 接口。
`ThroughputSpec.mfu_estimate()` 将吞吐量从 tokens/sec 升维为模型浮点利用率（MFU）。
`TrainingEngine` 类封装训练全流程，各阶段可注入 hook。_dbg 断点 15 处。

### 三维度审查（Knuth）

- **正确性**：三文件均含 `self_check()`，运行通过（`ALL PASS`）。
  `TransformerSpec.__post_init__` 断言 hidden_size 整除 num_attention_heads，
  防止静默的矩阵形状错误。`OptimizerSpec.loss_scale_guard()` 的 `min_scale >= 1.0`
  断言防止 FP16 损失缩放无限下降。`WeightDecayManifest.self_check()` 验证
  所有参数均被分组，无遗漏。

- **性能**：核心训练路径（forward/backward/allreduce）均保留上游原始实现，
  Walpurgis 封装仅在初始化路径引入额外 Python 对象创建（dataclass 实例化），
  对 GPU 训练吞吐无影响。`ThroughputSpec.mfu_estimate()` 为纯 Python 浮点计算，
  仅在日志时调用。

- **可读性**：本次 commit 的核心贡献是「用共享骨架替代复制粘贴」。
  Walpurgis 进一步将剩余的隐式差异（布尔值、字符串）外显为枚举，
...
## migrate 2ef9dba: remove cugraph-dgl CODEOWNERS rule, skip most CI on CODEOWNERS-only PRs

- **Commit**: `2ef9dba`
- **Commit message**: `remove cugraph-dgl CODEOWNERS rule, skip most CI on CODEOWNERS-only PRs (#264)`

- **Context**: 双管齐下的 housekeeping commit：
  1. `.github/CODEOWNERS`：删除 `python/cugraph-dgl @rapidsai/cugraph-gnn-python-codeowners` 条目——cugraph-dgl 模块已废弃，其 CODEOWNERS 条目成了无主僵尸规则；
  2. `.github/workflows/pr.yaml`：在 `test_cpp` / `test_notebooks` / `test_python` 三条 CI 矩阵的 `paths-filter` 中各新增 `- '!.github/CODEOWNERS'`，使「仅更新 reviewer 名单」的 PR 跳过全量 GPU CI。

- **迁移位置**:
  - `.github/CODEOWNERS` — 直接应用：删除 cugraph-dgl 行，追加 cugraph-pyg / pylibwholegraph / mg_utils 条目，更新迁移来源注释
  - `src/walpurgis/core/ci_codeowners_skip_policy` — 新增（无后缀，铁律）

- **SKIP 说明**:
  - `.github/workflows/pr.yaml` — SKIP：Walpurgis 无 GitHub Actions CI 体系，pr.yaml 的 paths-filter 配置在本仓库无实际运行环境；CI 跳过策略语义已以可程序化结构保存至 `ci_codeowners_skip_policy`

- **鲁迅拿法改写（≥20%）**:
  1. **`CiPipelineKind(Enum)`**: 将上游三条 YAML 矩阵键（test_cpp/test_notebooks/test_python）枚举化，携带 `upstream_yaml_key()`/`is_code_pipeline()` 属性——上游只有三处相同 YAML 块
  2. **`SkipReason(Enum)`**: 将「跳过原因」分类枚举（CONFIG_ONLY/DOC_ONLY/INFRA_ONLY），携带 `human_description()`/`blocks_code_execution()` 属性——上游无此语义层，只有隐含的「不写 `!pattern` 就会触发 CI」逻辑
  3. **`PathExemptionRule(frozen dataclass)`**: 将每条 `!pattern` 豁免规则结构化为「glob + 适用流水线 + 跳过原因 + 引入 commit」四元组，`matches_path()` 封装 fnmatch 判定，`audit_line()` 产出对应 YAML 注释——上游每条规则在三个 YAML 块中各重复一次
  4. **`CiSkipManifest`**: 单点定义（Single Source of Truth）替代上游三份重复 YAML 块；`can_skip_pipeline()` 实现「所有变更文件均被豁免覆盖 → 可跳过」完整判定逻辑，返回可审计的 `CiSkipDecision`——上游此逻辑完全隐藏在 GitHub Actions runner 内部
  5. **`CiSkipDecision(frozen dataclass)`**: 将每次跳过判定显式化为可审计记录，`summary()` 单行输出——上游开发者只能通过观察 CI 是否触发来倒推豁免是否生效
  6. **`CiSkipAuditReport(dataclass)`**: 结构化审计报告，统计 total_rules/from_2ef9dba/config_only 三项指标——上游无此统计层
  7. **断点调试**: 全链路 10 处 `WALPURGIS_DEBUG=1` 断点，覆盖模块加载、枚举构造、规则匹配、审计报告生成全路径
