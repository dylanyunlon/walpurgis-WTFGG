...
## migrate 5f12b2f: Update DGL Packages to PyTorch 2.4 (#148)

- **Upstream commit**: 5f12b2fa2e6cb5942118d001cb09dac84efc6abc (cugraph-gnn, Alex Barghi / alexbarghi-nv, 2025-02-19)
- **Commit message**: `Update DGL Packages to PyTorch 2.4 (#148)`
- **Author**: Alex Barghi (alexbarghi-nv, NVIDIA)
- **Approvers**: Tingyu Wang (tingyu66), James Lamb (jameslamb)
- **Context**: DGL 2.4 for PyTorch 2.3（th23 渠道）已不再提供；将所有 conda 渠道引用从 `dglteam/label/th23_cu*` 切换到 `dglteam/label/th24_cu*`，解除 CI 阻塞。

- **Upstream diff** (6 files changed, 8 insertions(+), 8 deletions(−)) — 全部为渠道标签替换，无逻辑变更:
  | 文件 | 变更内容 |
  |------|----------|
  | `conda/environments/all_cuda-118_arch-x86_64.yaml` | `th23_cu118::dgl>=2.4.0.th23.cu*` → `th24_cu118::dgl` |
  | `conda/environments/all_cuda-121_arch-x86_64.yaml` | `th23_cu121::dgl>=2.4.0.th23.cu*` → `th24_cu124::dgl` |
  | `conda/environments/all_cuda-124_arch-x86_64.yaml` | `th23_cu121::dgl>=2.4.0.th23.cu*` → `th24_cu124::dgl` |
  | `conda/recipes/cugraph-dgl/meta.yaml` | `dgl >=2.4.0.th23.cu*` → `dgl >=2.4.0.th24.cu*` |
  | `dependencies.yaml` | cuda 12.*/11.*/null 三个矩阵行均由 th23 → th24 |
  | `python/cugraph-dgl/conda/cugraph_dgl_dev_cuda-118.yaml` | `th23_cu118::dgl>=2.4.0.th23.cu*` → `th24_cu118::dgl` |

- **CI/merge → SKIP**（全部 6 个文件）:
  - `conda/environments/**` — Walpurgis 无 conda 构建矩阵，conda env yaml 无迁移目标
  - `conda/recipes/cugraph-dgl/meta.yaml` — 上游 cugraph-dgl conda recipe，Walpurgis 不发布此包
  - `dependencies.yaml` — RAPIDS 构建依赖清单，Walpurgis 用 pyproject.toml 独立管理
  - `python/cugraph-dgl/conda/**` — cugraph-dgl 已在 fb8296e 中永久删除，无迁移目标

- **迁移位置**: `src/walpurgis/core/dgl_package_policy.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`PyTorchLabel` dataclass**: 将 th23/th24 渠道标签强类型化，携带 `pytorch_version`/`cuda_variants`/`eol` 字段；`eol=True` 标记 th23（上游 PR #148 注释"no longer available"的根因），`is_newer_than()`/`conda_label_prefix()` 方法——上游只有裸字符串替换，无任何类型化表示
  2. **`DglChannelSpec` dataclass**: 封装 conda `channel::package[>=constraint]` 完整规格，`parse()` / `as_conda_dep()` / `as_meta_yaml_pin()` 派生多种格式——上游通过 sed 就地替换，无此抽象层
  3. **`MigrationKind` 枚举 + `DglPackageMigration` dataclass**: 区分 LABEL_ONLY / VERSION_RELAXED / CUDA_VARIANT_CHANGED 三类跃迁语义，携带 `commit_sha`/`pr_number`/`author`/`rationale`，`migration_kind()`/`is_forward()`/`as_patch_line()` 方法——上游只有 sed，无任何跃迁语义建模
  4. **`CudaChannelMatrix`**: 将 `dependencies.yaml` matrices 段类型化，`register()`/`lookup()`/`validate_all_covered()` 方法，`POST_5F12B2F_MATRIX` 实例化迁移后状态——上游散落在 yaml 文件中，无统一查询接口
  5. **`DglEolProbe`**: 在 Python import 层主动探测已安装 DGL 版本是否来自 th23（EOL 渠道），发出 DeprecationWarning——上游通过 CI 失败才发现包不可用，无主动探测机制
  6. **`DglPackageAudit`**: 扫描文件检查 th23 旧渠道标签残留，`assert_clean()` 供 CI 调用验证迁移彻底性——上游 sed 后无回头检查机制
  7. 全链路 `WALPURGIS_DEBUG=1` 断点（6 处）: 覆盖 label 解析→跃迁计算→矩阵查找→EOL 探测→残留扫描→自测各阶段

- **自测结果**: 15 项全部 [PASS]（PyTorchLabel 解析/排序、eol 标志、is_newer_than、DglChannelSpec parse/format、MigrationKind LABEL_ONLY/CUDA_VARIANT_CHANGED、is_forward、CudaChannelMatrix lookup cu124/cu118、DglPackageAudit dirty/clean/assert_clean、迁移条目数=8、所有 is_forward=True）
