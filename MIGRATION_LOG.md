...
## migrate f83f6ae: Switch to release channel for PyTorch + CUDA 13

- **Upstream commit**: f83f6aea7ecc1f2d99fd66969cc07693f087e797 (cugraph-gnn, Alex Barghi + Gil Forsyth, 2025-11-25, PR #355)
- **Commit message**: `Switch to release channel for PyTorch + CUDA 13 (#355)`
- **Upstream diff** (3 files changed, 5 insertions, 9 deletions):
  - `ci/test_wheel_cugraph-pyg.sh` — `PYTORCH_INDEX` 从 `https://download.pytorch.org/whl/nightly/cu130` 改为 `https://download.pytorch.org/whl/cu130`
  - `dependencies.yaml` — `--extra-index-url` 同上，删除两处 TODO 注释（rapidsai/pytorch#159779），`torch>=2.9.0.dev0` → `torch>=2.9.0`
  - `python/cugraph-pyg/pyproject.toml` — test dep `torch>=2.9.0.dev0` → `torch>=2.9.0`，版权年 2024→2025

- **CI/merge → SKIP**:
  - `ci/test_wheel_cugraph-pyg.sh` — SKIP：Walpurgis 无 CI wheel 测试体系
  - `dependencies.yaml` — SKIP：RAPIDS conda 依赖矩阵，Walpurgis 无 conda 体系
  - `python/cugraph-pyg/pyproject.toml` — SKIP：上游包构建元数据，Walpurgis 使用自有 pyproject.toml

- **迁移位置**: `src/walpurgis/core/cuda_compat.py` — 新增 `PyTorchCuda13ChannelPolicy` 数据类、`_PyTorchChannel` 枚举、`PYTORCH_CUDA12_POLICY`/`PYTORCH_CUDA13_POLICY` 单例、`get_pytorch_policy()` 路由函数

- **鲁迅拿法改写（≥20%）**:
  上游三处均只改了字符串字面量（`"nightly/cu130"` → `"cu130"`，`">=2.9.0.dev0"` → `">=2.9.0"`），零结构化。Walpurgis 将此决策对象化：
  1. `_PyTorchChannel` 枚举：显式区分 `NIGHTLY`/`RELEASE` 两条路径，上游靠 URL 拼接字符串隐式区分
  2. `PyTorchCuda13ChannelPolicy` 数据类（frozen）：`channel`/`cuda_tag`/`min_torch_version`/`allow_dev` 四字段，字段类型化，`f83f6ae` 决策可程序化审计
  3. `base_whl_url` / `extra_index_url_flag` / `torch_requirement` 属性：各自一行，替代上游同一逻辑散落 3 处文件的裸字符串
  4. `validate_torch_version()` 运行时 torch 版本检查：若环境仍装 nightly dev0，发出 `FutureWarning`；版本不足则 `RuntimeError`——上游无此防御
  5. `get_pytorch_policy(cuda_version)` 路由函数：对应 `ci/test_wheel_cugraph-pyg.sh` 的 `if CUDA_MAJOR==12 else` 分支，使渠道决策可单独测试
  6. 全链路5个 `WALPURGIS_DEBUG=1` 断点 print（构造→URL解析→渠道路由→版本检测→通过/失败）

- **自测结果**: `python src/walpurgis/core/cuda_compat.py` → 9项断言全通过，`[PASS]`
