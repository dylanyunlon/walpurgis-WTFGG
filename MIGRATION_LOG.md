...
## migrate bf64914: [SKIP] add full CI for wholegraph — 纯 CI/conda 基础设施，无迁移价值

- **Upstream commit**: bf64914 (cugraph-gnn, James Lamb, 2024-10-21, PR #58)
- **Commit message**: `add full CI for wholegraph (#58)`
- **Upstream diff** (26 files changed, 468 insertions, 289 deletions):
  - `.github/workflows/pr.yaml` — 新增 conda-cpp/python build/test 及 wheel-build/test-pylibwholegraph CI job
  - `build.sh` — 新增 `pylibwholegraph`/`libwholegraph`/`tests` 构建目标，重构默认构建顺序
  - `ci/build_cpp.sh` — 新增 (22行): conda libwholegraph 构建入口
  - `ci/build_python.sh` — 重写: 从 cugraph-pyg/dgl 构建改为 pylibwholegraph conda 构建
  - `ci/build_wheel.sh` — 重构: 新增 pylibwholegraph wheel 排除规则 (libcuda/nvidia-ml)
  - `ci/build_wheel_pylibwholegraph.sh` — 新增 (9行): pylibwholegraph wheel 构建入口
  - `ci/run_ctests.sh` — 新增 (9行): libwholegraph C++ 测试运行脚本
  - `ci/run_pylibwholegraph_pytests.sh` — 新增 (9行): pylibwholegraph pytest 入口
  - `ci/test_cpp.sh` — 重写: 从 libcugraph 测试改为 libwholegraph 测试
  - `ci/test_python.sh` — 重写: 从 cugraph-pyg/dgl 测试改为 pylibwholegraph 测试
  - `ci/test_wheel_pylibwholegraph.sh` — 新增 (42行): pylibwholegraph wheel 测试
  - `conda/environments/all_cuda-118/121/124_arch-x86_64.yaml` — 更新依赖 (nccl>=2.19, pytest-forked, pytorch<2.4)
  - `conda/recipes/libwholegraph/conda_build_config.yaml` — nccl 版本 >=2.9.9→>=2.19
  - `conda/recipes/pylibwholegraph/build.sh` — 移除 CMAKE_EXTRA_ARGS
  - `conda/recipes/pylibwholegraph/meta.yaml` — 新增 numpy/license 字段
  - `cpp/cmake/thirdparty/get_raft.cmake` — RAPIDS_VERSION 变量修复
  - `dependencies.yaml` — 新增 py_build/run/test_pylibwholegraph 配置块，删除旧的 depends_on_pylibwholegraph
  - `python/.coveragerc` — 新增 (21行): pytest coverage 配置
  - `python/cugraph-dgl/conda/cugraph_dgl_dev_cuda-118.yaml` — pytorch 版本约束
  - `python/cugraph-dgl/pyproject.toml` — build-system 从 setuptools/wheel 改为 cmake/cython/ninja
  - `python/cugraph-pyg/conda/cugraph_pyg_dev_cuda-118.yaml` — pytorch 版本约束
  - `python/cugraph-pyg/pyproject.toml` — build-system 同上
  - `python/pylibwholegraph/pylibwholegraph/binding/wholememory_binding.pyx` — 移除 `import numpy as np`
  - `python/pylibwholegraph/pyproject.toml` — 新增 numpy 依赖及 test extras

- **迁移价值**: 无
- **原因**: 全部为 CI/conda/CMake/packaging 基础设施，无任何算法、运行时 Python 或 C++ 代码改动。
  - `.github/workflows/pr.yaml`：GitHub Actions CI 配置，Walpurgis 无此 CI 体系
  - `ci/*.sh`：RAPIDS conda/wheel 构建脚本，Walpurgis 不维护 conda 打包流程
  - `conda/environments/*.yaml`、`conda/recipes/**`：conda 环境矩阵，Walpurgis 无 conda 体系
  - `dependencies.yaml`：RAPIDS 专用依赖矩阵生成器格式，Walpurgis 不使用此工具链
  - `python/*/pyproject.toml`：build-system 依赖更新（setuptools→cmake/cython/ninja），属打包配置
  - `python/.coveragerc`：pytest coverage 配置，Walpurgis 有独立测试体系
  - `python/pylibwholegraph/pyproject.toml`：pylibwholegraph 发布元数据，Walpurgis 通过 pip 使用预编译版本
  - `wholememory_binding.pyx` 的 `import numpy as np` 删除：属于 C 扩展绑定层清理，无 Python 算法价值
  - `get_raft.cmake`：CMake 变量修复，Walpurgis 无 CMake 构建体系

- **Knuth 审查**:
  1. **diff 完整性**: 26 个文件全量审查；所有 `.py`/`.pyx`/`.cu`/`.cpp` 改动均仅涉及 import 清理（`import numpy` 删除一行）或测试脚本入口，无业务逻辑
  2. **用户角度 bug**: 无运行时代码改动，零 bug 风险；`import numpy as np` 删除是因为 Cython 3.x 不再需要此 import（Cython 内置 NumPy 支持），不影响 Python 调用方
  3. **系统安全**: 无安全影响；nccl>=2.19 版本约束提升了通信库最低要求，但这是 conda 打包层约束，不影响 Walpurgis pip 安装路径
