...
- **Upstream commit**: 140908eb13f5bb78610abfc165f99d3e66be7fad (cugraph-gnn, Alex Barghi, 2025-03-05, PR #153)
- **Commit message**: `Drop PyTorch Constraint for OGB (#153)`
- **Upstream diff** (18 files changed, 98 insertions, 81 deletions):

  **CI/配置 → SKIP**（Walpurgis 无对应体系）：
  - `.github/workflows/pr.yaml` / `test.yaml` — 容器镜像从固定 `cuda11.8.0-py3.12` → `latest`
  - `ci/test_notebooks.sh` — DGL channel `th24_cu118` → `th24_cu124`
  - `ci/test_python.sh` — 删除 3 处 `--prepend-channel pytorch`（解除 OGB pytorch 约束）
  - `conda/environments/all_cuda-118/121/124_arch-x86_64.yaml` — pytorch 约束去除 `<2.6a0` 重复行
  - `dependencies.yaml` — 删除 `depends_on_ogb` 段（共 8 处引用），重构 `depends_on_pytorch`（新增 `tensordict` 到 requirements/pyproject 输出）
  - `python/cugraph-dgl/conda/cugraph_dgl_dev_cuda-118.yaml` — 同 conda yaml 整理
  ...
  **Python examples（核心逻辑）→ 迁移**：
  - `cugraph_pyg/examples/gcn_dist_sg.py` — `load_data()` 中加载 `PygNodePropPredDataset` 包裹 `safe_globals`
  - `cugraph_pyg/examples/gcn_dist_snmg.py` / `gcn_dist_mnmg.py` — `if rank==0` 分支同上
  - `cugraph_pyg/examples/rgcn_link_class_sg.py` / `snmg.py` / `mnmg.py` — 加载 `PygLinkPropPredDataset` 包裹 `safe_globals`，额外白名单 4 个 numpy 内部类

- **迁移位置**: `src/walpurgis/core/ogb_safe_load.py` — 新建
- **鲁迅拿法改写（≥20%）**:
  1. `SafeGlobalsScope` enum: 将节点预测/链接预测/异构链接预测三种场景语义化，上游六个文件硬编码不同长度白名单列表，无任何分类
  2. `OgbSafeGlobalsConfig` dataclass: 按 Scope 集中管理白名单类列表，延迟导入（`torch_geometric`/`numpy` 未安装时不报错），上游无此抽象
  3. `OgbCompatGuard` dataclass: 检测 `torch` 版本是否需要 `safe_globals`（<2.4 时 API 不存在，≥2.6 时为必须），上游无版本检测无条件包裹
  4. `safe_load_node_dataset()`: 封装 `PygNodePropPredDataset` 安全加载，返回 `(dataset, split_idx)`，屏蔽 torch 版本差异
  5. `safe_load_link_dataset()`: 封装 `PygLinkPropPredDataset` 安全加载，返回 `(data, dataset, splits)`
  6. `OgbSafeLoadAudit` dataclass: 加载后验证 dataset 非空、split_idx 含 train/valid/test 键、link dataset 有 num_nodes 属性，上游 examples 无任何加载后验证
  7. 全链路 `WALPURGIS_DEBUG=1` 断点（6 处）
- **自测结果**: 14 项全部 [PASS]
- **技术说明**: PyTorch ≥2.6 默认对 `torch.load` 启用全局变量白名单（`weights_only=True`），OGB 的 `PygDataset` 序列化依赖未白名单的 PyG/numpy 类，触发 `UnpicklingError`（snap-stanford/ogb#497）。`safe_globals` API 在 torch 2.4 引入，2.6 起变为必须。本模块将此 workaround 结构化，使适用范围、白名单内容、版本门槛均可审计。
