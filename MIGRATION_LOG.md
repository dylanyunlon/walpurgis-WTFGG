...
- **Upstream commit**: 958783cc77880d0440347259c1247b46f7e2b852 (cugraph-gnn, Nate Rock <nrock@nvidia.com>, 2025-11-10)
- **Commit message**: `refactored update-version.sh to handle new branching strategy`
- **Upstream diff** (2 files changed, 67 insertions, 6 deletions):
  - `ci/release/update-version.sh`: 新增 `--run-context=main|release` CLI 参数；优先级链 `CLI > RAPIDS_RUN_CONTEXT 环境变量 > 默认 main`；main 上下文 → `RAPIDS_BRANCH_NAME="main"`，release 上下文 → `RAPIDS_BRANCH_NAME="release/{YY.MM}"`；新增 `validate RUN_CONTEXT`；文档引用更新策略：main 上下文 noop，release 上下文 sed 替换 cpp/scripts/run-cmake-format.sh 中的 main 引用
  - `cpp/scripts/run-cmake-format.sh`: 文档链接从具体 branch-25.02 改为 main（反映新分支策略）
...
- **迁移位置**: `src/walpurgis/core/upstream_version_updater.py` — 新增

- **鲁迅拿法改写（≥20%）**:
  1. **`VersionSpec` dataclass**: 将上游 `NEXT_FULL_TAG/MAJOR/MINOR/SHORT_TAG` 四个散落 bash 变量收口为单一冻结 dataclass，`__post_init__` 校验 `YY.MM.PP` 格式，属性懒派生（`major/minor/patch/short_tag/pep440`），上游无任何结构化版本表示
  2. **`RunContextSource` 值对象**: 携带 `(context, source_label)` 二元组，使决策来源可审计——上游三段 `if/elif/else` 中来源信息只通过 `echo` 临时打印，无记录
  3. **`RunContextResolver` 三工厂方法**: `from_cli()/from_env()/default()` 各命名一路，替代上游三段无名 `if/elif/else`；`resolve()` 统一入口，优先级链可独立单元测试
  4. **`BranchNameStrategy` 两路分支**: `_main_branch()/release_branch()` 各自命名，`compute()` 静态方法返回 `BranchNameResult(branch_name, description)` 具名二元组，上游是内联 `if/elif` 无名赋值
  5. **`DocRefUpdatePolicy` 策略对象**: 封装 `Documentation references - context-aware` 的两路行为（`NOOP vs UPDATE_TO_RELEASE`），携带 `action/reason/target_branch`，上游 `if/elif` 内联无命名无可测试性；注：Walpurgis 无 cmake 体系，此策略仅文档化迁移，不执行 sed
  6. **`RunContextPolicy` 顶层策略对象**: 汇总所有决策（version + ctx_source + branch_result + doc_policy），替代上游四个散落 bash 变量，`dump()` 方法集中打印所有 echo 输出
  7. **全链路 8 处 `WALPURGIS_DEBUG=1` 断点**: 覆盖 VersionSpec 解析 → RunContextResolver 三段决策 → BranchNameStrategy 分支选择 → DocRefUpdatePolicy 决策路径 → RunContextPolicy 构建完成

- **自测结果**: `python upstream_version_updater.py` → 5 项断言全部通过，输出 `[ALL PASS] 958783c smoke test 全部通过`
