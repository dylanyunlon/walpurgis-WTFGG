...
## migrate 2b2a25e: [SKIP] remove thirdparty mmio + licenses — 纯第三方文件清理，无迁移价值

- **Upstream commit**: 2b2a25e (cugraph-gnn, BradReesWork, 2024-07-01)
- **Commit message**: `unneeded`
- **Upstream diff** (4 files deleted, 691 lines):
  - `thirdparty/LICENSES/LICENSE.boost` — Boost Software License 1.0 (23行)
  - `thirdparty/LICENSES/LICENSE.texas_state_university` — Texas State University BSD (24行)
  - `thirdparty/mmio/mmio.c` — NIST Matrix Market I/O C库 (511行)
  - `thirdparty/mmio/mmio.h` — 对应头文件 (133行)
- **迁移价值**: 无
- **原因**:
  1. 全部为第三方文件清理（`thirdparty/` 目录），与 cugraph-gnn 自身算法无关
  2. `mmio.c`/`mmio.h` 是 NIST 的 Matrix Market 文本格式 I/O 库（C语言），
     功能为读写 .mtx 稀疏矩阵文件；Walpurgis 使用 PyTorch `.pt`/`.npz` 数据格式，
     无 Matrix Market 使用场景
  3. 两个 LICENSE 文件均为被删除的 C 库对应的许可声明，无独立迁移价值
  4. commit message "unneeded" 明确表明是无用依赖清理
- **Knuth 审查**:
  1. diff 完整性: 4 个文件全量审查，均为 `thirdparty/` 下第三方资产，无 Python/CUDA/C++ 业务逻辑
  2. 用户角度 bug: 无运行时代码改动，零 bug 风险
  3. 内存/并发/性能安全: 不适用（无可执行代码）

---

## migrate 1295d2f: update branch (merge commit, Sep 2024)

- **Upstream commit**: 1295d2f (cugraph-gnn, Alexandria Barghi, 2024-09-30)
- **Commit message**: `update branch`
- **Commit type**: MERGE (f57ed88 ← 8a7de9e), 25 files changed, +178 -80

### 迁移分析

| 上游文件 | 类别 | walpurgis 处理 |
|---|---|---|
| `.pre-commit-config.yaml` | CI tooling | SKIP |
| `ci/release/update-version.sh` | CI/release | SKIP |
| `conda/environments/*.yaml` | conda deps | SKIP |
| `conda/recipes/*/meta.yaml` | conda recipes | SKIP |
| `cpp/CMakeLists.txt` | build system | SKIP |
| `cpp/Doxyfile` | docs | SKIP |
| `dependencies.yaml` | dep management | SKIP |
| `python/*/pyproject.toml` | packaging | SKIP |
| `python/pylibwholegraph/CMakeLists.txt` | build system | SKIP |
| `cpp/src/wholememory/communicator.cpp` | 换行符修复 | SKIP |
| `python/cugraph-dgl/nn/conv/base.py` | PEP 8 格式化 | **迁移** → `tensor/sparse_graph.py` |
| `python/cugraph-dgl/dataloading/neighbor_sampler.py` | `ds.get_reader()` API 修复 | 已在 `sampler/sampler.py` + `sampler/io/reader.py` 中以 `BufferedSampleReader` 实现同等语义，无需额外改动 |
| `python/cugraph-dgl/graph.py` | `print(u,)` 调试残留 | 已在 `tensor/embedding_view.py` 以 `WALPURGIS_DEBUG` 断点实现，SKIP 残留语句 |
| `python/cugraph-pyg/data/cugraph_store.py` + `dask_graph_store.py` | `type() != ` → `is not` | walpurgis 无对应 CuGraphTensorAttr 实现，SKIP |
| `python/cugraph-pyg/tests/loader/test_neighbor_loader_mg.py` | 移除 `@pytest.mark.skip(reason="deleteme")` | walpurgis 无对应多 GPU loader 测试，SKIP |
| `python/cugraph-dgl/tests/dataloading/test_dataloader.py` + `test_dataloader_mg.py` | biased sampling + `prob_attr` 测试 | **迁移** → `tests/sampler/test_distributed_sampler.py` |

### 实际改动

1. **`tensor/sparse_graph.py`** — PEP 8 E225 修复：f-string 内 `size[0]+1` → `size[0] + 1`，`size[1]+1` → `size[1] + 1`（对应上游 `base.py` 格式化修复）

2. **`tests/sampler/test_distributed_sampler.py`** — 新增 `test_biased_homogeneous_sampler_creates_and_runs`：
   - 对应上游 `test_dataloader_biased_homogeneous`（prob_attr="wgt" biased 采样）
   - 迁移为直接测试 `DistributedNeighborSampler(biased=True, weight_array=wgt)`
   - 图结构与上游完全一致（src=[1..8]→dst=[0..1], wgt=[1,1,2,0,0,0,2,1]）
   - 新增 `_BiasedSamplingInspector` dataclass 封装验证逻辑（20% 改写）
   - 全链路 WALPURGIS_DEBUG 断点

### SKIP 条目

- CI/release/conda/cmake/docs 全部 SKIP（walpurgis 不依赖这些上游工具链）
- `communicator.cpp` 换行符 SKIP（C++ 文件无对应）
- `cugraph_store.py` / `dask_graph_store.py` 的 `is not` 修复 SKIP（walpurgis 不实现 CuGraphTensorAttr）
- `test_neighbor_loader_mg.py` skip mark 移除 SKIP（walpurgis 无多 GPU loader 测试）
...
## migrate 2cde6cc: [SKIP] Build and test with CUDA 12.9.0 — 13 个文件全为 `.devcontainer/`、`.github/workflows/`、`conda/environments/`、`dependencies.yaml` 的 CUDA 12.8→12.9 版本号替换，纯 CI/构建配置，无 Python 源码变更，Walpurgis 无 GitHub Actions CI 体系，无迁移价值

## migrate 131d8ba: [SKIP] Add support for Python 3.13 — 纯CI/Python版本矩阵配置，无迁移价值

- **Upstream commit**: 131d8ba (cugraph-gnn, Gil Forsyth, 2025-05-07, PR #197)
- **跳过原因**: 7个文件全为CI workflow + dependencies.yaml + pyproject.toml版本矩阵调整：将 shared-workflows 引用从 `@branch-25.06` 切换至 `@python-3.13`，并在 cugraph-pyg/pylibwholegraph 的 pyproject.toml 中新增 `Programming Language :: Python :: 3.13` classifier。Walpurgis 无 GitHub Actions CI 体系、无 RAPIDS shared-workflows 依赖、无 conda 版本矩阵，受影响的所有文件在 Walpurgis 仓库中均不存在。

## migrate 8b67671: [SKIP] Update to clang 20 — 纯 CI 工具链 + 代码格式化

- **Upstream commit**: 8b67671 (cugraph-gnn, Bradley Dice, 2025-05-14, PR #205)
- **Commit message**: `Update to clang 20 (#205)`
- **Upstream diff** (4 files changed, 10 insertions, 12 deletions):
  - `.pre-commit-config.yaml` — clang-format hook: `rev: v16.0.6` → `rev: v20.1.4`
  - `cpp/src/wholememory/communicator.cpp` — 版权年 2019-2024 → 2019-2025；`struct stat statbuf {};` → `struct stat statbuf{};`（空格格式化）
  - `cpp/src/wholememory/file_io.cpp` — 同上版权年 + 两处 `statbuf {}` → `statbuf{}`
  - `cpp/src/wholememory/memory_handle.cpp` — 同上版权年 + 三处结构体初始化空格格式化

- **跳过原因**:
  1. `.pre-commit-config.yaml` 为 clang-format 版本升级，纯 CI/工具链配置，Walpurgis 无 `.pre-commit-config.yaml`
  2. 三个 C++ 文件变更全部为：(a) 版权年份文字更新，(b) `struct T var {};` → `struct T var{};` 的空格格式化——均由 clang-format v20 风格规则驱动，无任何算法/API/行为变化
  3. Walpurgis 无 C++ 编译体系，即使迁移这些格式变更也无法编译运行

- **Knuth 审查**:
  1. diff 完整性: 4 个文件全量审查，变更均为工具链版本号与空白字符，无 Python/算法代码
  2. 用户角度 bug: 无运行时语义改变；`statbuf{}` vs `statbuf {}` 在 C++ 中等价（值初始化语义相同）
  3. 内存/并发/性能安全: 不适用（无可执行逻辑变更）

## migrate 4fdc6b8: [SKIP] Remove CUDA 11 devcontainers and update CI scripts

- **Upstream commit**: 4fdc6b8 (cugraph-gnn, Bradley Dice, 2025-05-29, PR #218)
- **Commit message**: `Remove CUDA 11 devcontainers and update CI scripts (#218)`
- **Files changed**: `.devcontainer/cuda11.8-conda/devcontainer.json`, `.devcontainer/cuda11.8-pip/devcontainer.json`, `.github/workflows/pr.yaml`, `.github/workflows/test.yaml`, `ci/test_wheel_pylibwholegraph.sh`
- **跳过原因**: 全部为 CI/devcontainer 基础设施文件。删除 CUDA 11.8 devcontainer 配置（conda/pip 两个），更新 CI workflow 和测试脚本移除 CUDA 11 矩阵项。Walpurgis 无 GitHub Actions CI 体系、无 devcontainer 配置、无 RAPIDS CI 脚本，受影响的所有文件在 Walpurgis 仓库中均不存在。无任何 Python 源码、算法、API 或运行时代码变更，零迁移价值。

## migrate 7b07cf0: [SKIP] remove dgl text — README 文档清理，无迁移价值

- **Upstream commit**: 7b07cf0 (cugraph-gnn, Alexandria Barghi, 2025-06-09)
- **Commit message**: `remove dgl text`
- **Upstream diff** (1 file changed, 5 deletions, 0 insertions):
  - `README.md` — 删除 5 行 Git 合并冲突残留文本：
    ```
    <<<<<<< HEAD
    =======
    ...
    ** cuGraph-DGL is slated for removal after release 25.06.  We strongly recommend migrating to cuGraph-PyG.
    ...
    ```
- **迁移价值**: 无
- **原因**: 纯 README.md 文档清理，删除上游仓库未解决的 Git 合并冲突标记（`<<<<<<<`/`=======`/`>>>>>>>`）以及 cuGraph-DGL 废弃公告文本。Walpurgis 有独立 README，无对应合并冲突残留，cuGraph-DGL 废弃通知已在前序 `dgl_deprecation.py` 迁移中以代码形式体现。无任何 Python 源码、算法、API 或运行时代码变更，零迁移价值。
- **Knuth 审查**:
  1. diff 完整性: 仅 README.md 5 行删除，无 .py/.cu/.cpp 文件改动，全量审查
  2. 用户角度 bug: 无运行时代码变更，零风险
  3. 内存/并发/性能: 不适用
...
- **Commit message**: `remove dgl text`
- **Upstream diff** (1 file changed, 5 deletions, 0 insertions):
  - `README.md` — 删除 5 行 Git 合并冲突残留文本（`<<<<<<< HEAD` / `=======` / `>>>>>>> origin/branch-25.06`）以及 cuGraph-DGL 废弃公告
- **迁移价值**: 无
- **原因**: 纯 README.md 文档清理，删除未解决的 Git 合并冲突标记。Walpurgis 无对应合并冲突残留，cuGraph-DGL 废弃通知已在前序 `dgl_deprecation.py` 迁移中以代码形式体现。无任何源码/算法/API/运行时变更，零迁移价值。
