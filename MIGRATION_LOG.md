...
## migrate f251cce: [SKIP] Update `RAPIDS_BRANCH`, codify changes in `update-version.sh` (#322) — 纯 CI release 脚本

- **上游 PR**: https://github.com/rapidsai/cugraph-gnn/pull/322
- **作者**: Kyle Edwards (KyleFromNVIDIA)
- **变更文件**: `ci/release/update-version.sh`（1 file changed, 1 insertion）
- **变更内容**: 在版本更新脚本末尾追加 `echo "branch-${NEXT_SHORT_TAG}" > RAPIDS_BRANCH`，将 RAPIDS_BRANCH 文件写入自动化

- **SKIP 理由**:
  - 变更文件为 `ci/release/update-version.sh`，属于 RAPIDS CI/release 自动化流程专用脚本
  - Walpurgis-WTFGG 无 RAPIDS release 流水线，无 `RAPIDS_BRANCH` 文件概念，无对应 CI 体系
  - 纯 CI/release 基础设施变更，与 Python GNN 源码、数据加载、训练逻辑完全无关

---

## migrate a4d6bc2: [SKIP] Quote head_rev in conda recipes (#204) — conda recipe YAML 格式修复，Walpurgis 无 conda 构建体系

- **Upstream commit**: a4d6bc2be5dd3c5ba417d28a71b711334c002592 (cugraph-gnn, Bradley Dice, 2025-05-13, PR #204)
- **Commit message**: `Quote head_rev in conda recipes (#204)`
- **Upstream diff** (4 files changed, 4 insertions(+), 4 deletions(-)):
  ...
  - 每个文件均将 `head_rev: ${{ git.head_rev(".")[:8] }}` 改为带单引号形式，防止 YAML 解析器将前导零 SHA（如 `01234567`）误解为八进制/整数，导致 conda 构建产物包名截断。xref: rapidsai/build-planning#176
- **CI/conda/merge → SKIP**:
  - `conda/recipes/*/recipe.yaml` — RAPIDS rattler-build conda recipe，Walpurgis 无 conda 构建/打包体系
- **迁移位置**: `src/walpurgis/core/conda_recipe_revision.py` — 新建
- **鲁迅拿法改写（≥20%）**:
  1. `HeadRevFormat` 枚举：将\"带引号/不带引号\"两种格式显式化，上游只有 4 行 YAML 字符串修改，无任何抽象
  2. `RecipeRevisionSpec` dataclass：封装包名+SHA字符串+引号状态，含 `has_leading_zero_risk()` 前导零风险检测和 `safe_yaml_value()` 安全值生成，上游无结构化表示
  3. `RevisionGuard` dataclass：运行时守卫，`validate()` 检测未加引号且有风险的 spec，strict/宽松两模式，上游无任何防御层
  4. `RecipeRevisionAudit` 类：程序化扫描 recipe.yaml 文本，`scan()` 发现未加引号行，`apply_fix()` 批量修复，上游无审计工具（纯手工 grep+编辑）
  5. `WalpurgisRevisionEnv` dataclass：通过 subprocess 获取当前 git HEAD 前8位，`dump()` 打印快照，上游仅在 rattler-build 构建期通过内置函数获取
  6. `simulate_head_rev_quote()` 函数：演示加引号前后 YAML 解析差异（PyYAML 1.1 行为），上游无诊断工具
  7. 全链路 `WALPURGIS_DEBUG=1` 断点（6 处）：模块加载、前导零检测、守卫校验、审计扫描、修复应用、环境快照
- **自测结果**: 15 项全部 [PASS]
