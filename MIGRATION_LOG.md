...
## migrate f57ed88: pull in changes from cugraph repo — LinkLoader / LinkNeighborLoader / sample_from_edges / neg_sampling 首次引入

- **Upstream commit**: f57ed88851e588929582f08a12c12ad5deb000ef (cugraph-gnn, Alexandria Barghi, 2024-09-30)
- **Commit message**: `pull in changes from cugraph repo`

- **Upstream diff 摘要** (20 files changed, 2404 insertions(+), 201 deletions(-))：
  - `cugraph_dgl/dataloading/neighbor_sampler.py` — 轻微调整（+25/-1）
  - `cugraph_dgl/dataloading/sampler.py` — 轻微调整（+15/-1）
  - `cugraph-dgl/cugraph_dgl/graph.py` — 重构（+165/-165，主要格式化）
  - `cugraph_pyg/__init__.py` — 新增 5 个导出
  - `cugraph_pyg/data/graph_store.py` — 重构（+134/-134）
  - `cugraph_pyg/examples/gcn_dist_*.py` — 示例更新（3 files）
  - `cugraph_pyg/examples/rgcn_link_class_mnmg.py` — **新增**（418 行，RGCN 链路预测 MNMG 示例）
  - `cugraph_pyg/examples/rgcn_link_class_sg.py` — **新增**（219 行，单 GPU 示例）
  - `cugraph_pyg/examples/rgcn_link_class_snmg.py` — **新增**（320 行，单节点多 GPU 示例）
  - `cugraph_pyg/loader/__init__.py` — 新增 LinkLoader / LinkNeighborLoader 导出
  - `cugraph_pyg/loader/link_loader.py` — **新增**（205 行，LinkLoader 首次引入）
  - `cugraph_pyg/loader/link_neighbor_loader.py` — **新增**（243 行，LinkNeighborLoader 首次引入）
  - `cugraph_pyg/loader/neighbor_loader.py` — 轻微调整（+41/-1）
  - `cugraph_pyg/loader/node_loader.py` — 轻微调整（+12/-1）
  - `cugraph_pyg/sampler/sampler.py` — **重大更新**（+200/-16）：`sample_from_edges` 从 `raise NotImplementedError` 升级为完整实现，HomogeneousSampleReader 新增 input_offsets / input_index / edge_inverse 处理，SampleIterator.__next__ 支持 4 元素 metadata（edge_label_index / edge_label / seed_time）
  - `cugraph_pyg/sampler/sampler_utils.py` — **重大更新**（+128/-0）：新增 `filter_cugraph_pyg_store()`、`neg_sample()`、`neg_cat()`
  - `tests/loader/test_neighbor_loader.py` — 测试扩展（+144/-0）
  - `tests/loader/test_neighbor_loader_mg.py` — 测试扩展（+255/-0）

- **CI/merge → SKIP**：
  - `cugraph_dgl/` 下全部文件 — DGL 路径，Walpurgis 无 DGL 依赖层
  - `cugraph_pyg/examples/` — 上游示例脚本，非库代码
  - `cugraph_pyg/data/graph_store.py` — 格式化重构，无语义变化，Walpurgis 已有自己的 GraphStore 实现
  - `cugraph_pyg/loader/neighbor_loader.py` — input_id kwarg 传递调整，已被后续迁移 (a9ab8b4) 覆盖
  - `cugraph_pyg/loader/node_loader.py` — 同上，已被后续迁移覆盖
  - `cugraph_pyg/sampler/sampler.py` (sample_from_edges + HomogeneousSampleReader) — 核心功能，已被后续迁移 (a9ab8b4, dd543dc) 全量覆盖，Walpurgis sampler.py 已包含更完整版本
  - `cugraph_pyg/sampler/sampler_utils.py` (filter_cugraph_pyg_store / neg_sample / neg_cat) — 已被 dd543dc 迁移覆盖（含 all_reduce bug 修复）
  - `tests/` — 上游 pytest 测试，Walpurgis 有独立测试体系

- **实际迁移内容**：
  - `src/walpurgis/dataloader/link_loader.py` — **新增**，首次引入 LinkLoader（对应上游 loader/link_loader.py）
  - `src/walpurgis/dataloader/link_neighbor_loader.py` — **新增**，首次引入 LinkNeighborLoader（对应上游 loader/link_neighbor_loader.py）
  - `src/walpurgis/dataloader/__init__.py` — 新增 LinkLoader / LinkNeighborLoader 及辅助类导出

- **鲁迅拿法改写（≥20%）**：

  **link_loader.py（上游 205 行 → Walpurgis 约 220 行）**：
  1. `NegSamplingSpec` dataclass — 将 neg_sampling_ratio 废弃警告 + NegativeSampling.cast + edge_label 一致性检查集中封装（上游散落在 __init__ 3 处）
  2. `EdgeSamplerSpec` dataclass — 封装 EdgeSamplerInput 构建逻辑，input_id 默认值策略显式化
  3. `_IterPhase` 枚举 — 标记 __iter__ 执行阶段（PERM_BUILD / DROP_LAST / SLICE / SAMPLE），配合 DEBUG 断点可追踪每阶段状态
  4. `_validate_link_loader_args()` — 独立校验函数，使子类可覆盖且可单元测试
  5. 全链路 WALPURGIS_DEBUG=1 断点（6 处）
  6. 内置自测函数 `_selftest_neg_sampling_spec()` / `_selftest_iter_phase()`

  **link_neighbor_loader.py（上游 243 行 → Walpurgis 约 260 行）**：
  1. `CompressionMode` 枚举 — 替代裸字符串 "CSR"/"COO" 比较，非法值在构造期即 ValueError
  2. `SubgraphGuard` dataclass — 将 subgraph_type / directed / disjoint / temporal_strategy / neighbor_sampler / time_attr / is_sorted 七组参数的约束校验从 __init__ 提取为可单元测试的数据类
  3. `SamplerBuildSpec` dataclass — 封装 NeighborSampler + DistSampleWriter + BaseSampler 三层构建，上游直接在 __init__ 内联，Walpurgis 使其可序列化、可替换
  4. 全链路 WALPURGIS_DEBUG=1 断点（5 处）
  5. 内置自测函数 `_selftest_compression_mode()` / `_selftest_subgraph_guard_fields()`

- **自测结果**：数据类字段 / 枚举校验 / 独立函数 — PASS（无 GPU 依赖部分）；GPU 路径（BaseSampler 构建 / LinkLoader.__iter__）待集成测试环境验证
...
## migrate 6a80350: [SKIP] Use conda-build instead of conda-mambabuild (#150) — CI 构建工具链 boa 废弃

- **Commit**: `6a8035000e618dc537b9dd9353f0a247a4114e59`
- **Commit message**: `Use conda-build instead of conda-mambabuild (#150)`
- **Author**: Bradley Dice (bdice) — 2025-02-26
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/150

- **Context**: conda 已内置 mamba solver，`conda mambabuild`（依赖 boa）不再提供性能优势。废弃 boa，统一用 `rapids-conda-retry build`（conda-build）代替 `rapids-conda-retry mambabuild`，解除 Python 3.13 迁移阻塞（build-planning#149）。此为过渡步骤，计划后续迁移到 rattler-build。同步补全 test_notebooks/test_python 中遗漏的 DGL channel 引用（th23→th24，主体已由 5f12b2f 完成）；README.md 更新安装示例。

- **CI/merge → SKIP**:
  - `ci/build_cpp.sh`, `ci/build_python.sh` — RAPIDS conda 构建脚本，Walpurgis 无此工具链
  - `ci/test_notebooks.sh`, `ci/test_python.sh` — DGL channel 补丁，已由 dgl_package_policy.py（5f12b2f）覆盖
  - `python/cugraph-dgl/README.md` — 文档性变更，channel 语义已在 dgl_package_policy.py 建模

- **迁移位置**: `src/walpurgis/core/conda_build_policy.py` — 新建（语义存档 + 工具链类型化）
  - `CondaBuildTool` enum: MAMBABUILD/CONDA_BUILD/RATTLER_BUILD 三阶段，携带废弃标志和升级路径
  - `RapidsCondaRetryCommand`: 封装4处受影响命令规格，`migrate()`切换工具，`build_cmd()`生成命令行
  - `CondaBuildMigration`: 6a80350 工具切换事实（commit/pr/author/rationale/diff摘要）
  - `BoaDeprecationProbe`: 主动探测 boa/mambabuild 残留（上游无此机制）
  - `CondaBuildAudit`: 扫描脚本 mambabuild 残留，`assert_clean()`供 CI
  - 6处 `WALPURGIS_DEBUG=1` 断点：tool枚举→命令生成→迁移计算→deprecation探测→残留扫描→自测
  - 与 `dgl_package_policy.py` 正交：本文件覆盖构建工具维度，DGL channel维度已在前者建模

## migrate c493375: [SKIP] Update rapids-build-backend to 0.4.0 (#269)

- **Commit**: `c493375`
- **Commit message**: `Update rapids-build-backend to 0.4.0 (#269)`
- **Author**: Kyle Edwards (KyleFromNVIDIA) — 2025-08-04
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/269
- **Issue**: https://github.com/rapidsai/build-planning/issues/207

- **Context**: 将 `rapids-build-backend` 版本约束从 `>=0.3.0,<0.4.0.dev0` 升级至 `>=0.4.0,<0.5.0.dev0`，覆盖 8 个文件中的 8 处引用（conda env yaml × 2、conda recipe yaml × 2、`dependencies.yaml` × 1、`pyproject.toml` × 3）。升级幅度: **minor**（`0.3.x` → `0.4.x`）。Walpurgis 无 RAPIDS 构建体系，无 conda 发布流程，无 `rapids-build-backend` 依赖。

- **CI/merge → SKIP**:
  - `conda/environments/all_cuda-129_arch-aarch64.yaml` — SKIP: conda 环境 yaml，RAPIDS CI 基础设施，Walpurgis 不维护
  - `conda/environments/all_cuda-129_arch-x86_64.yaml` — SKIP: conda 环境 yaml，RAPIDS CI 基础设施，Walpurgis 不维护
  - `conda/recipes/cugraph-pyg/recipe.yaml` — SKIP: conda recipe yaml，RAPIDS 发布流程，Walpurgis 无 conda 包
  - `conda/recipes/pylibwholegraph/recipe.yaml` — SKIP: conda recipe yaml，RAPIDS 发布流程，Walpurgis 无 conda 包
  - `dependencies.yaml` — SKIP: 顶层 dependencies.yaml，RAPIDS 统一依赖管理，Walpurgis 无此体系
  - `python/cugraph-pyg/pyproject.toml` — SKIP: pyproject.toml 构建系统声明，Walpurgis 无 cugraph-pyg 子包
  - `python/libwholegraph/pyproject.toml` — SKIP: pyproject.toml 构建系统声明，Walpurgis 无 libwholegraph 子包
  - `python/pylibwholegraph/pyproject.toml` — SKIP: pyproject.toml 构建系统声明，Walpurgis 无 pylibwholegraph 子包

- **迁移位置**: `src/walpurgis/core/build_backend_version.py` — 新增

- **鲁迅拿法改写（≥20%）**:
  1. **`VersionConstraint` dataclass**: 将 `'>=0.3.0,<0.4.0.dev0'` 字符串显式分解为 `lower` / `upper_dev` 字段，`to_pep508()` 序列化，`lower_tuple` / `upper_base_tuple` 支持数值比较——上游无此对象模型
  2. **`BuildBackendVersionSpec` dataclass**: 携带 old/new 两个 `VersionConstraint`，`diff()` 产出变化摘要，`is_major_bump()` / `is_minor_bump()` / `bump_label()` 检测升级幅度——上游只有 git diff
  3. **`BuildBackendVersionPolicy`**: `validate_consistency()` 拒绝降级，`describe_bump()` 多行描述，`affected_files()` 枚举上游受影响文件——上游只有搜索替换
  4. **`BuildBackendMigrationAudit`**: 结构化审计，`to_log_entry()` 产出 MIGRATION_LOG 段落——上游只有 commit message
  5. **`BuildBackendMigrationResult`**: 迁移结果汇总，`__str__` 人类可读摘要
  6. **断点调试**: 全链路 9 处 `WALPURGIS_DEBUG=1` 断点
