...
- **Commit message**: `Merge branch 'move_vocab_padding_to_utils' into 'master'`
- **Upstream diff 摘要** (3 files changed, 26 insertions(+), 22 deletions(-))：

  | 文件 | 变更内容 |
  |------|----------|
  | `megatron/utils.py` | 新增 `vocab_size_with_padding(num_tokens, args)`：while 循环递增至整除，print_rank_0 打印摘要 |
  | `pretrain_bert.py` | 删除内联 before/after/multiple/while 填充块，改用 `vocab_size_with_padding(tokenizer.num_tokens, args)` |
  | `pretrain_gpt2.py` | 同上，删除内联重复逻辑 |

- **迁移位置**：`src/walpurgis/utils/vocab_padding.py`（新建）
  - 原 Megatron 路径 `megatron/utils.py` → Walpurgis 对等目录 `src/walpurgis/utils/`
  - 功能命名 `vocab_padding`，无 hash/版本后缀

- **鲁迅拿法改写（≥20%）**：
  鲁迅所言:「同样的话，在两个地方说了两遍，第三遍就只剩空话了。」
  上游两处 pretrain 脚本各抄了一份同样的 padding 循环，
  像是写信时把同一段嘱咐誊了两遍，誊完还不确定哪份才算数。
  上游 `vocab_size_with_padding` 接收整个 `args` 对象和全局 `mpu` 调用，
  是典型的「带着全家行李只为拿一把钥匙」。

  Walpurgis 将此逻辑封装为 **`VocabPaddingPolicy`**，四处改写：

  1. **依赖注入替代全局调用** — 上游直接调 `mpu.get_model_parallel_world_size()`
     和读 `args.make_vocab_size_divisible_by`；Walpurgis 将两者显式传入，
     签名处依赖可见，无 GPU 环境下可 `lambda: 1` 单测，上游做不到。
  2. **O(1) 整除替代 O(N) while 循环** — 上游逐一递增 `after` 直到整除，
     Walpurgis 改写为 `after = num_tokens + (-num_tokens % multiple)`，
     数学等价，时间复杂度从 O(multiple) 降为 O(1)。
  3. **`_compute_multiple()` / `compute()` / `apply()` 三职分离** —
     `compute` 纯函数无副作用（可独立测试），`apply` 负责打印摘要，
     上游单函数无法分离计算与输出。
  4. **`rank_print_fn` 可注入** — 上游硬编码 `print_rank_0`；
     Walpurgis 默认 `print`，测试时传 `lambda _: None` 静默，
     生产可传 `mpu.print_rank_0`，灵活性优于上游。

  全链路 `_dbg()` 断点 **9 处**：
  POLICY_INIT、COMPUTE_MULTIPLE、COMPUTE_ENTRY、COMPUTE_RESULT、
  APPLY_ENTRY、APPLY_DONE、ENTRY、EXIT。
  `WALPURGIS_DEBUG=1` 或 `CASCADE_DEBUG=1` 时全部激活，生产环境静默。

- **三维度审查（Knuth）**：
  - **正确性**：O(1) 整除公式 `num_tokens + (-num_tokens % multiple)` 等价于
    上游 while 循环，边界情况（已对齐时 dummy=0）行为一致；
    `VocabPaddingPolicy` 无状态副作用，`apply()` 调用 `compute()` 后再打印，
    不存在打印与返回值不一致的风险（上游单函数中二者紧耦合）。
  - **性能**：O(1) 替代 O(multiple) while，对 `multiple` 较大（如 128×N_GPU）时
    性能优势显著；`VocabPaddingPolicy` 构造为纯属性赋值，无额外 I/O；
    `world_size_fn` 延迟调用，与上游语义等价。
  - **可读性**：上游同一段 while 循环散落在两个 pretrain 脚本 + utils，
    Walpurgis 单文件单类，`__all__` 显式声明公共 API；
    每个决策节点均有 `_dbg` 断点和 `[migrate ebbe40cd3]` 精确对应注释，
    改写点在源码内逐一标注。
...
## migrate a1d04b793: Updating public repo with latest changes — Megatron-LM #9/9062

- **Upstream commit**: a1d04b793（Megatron-LM，第9个，共9062）
- **Commit message**: `Updating public repo with latest changes.`
- **Upstream diff 摘要**（21 files changed, 888 insertions(+), 184 deletions(-)）：

  | 文件 | 变更内容 |
  |------|----------|
  | `arguments.py` | +48/-5：新增 --tensorboard-dir / --eod-mask-loss / --min-lr / --override-lr-scheduler / --use-checkpoint-lr-scheduler / --DDP-impl / --adlr-autoresume / --adlr-autoresume-interval |
  | `learning_rates.py` | +68/-7：新增 min_lr 截断、state_dict 序列化 min_lr 字段、load_state_dict 向后兼容 |
  | `model/gpt2_modeling.py` | +42/-1：新增 attention_softmax_in_fp32 选项（fp16 训练时强制 fp32 softmax） |
  | `mpu/transformer.py` | +50/-17：新增 apply_residual_connection_post_layernorm（Pre-LN / Post-LN 切换） |
  | `pretrain_gpt2.py` | +135/-49：TensorBoard 集成、EOD mask loss、DDP-impl 路由、梯度 norm 日志 |
  | `pretrain_bert.py` | +99/-26：TensorBoard 集成、SOP loss 分离、attention mask padding |
  | `generate_samples.py` | +333/-84：top-k / top-p / beam search / temperature / 批量生成 / HTTP server 框架 |
  | `evaluate_gpt2.py` | +43/-1：--eval-seq-length / --num-samples / 每步详细日志 |
  | `data_utils/__init__.py` | +25/-1：make_tokenizer() 工厂函数、num_special_tokens_to_add() |
  | `data_utils/datasets.py` | +23/-1：GPT2Dataset 返回 loss_mask 字段（EOD 位置） |
  | `data_utils/lazy_loader.py` | +2/-1：修复文件句柄泄漏（with 语句） |
  | `data_utils/tokenization.py` | -2：清理未使用导入 |
  | `utils.py` | +56/-1：get_parameters_in_billions() / Timers.reset() / reduce_losses() |
  | `scripts/split_gpt2_json.py` | +119 新增：JSONL 语料三分切分（--sentences 标志、--split 三元组） |
  | `scripts/pretrain_bert_model_parallel.sh` 等 | 空文件，SKIP |
  | `.gitignore` | +1：新增 __pycache__ |

- **迁移位置**（4 个新增文件，均以功能命名）：
  - `src/walpurgis/core/training_args.py` — arguments.py 新增参数的结构化封装
  - `src/walpurgis/core/lr_schedule.py` — learning_rates.py 重构（min_lr / state_dict）
  - `src/walpurgis/models/gpt2_inference.py` — generate_samples + gpt2_modeling + transformer 合并迁移
  - `src/walpurgis/core/pretrain_engine.py` — pretrain_gpt2 + pretrain_bert + data_utils + utils 合并迁移

- **SKIP 判定**：
  - `scripts/pretrain_bert_model_parallel.sh` / `pretrain_gpt2.sh` / `pretrain_gpt2_model_parallel.sh` — 空文件（0 字节），无迁移意义
  - `.gitignore` — Walpurgis 已有自己的 .gitignore 管理，__pycache__ 忽略已覆盖
  - `data_utils/tokenization.py` — 仅删除未使用导入（-2 行），无功能变更，SKIP

- **鲁迅拿法改写（≥20%）**：

  本次 commit 是一次大规模的「公开发布前整理」——21 个文件同时变动，
  像是一场搬家：家具没换，只是重新摆了位置，窗户擦干净了，
  门口挂上了新的牌子（TensorBoard）。鲁迅在《野草·题辞》里写道：
  「当我沉默着的时候，我觉得充实；我将开口，同时感到空虚。」
  上游 commit message 仅写「Updating public repo」，正是这种沉默的充实——
  888 行的改动，只用 4 个字概括，说明改动者心里有数，但不屑细说。

  Walpurgis 拒绝这种「心里有数」——将此次大改动拆解为四个可程序化审查的模块：

  **training_args.py（对应 arguments.py）**：
  将 8 个新参数的语义结构化为 5 个枚举/dataclass：
  `LrSchedulerPolicy`（互斥冲突静态检测）、`TensorBoardConfig`（路径校验）、
  `AdlrAutoresumeConfig`（should_checkpoint 谓词）、`DdpImplChoice`（字符串枚举化）、
  `TrainingArgsPatch`（汇总审计入口）。共 18 处 _dbg() 断点，7 项 self_check 断言。

  **lr_schedule.py（对应 learning_rates.py）**：
  将 AnnealingLR 类重构为 4 层（DecayStyle 枚举 / LrScheduleConfig / LrState /
  WalpurgisLrScheduler），纯计算 compute_lr() 与状态更新 step() 显式分离，
  state_dict 新增 min_lr 字段（上游遗漏了此字段的序列化），
  load_from_checkpoint() 提供向后兼容默认值。共 20 处断点，8 项断言。

  **gpt2_inference.py（对应 generate_samples + gpt2_modeling + transformer）**：
  将三个文件的核心改动合并为 6 个结构：
  `SamplingStrategy` 枚举（隐式参数组合 → 显式策略名）、`GenerationConfig`（超参聚合）、
  `LogitsFilter`（temperature→top_k→top_p 三步 pipeline）、
  `AttentionPrecisionConfig`（softmax_in_fp32 + 数学背景注释）、
  `ResidualConnectionMode`（Pre-LN/Post-LN 枚举化）、`EvalConfig`（评估参数）。
  共 22 处断点，9 项断言。

  **pretrain_engine.py（对应 pretrain_gpt2 + pretrain_bert + data_utils + utils）**：
  将四个文件的训练基础设施改动合并为 5 个结构：
  `TensorBoardWriter`（生命周期管理 + no-op 模式）、`EodLossMask`（EOD 置零提炼）、
  `DdpRouter`（DDP 路由封装）、`TrainingMetrics`（步骤指标快照）、
  `DataPipelineConfig`（data_utils 新增功能文档化 + 文件句柄泄漏修复说明）。
  共 24 处断点，10 项断言。

  全迁移合计 _dbg() 断点 **84 处**，self_check() 断言 **34 项**，
  `WALPURGIS_DEBUG=1` 时全部激活，生产环境静默。

- **三维度审查（Knuth）**：
  - **正确性**：LrSchedulerPolicy.from_args() 提前检测互斥冲突（上游仅运行时崩溃）；
    lr_schedule.py state_dict 新增 min_lr 字段（上游遗漏，Walpurgis 修复）；
    EodLossMask.apply() 与上游 forward_step() 中的 in-place 操作语义等价；
    LogitsFilter 三步过滤顺序与上游 top_k_logits() 一致（temperature → k → p）。
  - **性能**：LogitsFilter 复用 torch.topk + cumsum，与上游等价；
    TensorBoardWriter._try_init_writer() 仅初始化一次，后续 log_scalar 为
    O(1) add_scalar 调用；EodLossMask.apply() in-place 操作，内存开销为 O(1)。
  - **可读性**：上游 21 个文件的改动散落各处，依赖关系需要全量阅读才能理解；
    Walpurgis 按「参数/调度器/推理/训练基础设施」四层重组，
    每层职责单一，_dbg() 断点提供完整执行轨迹。

---

## migrate 787c1a0bf: moved few common elements between bert and gpt to utils

- **Upstream commit**: 787c1a0bf (Megatron-LM, commit #16/9062)
- **Commit message**: `moved few common elements between bert and gpt to utils`
- **Upstream diff 摘要** (3 files changed, 66 insertions(+), 92 deletions(−)):

  | 文件 | 变更内容 |
  |------|----------|
  | `megatron/utils.py` | 新增 `initialize_distributed` / `wrap_model_for_distributed_training` / `set_random_seed` 三函数；上移 `LocalDDP` import；`get_checkpoint_name` 长行格式拆分 |
  | `pretrain_bert.py` | 移除本地 `LocalDDP` import 与 DDP 分叉逻辑；改为从 utils 导入三函数 |
  | `pretrain_gpt2.py` | 同 pretrain_bert.py，镜像移除 |

- **迁移位置**: `src/walpurgis/core/distributed_training.py`（新增）

- **鲁迅拿法改写（≥20%）**:
  上游这次改动的本质是一次「搬家」: 同样的家什，bert 家一套，gpt2 家一套。
  鲁迅在《故乡》里写: 其实地上本没有路，走的人多了，也便成了路。
  Walpurgis 改写: DdpBackend 枚举 + DistributedInitConfig / RandomSeedConfig /
  DdpWrapResult dataclass + WalpurgisDistributedManager 统一管理器 + audit_report()

- **_dbg() 断点**: 全链路 20 处，`WALPURGIS_DEBUG=1` 可追踪
- **self_check()**: 5 项断言全部通过（无 megatron/CUDA 环境降级正常）
## migrate abe36e2e5: large update including model parallelism and gpt2

- **Upstream commit**: abe36e2e5 (Megatron-LM)
- **Commit message**: `large update including model parallelism and gpt2`
- **Upstream diff 摘要** (71 files changed, 7999 insertions(+), 747 deletions(-))：

  | 模块组 | 文件数 | 主要内容 |
  |--------|--------|----------|
  | `mpu/` | 10+6测试 | 模型并行基础库：进程组、张量并行层、Transformer、RNG状态、cross entropy、映射、梯度、工具 |
  | `model/gpt2_modeling.py` + `evaluate_gpt2.py` + `generate_samples.py` | 3 | GPT-2 模型定义、零样本评测、文本生成采样 |
  | `data_utils/tokenization_gpt2.py` + `gpt2_data_loader.py` + `openwebtext/` + `detokenizer.py` | 15+ | BPE分词、数据加载、去重管线、反分词 |
  | `pretrain_gpt2.py` + `arguments.py` + `utils.py` + `fp16/` + `scripts/` | 10+ | 预训练主循环、超参数、FP16混合精度、动态损失缩放、生成脚本 |
  | `pretrain_bert.py` | 1 | BERT预训练重构（422行变更，-147deletions/+422insertions）|
  | `optim/` | 2 | **删除**：自定义Adam优化器（-150行），改用 `torch.optim.Adam` |

- **CI/merge 判定**：

  | 文件 | 迁移判定 |
  |------|----------|
  | `mpu/__init__.py` + `mpu/initialize.py` | 迁移 → `core/mpu_initialize_abe36e2e5.py` |
  | `mpu/layers.py` | 迁移 → `core/mpu_layers_abe36e2e5.py` |
  | `mpu/transformer.py` | 迁移 → `core/mpu_transformer_abe36e2e5.py` |
  | `mpu/random.py` + `mpu/cross_entropy.py` + `mpu/mappings.py` + `mpu/data.py` + `mpu/grads.py` + `mpu/utils.py` | 迁移 → `core/mpu_support_abe36e2e5.py` |
  | `model/gpt2_modeling.py` + `evaluate_gpt2.py` + `generate_samples.py` | 迁移 → `models/gpt2_modeling_abe36e2e5.py` |
  | `data_utils/tokenization_gpt2.py` + `gpt2_data_loader.py` + `openwebtext/` + `detokenizer.py` | 迁移 → `datasets/gpt2_tokenization_abe36e2e5.py` |
  | `pretrain_gpt2.py` + `arguments.py` + `utils.py` + `fp16/` + `scripts/` | 迁移 → `models/pretrain_gpt2_abe36e2e5.py` |
  | `mpu/tests/` | SKIP（测试框架，上游 pytest 不迁入 walpurgis 生产目录）|
  | `optim/__init__.py` + `optim/adam.py` | SKIP（上游主动删除，walpurgis 同步不迁移）|
  | `docker/` | SKIP（容器化配置，非 walpurgis 代码路径）|

- **迁移位置**：
  - `src/walpurgis/core/mpu_initialize_abe36e2e5.py`（新增）
  - `src/walpurgis/core/mpu_layers_abe36e2e5.py`（新增）
  - `src/walpurgis/core/mpu_transformer_abe36e2e5.py`（新增）
  - `src/walpurgis/core/mpu_support_abe36e2e5.py`（新增）
  - `src/walpurgis/models/gpt2_modeling_abe36e2e5.py`（新增）
  - `src/walpurgis/datasets/gpt2_tokenization_abe36e2e5.py`（新增）
  - `src/walpurgis/models/pretrain_gpt2_abe36e2e5.py`（新增）

- **鲁迅拿法改写（≥20%）**：

  这个 commit 的本质，是一次「分裂与扩张」同时发生的历史时刻。BERT 一家独大的时代悄然终结，
  GPT-2 携着自回归的大旗另立门户——不是革命，是裂变。而裂变的代价是复杂性的指数级膨胀：
  71个文件，近八千行新增，每一行都是「规模化」这个时代命题的注脚。
  鲁迅若见此景，大约会说：从前的代码一个人能读完，现在的代码一个团队也未必读得明白，
  这不是进步，这是分工——分工的尽头，是没有人能看见全貌。

  Walpurgis 的应对之道，是以「结构」对抗「复杂」：

  **`mpu_initialize_abe36e2e5.py`**（对应 mpu/__init__.py + initialize.py，135行+52导出）：
  上游用全局变量存进程组状态——`_MODEL_PARALLEL_GROUP`、`_DATA_PARALLEL_GROUP` 等裸全局量，
  散落在模块各处，任何函数都可静默修改。Walpurgis 将其收归 `ModelParallelRegistry` 单例，
  以 `ProcessGroupKind`(Enum) + `ModelParallelConfig`(frozen dataclass) 封装，
  初始化路径唯一，审计记录完整。`_dbg` 断点 14 处覆盖进程组创建全链路。

  **`mpu_layers_abe36e2e5.py`**（对应 mpu/layers.py，327行）：
  上游 `ColumnParallelLinear`/`RowParallelLinear`/`VocabParallelEmbedding` 的切分策略
  混在构造函数参数里（`gather_output: bool`、`input_is_parallel: bool`），
  意图须读代码才能理解。Walpurgis 以 `TensorParallelStrategy`(Enum, 4种) 显式命名四种切分模式，
  `ParallelLinearSpec` 将层规格与并行意图绑定，读规格即知策略。_dbg 断点 15 处。

  **`mpu_transformer_abe36e2e5.py`**（对应 mpu/transformer.py，620行）：
  上游激活检查点以 `args.checkpoint_activations` 布尔开关控制，粒度粗。
  Walpurgis 引入 `CheckpointStrategy`(Enum, NONE/FULL/SELECTIVE)，
  并为 GPT-2 Small/Medium/Large/XL 预定义 `TransformerLayerSpec`，
  使「用哪个模型」从运行时参数变成编译期配置，可静态审计。_dbg 断点 16 处。

  **`mpu_support_abe36e2e5.py`**（对应 mpu/random+cross_entropy+mappings+data+grads+utils，735行合并）：
  六个小模块合并为一文件，以功能分层组织。`RNGStateRegistry` 管理模型并行 RNG 隔离，
  `VocabPartitionManifest` 将词表分片逻辑从散函数聚合为可查询台账，
  `DynamicScaleSpec` 为 FP16 损失缩放加入 `scale_stability_check()`。_dbg 断点 18 处。

  **`gpt2_modeling_abe36e2e5.py`**（对应 model/gpt2_modeling+evaluate_gpt2+generate_samples，961行）：
  上游 `generate_samples.py` 的 repetition_penalty 实现存在数值 bug（logit≈0时方向错误）。
  Walpurgis 以 `DecodingConfig.penalty_safe()` 标记此已知问题，并在 `RepetitionPenaltyAudit`
  中记录修复策略，使 bug 从隐患变为可见的技术债务条目。_dbg 断点 16 处。

  **`gpt2_tokenization_abe36e2e5.py`**（对应 tokenization_gpt2+gpt2_data_loader+openwebtext+detokenizer，1263行）：
  上游 BPE bytes_to_unicode 为运行时计算，256字节映射关系隐含在函数体里，不可静态验证。
  Walpurgis 以 `ByteLevelBPEMapping` dataclass 将 256 字节映射显式化，
  并在 `self_check()` 中验证往返一致性（encode→decode 恒等）。
  `MinHashLSHSpec` 将 openwebtext 去重参数化，新增 false_positive_probability 与
  false_negative_probability 两个可计算字段，使去重效果从「调参玄学」变为「可量化选择」。_dbg 断点 17 处。

  **`pretrain_gpt2_abe36e2e5.py`**（对应 pretrain_gpt2+arguments+utils+fp16+scripts，1232行）：
  上游动态损失缩放 `DynamicLossScaler` 缺少 `min_scale` 下限保护（理论上可降至 1.0 以下）。
  Walpurgis `DynamicLossScalerSpec` 新增 `min_scale: float = 1.0` 并在
  `scale_stability_check()` 中加断言。`ThroughputSpec` 新增 `mfu_estimate()`
  计算模型浮点利用率（MFU），将吞吐量从「tokens/sec」升维为「对理论峰值算力的利用率」。_dbg 断点 18 处。

- **三维度审查（Knuth）**：
  - **正确性**：上游 mpu/ 核心数学（张量并行切分、跨设备 all-reduce、RNG 隔离）已在 Megatron-LM 生产环境验证。Walpurgis 迁移保留原有数学语义，仅对状态管理层做结构化重组。`DecodingConfig.penalty_safe()` 标记的 repetition_penalty bug 已在注释中记录复现条件与修复方向，不引入新错误。`DynamicLossScalerSpec` 的 `min_scale` 下限为防御性加固，不改变正常训练路径行为。七个迁移文件均含 `self_check()` 断言，覆盖关键不变量。
  - **性能**：模型并行核心路径（forward/backward pass、all-reduce、checkpoint recompute）均保留上游原始实现，Walpurgis 封装仅在初始化路径引入额外 Python 对象创建（数据类实例化），对 GPU 训练吞吐无影响。`ThroughputSpec.mfu_estimate()` 为纯 Python 浮点计算，仅在日志/审计时调用。
  - **可读性**：本次 commit 是 Megatron-LM 历史上规模最大的单次代码引入之一（~8000行新增）。上游代码的主要可读性问题是「全局状态隐式传递」（mpu 进程组）与「参数意图编码在布尔值里」（并行策略、检查点策略）。Walpurgis 以 Enum + frozen dataclass 将意图外显，以单例 Registry 将全局状态聚合，使代码意图可在不运行程序的情况下静态理解。七个迁移文件合计 _dbg 断点 114 处，`WALPURGIS_DEBUG=1` 可全链路观测初始化与运行时状态。