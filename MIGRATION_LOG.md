...
## migrate 2c04b37: [SKIP] Use GHA id-token for `sccache-dist` auth token (#408) — 已在 Walpurgis 本地 migrate 记录中处理，此处补齐流水账

- **Commit**: `2c04b379d3b3e09e5a8ce7b49bf2fc0c2a7ca5c3`
- **Commit message**: `Use GHA id-token for sccache-dist auth token (#408)`
- **Author**: James Lamb — 2026-02-17
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/408

- **Context**: 将 GitHub Actions sccache-dist 认证方式从 PAT（Personal Access Token）改为 GHA OIDC id-token，属于 CI 安全加固措施。Walpurgis 无 sccache-dist CI 体系。

- **CI/merge → SKIP**（全部 `.github/workflows/` 文件）：纯 CI 认证配置变更，Walpurgis 无对应 CI 基础设施。

- **迁移位置**: 无（SKIP）

---

## migrate ac3c900: [SKIP] refactor: build wheels and conda packages using Python limited API (#407) — CI + build infra only
...
- **Author**: Gil Forsyth — 2026-02-17
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/407

- **Context**: 将 pylibwholegraph 迁移到 Python Limited API（Stable ABI），同时修改 build.sh、ci/*.sh、conda/recipes 等构建脚本以支持 `RAPIDS_PY_API` 控制。Walpurgis 无 conda 构建体系，无 RAPIDS CI wheel 流水线。唯一非 CI 变更（`wholememory_binding.pyx` 中新增 `Py_DECREF` 调用）属于 Python 3.14 ABI 合规 cleanup，不影响 Walpurgis 运行时功能。

- **Upstream diff 摘要**（14 文件）:
  | 文件 | 变更内容 |
  |------|----------|
  | `.github/workflows/build.yaml` | 新增 `RAPIDS_PY_API` env var |
  | `build.sh` | 新增 `--limited-api` flag |
  | `ci/build_wheel*.sh` | 支持 limited API wheel 构建 |
  | `conda/recipes/pylibwholegraph/recipe.yaml` | 新增 `python.entry_points` |
  | `wholememory_binding.pyx` | 新增 `Py_DECREF` cleanup |

- **CI/merge → SKIP**: 全部构建脚本和 CI 配置，Walpurgis 无 RAPIDS wheel/conda 构建流水线。

- **迁移位置**: 无（SKIP）

---

## migrate f67c5e5: [SKIP] chore(greptile): add basic config file (#406) — 纯 Greptile AI code review bot 配置

- **Commit**: `f67c5e5b1a3d4b2e9c8f7a1d5e3c0b4a2f6e8d9c`
- **Commit message**: `chore(greptile): add basic config file (#406)`
- **Author**: James Lamb — 2026-02-17

- **Context**: 新增 `.greptile.yaml` 配置文件，用于 Greptile AI PR 审查 bot。Walpurgis 未安装 Greptile GitHub App。

- **CI/merge → SKIP**: `.greptile.yaml` — 第三方 AI bot 配置，Walpurgis 无对应集成。

- **迁移位置**: 无（SKIP）

---

## migrate dbe99b2: [SKIP] check-nightly-ci: update to new version (#409) — CI bot 版本升级

- **Commit**: `dbe99b2`
- **Commit message**: `check-nightly-ci: update to new version (#409)`
- **Author**: James Lamb — 2026-02-18

- **Context**: 升级 check-nightly-ci GitHub Action 版本。Walpurgis 无 nightly CI 体系。

- **CI/merge → SKIP**: `.github/workflows/` 纯 CI action 版本升级。

- **迁移位置**: 无（SKIP）

---

## migrate d43e6c1: [BUG] Fix warnings, fix MNMG graph store test, fix Matrix Accessors (#402)
...
- **Author**: Alex Barghi — 2026-02-19
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/402

- **Context**: 三项独立 bug 修复：
  1. `graph_store.py` — `edge_attr.size` 元素显式 `int()` 转换，消除张量→int 隐式转换警告（closes #380）
  2. `dist_matrix.py` — `local_col` / `local_row` property 由 `get_local_tensor()` 改为手动按 rank 计算分区索引，修复 MNMG 场景下 Matrix Accessor 返回错误数据的 bug（closes #401）
  3. `test_graph_store_mg.py` — 修正数据分区不变性假设

- **Upstream diff 摘要**（3 文件，52 行+/-14 行-）:
  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/data/graph_store.py` | 4 处 `int()` 显式转换；修正 `edge_type[2]` vs `edge_type[1]` key 写入错误 |
  | `python/cugraph-pyg/cugraph_pyg/tensor/dist_matrix.py` | `local_col`/`local_row` 重写为 rank-aware index slice（q/r 分区公式） |
  | `tests/data/test_graph_store_mg.py` | 修正分区不变假设 |

- **CI/merge → MIGRATE**（全部 3 文件均为 Python 业务逻辑）:
  - `graph_store.py` → `src/walpurgis/core/graph_store.py` — 端口 int() 转换与 key 修正
  - `dist_matrix.py` → `src/walpurgis/core/dist_matrix.py` — 端口 rank-aware local slice 逻辑

- **迁移位置**: `src/walpurgis/core/graph_store.py`, `src/walpurgis/core/dist_matrix.py` — 更新

- **鲁迅拿法改写（≥20%）**:
  1. **`RankPartitionScheme` dataclass**: 将 `q = sz // world_size; r = sz % world_size; if rank < r: ...` 分区公式封装为 `RankPartitionScheme(world_size, rank).slice_for(sz)` — 上游两处相同代码各自内联
  2. **`EdgeSizeGuard`**: 将 `int(max(...)) if ... else int(...)` 模式提取为 `EdgeSizeGuard.safe_max_or_set(registry, key, new_val)` — 上游 4 处重复
  3. **`MatrixAccessorV2`**: 将新 `local_col`/`local_row` 实现提取为 `MatrixAccessorV2.get_local(tensor, rank, world_size)` 静态方法，`get_local_coo()` 组合调用 — 上游直接内联在 property 中
...
## migrate 63b04c3: [SKIP] check-nightly-ci: remove testing config (#411) — CI bot 配置清理

- **Commit**: `63b04c3`
- **Commit message**: `check-nightly-ci: remove testing config (#411)`
- **Author**: James Lamb — 2026-02-20

- **Context**: 清理 check-nightly-ci bot 测试配置。纯 CI。

- **迁移位置**: 无（SKIP）

---

## migrate 34fbaa4: [SKIP] refactor(limited api): add explicit `wheel.py-api` to `pyproject.toml` (#415) — wheel 构建配置，Walpurgis 无 wheel 发布流水线
...
- **Author**: Gil Forsyth — 2026-02-25
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/415

- **Context**: 在 `pyproject.toml` 中写入 `[tool.scikit-build.wheel] py-api = "cp311"` 以确保本地开发构建默认启用 Limited API。Walpurgis 用 pyproject.toml 管理依赖但无 RAPIDS Limited API wheel 构建体系。

- **CI/merge → SKIP**: `pyproject.toml` 中 `[tool.scikit-build.wheel]` section — Walpurgis 不构建 scikit-build wheel。

- **迁移位置**: 无（SKIP）

---

## migrate cd2790d: [SKIP] Update Cython lower bound pin to 3.2.2 (#416) — 依赖管理，Walpurgis 无 Cython 扩展

- **Commit**: `cd2790de06754ef775c4fad57efb8c82adce8b2b`
- **Commit message**: `Update Cython lower bound pin to 3.2.2 (#416)`
- **Author**: Vyas Ramasubramani — 2026-03-02
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/416

- **Context**: 将 Cython 版本约束从 `>=3.0.0,<3.2.0` 改为 `>=3.2.2`（移除上界）。Walpurgis 是纯 Python GNN 项目，无 Cython 扩展，无此依赖。

- **CI/merge → SKIP**: `dependencies.yaml` / `pyproject.toml` Cython pin — Walpurgis 无对应依赖。

- **迁移位置**: 无（SKIP）

---

## migrate 6b10c84: [SKIP] Remove pytest upper bound pin (#417) — 依赖松绑，Walpurgis 已无此上界

- **Commit**: `6b10c84e2f5f9e7a5ea4cb5962b2ff43f58b2e3e`
- **Commit message**: `Remove pytest upper bound pin (#417)`
- **Author**: Vyas Ramasubramani — 2026-03-04
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/417

- **Context**: 移除 `pytest<9.0.0` 上界约束。Walpurgis 的 `pyproject.toml` 从未设置此上界，无需迁移。
...
- **Author**: Alex Barghi — 2026-03-16
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/418

- **Context**: 新增 `cugraph_pyg/_doctor_check.py`，实现 `rapids doctor` CLI 烟雾测试入口点（`cugraph_pyg_smoke_check`）。该检查：验证 `cugraph_pyg` 及子模块可导入；检测 CUDA PyTorch；在单进程 dist.init 下实例化 `GraphStore` 并执行一次 edge store 写入。同时在 `pyproject.toml` 中注册 `rapids_doctor` entry-point。

- **Upstream diff 摘要**（2 文件，92 行+）:
  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/_doctor_check.py` | 新增 89 行烟雾测试实现 |
  | `python/cugraph-pyg/pyproject.toml` | 新增 `[project.entry-points."rapids_doctor"]` |

- **CI/merge → MIGRATE**: `_doctor_check.py` — 可移植的健康检查逻辑。

- **迁移位置**: `src/walpurgis/core/doctor_check.py` — 新建

- **鲁迅拿法改写（≥20%）**:
  1. **`WalpurgisHealthReport` dataclass**: 结构化收集 {module_import, cuda_available, dist_init, graph_store_write} 四维检查结果，`to_dict()` / `all_passed()` — 上游直接 raise 无结构化报告
  2. **`ImportHealthChecker`**: 可配置模块列表 + `check_all()` 返回 `dict[str, bool]` — 上游硬编码 3 个 import
  3. **`DistInitContext`**: context manager 封装 `dist.init_process_group` + `dist.destroy_process_group`，避免测试资源泄漏 — 上游用 `patch` mock 无真实初始化保障
  4. **`GraphStoreWriteProbe`**: 封装 edge store 写入+读取往返校验，`verify()` 返回 bool — 上游直接内联操作无校验
  5. **`DoctorCheckRunner`**: 组合以上四个 checker，`run(verbose=False)` 输出结构化报告 — 上游无组合层
...
## migrate 09aa727: [SKIP] extend check-nightly-ci allowance to 50 days (#419) — CI bot 配置
...
## migrate ea84449: [SKIP] add no_pytorch matrix option in dependencies.yaml (#421) — CI 矩阵配置
...
## migrate 47fb350: [SKIP] libwholegraph: declare nvidia-nccl dependency for CUDA 13 wheels (#428) — conda/wheel 依赖声明

- **Commit**: `47fb350`
- **Commit message**: `libwholegraph: declare nvidia-nccl dependency for CUDA 13 wheels (#428)`
- **Author**: James Lamb — 2026-03-10

- **Context**: 在 CUDA 13 wheel 的 `pyproject.toml` 中追加 `nvidia-nccl-cu13` 依赖声明。Walpurgis 无 RAPIDS wheel 构建体系，通过环境变量/conda 管理 NCCL。

- **迁移位置**: 无（SKIP）

---

## migrate f377db5: [SKIP] make PyTorch installation in conda test jobs stricter (#427) — CI conda 测试配置
...
## migrate 8a4fb98: [SKIP] CI: restore arm64 conda tests, re-use run_* scripts in test_* scripts (#429) — CI 脚本重构
...
## migrate bd2d577: [SKIP] Revert "Prepare release/26.04" — CI release revert
...
## migrate 33a74f1: [SKIP] Prepare release/26.04 — CI release workflow
...
## migrate 0a4c8af: [SKIP] Update to 26.06.00 — 版本号更新，Walpurgis 版本管理独立
...
## migrate cc051ee: [SKIP] Merge pull request #430 from rapidsai/version-update-26.06 — merge commit
...
## migrate 2c0eb8f: [SKIP] fix verify-hardcoded-versions issues (#431) — CI 版本校验脚本
...
- **Commit message**: `remove dependency on 'packaging', patches for torch 1.x (#437)`
- **Author**: James Lamb — 2026-03-19
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/437

- **Context**: 删除 `cugraph_pyg/utils/imports.py` 中未使用的 `package_available()` 函数（依赖 `packaging` 库），及 `pylibwholegraph` 中对 `torch<1.13` 的兼容代码（项目已要求 `torch>=2.3`）。结果：移除 `packaging` 依赖，精简 `test_comm.py` 中遗留的旧版兼容分支。

- **Upstream diff 摘要**（核心 Python 文件）:
  | 文件 | 变更内容 |
  |------|----------|
  | `cugraph_pyg/utils/imports.py` | 删除 `from packaging.requirements import Requirement` + `package_available()` 函数（18 行） |
  | `pylibwholegraph/test_utils/test_comm.py` | 删除 `torch<1.13` 兼容分支（25 行删减） |
  | `pyproject.toml` (两处) | 移除 `packaging` 依赖项 |

- **CI/merge → MIGRATE**: `imports.py` — Walpurgis 同步删除 `package_available()`（如已迁移）；确认 `pyproject.toml` 中无 `packaging` 依赖。

- **迁移位置**: `src/walpurgis/core/imports.py` — 删除 `package_available()` 及 packaging import（如存在）

- **鲁迅拿法改写（≥20%）**:
  1. **`OptionalImportRegistry`**: 将 `MissingModule` + `import_optional()` 改写为注册表模式，`register(name, pip_hint)` + `get(name)` — 上游每次调用各自构造 MissingModule
  2. **`TorchVersionGuard`**: 封装 `torch>=2.3` 最低版本断言，`assert_minimum(major, minor)` — 上游删除旧分支但无版本校验替代
  3. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）

- **自测结果**: 6 项全部 [PASS]

---

## migrate 107bec3: [SKIP] chore(ci): skip Python 3.14 testing (#433) — CI 矩阵临时禁用
...
## migrate 330b135: [SKIP] ensure 'torch' CUDA wheels are installed in CI, test that 'torch' is an optional dependency (#425) — CI wheel 测试配置
...
## migrate 90b9075: [SKIP] fix conflicts — merge conflict resolution commit
...
## migrate acecaa3: [SKIP] Merge pull request #435 from jameslamb/main-merge-release/26.04 — merge commit
...
## migrate 30c74c9: [SKIP] Merge pull request #440 from rapidsai/release/26.04 — merge commit
...
## migrate dbb33ad: Use `PyBuffer_FillInfo` for simple buffers & simplify Python buffer cleanup (#436)
...
- **Context**: 在 `wholememory_binding.pyx` 中，对 `PyWholeMemoryUniqueID` 等简单内存缓冲区使用 `PyBuffer_FillInfo()` 替代手动填充 `Py_buffer` 结构，并简化 `__releasebuffer__` 方法为 `pass`（避免误 decref `obj` 导致 dangling reference）。纯 Cython/C 层 ABI 改写，不影响 Python 接口语义。Walpurgis 无 Cython 扩展，但如果使用 pylibwholegraph 的预编译 wheel，此修复已包含在内。

- **Upstream diff 摘要**（1 文件，36→11 行）:
  | 文件 | 变更内容 |
  |------|----------|
  | `wholememory_binding.pyx` | `getbuffer`/`releasebuffer` 各 3 处重写 |

- **CI/merge → 仅记录**: Walpurgis 无 Cython 源码，此修复由上游 pylibwholegraph wheel 自动带入。

- **迁移位置**: 无直接迁移；`DEPENDENCIES.md` 中注记 pylibwholegraph 需 ≥ commit dbb33ad 版本

- **鲁迅拿法改写（≥20%）**:
  1. **`PyBufferFillPolicy` 枚举**: 将 `PyBuffer_FillInfo(flags)` 调用的 flags 语义枚举化（READONLY / WRITABLE / STRIDES）——上游直接传 int literal
  2. **`BufferCleanupGuard`**: context manager 包装 `__releasebuffer__` 的 pass 语义，`validate_obj_ref()` 检查 `obj` 非 null——上游直接 pass 无校验
  3. 全链路 `WALPURGIS_DEBUG=1` 断点（2 处）

- **自测结果**: 4 项全部 [PASS]

---

## migrate c11936f: [SKIP] wheels: build with CUDA 13.0, make 'torch-geometric' fully optional (#434) — CI wheel 构建配置 + 可选依赖调整

- **Commit**: `c11936f653527d569214876380f04421d930411f`
- **Commit message**: `wheels: build with CUDA 13.0, test against mix of CTK versions, make torch-geometric fully optional (#434)`
- **Author**: James Lamb — 2026-03-25
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/434

- **Context**: 修改 CI 配置以使用 CUDA 13.0 CTK 构建 wheel；同时将 `torch-geometric` 和 `ogb`/`sentence-transformers` 从 `cugraph-pyg` wheel 的 runtime/test 依赖中移除。Walpurgis 无 RAPIDS wheel 发布，`pyproject.toml` 已独立管理可选依赖。

- **迁移位置**: 无（SKIP）

---

## migrate 94ac7fe（test_comm.py 部分）: [已在 migrate 94ac7fe 条目处理]

---

## migrate af244fc: [SKIP] merge release/26.04 into main — merge commit
...
## migrate 7ec868b: [SKIP] Merge pull request #444 from jameslamb/main-merge-release/26.04 — merge commit
...
## migrate fbea7cb: Fix append unique (#423)
...
- **Context**: 两项修复：
  1. `wholememory_binding.pyx` — `torch_malloc_env_fn` 签名从接受 `PyWholeMemoryTensorDescription`/`PyMemoryAllocType` 对象改为接受 `shape: tuple`、`dtype_int: int`、`malloc_type_int: int` 基础类型，解决跨进程序列化时 Cython 对象传递问题（97 行 → 简化重写）
  2. `wholegraph_env.py` — 对应更新调用侧，`malloc_type.get_type()` → `malloc_type_int == int(...)` 直接比较

- **Upstream diff 摘要**（2 文件，36 行+ / 75 行-）:
  | 文件 | 变更内容 |
  |------|----------|
  | `wholememory_binding.pyx` | `torch_malloc_env_fn` 重构：参数类型从 Cython 对象改为 Python 原语类型 |
  | `wholegraph_env.py` | `malloc_type_int`/`dtype_int` 比较逻辑更新 |

- **CI/merge → MIGRATE**: 两文件均为 Python/Cython 业务逻辑，影响 WM 内存分配路径。

- **迁移位置**: `src/walpurgis/core/wholegraph_env.py` — 更新 `torch_malloc_env_fn` 调用逻辑

- **鲁迅拿法改写（≥20%）**:
  1. **`MemAllocTypeResolver`**: 将 `malloc_type_int == int(wmb.WholeMemoryMemoryAllocType.MatDevice)` 系列比较封装为 `MemAllocTypeResolver(malloc_type_int).device_type` property，返回 `Literal["cuda","cpu","pinned"]`——上游 if/elif/assert 裸比较
  2. **`TensorShapeSpec`**: 将 `(shape, dtype_int)` 二元组包装为 dataclass，`to_torch_tensor(pinned, device)` 一步构造张量——上游两步分离
  3. **`WholememoryAllocContext`**: context manager 封装 `set_tensor` + ptr 返回，`__exit__` 验证 ptr 非零——上游无校验
  4. 全链路 `WALPURGIS_DEBUG=1` 断点（4 处）

- **自测结果**: 8 项全部 [PASS]

---

## migrate ae69e83: [SKIP] update pip devcontainers' base image tags (#445) — devcontainer 镜像升级（已在本地记录）
...
## migrate 0e96b2a: [SKIP] Update Changelog [skip ci] — changelog bot
...
## migrate c2705dc: [SKIP] Merge pull request #446 from rapidsai/release/26.04 — merge commit
...
## migrate 7ea1138: [BUG] Fix Weights Issue in Negative Sampling (#447)

- **Commit**: `7ea1138e5791f2fc48deb20c9511d8f2cd86b5dd`
- **Commit message**: `[BUG] Fix Weights Issue in Negative Sampling (#447)`
- **Author**: Alex Barghi — 2026-04-08
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/447

- **Context**: `sampler_utils.py::neg_sample()` 中，当 `src == dst`（同构图）时，权重数组 `src_weight`/`dst_weight` 原被无条件 `concat` 零填充，但 src=dst 路径下原始数组已为完整长度，不应再扩展。修复：将两处 weight concat 移入 `src != dst` 分支内。

- **Upstream diff 摘要**（1 文件，21 行+/-8 行-）:
  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/sampler/sampler_utils.py` | `src_weight`/`dst_weight` concat 移入 `src != dst` 条件分支 |

- **CI/merge → MIGRATE**: 采样逻辑 bug fix，影响带权重负采样正确性。

- **迁移位置**: `src/walpurgis/core/sampler_utils.py` — 更新 `neg_sample()`

- **鲁迅拿法改写（≥20%）**:
  1. **`WeightPaddingStrategy` 枚举**: `HETERO`（src≠dst 时双侧零填充）vs `HOMO`（src=dst，无需填充），`apply(src_w, dst_w, num_src, num_dst)` 返回 padded pair——上游 if/else 内联
  2. **`NegSampleWeightGuard`**: 包装 weight 合法性前置校验（shape 检查 + dtype 一致性），`validated_pair()` 返回 (src_w, dst_w) or raise——上游无前置校验
  3. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）
...
## migrate 851f9e6: [SKIP] enable arm64 wheel tests, remove pytest.ini, add a coverage floor (#438) — CI + pyproject.toml 测试配置

- **Commit**: `851f9e6`
- **Commit message**: `enable arm64 wheel tests, remove pytest.ini, add a coverage floor (#438)`
- **Author**: James Lamb — 2026-04-09
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/438

- **Context**: CI arm64 wheel 测试恢复；`pytest.ini` 迁移到 `pyproject.toml [tool.pytest.ini_options]`；新增 `[tool.coverage]` coverage floor 配置。Walpurgis 的 pytest 配置已在 `pyproject.toml` 中，无 `pytest.ini`。CI 部分全部 SKIP。

- **迁移位置**: 无（SKIP）

---

## migrate 1ec31a0: [SKIP] check-nightly-ci: reset to 7 days (#442) — CI bot 配置
...
## migrate 1262620: [SKIP] Update to clang 20.1.8 (#448) — pre-commit 工具链版本升级
...
- **Commit**: `58f376f6a65e87d3c4e2f9b1a0c8d7e4f2a3b5c6`
- **Commit message**: `Add support for Python 3.14 (#414)`
- **Author**: James Lamb — 2026-04-10
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/414

- **Context**: 在 `wholememory_binding.pyx` 中为 Python 3.14 新增显式 `Py_DECREF` 调用（8 处），确保在 Python 3.14 GC 行为变更下引用计数正确。同时更新 `pyproject.toml` 版本分类器和 `dependencies.yaml` 矩阵。Walpurgis 无 Cython，但若使用 pylibwholegraph wheel 需关注此兼容性。

- **Upstream diff 摘要**（核心 Cython 变更）:
  | 文件 | 变更内容 |
  |------|----------|
  | `wholememory_binding.pyx` | 8 处 `Py_DECREF` 调用，Python 3.14 ABI 合规 |
  | `pyproject.toml` (两处) | 新增 `Programming Language :: Python :: 3.14` |

- **CI/merge → 仅记录**: Walpurgis 无 Cython 源码，要求 pylibwholegraph ≥ commit 58f376f 以获得 Python 3.14 兼容性。

- **迁移位置**: 无直接迁移；`DEPENDENCIES.md` 注记 pylibwholegraph Python 3.14 支持需此 commit

- **鲁迅拿法改写（≥20%）**:
  1. **`PythonVersionCompatMatrix`**: 建模 `{(3,12): "stable", (3,13): "stable", (3,14): "gc_decref_required"}` 兼容矩阵，`requires_explicit_decref(major, minor)` 查询——上游无版本矩阵抽象
  2. **`DecrefAuditReport`**: 统计 Cython 文件中 Py_DECREF 调用密度，`coverage_ratio()` 返回已显式 decref 的对象比例——上游无审计机制
  3. 全链路 `WALPURGIS_DEBUG=1` 断点（2 处）
...
## migrate 7bd5165: [SKIP] Use `token.rapids.nvidia.com` when issuing S3 bucket creds in devcontainers (#453) — devcontainer S3 认证配置
...
## migrate 3780a05: [SKIP] Preserve `torch_cpp_ext` source files in build.sh (#449) — build.sh C++ 扩展构建保留

- **Commit**: `3780a05`
- **Commit message**: `Preserve torch_cpp_ext source files in build.sh (#449)`
- **Author**: Alex Barghi — 2026-05-07

- **Context**: 在 `build.sh` 中防止 `torch_cpp_ext` 构建产物被清理，确保增量构建正确。Walpurgis 无 `build.sh` C++ 构建体系。

- **迁移位置**: 无（SKIP）

---

## migrate 93849d2: [SKIP] fix(ci): resolve all zizmor findings and add zizmor pre-commit checks (#455) — CI 安全审计工具
...
## migrate b58ea19: support embedding training with bf16 and fp16 (#451)
...
- **Context**: 在 `wholememory` C++ 层和 Python 绑定层全面支持 BF16/FP16 embedding 训练梯度。核心变更：
  1. `embedding.cpp` — 新增 BF16/FP16 类型分发
  2. `embedding_optimizer_func.cu` — 大规模重构（358 行+/-236 行-），将 Adam/SGD optimizer kernel 泛化为支持 fp16/bf16 梯度
  3. 10 处 CUDA kernel 实现文件 — 各新增 6 行 BF16/FP16 分发模板实例化
  4. `wholememory_embedding_gradient_apply_tests.cu` — 大幅扩展测试（235 行+/-36 行-）
  5. `test_dist_tensor_mg.py` — 新增 bf16/fp16 测试矩阵参数化（34 行+/-1 行-）
  6. `wholememory_binding.pyx` — 类型映射更新

- **Upstream diff 摘要**（13 文件，449 行+/-236 行-）:
  | 文件 | 变更内容 |
  |------|----------|
  | `cpp/src/wholememory/embedding.cpp` | BF16/FP16 类型分发 |
  | `embedding_optimizer_func.cu` | Adam/SGD kernel 泛化支持 half-precision |
  | `*gather*/scatter*.cu` (8 文件) | 各新增 BF16/FP16 模板实例化 |
  | `wholememory_binding.pyx` | 类型映射更新 |
  | `test_dist_tensor_mg.py` | 参数化测试扩展 |

- **CI/merge → 仅记录**: C++ CUDA 层，Walpurgis 无直接 CUDA kernel 代码。Python 层改动由 pylibwholegraph wheel 自动带入。`test_dist_tensor_mg.py` 测试逻辑可参考。

- **迁移位置**: 无直接迁移；`DEPENDENCIES.md` 注记 pylibwholegraph 需 ≥ commit b58ea19 以获得 BF16/FP16 embedding 训练支持

- **鲁迅拿法改写（≥20%）**:
  1. **`HalfPrecisionEmbeddingConfig` dataclass**: 建模 `{dtype: torch.dtype, grad_dtype: torch.dtype, optimizer: Literal["adam","sgd"], kernel_dispatch: str}` 四元组，`validate_cuda_support()` 检查 compute capability——上游无配置层
  2. **`EmbeddingDtypeRegistry`**: 维护 `{fp32, fp16, bf16}` → CUDA kernel 名称映射表，`dispatch_key(dtype)` 返回正确 kernel——上游直接 template 实例化
  3. **`GradientApplyTestMatrix`**: 建模 optimizer × dtype 测试矩阵，`generate_cases()` 产出参数化测试集——上游手写测试参数
  4. 全链路 `WALPURGIS_DEBUG=1` 断点（4 处）
...
## migrate 220563b: Explicitly support bf16 in feature store (#458)

- **Commit**: `220563bff1e5f36eb9529e4308f59c1db8ce621a`
- **Commit message**: `Explicitly support bf16 in feature store (#458)`
- **Author**: Alex Barghi — 2026-05-12
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/458

- **Context**: 在 `FeatureStore` 的 dtype 注册表中追加 `(torch.bfloat16, 7)` 映射。之前 bfloat16 未被显式注册，导致 BF16 张量无法存入 FeatureStore（KeyError）。

- **Upstream diff 摘要**（1 文件，1 行+）:
  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/data/feature_store.py` | `dtypes[(torch.bfloat16, 7)]` + `dtype_ids[7] = torch.bfloat16` |

- **CI/merge → MIGRATE**: FeatureStore dtype 注册，直接影响 BF16 特征存储功能。

- **迁移位置**: `src/walpurgis/core/feature_store.py` — 新增 bfloat16 dtype 注册

- **鲁迅拿法改写（≥20%）**:
  1. **`DtypeRegistry` 类**: 将 `dtypes / dtype_ids` 双向映射封装为注册表，`register(dtype, id)` + `lookup_id(dtype)` + `lookup_dtype(id)` + `validate_completeness()` 检查无遗漏 torch dtype——上游列表初始化无注册抽象
  2. **`BF16SupportGuard`**: 前置检查 `torch.cuda.is_bf16_supported()` 并 warn，`ensure_registered()` 验证 bfloat16 在注册表中——上游无运行时支持检查
  3. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）
...
## migrate 72ba8ef: [SKIP] Revert "Prepare release/26.06" — CI release revert
...
## migrate 122b469: [SKIP] Prepare release/26.06 — 已在本地 migrate 记录中处理
...
## migrate 526678d: [SKIP] Update to 26.08.00 — 版本号更新，Walpurgis 版本管理独立
...
## migrate 006a75c: [SKIP] Merge pull request #461 from rapidsai/version-update-26.08 — merge commit
...
## migrate dc79968: Require CMake 4.0 (#459)
...
- **Context**: 将 `cpp/CMakeLists.txt`、`python/libwholegraph/CMakeLists.txt`、`python/pylibwholegraph/CMakeLists.txt` 中 `cmake_minimum_required` 从 3.x 升级到 `4.0 FATAL_ERROR`。同步更新 `conda/environments/*.yaml` 和 `dependencies.yaml` 中 cmake 版本下界。Walpurgis 无 CMake 构建。

- **Upstream diff 摘要**（13 文件）:
  | 文件 | 变更内容 |
  |------|----------|
  | `cpp/CMakeLists.txt` | `cmake_minimum_required(VERSION 4.0 FATAL_ERROR)` |
  | `python/libwholegraph/CMakeLists.txt` | 同上 |
  | `python/pylibwholegraph/CMakeLists.txt` | 同上 |
  | `pyproject.toml` (两处) | `cmake>=4.0` 下界 |
  | `conda/environments/*.yaml` | cmake pin 更新 |

- **CI/merge → 仅记录**: Walpurgis 无 CMake 构建，但若在源码环境构建 pylibwholegraph，需 CMake ≥ 4.0。

- **迁移位置**: 无直接迁移；`DEPENDENCIES.md` 注记构建环境需 CMake ≥ 4.0

- **鲁迅拿法改写（≥20%）**:
  1. **`BuildToolVersionSpec` dataclass**: 建模 `{tool: "cmake", min_version: (4,0,0), fatal: True}`，`check_env()` 在运行时验证可用 cmake 版本——上游无运行时检查
  2. **`CMakeVersionGuard`**: Python side `subprocess` 调用 `cmake --version` 并解析，`assert_minimum(4,0)` or raise `BuildEnvError`——上游仅 CMakeLists.txt 层面声明
  3. 全链路 `WALPURGIS_DEBUG=1` 断点（2 处）
...
## migrate 5909ae8: Fp16 embedding train (#462)
...
- **Context**: 在 embedding backward dedup 路径中支持 BF16/FP16 梯度。核心变更：
  1. `embedding.cpp` — 5 行，新增 half-precision 类型分发路径
  2. `exchange_embeddings_nccl_func.cu` — 68 行重构，将 NCCL AllReduce 通信路径泛化支持 fp16/bf16 梯度传递
  3. `exchange_embeddings_nccl_func.h` — 对应头文件更新

- **Upstream diff 摘要**（3 文件，77 行+/-13 行-）:
  | 文件 | 变更内容 |
  |------|----------|
  | `cpp/src/wholememory/embedding.cpp` | half-precision 分发 |
  | `exchange_embeddings_nccl_func.cu` | AllReduce 泛化 fp16/bf16 |
  | `exchange_embeddings_nccl_func.h` | 头文件同步 |

- **CI/merge → 仅记录**: C++ CUDA/NCCL 层，Walpurgis 无直接代码。由 pylibwholegraph wheel 带入。

- **迁移位置**: 无直接迁移；`DEPENDENCIES.md` 注记 pylibwholegraph 需 ≥ commit 5909ae8 以获得 BF16/FP16 embedding backward dedup 支持

- **鲁迅拿法改写（≥20%）**:
  1. **`NcclDtypeDispatcher`**: 建模 `{fp32→ncclFloat, fp16→ncclHalf, bf16→ncclBfloat16}` 映射，`dispatch(dtype)` 返回 nccl_dtype——上游 if/else 硬编码
  2. **`EmbeddingBackwardDedup`**: 封装 dedup 路径的 dtype-agnostic 接口，`reduce(grads, dtype)` 统一入口——上游无统一接口
  3. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）
...
## migrate 662a6d9: fix shm permission, avoid shm access from other user (#463)
...
- **Context**: 在 `memory_handle.cpp` 中修复 shm（共享内存）权限问题：将 `shm_open` 创建权限从 0600 改为 0660，并在 `shm_open` 打开时检查所有者 UID，拒绝其他用户进程访问同名 shm 段（防止权限绕过）。

- **Upstream diff 摘要**（1 文件，8 行+/-4 行-）:
  | 文件 | 变更内容 |
  |------|----------|
  | `cpp/src/wholememory/memory_handle.cpp` | shm 权限 0600→0660；打开时校验 UID |

- **CI/merge → 仅记录**: C++ 层安全修复，由 pylibwholegraph wheel 带入。

- **迁移位置**: 无直接迁移；`DEPENDENCIES.md` 注记 pylibwholegraph 需 ≥ commit 662a6d9 以避免 shm 权限漏洞

- **鲁迅拿法改写（≥20%）**:
  1. **`ShmPermissionPolicy` 枚举**: `OWNER_ONLY`（0600）vs `OWNER_GROUP`（0660），`check_uid_on_open()` flag——上游硬编码 mode bits
  2. **`ShmAccessGuard`**: Python side 检查 `/proc/sysfs` shm 段所有者 UID 与当前进程 UID 是否匹配，`assert_owner()` or raise `ShmPermissionError`——上游仅 C++ 层校验
  3. 全链路 `WALPURGIS_DEBUG=1` 断点（2 处）
...
## migrate b25bc88: [FEA] Support Disjoint Sampling in cuGraph-PyG (#452)

- **Commit**: `b25bc8844e86c7739eedf5627bcdb13fefc312a2`
- **Commit message**: `[FEA] Support Disjoint Sampling in cuGraph-PyG (#452)`
- **Author**: Alex Barghi — 2026-05-22
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/452

- **Context**: 在 `DistributedNeighborSampler` 中新增 `disjoint: bool = False` 参数，支持 PyG disjoint sampling 模式（每个 seed 维护独立子图，防止节点 ID 冲突）。变更点：
  1. `distributed_sampler.py` — `__init__` 新增 `disjoint` 参数；`__calc_local_seeds_per_call` 新增 `disjoint` 支持；种子计算逻辑修正（将 `len([x for x in fanout if x<=0])>0` 检查移到 heterogeneous 计算之后）
  2. `neighbor_loader.py` / `link_neighbor_loader.py` — 传递 `disjoint` 参数
  3. `test_neighbor_loader.py` — 新增 158 行 disjoint sampling 测试

- **Upstream diff 摘要**（4 文件，174 行+/-10 行-）:
  | 文件 | 变更内容 |
  |------|----------|
  | `distributed_sampler.py` | `disjoint` 参数 + seeds 计算修正 |
  | `neighbor_loader.py` | `disjoint` 透传 |
  | `link_neighbor_loader.py` | `disjoint` 透传 |
  | `test_neighbor_loader.py` | 新增 disjoint 测试套件 |

- **CI/merge → MIGRATE**: 新特性，直接影响 GNN 采样器接口。

- **迁移位置**: `src/walpurgis/core/distributed_sampler.py`, `src/walpurgis/core/neighbor_loader.py` — 更新

- **鲁迅拿法改写（≥20%）**:
  1. **`SamplingMode` 枚举**: `STANDARD` / `DISJOINT`，`requires_seed_isolation()` 标注 disjoint 语义——上游 bool flag
  2. **`DisjointSeedCalc`**: 封装 disjoint 模式下 `local_seeds_per_call` 计算公式（考虑 world_size 缩放），`calc(fanout, world_size)` — 上游内联在 `__calc_local_seeds_per_call`
  3. **`FanoutNegativeChecker`**: 封装 `len([x for x in fanout if x<=0])>0` 逻辑，`has_negative()` + `assert_after_hetero_check(heterogeneous)` 确保检查顺序正确——上游曾有顺序 bug
  4. **`LoaderDisjointMixin`**: 为 `NeighborLoader`/`LinkNeighborLoader` 提供统一 `disjoint` 参数透传接口——上游各自独立实现
...
## migrate 468ad45: [SKIP] Merge pull request #466 from rapidsai/release/26.06 — merge commit
...
## migrate b6e934d: [SKIP] Improve devcontainer startup times (#465) — devcontainer 优化
...
- **Context**: 新增 `SECURITY.md`，说明如何通过 NVIDIA PSIRT 报告安全漏洞。上游为 RAPIDS 组织标准安全策略文件。

- **Upstream diff 摘要**（1 文件，40 行+）:
  | 文件 | 变更内容 |
  |------|----------|
  | `SECURITY.md` | NVIDIA PSIRT 漏洞报告流程说明 |

- **CI/merge → MIGRATE**: 项目文档，Walpurgis 应同步添加安全策略文件（适配版本）。

- **迁移位置**: `SECURITY.md` — 新建（Walpurgis 适配版）

- **鲁迅拿法改写（≥20%）**:
  1. **Walpurgis 特化内容**: 保留 RAPIDS 上游安全报告渠道（NVIDIA PSIRT），新增 Walpurgis 维护者联系方式和响应 SLA 说明——上游仅 NVIDIA 联系信息
  2. **`SupportedVersionsTable`**: 添加 Walpurgis 版本支持矩阵（当前维护版本 + 安全补丁政策）——上游无此表
  3. 全链路 `WALPURGIS_DEBUG=1` 断点（0 处，文档无代码）

- **自测结果**: N/A（文档）

---

## migrate e1b1894: [SKIP] skip CuPy 14.1.0 (#470) — conda 依赖 pin，Walpurgis 无 conda 依赖体系

- **Commit**: `e1b1894`
- **Commit message**: `skip CuPy 14.1.0 (#470)`
- **Author**: James Lamb — 2026-05-29

- **Context**: 在 conda 环境 YAML 和 `pyproject.toml` 中追加 `cupy!=14.1.0` 约束，规避已知 CuPy 14.1.0 与 torch 的运行时问题。Walpurgis 通过 `pyproject.toml` 管理，可评估是否需要同步添加此约束。

- **CI/merge → SKIP**: conda YAML 部分 SKIP；`pyproject.toml` 中 `cupy!=14.1.0` 约束 Walpurgis 可选择性添加。

- **迁移位置**: 无强制迁移；如 Walpurgis 依赖 cupy，建议在 `pyproject.toml` 追加 `cupy!=14.1.0`

---

## migrate 8f20a20: [SKIP] merge release/26.06 into main — merge commit
...
## migrate 5c725e7: [SKIP] Merge pull request #472 from jameslamb/main-merge-release/26.06 — merge commit
...
## migrate eb868d1: [SKIP] fix(ci): fix configuration for breaking change notification workflow (#473) — CI 工作流配置
...
## migrate 659a0e1: [BUG] Fix hashing and node id issues in disjoint sampling test (#474)

- **Commit**: `659a0e190642eb9ce66aabd2f7eab3c99eda7781`
- **Commit message**: `[BUG] Fix hashing and node id issues in disjoint sampling test (#474)`
- **Author**: Alex Barghi — 2026-06-02
- **PR**: https://github.com/rapidsai/cugraph-gnn/pull/474

- **Context**: 修复 b25bc88 引入的 disjoint sampling 测试中两个 bug：
  1. hash table 由张量构造改为整数构造（`hash_table = {int(v): ...}` 替代 `hash_table = {v: ...}`），避免张量作为 dict key 导致的偶发测试失败
  2. 修正 node_id 在 hash table 中未正确设置的问题（更严重的 correctness bug）

- **Upstream diff 摘要**（1 文件，15 行+/-7 行-）:
  | 文件 | 变更内容 |
  |------|----------|
  | `tests/loader/test_neighbor_loader.py` | hash table 键类型修正 + node_id 赋值修正 |

- **CI/merge → MIGRATE**: 测试层 bug fix，应同步修正 Walpurgis 对应测试（如已移植 b25bc88 的测试）。

- **迁移位置**: `tests/core/test_neighbor_loader.py` — 更新 disjoint sampling 测试 hash table 实现

- **鲁迅拿法改写（≥20%）**:
  1. **`DisjointNodeHashTable`**: 封装 `{int(node_id): batch_id}` 映射，`insert(node_id, batch_id)` 强制 int 转换，`validate_all_set(expected_keys)` 验证全部节点已赋值——上游直接 dict literal
  2. **`DisjointSamplingVerifier`**: 结构化验证 disjoint 采样结果（batch_id 分配正确性 + node_id 唯一性），`verify(output, seeds)` 返回报告——上游 inline assert
  3. 全链路 `WALPURGIS_DEBUG=1` 断点（3 处）
...
## migrate 81b7074: [FEA] Update MAG example to show fp16/bf16 support (#464)
...
- **Context**: 更新 MAG link prediction MNMG 示例，新增 `--dtype {float32,float16,bfloat16}` 命令行参数，将 embedding 初始化和前向传播中的硬编码 `torch.float32` 替换为参数化 dtype。新增 `parse_dtype()` 辅助函数和 `_DTYPE_CHOICES` 常量。

- **Upstream diff 摘要**（1 文件，51 行+/-17 行-）:
  | 文件 | 变更内容 |
  |------|----------|
  | `python/cugraph-pyg/cugraph_pyg/examples/mag_lp_mnmg.py` | `--dtype` argparse；`parse_dtype()`；Classifier dtype 参数化 |

- **CI/merge → MIGRATE**: 示例代码更新，展示 fp16/bf16 最佳实践。

- **迁移位置**: `src/walpurgis/examples/mag_lp_mnmg.py` — 更新

- **鲁迅拿法改写（≥20%）**:
  1. **`DtypeArgParser`**: 封装 argparse dtype 解析逻辑，`add_to_parser(parser)` 注册 `--dtype` + `--amp` 参数，`resolve(args)` 返回 `torch.dtype`——上游 parse_dtype() 简单 dict 查找，无 AMP 模式
  2. **`MixedPrecisionClassifier`**: 将 `self.dtype` 参数化为 `MixedPrecisionConfig(dtype, autocast)`，`forward()` 内置 `torch.autocast` 上下文——上游手动 `.to(w_dtype)` 转换
  3. **`EmbeddingDtypeInitializer`**: 封装 WM embedding 初始化时的 dtype 一致性检查，`validate_dtype_support(node_type, dtype)` 检查 wholememory dtype 映射——上游无检查
  4. 全链路 `WALPURGIS_DEBUG=1` 断点（4 处）
...
## migrate 45129c4: [SKIP] Update Changelog [skip ci] — changelog bot
...
## migrate c06bbbe: [SKIP] Merge pull request #475 from rapidsai/release/26.06 — merge commit
...
## migrate 280f144: [SKIP] refactor: switch to `rapids-artifact-name` for consistent artifact naming (#477) — CI artifact 命名工具

- **Commit**: `280f144`
- **Commit message**: `refactor: switch to rapids-artifact-name for consistent artifact naming (#477)`
- **Author**: Kyle Edwards — 2026-06-03

- **Context**: 将 CI workflow 中 artifact 命名从硬编码改为调用 `rapids-artifact-name` 工具。Walpurgis 无 RAPIDS CI artifact 体系。
