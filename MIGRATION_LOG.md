...
## migrate fb8296e: Remove cuGraph-DGL (#210)

- **Upstream commit**: fb8296e3afc70f965cf7d79b3e887c3233f3681b (cugraph-gnn, Alex Barghi, 2025-05-22, PR #210)
- **Commit message**: `Remove cuGraph-DGL (#210)`
- **Approvers**: Tingyu Wang, linhu-nv, Vyas Ramasubramani, Don Acosta
- **Upstream diff** (68 files changed, 23 insertions, 6610 deletions):
  - 永久删除 `python/cugraph-dgl/` 整包（含 graph.py 1133行、sampling_helpers.py 692行等全部源码）
  - `.github/workflows/` build/pr/test.yaml — 删除所有 `wheel-build-cugraph-dgl` / `wheel-tests-cugraph-dgl` / `wheel-publish-cugraph-dgl` job
  - `ci/build_wheel_cugraph-dgl.sh` / `ci/run_cugraph_dgl_pytests.sh` / `ci/test_wheel_cugraph-dgl.sh` / `ci/test_python.sh` — 删除 DGL CI 构建测试脚本
  - `conda/recipes/cugraph-dgl/` — 删除 conda 构建配方（50行）
  - `dependencies.yaml` — 删除 `cugraph-dgl` 依赖块及所有引用（90行删减）
  - `build.sh` — 删除 `cugraph-dgl` 构建目标 + default action 描述
  - `README.md` — 升级通知: "cuGraph-DGL has been removed in release 25.08."
  - `python/pylibwholegraph/torch/gnn_model.py` — 删除所有 `elif framework_name == "dgl"` 分支:
    * `set_framework()`: 删除 `import dgl + from dgl.nn.pytorch.conv import SAGEConv, GATConv`
    * `create_sub_graph()`: 删除 `dgl.create_block(("csc", (csr_row_ptr, csr_col_ind, ...)))` 路径
    * `layer_forward()`: 删除 `layer(sub_graph, (x_feat, x_target_feat))` 路径
  - `python/pylibwholegraph/torch/common_options.py` — `--framework` help 从 `"dgl, pyg, wg"` 改为 `"pyg, wg"`
  - `python/pylibwholegraph/torch/gnn_model.py` — 删除 `from .graph_ops import add_csr_self_loop`

- **迁移位置**:
  - `src/walpurgis/models/gnn_framework.py` — 更新: `SubgraphAdapter.build()` 和 `LayerForwardDispatch.forward()` 中 `dgl` 路径从通用 import 改为 RuntimeError + 迁移说明
  - `src/walpurgis/core/dgl_deprecation.py` — 更新: DGL 状态从"deprecated（FutureWarning）"升级为"permanently removed（RuntimeError）"

- **CI/merge/docs 文件 → SKIP**:
  - `.github/workflows/build.yaml` / `pr.yaml` / `test.yaml` — CI wheel 发布 pipeline
  - `ci/build_wheel_cugraph-dgl.sh` / `ci/test_wheel_cugraph-dgl.sh` / `ci/run_cugraph_dgl_pytests.sh` — CI 构建测试脚本
  - `conda/recipes/cugraph-dgl/` — conda 构建配方
  - `dependencies.yaml` — RAPIDS 依赖矩阵
  - `README.md` / `readme_pages/` — 文档
  - `python/cugraph-dgl/` 整包 Python 源码 — 已在前序迁移 f4ca484（db74d87 batch）中通过 Walpurgis 改写版本覆盖
  - `ci/release/update-version.sh` — 版本脚本删除 cugraph-dgl 条目
  - `.python/.coveragerc` — 覆盖率配置删除 cugraph-dgl 路径
  - `python/cugraph-pyg/pyproject.toml` — 删除 cugraph-dgl test dep

- **鲁迅拿法改写 (>20%)**:

  **gnn_framework.py (SubgraphAdapter.build)**:
  1. 上游 gnn_model.py 的 DGL 分支是 `elif framework_name == "dgl": import dgl; block = dgl.create_block(...)`，fb8296e 直接删行无任何错误提示。Walpurgis 版改为 `RuntimeError` + 4行迁移说明，明示删除原因（fb8296e, PR #210）和推荐路径（'wg' 或 'pyg'）。
  2. 同时注明上游删除的具体路径内容（`add_csr_self_loop` + `dgl.create_block` CSC 格式），供历史追溯。

  **gnn_framework.py (LayerForwardDispatch.forward)**:
  1. 上游 `elif framework_name == "dgl": x_feat = layer(sub_graph, (x_feat, x_target_feat))` 静默删除。Walpurgis 版改为 `RuntimeError` + 3行迁移说明，包含原 API 签名，帮助现有代码找到对应的 wg/pyg 替代。

  **dgl_deprecation.py**:
  1. `CuGraphStorageCompat.__call__()`: 上游 456d5a2 是 `FutureWarning` + 尝试调用 cugraph-dgl；fb8296e 后 cugraph-dgl 已删除，Walpurgis 升级为 `RuntimeError` + 精准迁移路径 (`walpurgis.core.unified_store.UnifiedStore`)。
  2. `DaskDataLoaderCompat.__call__()`: 同上，升级为 `RuntimeError` + 迁移路径 (`walpurgis.dataloader.DataLoader`)。
  3. 新增 `is_dgl_permanently_removed()` 公开函数：上游无此机制，调用方可在 import 前查询删除状态，结合 `_FB8296E_REMOVED_RELEASE="25.08"` 和 `_FB8296E_MIGRATION_TARGET` 做版本感知判断。
  4. 新增 `_FB8296E_DGL_PERMANENTLY_REMOVED` / `_FB8296E_REMOVED_RELEASE` / `_FB8296E_MIGRATION_TARGET` 三个模块常量，使删除状态可程序化查询，替代上游 README 散文段落。
  5. 文件头 docstring 更新：从纯"废弃"语义升级为"废弃 → 永久删除"全历史叙述，含上游 PR #210 原文引用。

- **自测结果**:
  - `python -c "import ast; ast.parse(open('src/walpurgis/models/gnn_framework.py').read())"` → OK
  - `python -c "import ast; ast.parse(open('src/walpurgis/core/dgl_deprecation.py').read())"` → OK
  - `SubgraphAdapter.build(..., framework='dgl')` → RuntimeError: "migrate fb8296e: 'dgl' framework 已在 fb8296e (Remove cuGraph-DGL) 中永久删除"
  - `LayerForwardDispatch.forward(..., framework='dgl')` → RuntimeError: 同上
  - `CuGraphStorage(g)` → RuntimeError: "cuGraph-DGL has been permanently removed in release 25.08 (PR #210)"
  - `DaskDataLoader(graph)` → RuntimeError: 同上
  - `is_dgl_permanently_removed()` → True
## migrate a24978e: [SKIP] Temporarily Disable Bitcoin Example Tests

- **Upstream commit**: a24978e (cugraph-gnn, Alex Barghi, 2025-06-18, PR #230)
- **Commit message**: `Temporarily Disable Bitcoin Example Tests (#230)`
- **Upstream diff** (2 files changed, 6 insertions, 6 deletions):
  - `ci/run_cugraph_pyg_pytests.sh`: 注释掉 bitcoin example 的 torchrun 和 python 调用行
  - `ci/test_wheel_cugraph-pyg.sh`: 同上，注释掉 rapids-logger + bitcoin 两行
  - 原因: SSL 证书问题导致 CI 中 bitcoin 数据集下载失败，暂时禁用，待数据集迁移至其他托管后恢复

- **跳过原因（主体 SKIP）**:
  两个文件均为 `ci/` 目录下的 CI 流水线脚本，属于 RAPIDS CI 基础设施。Walpurgis 无 `ci/` 目录、无 RAPIDS CI 体系、无 `run_cugraph_pyg_pytests.sh` 等 CI 入口脚本。两个文件的改动仅为注释掉 3 行 shell 命令，不涉及算法、运行时代码或 API 变更，零直接迁移价值。

- **派生迁移（鲁迅拿法改写 ≥20%）**:

  上游注释掉 bitcoin 示例的根因是「SSL 下载失败致 CI 崩溃」。此模式揭示了 Walpurgis `bitcoin_mnmg.py` 和 `bitcoin_rf.py` 同样存在的防御空白：外部数据集下载失败时，示例会以不可读的 Python urllib/requests 错误退出，无任何有用诊断。

  **改写文件**: `src/walpurgis/examples/fraud/bitcoin_mnmg.py`（已存在，追加 `_DatasetDownloadGuard`）

  改写点：
  1. `_DatasetDownloadGuard` 类: 封装 EllipticBitcoin 数据集访问，检测 `WALPURGIS_SKIP_BITCOIN=1` 环境变量（对应上游 CI 注释逻辑），`check_or_skip()` 在 SSL/网络不可用时发出 `RuntimeWarning` 并返回 `False`，不崩溃
  2. `_ssl_probe()` 静态方法: 提前探测 Bitcoin dataset 托管 URL 可达性（HEAD 请求，3秒超时），不可达时在 `WALPURGIS_DEBUG=1` 下打印详细错误链
  3. `_CI_SKIP_MSG` 常量: 完整记录上游 a24978e 的临时禁用原因，方便未来维护者追踪
  4. 断点1: `_DatasetDownloadGuard.__init__` — 打印 `WALPURGIS_SKIP_BITCOIN` 环境变量状态
  5. 断点2: `_ssl_probe` — 打印 URL 可达性结果 + SSL 证书错误类型
  6. 断点3: `check_or_skip` — 打印跳过决策路径（env_skip / ssl_fail / ok）
  7. `bitcoin_rf.py` 追加相同的 `_DatasetDownloadGuard` 防御

- **自测结果**:
  - `WALPURGIS_SKIP_BITCOIN=1 python -c "from walpurgis.examples.fraud.bitcoin_mnmg import _DatasetDownloadGuard; g=_DatasetDownloadGuard(); print(g.check_or_skip())"` → `False` (跳过), 无崩溃
  - `WALPURGIS_DEBUG=1 python -c "..."` → 断点 print 正常输出

- **CI/merge/docs 文件 → SKIP**:
  - `ci/run_cugraph_pyg_pytests.sh` — SKIP：纯 CI 脚本
  - `ci/test_wheel_cugraph-pyg.sh` — SKIP：CI wheel 测试脚本
