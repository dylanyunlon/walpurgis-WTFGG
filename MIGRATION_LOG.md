...
## migrate bc52d9d: [SKIP] Migrate to NVKS for amd64 CI runners — 纯 CI/GitHub Actions workflow 迁移，将 shared-workflows 引用从 `branch-25.04` 改为 `nvks-runners`，同时注释掉 example 测试以避免网络带宽滥用；Walpurgis 无 GitHub Actions CI 体系，无迁移价值

## migrate bc52d9d: [SKIP] Migrate to NVKS for amd64 CI runners — 纯 CI/GitHub Actions workflow 迁移，将 shared-workflows 引用从 `branch-25.04` 改为 `nvks-runners`，同时注释掉 example 测试以避免网络带宽滥用；Walpurgis 无 GitHub Actions CI 体系，无迁移价值

## migrate ab309ef: [SKIP] Require CMake 3.30.4 — 纯构建基础设施版本升级，Walpurgis 无 CMake 体系

- **Upstream commit**: ab309ef (cugraph-gnn, Robert Maynard, 2025-02-22, PR #147)
- **Commit message**: `Require CMake 3.30.4 (#147)`
- **Upstream diff** (10 files changed, 10 insertions, 10 deletions):
  - `conda/environments/all_cuda-118_arch-x86_64.yaml` — `cmake>=3.26.4,!=3.30.0` → `cmake>=3.30.4`
  - `conda/environments/all_cuda-121_arch-x86_64.yaml` — 同上
  - `conda/environments/all_cuda-124_arch-x86_64.yaml` — 同上
  - `conda/recipes/cugraph-pyg/conda_build_config.yaml` — cmake_version 升级
  - `conda/recipes/libwholegraph/conda_build_config.yaml` — cmake_version 升级
  - `conda/recipes/pylibwholegraph/conda_build_config.yaml` — cmake_version 升级
  - `cpp/CMakeLists.txt` — `cmake_minimum_required(VERSION 3.26.4)` → `3.30.4`
  - `dependencies.yaml` — cmake 版本约束升级
  - `python/pylibwholegraph/CMakeLists.txt` — `cmake_minimum_required(VERSION 3.26.4)` → `3.30.4`
  - `python/pylibwholegraph/pyproject.toml` — `cmake>=3.26.4,!=3.30.0` → `cmake>=3.30.4`
- **跳过原因**: 纯 CMake/conda 构建基础设施版本升级，全面更新 RAPIDS 生态的最低 CMake 要求。
  Walpurgis-WTFGG 是纯 Python 项目（无 `CMakeLists.txt`、无 `conda/` 目录、无 C++/CUDA 编译步骤），
  此 commit 影响的所有 10 个文件在 Walpurgis 仓库中均不存在。
- **Knuth 审查**:
  1. diff 完整性: 10 个文件全量审查，均为构建配置文件，无 Python 算法逻辑
  2. 用户角度 bug: 无运行时代码改动，零 bug 风险
  3. 内存/并发/性能安全: 不适用（无可执行代码）

## migrate 247d10e: [SKIP] filter 11.4.3 — 仅修改 `.github/workflows/pr.yaml` 删除 CUDA 11.4.3 测试矩阵条目，纯 CI 配置，Walpurgis 无 GitHub Actions CI 体系，无迁移价值

- **Upstream commit**: b173063 (cugraph-gnn, Alex Barghi, 2025-03-18, PR #161)
- **Commit message**: `[FIX] Add mkl version, limit tensordict to 0.6.2 (#161)`
- **Upstream diff** (5 files changed, 8 insertions, 6 deletions):
  - `dependencies.yaml` — test_cugraph_dgl/test_cugraph_pyg 新增 `depends_on_mkl`；`tensordict>=0.1.2` → `tensordict>=0.1.2,<=0.6.2`；pyproject 路径 `tensordict` → `tensordict>=0.1.2,<=0.6.2`
  - `python/cugraph-dgl/conda/cugraph_dgl_dev_cuda-118.yaml` — `tensordict>=0.1.2` → `tensordict>=0.1.2,<=0.6.2`
  - `python/cugraph-dgl/pyproject.toml` — `tensordict` → `tensordict>=0.1.2,<=0.6.2`
  - `python/cugraph-pyg/conda/cugraph_pyg_dev_cuda-118.yaml` — `tensordict>=0.1.2` → `tensordict>=0.1.2,<=0.6.2`
  - `python/cugraph-pyg/pyproject.toml` — `tensordict` → `tensordict>=0.1.2,<=0.6.2`
- **跳过原因**: 纯依赖版本约束修复，防止 CPU-only PyTorch + tensordict 0.7.2 不兼容。
  Walpurgis 无 `dependencies.yaml`、无 conda 环境文件、无 `pyproject.toml` 中的 tensordict 依赖，
  5 个受影响文件在 Walpurgis 仓库中均不存在。
- **Knuth 审查**:
  1. diff 完整性: 全量审查，均为版本约束配置，无 Python 算法逻辑
  2. 用户角度: mkl 版本缺失导致安装 CPU-only PyTorch，tensordict 0.7.x 与 PyTorch 2.6 部分版本不兼容——均为上游 CI 环境问题，与 Walpurgis 运行时无关
  3. 内存/并发/性能: 不适用（无可执行代码改动）

## migrate 1132187: [SKIP] fix python tests filter and filter out 11.4 in wheel tests

- **Upstream commit**: 1132187 (cugraph-gnn, Alexandria Barghi, 2025-03-21, PR CI)
- **Commit message**: `fix python tests filter and filter out 11.4 in wheel tests`
- **Upstream diff** (1 file changed, 4 insertions, 4 deletions):
  - `.github/workflows/pr.yaml` — 4处 jq matrix_filter 语法修复:
    - L129: `map(select(.ARCH == "amd64") && select(.CUDA_VER != "11.4.3"))` → `map(select(.ARCH == "amd64" and .CUDA_VER != "11.4.3"))` (语法修正: `&&` 在 jq 中应为 and 关键字, 且须在 select 内)
    - L154: `map(select(.ARCH == "amd64"))` → `map(select(.ARCH == "amd64" and .CUDA_VER != "11.4.3"))` (新增 CUDA 11.4.3 过滤)
    - L170: 同上 (test_wheel_cugraph-dgl)
    - L186: 同上 (test_wheel_cugraph-pyg)
- **跳过原因**: 纯 CI workflow 配置修复, 全部变更位于 `.github/workflows/pr.yaml`。Walpurgis 无 GitHub Actions CI 体系, 无对应 workflow 文件。
- **Knuth 审查**:
  1. diff 完整性: 1 个文件全量审查, 纯 jq filter 语法修正, 无 Python/CUDA/C++ 算法代码
  2. 用户角度 bug: 原 `&&` 在 jq 中是 alternative operator 而非逻辑 AND, 导致 CUDA 11.4.3 过滤失效, CI 矩阵仍跑 11.4.3 wheel 测试并失败; 修复后三个 wheel job 统一排除 CUDA 11.4.3
  3. 内存/并发/性能安全: 不适用 (无可执行代码)
