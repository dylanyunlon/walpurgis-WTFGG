...
## migrate d361d16: [SKIP] Use SPDX for all copyright headers (#338)

**上游 commit**: `d361d167207ed2510f59350ccb75d184042bd4a6`
**上游 PR**: rapidsai/cugraph-gnn#338
**作者**: Kyle Edwards (KyleFromNVIDIA)
**日期**: 2025-10-31

**变更摘要**: 全仓库 296 个文件将冗长的 Apache 2.0 license 块（12–15 行）统一替换为 SPDX 两行格式：
```
# SPDX-FileCopyrightText: Copyright (c) XXXX-YYYY, NVIDIA CORPORATION.
# SPDX-License-Identifier: Apache-2.0
```
同步升级 `.pre-commit-config.yaml`：`rapidsai/pre-commit-hooks` hook 从 `v0.7.0` → `v1.2.0`，新增 `--fix --spdx` 参数及覆盖范围（`dependencies.yaml`、`pyproject.toml`、`recipe.yaml`、`.flake8`、`.coveragerc` 等）。

**涉及文件分类**:
- CI 脚本: `ci/*.sh`、`ci/utils/*.py`（全套 build/test/wheel CI 流水线）
- CMake 基础设施: `cmake/RAPIDS.cmake`、`cmake/rapids_config.cmake`、各级 `CMakeLists.txt`
- C++/CUDA 源码: `cpp/src/**`、`cpp/include/**`、`cpp/tests/**`（wholegraph/wholememory/graph_ops）
- Python 绑定层: `python/pylibwholegraph/**`、`python/cugraph-pyg/**`、`python/libwholegraph/**`
- 配置文件: `.pre-commit-config.yaml`、`.flake8`、`dependencies.yaml`、`mg_utils/*.sh`

**SKIP 理由**:
1. **零业务逻辑变更** — 所有 296 个文件修改内容 100% 为 license header 格式替换，无一行功能代码改动
2. **架构不对应** — `cpp/` 目录（C++/CUDA Wholememory 实现）、`ci/` 目录、`cmake/` 基础设施在 Walpurgis 中均无对应文件；Walpurgis 是纯 Python GNN 框架
3. **CI 体系独立** — Walpurgis 无 rapidsai pre-commit-hooks，无 `verify-copyright` hook，不使用 RAPIDS cmake 体系
4. **License 策略独立** — Walpurgis 在自有 `pyproject.toml` 中声明独立 license，不继承 NVIDIA CORPORATION copyright 表述
5. **依赖清单独立** — `dependencies.yaml`（RAPIDS dependency-file-generator 格式）与 Walpurgis 的 `pyproject.toml` 依赖声明体系不兼容

**迁移位置**: 无（SKIP）

## 525ca06 — Allow CMake 4 (#307)

- **Commit**: `525ca06139d254e652065d9d0588e941095426f2`
- **Commit message**: `Allow CMake 4 (#307)`
- **Author**: Bradley Dice (bdice) / Approver: James Lamb (jameslamb)
- **Context**: CMake 4.0 发布后，上游 7 个构建配置文件中 `cmake>=3.30.4,<4.0.0` 的上界封锁导致 CI 构建失败。统一移除 `<4.0.0` 上界约束，改为 `cmake>=3.30.4`（无上界）。

- **Upstream diff** (7 files changed, 7 insertions(+), 7 deletions(−))：
  | 文件 | 变更内容 |
  |------|----------|
  | `conda/environments/all_cuda-129_arch-aarch64.yaml` | `cmake>=3.30.4,<4.0.0` → `cmake>=3.30.4` |
  | `conda/environments/all_cuda-129_arch-x86_64.yaml` | 同上 |
  | `conda/environments/all_cuda-130_arch-aarch64.yaml` | 同上 |
  | `conda/environments/all_cuda-130_arch-x86_64.yaml` | 同上 |
  | `dependencies.yaml` | `&cmake_ver` YAML 锚点约束同步更新 |
  | `python/libwholegraph/pyproject.toml` | `build-system.requires` cmake 约束更新 |
  | `python/pylibwholegraph/pyproject.toml` | 同上 |

- **CI/merge → SKIP**（全部 7 个上游文件）：
  - `conda/environments/*.yaml` × 4 — Walpurgis 无 conda 构建矩阵，不维护 CUDA 环境 YAML
  - `dependencies.yaml` — RAPIDS 专属依赖生成器，Walpurgis 用 pyproject.toml 独立管理
  - `python/libwholegraph/pyproject.toml` — libwholegraph C 扩展包元数据，非 Walpurgis 源码
  - `python/pylibwholegraph/pyproject.toml` — pylibwholegraph 包元数据，非 Walpurgis 源码

- **迁移位置**: `src/walpurgis/core/cmake_version_policy.py` — 新建

- **鲁迅拿法改写（≥20%）**：
  1. **`BoundKind` 枚举**：LOWER_INCLUSIVE / UPPER_EXCLUSIVE / UPPER_INCLUSIVE / EXACT，将上游裸字符串操作符语义化——上游只有 diff 减号/加号行，无版本语义结构
  2. **`VersionBound` dataclass**：结构化单条约束边界，携带 `kind/version(三元组)/raw`，`parse()` 静态方法从约束字符串构造实例——上游无此层
  3. **`VersionRange` dataclass**：`lower + Optional[upper]`，`satisfies()` 程序化区间检测，`is_open_upper()` 直接映射"移除上界"的核心语义，`parse()` 支持逗号分隔多约束——上游只有字符串替换
  4. **`ConstraintRevision` dataclass**：历史变更台账条目，携带 `upstream_commit/upstream_pr/upstream_author/source_file/before/after/rationale`，`is_upper_bound_removal()` 程序化识别变更类型——上游无此汇总层
  5. **`REVISION_HISTORY`**：7 条结构化变更记录，对应 upstream 7 个文件，可程序化检索——上游只有 7 行 diff
  6. **`ToolchainCompatEntry` + `TOOLCHAIN_COMPAT_MATRIX`**：4 × (cmake_range, cuda_version, arch, walpurgis_affected, note) 兼容性矩阵，`walpurgis_affected=False` 全局声明训练路径零影响——上游无此推理层
  7. **`CMakeVersionPolicy`**：可调用策略对象，`check_version()`/`upper_bound_removals()`/`compat_entries_for()`/`walpurgis_impact_summary()` 四个查询接口——上游无任何 Python 层
  8. **`probe_cmake_version()`**：`subprocess` 调用 `cmake --version`，正则提取三元组，运行时环境自检——上游无
  9. **`verify_environment(strict=)`**：一键环境验证，strict 模式下 cmake 缺失视为失败——上游无
  10. 全链路 `WALPURGIS_DEBUG=1` 断点（`_bp()` 守卫，2 处 dataclass `__post_init__`/`parse` + 3 处 policy 方法 + 1 处 `verify_environment` + 1 处 `probe_cmake_version` + 1 处 `_self_test`，共 8 处）

- **自测结果**: 12/12 全部 [PASS]

---

## migrate 920d309: [SKIP] Configure repo for automatic release notes generation (#308) — `.github/release.yml` 纯 CI/repo 配置，Walpurgis 无 GitHub Release 自动化体系
