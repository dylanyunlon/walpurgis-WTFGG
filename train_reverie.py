"""
train_reverie.py — Reverie训练入口
从项目根目录运行: python train_reverie.py --dataset SYNTH --debug

算法特性 (vs upstream):
  - Bilinear+Swish estimation gate
  - InstanceNorm residual decomp
  - Chebyshev polynomial graph conv
  - RBF kernel distance
  - Gumbel top-k mask
  - Sinkhorn doubly-stochastic normalization
  - MinGRU + exponential decay PE
  - LayerScale + GELU output head
  - Log-cosh loss + adaptive temperature
...
from walpurgis_reverie.utils.train import (
    set_config, EarlyStopping, data_reshaper, load_model)
from walpurgis_reverie.utils.load_data import (
    load_dataset, load_adj)
from walpurgis_reverie.utils.log import TrainLogger
from walpurgis_reverie.models.losses import (
...
def run(dataset, device_str='cpu', epochs_override=None,
        debug=False):
    if debug:
        os.environ['REVERIE_DEBUG'] = '1'

    set_config(0)
    config_dir = os.path.join(
        _REPO_ROOT, 'src', 'walpurgis_reverie', 'configs')
    ...
    if not os.path.exists(data_dir):
        nf_dir = os.path.join(
            _REPO_ROOT, 'src', 'walpurgis_reverie')
    ...
    save_path_resume = os.path.join(
        _REPO_ROOT, 'output',
        f'{model_name}_REVERIE_{dataset_name}_resume.pt')
...
def main():
    parser = argparse.ArgumentParser(
        description='Reverie (D2STGNN) Training')
