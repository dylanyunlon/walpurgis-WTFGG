"""
train_prism.py — Prism训练入口
从项目根目录运行: python train_prism.py --dataset SYNTH --debug

算法特性 (vs upstream):
  - 多视角融合 (空间/时间/频率三视角编码 + 自适应注意力融合)
  - 对比学习正则化 (InfoNCE相邻节点embedding拉近, 温度退火)
  - Mixup数据增强 (Beta分布输入插值 + mixup-aware MAE)
  - 频率域特征注入 (embedding后FFT注入频谱信息)
  - 交叉视角注意力 (inherent block中temporal→spatial cross-attention)
  - AdamW优化器 + OneCycleLR调度器
  - 对比感知边调制 (动态图构建中embedding相似度调制边权)
  - 频谱残差分解 (频域低通滤波平滑残差)
  - 完整诊断: contrastive temperature tracker / view fusion tracker
"""
import argparse
import os
import sys
...
from walpurgis_prism.utils.load_data import (
    load_dataset, load_adj)
from walpurgis_prism.utils.log import TrainLogger
from walpurgis_prism.models.losses import (
    masked_mae, masked_rmse, masked_mape, metric)
from walpurgis_prism.models import trainer
from walpurgis_prism.models.model import D2STGNN
...
def run(dataset, device_str='cpu', epochs_override=None,
    ...
    config_dir = os.path.join(
        _REPO_ROOT, 'src', 'walpurgis_prism', 'configs')
    ...
    if not os.path.exists(data_dir):
        pr_dir = os.path.join(
            _REPO_ROOT, 'src', 'walpurgis_prism')
        data_dir = _resolve_path(
            config['data_args']['data_dir'], pr_dir)
        adj_path = _resolve_path(
            config['data_args']['adj_data_path'], pr_dir)
    ...
    print(f"\n{'='*60}")
    print(f"  Prism Training Pipeline")
    ...
    # Prism特有: 传递邻接矩阵原始版本给trainer用于对比loss
    optim_args['adj_ori'] = adj_ori
    ...
    if mode != 'test':
        for epoch in range(
            ...
            # Prism特有: 记录对比学习和视角融合诊断
            contrast_loss_val = 0.0
            if hasattr(engine, 'contrast_tracker'):
                recent = engine.contrast_tracker.history
                if recent:
                    contrast_loss_val = recent[-1][2]
        ...
        if _is_debug():
            engine.perf.report()
            engine.contrast_tracker.report()
            engine.view_tracker.report()
    ...
    print(f"\n[PR] Pipeline complete. Saved: {save_path}")
    return early_stopping.val_loss_min


def main():
    parser = argparse.ArgumentParser(
        description='Prism (D2STGNN) Training')
